...
# This program will run the Kettle job to load Clear2Work Badge Activity files
...
RUN_DATE_LOG=$(date +"%Y%m%d_%H%M%S" --date='today')
	...
	JOB=Clear2Work_Process_Badge_Activity_Files
	...
	EMAIL_SUBJECT_FILE_NOT_FOUND="ALERT! – C2W $ENV_CODE - No Badge Activity files found in the Inbound Directory"
...
if [ "$(find $CLEAR2WORK_INBOUND_DIR -maxdepth 1 -not -type d -name "+_MG_Last*")" ]; then
