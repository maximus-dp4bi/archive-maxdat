...
echo "KETTLE_PROPERTIES_FILE=${KETTLE_PROPERTIES_FILE}"
...
echo "Clear2Work_DATA_DIR=${Clear2Work_DATA_DIR}"

if [ "${DB_PASSWORD:0:9}" = "Encrypted" ]; then
	DB_ENCRYPTED_PASSWORD=$(grep "^DB_MOTS_PASSWORD" ${KETTLE_PROPERTIES_FILE} | cut -d'=' -f2 | cut -d' ' -f2)
	
	echo "DB_ENCRYPTED_PASSWORD=${DB_ENCRYPTED_PASSWORD}"
	
	if [ "$CLASSPATH" = "" ]; then
		export CLASSPATH=$Clear2Work_SCRIPT_DIR
	else
