...
INIT_OK="$MAXDAT_ETL_PATH/${STCODE}_run_sc_pp_wfm_task_audit_check.txt"
CHILD_FAIL="/tmp/${STCODE}_sc_pp_wfm_task_audit_child_task_fail.txt"


#mail related variables
EMAIL="MAXDatSystem@maximus.com"
EMAIL_MESSAGE="/tmp/${STCODE}_run_sc_pp_wfm_task_audit-ERROR-LOG.txt"
EMAIL_SUBJECT="Errors With ${STCODE}_run_sc_pp_wfm_task_audit.sh in $ENV_CODE"
...
if [[ -e $INIT_OK ]] ; then
   ...
   echo "Starting ${STCODE}_run_sc_pp_wfm_task_audit.sh in ${ENV_CODE}."
   rm -f $EMAIL_MESSAGE $CHILD_FAIL
   touch $INIT_OK
fi

#init check
$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/dp_scorecard_Init_check.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/dp_scorecard_init_check_$(date +%Y%m%d_%H%M%S).log
rc=$?
if [[ $rc != 0 ]] ; then
	...
	$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/DP_Scorecard/PP_WFM_TASK_AUDIT.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/PP_WFM_TASK_AUDIT_$(date +%Y%m%d_%H%M%S).log &
	 wait
		if [[ -e $CHILD_FAIL ]]
		then
