...
INIT_OK="$MAXDAT_ETL_PATH/${STCODE}_PP_WFM_TASK_BO_30_DAY_SYNC_check.txt"  
CHILD_FAIL="/tmp/${STCODE}_PP_WFM_TASK_BO_30_DAY_SYNC_child_task_fail.txt"   


#mail related variables
EMAIL="MAXDatSystem@maximus.com"
EMAIL_MESSAGE="/tmp/${STCODE}_PP_WFM_TASK_BO_30_DAY_SYNC-ERROR-LOG.txt"   
EMAIL_SUBJECT="Errors With ${STCODE}_pp_wfm_task_bo)30_DAYS.sh in $ENV_CODE"  
...
if [[ $rc != 0 ]] ; then
	...
	$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/DP_Scorecard/PP_WFM_TASK_BO_30_DAY_SYNC.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/PP_WFM_TASK_BO_30_DAY_SYNC_$(date +%Y%m%d_%H%M%S).log &
	 wait                                                              
		if [[ -e $CHILD_FAIL ]]
		then
