...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/Run_Initialization_incident_stage.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/Run_Initialization_incident_stage_$(date +%Y%m%d_%H%M%S).log
   if [[ $? -eq 0 ]]
   then
