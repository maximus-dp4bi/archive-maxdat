...
#init check
$MAXDAT_ETL_PATH/run_kjb7.sh $MAXDAT_ETL_PATH/Planning_Init_check.kjb $KJB_LOG_LEVEL  >> $MAXDAT_ETL_LOGS/Planning_Init_check_$(date +%Y%m%d_%H%M%S).log
rc=$?
if [[ $rc != 0 ]] ; then
