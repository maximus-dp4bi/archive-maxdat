...
EMAIL_MESSAGE="/tmp/${STCODE}_PP_WFM_TASK_BO_30D_SYNC-ERROR-LOG.txt"   
EMAIL_SUBJECT="Errors With ${STCODE}_PP_WFM_TASK_BO_30D_SYNC.sh in $ENV_CODE"  
...
if [[ -e $INIT_OK ]] ; then
   ...
   echo "Starting ${STCODE}_PP_WFM_TASK_BO_30D_SYNC.sh in ${ENV_CODE}."
...
if [[ $rc != 0 ]] ; then
	...
	$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/DP_Scorecard/PP_WFM_TASK_BO_30D_SYNC.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/PP_WFM_TASK_BO_30D_SYNC_$(date +%Y%m%d_%H%M%S).log &
	 wait                                                              
		if [[ -e $CHILD_FAIL ]]
		then
