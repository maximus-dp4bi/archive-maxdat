...
if [[ $rc != 0 ]] ; then
   ...
   then
	...
	$MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/IDR/IDR_Incidents_RUN_ALL.kjb $KJB_LOG_ROWLEVEL >> $MAXDAT_ETL_LOGS/IDR_Incidents_RUN_ALL_$(date +%Y%m%d_%H%M%S).log &
	wait
