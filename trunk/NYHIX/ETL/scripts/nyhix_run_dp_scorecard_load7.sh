...
if [[ $rc != 0 ]] ; then
	...
	$MAXDAT_ETL_PATH/run_kjb7.sh $MAXDAT_ETL_PATH/DP_Scorecard/Process_DP_SCORECARD_Files.kjb $KJB_LOG_LEVEL  >> $MAXDAT_ETL_LOGS/Process_DP_SCORECARD_Files7_$(date +%Y%m%d_%H%M%S).log &
	 wait
		if [[ -e $CHILD_FAIL ]]
		then
