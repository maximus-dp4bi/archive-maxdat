...
EMAIL_MESSAGE="/tmp/fl_load_case_emrs-ERROR-LOG.txt"
EMAIL_SUBJECT="Errors With fl_load_case_emrs.sh"
...
if [[ $rc != 0 ]] ; then
     echo "exited with status: $rc - Init_check.kjb, aborting run" >> $EMAIL_MESSAGE
     mail -s "$EMAIL_SUBJECT" "$EMAIL" < $EMAIL_MESSAGE
     cat $EMAIL_MESSAGE
     exit $rc
else
     $EMRSSCRIPTS/run_ktr.sh $EMRSDATADIR/DIM_Load_CASES_firstime.ktr > $LOG_DIR/fl_load_case_emrs.sh_$(date +%Y%m%d_%H%M%S).log
     rc=$?
     if [[ $rc != 0 ]] ; then
     echo "fl_load_case_emrs.sh failed, review error log for additional detail." >> $EMAIL_MESSAGE
     mail -s "$EMAIL_SUBJECT" "$EMAIL" < $EMAIL_MESSAGE
     cat $EMAIL_MESSAGE
     error_exit "$LINENO: An error has occurred in fl_load_case_emrs.sh"
     fi
fi
