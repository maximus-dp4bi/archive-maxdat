create table FLHK_ETL_SUPP_MEMBR_INQY
(
...
INSTANCE_CREATE_DATE	DATE NOT NULL
  ...
  --------------------------------------------------------
CREATE UNIQUE INDEX "CONTACT_ID_PK" ON "FLHK_ETL_SUPP_MEMBR_INQY"
  (
    "CONTACT_ID"
  )
  ;
  ...
  --------------------------------------------------------
  ALTER TABLE "FLHK_ETL_SUPP_MEMBR_INQY" ADD CONSTRAINT "CONTACT_ID_PK" PRIMARY KEY ("CONTACT_ID") ENABLE;
  /
  --------------------------------------------------------
  --  DDL for Trigger FLHK_ETL_SMI_TRG
  --------------------------------------------------------
CREATE OR REPLACE TRIGGER "FLHK_ETL_SMI_TRG" before
  INSERT ON "FLHK_ETL_SUPP_MEMBR_INQY" FOR EACH row BEGIN IF inserting THEN IF :NEW."FEMSI_ID" IS NULL THEN
  SELECT FLHK_ETL_SMI_SEQ.nextval INTO :NEW."FEMSI_ID" FROM dual;
END IF;
END IF;
END;
/
ALTER TRIGGER "FLHK_ETL_SMI_TRG" ENABLE;
/
