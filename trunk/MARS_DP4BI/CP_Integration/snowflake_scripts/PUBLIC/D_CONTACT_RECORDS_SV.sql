...
CREATE OR REPLACE VIEW D_CONTACT_RECORDS_SV AS
...
LEFT JOIN (SELECT *
           FROM(SELECT project_id, contact_record_id,contact_record_actions_id,contact_reason,contact_action, 
                ...
                WHERE UPPER(contact_reason) in('APPEAL','APPLICATION','CHANGE OF ADDRESS','COMPLAINT','DMI','INBOUND DOCUMENT INQUIRY','RETURNED MAIL')
                AND UPPER(contact_action) LIKE 'UNABLE TO RESOLVE ISSUE%'
               )
...
SELECT 
    ...
    ,CUSV.BUSINESS_UNIT_ID AS CONTACT_RECORD_CREATED_BY_BUSINESS_UNIT_ID
    ,CUSV.BUSINESS_UNIT_NAME AS CONTACT_RECORD_CREATED_BY_BUSINESS_UNIT_NAME
    ...
    ,CASE WHEN (select 
               ...
               AND TASK_LINK.EFFECTIVE_END_DATE IS NULL) > 1 THEN 1 ELSE 0 END CONTACT_CREATING_TASKS_FLAG  
    ...
    ,CASE WHEN (SELECT COUNT(*) FROM MARSDB.MARSDB_EXTERNAL_LINKS_VW el JOIN MARSDB.MARSDB_CONSUMER_VW c ON el.external_ref_id = c.consumer_id AND el.project_id = c.project_id 
        ...
        ,CR.ORGANIZATION_NAME as organization_name 
FROM 
    MARSDB.MARSDB_CONTACT_RECORD_VW AS CR 
LEFT OUTER JOIN 
    MARSDB.MARSDB_EXTERNAL_LINKS_VW CONS_LINK
...
LEFT OUTER JOIN
    D_USER_SV CUSV
        ON CU.USER_ID = CUSV.USER_ID
        AND CU.PROJECT_ID = CUSV.USER_PROJECT_ID
LEFT OUTER JOIN 
