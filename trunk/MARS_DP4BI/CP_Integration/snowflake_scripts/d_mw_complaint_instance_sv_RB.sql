CREATE OR REPLACE VIEW MW_D_COMPLAINT_INSTANCE_SV AS
SELECT  ts.PROJECT_ID
       ...
       ,tdcr.SELECTION_VARCHAR AS complaint_reason
       ,tdca.SELECTION_VARCHAR AS complaint_about 
...
FROM (SELECT project_id,task_id,task_type_id,default_priority,task_status,status_date,staff_worked_by,staff_assigned_to
 ...
 LEFT JOIN MARSDB.MARSDB_TASK_DETAIL_VW tdca ON ts.TASK_ID = tdca.TASK_ID AND tdca.TASK_FIELD_NAME = 'Complaint About'
 LEFT JOIN MARSDB.MARSDB_TASK_DETAIL_VW tdcr ON ts.TASK_ID = tdcr.TASK_ID AND tdcr.TASK_FIELD_NAME = 'Reason'
 ...
 LEFT JOIN MARSDB.MARSDB_USER_PROJECT_ROLE_VW upr ON cu.USER_ID = upr.USER_ID AND upr.IS_DEFAULT = 1 AND COALESCE(upr.END_DATE,CURRENT_DATE()) >= CURRENT_DATE()
 ...
 LEFT JOIN MARSDB.MARSDB_USER_PROJECT_ROLE_VW satupr ON satu.USER_ID = satupr.USER_ID AND satupr.IS_DEFAULT = 1 AND COALESCE(satupr.END_DATE,CURRENT_DATE()) >= CURRENT_DATE()
