CREATE OR REPLACE VIEW MARS_DP4BI_PROD.PUBLIC.MW_D_COMPLAINT_INSTANCE_SV AS
SELECT PROJECT_ID
       ,complaint_id
...
FROM ( 
    SELECT  ts.PROJECT_ID
           ...
           ,CASE WHEN ts.ESCALATED_FLAG_HIST THEN 'Y' ELSE 'N' END AS complaint_flag
           ,CASE WHEN ts.ESCALATED_FLAG_HIST THEN thesc.escalated_to_user_id ELSE NULL END AS complaint_escalated_to_user_id
           ...
           ,COALESCE(tdsr.SELECTION_VARCHAR,tdsrh.SELECTION_VARCHAR,tdea.SELECTION_VARCHAR,tdeah.SELECTION_VARCHAR) AS external_application_id
    FROM  PUBLIC.D_DATES d
      JOIN (SELECT 
               ...
               FROM
                MARSDB.MARSDB_TASKS_VW AS ts) ts            
  ...
  -- get Locality
    ...
    LEFT OUTER JOIN
     ...
     --get SR ID
     LEFT JOIN (SELECT * FROM
                 (SELECT ex.PROJECT_ID,ex.INTERNAL_ID,ex.EXTERNAL_REF_ID AS service_request_id,RANK() OVER (PARTITION BY ex.PROJECT_ID,ex.INTERNAL_ID ORDER BY ex.EXTERNAL_LINK_ID DESC) exrnk
                      FROM MARSDB.MARSDB_EXTERNAL_LINKS_VW ex
                      WHERE UPPER(ex.INTERNAL_REF_TYPE) = 'TASK'
                      AND UPPER(ex.EXTERNAL_REF_TYPE) = 'SERVICE_REQUEST'
                      AND ex.EFFECTIVE_END_DATE IS NULL ) tmp 
                  WHERE exrnk = 1) sr ON sr.INTERNAL_ID = ts.TASK_ID AND sr.PROJECT_ID = ts.PROJECT_ID
     --get Application Id
     LEFT JOIN MARSDB.MARSDB_TASK_DETAIL_VW tdea ON ts.task_id = tdea.task_id AND ts.project_id = tdea.project_id AND tdea.task_field_id = 55
     LEFT JOIN (SELECT *
                FROM (SELECT task_id, task_field_name,project_id, selection_varchar,RANK() OVER (PARTITION BY task_id,project_id ORDER BY task_detail_history_id DESC) tdarn
                      FROM MARSDB.MARSDB_TASK_DETAIL_HISTORY_VW
                      WHERE task_field_id = 55 )
                WHERE tdarn = 1 ) tdeah ON ts.task_id = tdeah.task_id AND ts.project_id = tdeah.project_id  
     LEFT JOIN MARSDB.MARSDB_TASK_DETAIL_VW tdsr ON sr.service_request_id = tdsr.task_id AND sr.project_id = tdsr.project_id AND tdsr.task_field_id = 55
     LEFT JOIN (SELECT *
                FROM (SELECT task_id, task_field_name,project_id, selection_varchar,RANK() OVER (PARTITION BY task_id,project_id ORDER BY task_detail_history_id DESC) tdarn
                      FROM MARSDB.MARSDB_TASK_DETAIL_HISTORY_VW
                      WHERE task_field_id = 55 )
                WHERE tdarn = 1 ) tdsrh ON sr.service_request_id = tdsrh.task_id AND sr.project_id = tdsrh.project_id                  
