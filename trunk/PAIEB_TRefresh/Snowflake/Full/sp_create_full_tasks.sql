...
//use this in every task routine
    ...
    var completeTaskSql = "create task "+completeTaskName+"  \
    warehouse= "+dbwarehouse+" \
    ALLOW_OVERLAPPING_EXECUTION = FALSE \
    AFTER REPLACE_MERGELIST \
    as call control.UPDATE_JOB_COMPLETE('Y','0') \
    ; \
    ";
          ...
          while (ret_value.next()) {
            ...
            return_array.push(sqlCommand);
                try {
                    ...
                    snowflake.execute({sqlText:sqlCommand});
                    mergeTask_array.push(mergeTaskName+vtablename);
            ...
            var mergeTaskList = mergeTask_array.toString();
            return_array.push(mergeTaskList);
            if (mergeTask_array.length > 0  & mergeTaskList != null) {
              completeTaskSql = completeTaskSql.replace('REPLACE_MERGELIST',mergeTaskList);
              return_array.push(completeTaskSql);
              snowflake.execute({sqlText:completeTaskSql});
            }
