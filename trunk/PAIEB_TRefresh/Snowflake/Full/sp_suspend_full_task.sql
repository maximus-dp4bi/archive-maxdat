create or replace procedure control.SUSPEND_FULL_TASK(intaskprefix varchar)
  ...
  var isFatal = 'N';
  var vjobid = 9999999;
  var vjobname = 'SUSPEND_FULL_TASK';
...
try{
        ...
        //Cant suspend root first before child 
                   
        sCommand = 'alter task if exists '+fullLoadTaskName+' SUSPEND;';
        ...
        sCommand = 'alter task if exists '+listTaskName+' SUSPEND;';
        return_array.push(sCommand);
          try {
            snowflake.execute({sqlText:sCommand});
          } catch(err) {
          ...
          while (ret_value.next()) {
            vtablename = ret_value.getColumnValue('SOURCE_TABLE_NAME');
                sCommand = 'alter task if exists '+vtaskprefix+'_INFER_' + vtablename +' SUSPEND;';
                return_array.push(sCommand);
                try {
                    snowflake.execute({sqlText:sCommand});

                    sCommand = 'alter task if exists '+vtaskprefix+'_MERGE_' + vtablename +' SUSPEND;';
                    return_array.push(sCommand);
                    snowflake.execute({sqlText:sCommand});
        ...
        sCommand = 'alter task if exists '+completeTaskName+' SUSPEND;';
          return_array.push(sCommand);
          snowflake.execute({sqlText:sCommand});
