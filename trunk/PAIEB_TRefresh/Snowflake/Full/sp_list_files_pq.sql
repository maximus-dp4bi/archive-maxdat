create or replace procedure control.LIST_FILES_PQ(ISTASK varchar, PJOBID varchar)
    ...
    try {
        ...
        var updateJobSql = `update ` + strCtlTableSchema + `.paieb_job_ctrl s set 
          ...
          strSQLText = `SELECT primary_key_column, primary_key_limit
                     , replace(replace(merge_liststr,'REPLACE_JOBID','`+numMaxJobId+`'),'REPLACE_REINGEST','`+strReingest+`')  merge_liststr
