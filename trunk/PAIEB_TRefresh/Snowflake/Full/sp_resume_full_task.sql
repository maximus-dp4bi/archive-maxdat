create or replace procedure control.RESUME_FULL_TASK(intaskprefix varchar)
...
try{
          //Resume in reverse order child first
          sCommand = 'alter task if exists '+completeTaskName+' RESUME;';
          ...
          while (ret_value.next()) {
            vtablename = ret_value.getColumnValue('SOURCE_TABLE_NAME');
                sCommand = 'alter task if exists '+vtaskprefix+'_MERGE_' + vtablename +' RESUME;';
                return_array.push(sCommand);
                try {
                    snowflake.execute({sqlText:sCommand});

                    sCommand = 'alter task if exists '+vtaskprefix+'_INFER_' + vtablename +' RESUME;';
                    return_array.push(sCommand);
                    snowflake.execute({sqlText:sCommand});
        ...
        //Cant resume child when root is not suspend- so reverse order
                   
        sCommand = 'alter task if exists '+listTaskName+' RESUME;';
        return_array.push(sCommand);
          try {
            snowflake.execute({sqlText:sCommand});
          } catch(err) {
        ...
        sCommand = 'alter task if exists '+fullLoadTaskName+' RESUME;';
        return_array.push(sCommand);
          try {
            snowflake.execute({sqlText:sCommand});
          } catch(err) {
