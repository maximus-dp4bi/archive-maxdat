create or replace procedure control.START_FULL_LOAD(stageName varchar, awsfolder varchar, pq_schema_name varchar, target_schema_name varchar, source_table_name varchar, istask varchar, isreingest varchar)
  ...
  var vMessage = "";
  var return_array= [];
  var statusSQLStmt = `UPDATE control.paieb_job_ctrl SET params = OBJECT_INSERT(params, 'FULL_STATUS', '~PSTATUS~', true) WHERE jobid = PJOBID and params:FULL_STATUS::VARCHAR = 'STARTED';`;
  var taskStmt = "";
...
//use this in every task routine
   ...
   try {
      ...
      if (numMaxJobId == null) {
          ...
          var vparams = '{ "STAGENAME":"'+STAGENAME+'" , "AWSFOLDER":"'+AWSFOLDER+'" , "PQ_SCHEMA_NAME":"'+PQ_SCHEMA_NAME+'", "TARGET_SCHEMA_NAME":"'+TARGET_SCHEMA_NAME+'" , "SOURCE_TABLE_NAME":"'+SOURCE_TABLE_NAME+'" , "FULL_STATUS":"STARTED" , "REINGEST_FLAG":"'+ISREINGEST+'" }';
