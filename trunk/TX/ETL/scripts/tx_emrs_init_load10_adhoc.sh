...
then
   ...
   $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/EMRS_set_etl_run_date.ktr EMRS_set_etl_run_date >> $MAXDAT_ETL_LOGS/EMRS_set_etl_run_date_$(date +%Y%m%d_%H%M%S).log    
                        rc=$?
                        if [[ $rc == 0 ]] ; then
                        $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/FACT_Load_ENROLLMENTS_HIST_ADHOC.ktr FACT_Load_ENROLLMENTS_HIST_ADHOC >> $MAXDAT_ETL_LOGS/FACT_Load_ENROLLMENTS_HIST_ADHOC_$(date +%Y%m%d_%H%M%S).log 
     fi
else
