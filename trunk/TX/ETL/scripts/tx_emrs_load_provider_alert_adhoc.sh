...
then
   # run the sequence of scripts dependent on each one finishing successfully
      rc=$?
       if [[ $rc == 0 ]] ; then
           $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRS_DATA_DIR/ETL_Load_PROVIDERS_ADHOC.kjb ETL_Load_PROVIDERS_ADHOC >> $MAXDAT_ETL_LOGS/ETL_Load_PROVIDERS_ADHOC_$(date +%Y%m%d_%H%M%S).log      
           $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/DIM_Load_ALERTS_ONETIME.ktr DIM_Load_ALERTS_ONETIME >> $MAXDAT_ETL_LOGS/DIM_Load_ALERTS_ONETIME_$(date +%Y%m%d_%H%M%S).log  
    fi
else
