...
then
   # run the sequence of scripts dependent on each one finishing successfully
      rc=$?
       if [[ $rc == 0 ]] ; then
           $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRS_DATA_DIR/ETL_Load_Notifications_OneTime.kjb ETL_Load_Notifications_OneTime >> $MAXDAT_ETL_LOGS/ETL_Load_Notifications_OneTime_$(date +%Y%m%d_%H%M%S).log     
    fi
else
