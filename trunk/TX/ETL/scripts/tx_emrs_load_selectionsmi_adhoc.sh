...
then
   # run the sequence of scripts dependent on each one finishing successfully
      rc=$?
       if [[ $rc == 0 ]] ; then     
           $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/DIM_Load_SELECTION_MISSING_INFO_ADHOC.ktr DIM_Load_SELECTION_MISSING_INFO_ADHOC >> $MAXDAT_ETL_LOGS/DIM_Load_SELECTION_MISSING_INFO_ADHOC_$(date +%Y%m%d_%H%M%S).log  
    fi
else
