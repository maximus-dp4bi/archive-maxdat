...
1. Turn off tx_run_bpm.sh cron job, then do the steps 1 - 11 in that order.
...
BPM_D_SCI_CURRENT_DDL.sql
BPM_D_SCI_CURRENT_SV_DDL.sql
BPM_D_SCID_CURRENT_SV_DDL.sql
BPM_D_SCIE_CURRENT_SV_DDL.sql
BPM_F_SCI_BY_DATE_DDL.sql
BPM_F_SCI_BY_DATE_SV_DDL.sql
...
7. Update kettle.properties configuration file
Add JOB_STATISTIC_DIR variable pointing to ETL scripts directory
JOB_STATISTIC_DIR=<corp/scripts>

8. Create  Directory on App Server
Create directory for kettle scripts as follows:
MAXDAT_ETL_PATH/SupportClientInquiry


9. AS_TXEB_SCI_INIT_MAXDAT_BRIAN_1.zip patchset
Deploy Kettle files
...
10. Deploy UNIX Script
Copy files from AS_TXEB_SCI_INIT_MAXDAT_BRIAN_1.zip patchset
TO: MAXDAT_ETL_PATH
tx_run_bpm.sh

11. Convert Unix Script from Dos to Unix
