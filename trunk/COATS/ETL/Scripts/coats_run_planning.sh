...
#mail related variables - overwrite of default
EMAIL_MESSAGE="/tmp/${STCODE}-PLANNING-ERROR-LOG.txt"
...
#init check
$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/Run_Initialization.kjb" -level="$KJB_LOG_LEVEL" > $MAXDAT_ETL_LOGS/Planning_Init_check_$(date +%Y%m%d_%H%M%S).log
rc=$?
if [[ $rc != 0 ]] ; then
