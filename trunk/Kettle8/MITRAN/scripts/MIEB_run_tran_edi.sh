...
LETTER_INIT_OK="$MAXDAT_ETL_PATH/${STCODE}_run_tran_edi_check.txt"
function error_exit
{
...
EMAIL_SUBJECT="Errors With ${STCODE}_run_tran_edi.sh in ${ENV_CODE}"
...
rc=$?
if [[ $rc != 0 ]] ; then
	echo "exited with status: $rc - BPM_Init_check.kjb for MIEB_run_tran_edi.sh, aborting run in ${STCODE}" >> ${EMAIL_MESSAGE}
	rm -f ${LETTER_INIT_OK}
	mail -s "${EMAIL_SUBJECT}" "${EMAIL}" < ${EMAIL_MESSAGE}
	cat ${EMAIL_MESSAGE}
	exit $rc
else
        MODULE=${MODULE_LTR}; RUN_FILE=load_tran_edi; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/${MODULE}/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/run_kjb.sh ${MAXDAT_ETL_PATH}/${MODULE}/${RUN_FILE}.kjb ${KJB_LOG_LEVEL} >> ${LOG_FILE} 
		if [[ -e ${CHILD_FAIL} ]]
		then
