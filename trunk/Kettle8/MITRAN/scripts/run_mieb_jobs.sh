...
if [[ $rc != 0 ]] ; then
	...
	exit $rc
        ...
        MODULE=JobScheduler; RUN_FILE=run_mi_jobs; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/Letters/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/run_kjb.sh ${MAXDAT_ETL_PATH}/${MODULE}/${RUN_FILE}.kjb ${KJB_LOG_LEVEL} >> ${LOG_FILE} &
		if [[ -e ${CHILD_FAIL} ]]
		then
