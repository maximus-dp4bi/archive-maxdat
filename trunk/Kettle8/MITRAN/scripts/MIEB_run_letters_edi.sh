...
LETTER_INIT_OK="$MAXDAT_ETL_PATH/${STCODE}_run_letter_edi_check.txt"
function error_exit
{
...
EMAIL_SUBJECT="Errors With ${STCODE}_run_letters_edi.sh in ${ENV_CODE}"

#checking for run file, Abort if it exists, create if it does not exists
if [[ -e ${LETTER_INIT_OK} ]] ; then
   echo "Run Aborted - ${LETTER_INIT_OK} exists"
   exit;
else
   echo "Starting ${STCODE}_run_bpm.sh in ${ENV_CODE}."
   rm -f ${EMAIL_MESSAGE} ${CHILD_FAIL}
   touch ${LETTER_INIT_OK}
fi
...
rc=$?
if [[ $rc != 0 ]] ; then
	echo "exited with status: $rc - BPM_Init_check.kjb for MIEB_run_letters_edi.sh, aborting run in ${STCODE}" >> ${EMAIL_MESSAGE}
	rm -f ${LETTER_INIT_OK}
	mail -s "${EMAIL_SUBJECT}" "${EMAIL}" < ${EMAIL_MESSAGE}
	cat ${EMAIL_MESSAGE}
	exit $rc
else
        MODULE=${MODULE_LTR}; RUN_FILE=load_letters_edi; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/${MODULE}/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/run_kjb.sh ${MAXDAT_ETL_PATH}/${MODULE}/${RUN_FILE}.kjb ${KJB_LOG_LEVEL} >> ${LOG_FILE} 
		if [[ -e ${CHILD_FAIL} ]]
		then
