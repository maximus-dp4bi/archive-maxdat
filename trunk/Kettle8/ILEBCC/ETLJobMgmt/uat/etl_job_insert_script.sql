
TRUNCATE TABLE ETL_JOB_GLOBAL_CONFIG;
TRUNCATE TABLE ETL_JOB_CONFIG;

-- INSERT GLOBAL PARAMETERS - SHOULD RETAIN THESE 5 PARAMETERS FOR EACH PROJECT
-- Change PROJECT_NAME and PARAM_VALUE fied values only 

INSERT INTO ETL_JOB_GLOBAL_CONFIG (PROJECT_NAME, PARAM_KEY, PARAM_VALUE)
VALUES ('ILEB', 'ETL_AUDIT_DATA_RETENTION_DAYS', 365);

INSERT INTO ETL_JOB_GLOBAL_CONFIG (PROJECT_NAME, PARAM_KEY, PARAM_VALUE)
VALUES ('ILEB', 'ETL_RUN_DATA_RETENTION_DAYS', 365);
        
INSERT INTO ETL_JOB_GLOBAL_CONFIG (PROJECT_NAME, PARAM_KEY, PARAM_VALUE)
VALUES ('ILEB', 'ETL_LOG_DATA_RETENTION_DAYS', 365);

INSERT INTO ETL_JOB_GLOBAL_CONFIG (PROJECT_NAME, PARAM_KEY, PARAM_VALUE)
VALUES ('ILEB', 'MAX_PARALLEL_JOBS_ALLOWED', 7);

INSERT INTO ETL_JOB_GLOBAL_CONFIG (PROJECT_NAME, PARAM_KEY, PARAM_VALUE)
VALUES ('ILEB', 'JOB_TIMEOUT_SEC', 3000);

INSERT INTO ETL_JOB_CONFIG (
    PROJECT_NAME,
    JOB_NAME,
    JOB_TYPE,
    JOB_SCHEDULE,
    JOB_ENABLED,
    JOB_SCRIPT_NAME,
	JOB_LOG_PATH,
    LAST_UPD_DT,
    LAST_UPD_USER)
VALUES (
	'ILEB',
	'MANAGE ADHOC CC JOBS',
	'STANDALONE',
	'15 * * * *',
	'Y',
	'manage_adhoc_contact_center_jobs.sh',
	'/u01/maximus/maxdat-uat/ILEBCC8/ETLJobControl/logs',
	SYSDATE,
	USER
);

INSERT INTO ETL_JOB_CONFIG (
    PROJECT_NAME,
    JOB_NAME,
    JOB_TYPE,
    JOB_SCHEDULE,
    JOB_ENABLED,
    JOB_SCRIPT_NAME,
    JOB_LOG_PATH,
    LAST_UPD_DT,
    LAST_UPD_USER)
VALUES (
	'ILEB',
	'SCHEDULED CC JOB EXECUTIONS',
	'STANDALONE',
	'00 * * * *',
	'Y',
	'scheduled_contact_center_job_executions.sh',
	'/u01/maximus/maxdat-uat/ILEBCC8/ETLJobControl/logs',
	SYSDATE,
	USER
);

INSERT INTO ETL_JOB_CONFIG (
    PROJECT_NAME,
    JOB_NAME,
    JOB_TYPE,
    JOB_SCHEDULE,
    JOB_ENABLED,
    JOB_SCRIPT_NAME,
    JOB_LOG_PATH,
    LAST_UPD_DT,
    LAST_UPD_USER)
VALUES (
	'ILEB',
	'RUN DAILY IVR FILES',
	'STANDALONE',
	'40 * * * *',
	'Y',
	'Run_Daily_IVR_Files.sh',
	'/u01/maximus/maxdat-uat/ILEBCC8/ETLJobControl/logs',
	SYSDATE,
	USER
);

INSERT INTO ETL_JOB_CONFIG (
    PROJECT_NAME,
    JOB_NAME,
    JOB_TYPE,
    JOB_SCHEDULE,
    JOB_ENABLED,
    JOB_SCRIPT_NAME,
	JOB_LOG_PATH,
    LAST_UPD_DT,
    LAST_UPD_USER)
VALUES (
	'ILEB',
	'LOAD_NEW_QUEUE_FLAG',
	'STANDALONE',
	'00 8 * * *',
	'Y',
	'load_New_Queue_Flag_for_Alert.sh',
	'/u01/maximus/maxdat-uat/ILEBCC8/ETLJobControl/logs',
	SYSDATE,
	USER
);

INSERT INTO ETL_JOB_CONFIG (
    PROJECT_NAME,
    JOB_NAME,
    JOB_TYPE,
    JOB_SCHEDULE,
    JOB_ENABLED,
    JOB_SCRIPT_NAME,
    JOB_LOG_PATH,
    LAST_UPD_DT,
    LAST_UPD_USER)
VALUES (
	'ILEB',
	'CRON RUN DAILY IVR FILES',
	'STANDALONE',
	'00 * * * *',
	'Y',
	'cron_il_Run_Daily_IVR_Files.sh',
	'/u01/maximus/maxdat-uat/ILEBCC8/ETLJobControl/logs',
	SYSDATE,
	USER
);

COMMIT;

--SELECT * FROM ETL_JOB_GLOBAL_CONFIG;
--SELECT * FROM ETL_JOB_CONFIG;

