...
#init check
$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/${MODULE_INIT}/bpm_Init_check_test.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/${MODULE_INIT}/bpm_init_check_$(date +%Y%m%d_%H%M%S).log
 if [[ $rc == 0 || $rc = '' ]] ; then
        rm -f $INIT_OK
