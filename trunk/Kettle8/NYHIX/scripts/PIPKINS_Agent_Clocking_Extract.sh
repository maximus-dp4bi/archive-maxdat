...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/NYHIX/ETL/scripts/DP_Scorecard/20190125_1400_PIPKINS_Agent_Clocking_Extract.sh $
# $Revision: 25979 $
# $Date: 2019-01-17 12:53:30 -0500 (Thu, 17 Jan 2019) $
# $Author: wl134672 $
...
PIPKINS_Agent_Clocking_Extract_LOG="$MAXDAT_ETL_LOGS/${MODULE_DPSC}/PIPKINS_Agent_Clocking_Extract_$(date +%Y%m%d_%H%M%S).log"
echo $PIPKINS_Agent_Clocking_Extract_LOG
 
#check Data Base Connections

$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/${MODULE_INIT}/dp_scorecard_Init_check.kjb" -level="$KJB_LOG_LEVEL"  > $PIPKINS_Agent_Clocking_Extract_LOG
rc=$?
wait
if [[ $rc != 0 ]] ; then
...
$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/${MODULE_DPSC}/Pipkins_Agent_Clocking_Extract.kjb" -level="Detailed"  >> $PIPKINS_Agent_Clocking_Extract_LOG
rc=$?
wait
if [[ $rc != 0 ]] ; then
