...
EMAIL_SUBJECT="${ENV_CODE}-${STCODE}-Errors With Load Metrics"

#init check
$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/${MODULE_INIT}/bpm_Init_check.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/${MODULE_INIT}/chart_Init_check_$(date +%Y%m%d_%H%M%S).log
rc=$?
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/${MODULE_PCC}/load_ProcessMetrics.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/${MODULE_PCC}/load_ProcessMetrics_$(date +%Y%m%d_%H%M%S).log
   rc=$?
   if [[ $rc != 0 ]] ; then
