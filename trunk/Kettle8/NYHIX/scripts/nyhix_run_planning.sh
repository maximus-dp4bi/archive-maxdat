...
#init check
$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/${MODULE_INIT}/Planning_Init_check.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/${MODULE_INIT}/Planning_Init_check_$(date +%Y%m%d_%H%M%S).log
rc=$?
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/${MODULE_PP}/PP_Forecast_RUNALL.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/${MODULE_PP}/PP_Forecast_RUNALL_$(date +%Y%m%d_%H%M%S).log &
   wait
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/${MODULE_PP}/PP_Actuals_Import_RUNALL.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/${MODULE_PP}/PP_Actuals_Import_RUNALL_$(date +%Y%m%d_%H%M%S).log &
   wait
   if [[ -e $PLANNING_FAIL ]]
	then
