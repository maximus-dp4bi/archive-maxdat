...
# $URL: svn://rcmxapp1d.maximus.com/maxdat/trunk/NYHIX/ETL/scripts/nyhix_run_dp_scorecard_load.sh $
# $Revision: 19683 $
# $Date: 2017-04-14 09:35:43 -0700 (Fri, 14 Apr 2017) $
# $Author: gt83345 $
...
if [[ $rc != 0 ]] ; then
	...
	$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/${MODULE_DPSC}/Process_DP_SCORECARD_Files.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/${MODULE_DPSC}/Process_DP_SCORECARD_Files_$(date +%Y%m%d_%H%M%S).log &
	 wait
		if [[ -e $CHILD_FAIL ]]
		then
