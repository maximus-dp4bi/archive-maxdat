...
if [[ $rc != 0 ]] ; then
	...
	$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/${MODULE_DPSC}/PP_WFM_TASK_BO_30D_SYNC.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/${MODULE_DPSC}/PP_WFM_TASK_BO_30D_SYNC_$(date +%Y%m%d_%H%M%S).log &
	 wait                                                              
		if [[ -e $CHILD_FAIL ]]
		then
