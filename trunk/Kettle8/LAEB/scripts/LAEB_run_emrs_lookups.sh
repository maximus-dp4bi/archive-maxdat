...
LOOKUP_INIT_OK="$MAXDAT_ETL_PATH/${STCODE}_run_lookup_check.txt"
function error_exit
{
...
if [[ -e ${LOOKUP_INIT_OK} ]] ; then
   echo "Run Aborted - ${LOOKUP_INIT_OK} exists on ${ENV_CODE}."
   exit;
else
   echo "Starting ${STCODE}_run_bpm.sh on ${ENV_CODE}."
   rm -f ${EMAIL_MESSAGE} ${CHILD_FAIL}
   touch ${LOOKUP_INIT_OK}
fi
...
wait
         MODULE=${MODULE_EMRS}; RUN_FILE=EMRS_Load_Dimension_Enrl_Lookups; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/${MODULE}/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/run_kjb.sh ${MAXDAT_ETL_PATH}/${MODULE}/${RUN_FILE}.kjb ${KJB_LOG_LEVEL} >> ${LOG_FILE} 
