...
SGMT_INIT_OK="$MAXDAT_ETL_PATH/${STCODE}_run_segdtl_check.txt"
function error_exit
{
...
EMAIL_SUBJECT="Errors With ${STCODE}_run_emrs_eligsegmentdtl.sh in ${ENV_CODE}"

#checking for run file, Abort if it exists, create if it does not exists
if [[ -e ${SGMT_INIT_OK} ]] ; then
   echo "Run Aborted - ${SGMT_INIT_OK} exists on ${ENV_CODE}"
   exit;
else
