...
then
   ...
   $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/EMRS_set_etl_run_date.ktr EMRS_set_etl_run_date >> $MAXDAT_ETL_LOGS/EMRS_set_etl_run_date_$(date +%Y%m%d_%H%M%S).log 
                        rc=$?
   if [[ $rc == 0 ]] ; then
   $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRS_DATA_DIR/ETL_Enrollment_ADHOC_DAILY.kjb ETL_Enrollment_ADHOC_DAILY >> $MAXDAT_ETL_LOGS/ETL_Enrollment_ADHOC_DAILY_$(date +%Y%m%d_%H%M%S).log    
    fi
else
