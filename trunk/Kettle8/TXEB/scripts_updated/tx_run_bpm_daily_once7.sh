...
EMAIL_SUBJECT="Errors With ${STCODE}_run_bpm_daily_once7.sh in $ENV_CODE"
...
$MAXDAT_ETL_PATH/run_kjb7.sh $MAXDAT_ETL_PATH/ManageAlerts/ManageAlerts_Create_Instance.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/ManageAlerts_Create_Instance_$(date +%Y%m%d_%H%M%S).log &
