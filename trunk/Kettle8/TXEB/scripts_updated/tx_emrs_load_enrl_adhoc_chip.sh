...
then
   # run the sequence of scripts dependent on each one finishing successfully
      rc=$?
       if [[ $rc == 0 ]] ; then
           $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/STG_Load_S_ENROLLMENTS_CHIP_adhoc.ktr STG_Load_S_ENROLLMENTS_CHIP_adhoc >> $MAXDAT_ETL_LOGS/STG_Load_S_ENROLLMENTS_CHIP_adhoc_$(date +%Y%m%d_%H%M%S).log       
    fi
else
