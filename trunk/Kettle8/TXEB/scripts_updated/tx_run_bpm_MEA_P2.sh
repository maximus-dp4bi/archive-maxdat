...
EMAIL_SUBJECT="Errors With ${STCODE}_run_bpm_MEA_P2.sh in $ENV_CODE"
...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/Selection_txn_STG.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/Selection_txn_STG_$(date +%Y%m%d_%H%M%S).log &	
fi
