...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/TX/ETL/scripts/tx_emrs_load_aaclient_onetime.sh $
# $Revision: 7769 $
...
then
   ...
   $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/EMRS_set_etl_run_date.ktr EMRS_set_etl_run_date >> $MAXDAT_ETL_LOGS/EMRS_set_etl_run_date_$(date +%Y%m%d_%H%M%S).log 
             rc=$?
                        if [[ $rc == 0 ]] ; then
                        $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/DIM_Load_AA_CLIENT_HIST.ktr DIM_Load_AA_CLIENT_HIST >> $MAXDAT_ETL_LOGS/DIM_Load_AA_CLIENT_HIST_$(date +%Y%m%d_%H%M%S).log 
      fi
else
