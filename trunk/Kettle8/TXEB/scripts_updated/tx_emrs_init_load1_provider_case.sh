...
then
  ...
  rc=$?
   if [[ $rc == 0 ]] ; then
      $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRS_DATA_DIR/ETL_Load_AA_Dimensions_ONETIMERUN.kjb ETL_Load_AA_Dimensions_ONETIMERUN >> $MAXDAT_ETL_LOGS/EMRS_Load_AA_Dimensions_ONETIMERUN_$(date +%Y%m%d_%H%M%S).log
   rc=$?
   if [[ $rc == 0 ]] ; then
