...
then
   ...
   $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRS_DATA_DIR/ETL_Load_Dimension_Lookups.kjb ETL_Load_Dimension_Lookups >> $MAXDAT_ETL_LOGS/EMRS_Load_Dimension_Lookups_$(date +%Y%m%d_%H%M%S).log
      rc=$?
      if [[ $rc == 0 ]] ; then
         $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/DIM_Load_CLIENT_ELIGIBILITY_HIST.ktr DIM_Load_CLIENT_ELIGIBILITY_HIST >> $MAXDAT_ETL_LOGS/DIM_Load_CLIENT_ELIGIBILITY_HIST_$(date +%Y%m%d_%H%M%S).log
         rc=$?
         if [[ $rc == 0 ]] ; then
            $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/DIM_Load_CLIENT_ENRL_STATUS_HIST.ktr DIM_Load_CLIENT_ENRL_STATUS_HIST >> $MAXDAT_ETL_LOGS/DIM_Load_CLIENT_ENRL_STATUS_HIST_$(date +%Y%m%d_%H%M%S).log
            rc=$?
            if [[ $rc == 0 ]] ; then
