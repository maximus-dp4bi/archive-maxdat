...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/TX/ETL/scripts/tx_run_bpm_daily.sh $
# $Revision: 15999 $
...
EMAIL_SUBJECT="Errors With ${STCODE}_run_bpm_daily.sh in $ENV_CODE"
...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/Run_Initialization_Daily.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/${STCODE}_Run_Initialization_Daily_$(date +%Y%m%d_%H%M%S).log
 if [[ $? -eq 0 ]]
   then

      #ensure the directory structure matches and the desired kjb/ktr files are specified
      $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/ProcessLetters/Process_Letters_runall.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/Process_Letters_bpm_Let_runall_$(date +%Y%m%d_%H%M%S).log &
