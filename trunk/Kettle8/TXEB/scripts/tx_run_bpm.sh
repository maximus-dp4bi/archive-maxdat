...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/TX/ETL/scripts/tx_run_bpm.sh $
# $Revision: 16765 $
...
if [[ $rc != 0 ]] ; then
   ...
   then
      ...
      sleep 5
	  $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/ChipEligibilityEvents/ChipEligibilityEvents_RunAll.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/ChipEligibilityEvents_RunAll_$(date +%Y%m%d_%H%M%S).log &	
      sleep 5
          $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/ManageAlerts/ManageAlerts_Update_Activity.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/ManageAlerts_Update_Activity_$(date +%Y%m%d_%H%M%S).log &	      
      sleep 5
          $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/ManageAlerts/ManageAlerts_Update_Instance.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/ManageAlerts_Update_Instance_$(date +%Y%m%d_%H%M%S).log &	      
      sleep 5
          $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/ManageAlerts/ManageAlerts_Create_Review.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/ManageAlerts_Create_Review_$(date +%Y%m%d_%H%M%S).log &	      
	  wait
      if [[ -e $CHILD_FAIL ]]
         then
