...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/TX/ETL/scripts/tx_emrs_load_chip_notif_onetime.sh $
# $Revision: 7769 $
...
then
   # run the sequence of scripts dependent on each one finishing successfully
      rc=$?
       if [[ $rc == 0 ]] ; then
           $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/DIM_Load_NOTIFICATIONS_CHIP_ONETIME.ktr DIM_Load_NOTIFICATIONS_CHIP_ONETIME >> $MAXDAT_ETL_LOGS/DIM_Load_NOTIFICATIONS_CHIP_ONETIME_$(date +%Y%m%d_%H%M%S).log       
    fi
else
