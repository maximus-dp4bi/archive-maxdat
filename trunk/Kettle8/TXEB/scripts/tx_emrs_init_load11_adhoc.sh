...
then
   ...
   $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/EMRS_set_etl_run_date.ktr EMRS_set_etl_run_date >> $MAXDAT_ETL_LOGS/EMRS_set_etl_run_date_$(date +%Y%m%d_%H%M%S).log 
                        rc=$?
   if [[ $rc == 0 ]] ; then
   $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRS_DATA_DIR/ETL_Load_Cases_Clients_Adhoc.kjb ETL_Load_Cases_Clients_Adhoc >> $MAXDAT_ETL_LOGS/ETL_Load_Cases_Clients_Adhoc_$(date +%Y%m%d_%H%M%S).log    
                        rc=$?
                        if [[ $rc == 0 ]] ; then
                        $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/DIM_Load_CLIENTS_HIST.ktr DIM_Load_CLIENTS_HIST >> $MAXDAT_ETL_LOGS/DIM_Load_CLIENTS_HIST_$(date +%Y%m%d_%H%M%S).log 
             rc=$?
                        if [[ $rc == 0 ]] ; then
                        $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/DIM_Upd_CLIENTS.ktr DIM_Upd_CLIENTS >> $MAXDAT_ETL_LOGS/DIM_Upd_CLIENTS_$(date +%Y%m%d_%H%M%S).log 
      fi
     fi
    fi
else
