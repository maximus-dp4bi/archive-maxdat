...
set JOBS_DIR=%MAXDAT_ETL_PATH%\ContactCenter\main\jobs\initialization

REM set INIT_JOB=initialize_Contact_Center
set INIT_JOB=initialize_staging
...
REM 		else
			REM  failure
REM 		 	echo "%DATETIME%:  TX - Initializion of the Contact Center data mart failed.  Check error logs for more details." >> %EMAIL_MESSAGE%
REM 	  	mail -s "%EMAIL_SUBJECT%" "%EMAIL%" < %EMAIL_MESSAGE%
