...
ksh $MAXDAT_KETTLE_DIR/kitchen.sh -file="$JOBS_DIR/$JOB.kjb" -level="$LOG_LEVEL"  -param:projectName="$PROJNAME" -param:startDate=$YESTERDAY -param:endDate=$YESTERDAY >> "$LOG_DIR/$JOB$(date +%Y%m%d_%H%M%S).log"
