...
# This program will run the Kettle job necessary to load CC_S_VM_IN_QUEUE table
...
$SCRIPTS_DIR/kitchen.sh -file="$JOBS_DIR/$JOB.kjb" -log="$LOG_DIR/$JOB$(date +%Y%m%d_%H%M%S).log" -level="$LOG_LEVEL"
