...
-- Test case 2 
select 'TestCase2' as TestNumber, interval_date, contacts_handled, acd_calls_count from
...
--Test Case 5
select 'TestCase5' as TestNumber, cd.call_detail_id, cd.call_date, cd.source_call_id,cd.ani_phone_number, cd.agent_id, sa.First_name, sa.Last_name, sa.Login_id, cd.queue_name, cd.language, cd.talk_time_seconds, cd.call_abandoned_flag, cd.call_type
...
and queue_name not in ('InboundAban','No Queue')
and  (cd.queue_name='Unknown' or cd.language = 'Unknown' or cd.agent_id =0 or cd.talk_time_seconds=0)
...
from CC_S_CALL_DETAIL cd
inner join cc_s_agent sa on cd.agent_id = sa.agent_id
where cd.call_type = 'Inbound' 
and   cd.call_segment_id = 1
and   cd.queue_name not in ('InboundAban','No Queue')
and   cd.call_abandoned_flag = 0 
and  ((cd.language = 'Unknown' or cd.ani_phone_number = '0' or cd.agent_id = 0 or cd.disposition='Unknown')
      or (cd.language != 'Unknown' and cd.ani_phone_number != '0' and cd.agent_id != 0 and cd.disposition !='Unknown' and cd.talk_time_seconds=0))
and cd.call_date> to_date(:var_date1,'mm/dd/yyyy')+1--'19-SEP-2013'
order by cd.call_date desc;
...
--Test Case 9
select 'TestCase9' as TestNumber, ai.acd_interval_id, ai.interval_date, cq.queue_name, ai.service_level_answered_count, ai.contacts_handled
from cc_s_acd_interval ai
inner join cc_s_contact_queue cq on ai.contact_queue_id = cq.contact_queue_id
...
--Test Case 10
select 'TestCase10' as TestNumber, f.f_call_center_actls_intrvl_id, d.d_date, cq.queue_name, f.service_level_answered_count, f.contacts_handled, f.service_level_answered_count, f.speed_of_answer_period_8,f.speed_of_answer_period_9,f.speed_of_answer_period_10
...
--Test Case 12
select 'TestCase12' as TestNumber, agent_calls_dt, sa.agent_id, sa.Login_id, acd_agent_activity_id,login_seconds, 9*60*60 as ShiftLength, sa.First_name, sa.Last_name
from CC_S_acd_agent_activity aa
inner join cc_s_agent sa on aa.agent_id = sa.agent_id
where login_seconds > 9*60*60
and agent_calls_dt > to_date(:var_date1,'mm/dd/yyyy')--'18-SEP-2013'
order by agent_calls_dt desc;
...
--Test Case 18
select 'TestCase18' as TestNumber, faqi.F_CALL_CENTER_ACTLS_INTRVL_ID, di.interval_start_date, cq.queue_name, ROUND(mean_handle_time, 2), ROUND(((talk_time_total+after_call_work_time_total+hold_time_total)/contacts_handled), 2) as average, talk_time_total, after_call_work_time_total,hold_time_total,contacts_handled
...
order by dd.d_date desc, cq.queue_name;
...
--Test Case 21
select 'TestCase21' as TestNumber, acd_agent_activity_id, agent_calls_dt, sa.agent_id, sa.First_name, sa.Last_name, sa.Login_id
from cc_s_acd_agent_activity  aa
...
--Test Case 24
select 'Test Case 24'  as TestNumber, cq.queue_name, ai.* 
from CC_s_acd_interval ai
inner join cc_s_contact_queue cq on ai.contact_queue_id = cq.contact_queue_id
...
order by subq.agent_calls_dt desc;
...
--Test Case 35
select 'Test Case 35'  as TestNumber, si.interval_start_date, sub1.contacts_completed, sub2.contacts_created, (ABS(sub1.contacts_completed - sub2.contacts_created)/sub1.contacts_completed) as percentage from
...
and (ABS(sub1.contacts_completed - sub2.contacts_created)/sub1.contacts_completed)> 0.05
and trunc(si.interval_start_date,'DD') > to_date(:var_date1,'mm/dd/yyyy')
order by sub1.interval_id desc;
...
select 'Test Case 38 failed - no voicemail recorded in date range' as TestNumber
from 
(select count(*) as rowcount
from cc_s_call_detail cd 
where lower(queue_name) like '%vmail'
and cd.call_date > to_date(:var_date1,'mm/dd/yyyy')+1)--'19-SEP-2013'
where rowcount = 0;