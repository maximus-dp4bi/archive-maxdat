...
#  $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/TX/ETL/scripts/tx_run_emrs.sh $
#  $Revision: 5152 $
...
if [[ $rc != 0 ]] ; then
     ...
     $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRS_DATA_DIR/ETL_Enrollment.kjb ETL_Enrollment >> $MAXDAT_ETL_LOGS/ETL_Enrollment$(date +%Y%m%d_%H%M%S).log 
     rc=$?
     if [[ $rc != 0 ]] ; then
