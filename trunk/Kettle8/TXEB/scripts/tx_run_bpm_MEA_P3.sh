...
EMAIL_SUBJECT="Errors With ${STCODE}_run_bpm_MEA_P3.sh in $ENV_CODE"
...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/Client_Elig_Status_STG.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/Client_Elig_Status_STG_$(date +%Y%m%d_%H%M%S).log &	
fi
