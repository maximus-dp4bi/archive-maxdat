...
then
   ...
   $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRS_DATA_DIR/ETL_Load_Cases_Clients_Adhoc.kjb ETL_Load_Cases_Clients_Adhoc >> $MAXDAT_ETL_LOGS/ETL_Load_Cases_Clients_Adhoc_$(date +%Y%m%d_%H%M%S).log 
                        rc=$?
                        if [[ $rc == 0 ]] ; then
                        $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRS_DATA_DIR/FACT_Load_ENROLLMENTS_HIST.ktr FACT_Load_ENROLLMENTS_HIST >> $MAXDAT_ETL_LOGS/FACT_Load_ENROLLMENTS_HIST_$(date +%Y%m%d_%H%M%S).log 
     fi
    fi   
   fi
else
