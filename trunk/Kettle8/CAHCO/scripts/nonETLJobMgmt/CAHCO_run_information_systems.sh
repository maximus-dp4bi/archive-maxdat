...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/Kettle8/CAHCO/scripts/CAHCO_run_information_systems.sh $
# $Revision: 26603 $
...
INIT_OK=${MAXDAT_ETL_PATH}/${STCODE}_run_information_systems_check.txt
...
#	exit $rc
#else
 MODULE=${MODULE_IS}; RUN_FILE=Load_PIN_Query_Fact; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/${MODULE}/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/run_kjb.sh ${MAXDAT_ETL_PATH}/${MODULE}/${RUN_FILE}.kjb ${KJB_LOG_LEVEL} >> ${LOG_FILE} & 
