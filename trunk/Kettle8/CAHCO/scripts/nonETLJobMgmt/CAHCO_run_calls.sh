...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/Kettle8/CAHCO/scripts/CAHCO_run_calls.sh $
# $Revision: 26603 $
...
INIT_OK=${MAXDAT_ETL_PATH}/${STCODE}_run_calls_check.txt
...
if [[ $rc != 0 ]] ; then
	...
	exit $rc
         ...
         MODULE=${MODULE_EMRS}; RUN_FILE=HCO_Load_Call_Details; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/${MODULE}/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/run_kjb.sh ${MAXDAT_ETL_PATH}/${MODULE}/${RUN_FILE}.kjb ${KJB_LOG_LEVEL} >> ${LOG_FILE} & 
	  wait
		if [[ -e ${CHILD_FAIL} ]]
		then
