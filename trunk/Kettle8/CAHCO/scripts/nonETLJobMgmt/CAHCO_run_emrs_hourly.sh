...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/Kettle8/CAHCO/scripts/CAHCO_run_emrs_all.sh $
# $Revision: 26603 $
...
INIT_OK=${MAXDAT_ETL_PATH}/${STCODE}_run_emrs_hourly_check.txt
...
EMAIL_SUBJECT="Errors With ${STCODE}_run_emrs_hourly.sh in ${ENV_CODE}"
...
# Set the RUN_FILE to the kjb name (minus .kjb) and the MODULE the appropriate MODULE name.
          ...
          MODULE=${MODULE_EMRS}; RUN_FILE=EMRS_Load_ESR; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/${MODULE}/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/run_kjb.sh ${MAXDAT_ETL_PATH}/${MODULE}/${RUN_FILE}.kjb ${KJB_LOG_LEVEL} >> ${LOG_FILE} &
	wait
    ...
    wait
        MODULE=${MODULE_IM}; RUN_FILE=CAHCO_run_informing_materials; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/${MODULE}/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/CAHCO_run_informing_materials.sh >> ${LOG_FILE} &
    wait
	if [[ -e ${CHILD_FAIL} ]]
		then
