...
EMAIL_SUBJECT="Errors With ${STCODE}_r#un_bpm.sh in ${ENV_CODE}"
...
wait
         ...
         MODULE=${MODULE_EMRS}; RUN_FILE=HCO_Load_Research; LOG_NAME=${RUN_FILE}_$(date +%Y%m%d_%H%M%S).log; LOG_FILE="${MAXDAT_ETL_LOGS}/${MODULE}/${LOG_NAME}"; ${MAXDAT_ETL_PATH}/run_kjb.sh ${MAXDAT_ETL_PATH}/${MODULE}/${RUN_FILE}.kjb ${KJB_LOG_LEVEL} >> ${LOG_FILE} &
