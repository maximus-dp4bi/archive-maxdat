...
if [[ $rc != 0 ]] ; then
     ...
     $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRSDATADIR/FACT_Load_ENROLLMENTS_adhoc.ktr FACT_Load_ENROLLMENTS_adhoc >> $MAXDAT_ETL_LOGS/$MODULE_EMRS/EMRS_FACT_Load_ENROLLMENTS_adhoc_$(date +%Y%m%d_%H%M%S).log
     rc=$?
     if [[ $rc != 0 ]] ; then
