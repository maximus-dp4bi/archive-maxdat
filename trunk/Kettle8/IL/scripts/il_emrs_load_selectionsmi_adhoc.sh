...
if [[ $rc != 0 ]] ; then
     ...
     $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRSDATADIR/DIM_Load_SELECTION_MISSING_INFO_ADHOC.ktr DIM_Load_SELECTION_MISSING_INFO_ADHOC >> $MAXDAT_ETL_LOGS/$MODULE_EMRS/DIM_Load_SELECTION_MISSING_INFO_ADHOC_$(date +%Y%m%d_%H%M%S).log &
     rc=$?
     if [[ $rc != 0 ]] ; then
