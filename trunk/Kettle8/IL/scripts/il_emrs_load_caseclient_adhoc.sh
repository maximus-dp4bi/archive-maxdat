...
$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/emrs_Init_check.kjb" -level="$DETAIL_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/$MODULE_INIT/il_emrs_Init_check_$(date +%Y%m%d_%H%M%S).log
rc=$?
...
if [[ $rc != 0 ]] ; then
     ...
     $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRSDATADIR/DIM_Load_CASE_CLIENTS_ONETIME.ktr DIM_Load_CASE_CLIENTS_ONETIME >> $MAXDAT_ETL_LOGS/$MODULE_EMRS/DIM_Load_CASE_CLIENTS_ONETIME_$(date +%Y%m%d_%H%M%S).log &
     rc=$?
     if [[ $rc != 0 ]] ; then
