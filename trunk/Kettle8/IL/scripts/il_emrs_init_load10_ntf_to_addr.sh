...
# $Revision: 4317 $
# $Date: 2013-08-05 08:51:35 -0700 (Mon, 05 Aug 2013) $
# $Author: aa24065 $
...
then
   ...
   $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRSDATADIR/ETL_Load_Dimension_Lookups.kjb ETL_Load_Dimension_Lookups >> $MAXDAT_ETL_LOGS/$MODULE_EMRS/ETL_Load_Dimension_Lookups_$(date +%Y%m%d_%H%M%S).log
            rc=$?
            if [[ $rc == 0 ]] ; then
