...
if [[ $rc != 0 ]] ; then
     ...
     $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRSDATADIR/STG_Load_S_ENROLLMENTS_adhoc.ktr STG_Load_S_ENROLLMENTS_adhoc >> $MAXDAT_ETL_LOGS/$MODULE_EMRS/EMRS_STG_Load_S_ENROLLMENTS_adhoc_$(date +%Y%m%d_%H%M%S).log
     rc=$?
     if [[ $rc != 0 ]] ; then

	     echo "EMRS_STG_Load_S_ENROLLMENTS_adhoc failed in ILEB, review error log for additional detail." >> $EMAIL_MESSAGE
	     rm -f $INIT_OK
	     mail -s "$EMAIL_SUBJECT" "$EMAIL" < $EMAIL_MESSAGE
	     cat $EMAIL_MESSAGE
