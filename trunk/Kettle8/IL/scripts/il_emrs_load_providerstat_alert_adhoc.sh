...
EMRSDATADIR=$EMRSSCRIPTS/$MODULE_EMRS
...
if [[ $rc != 0 ]] ; then
     ...
     $MAXDAT_ETL_PATH/run_emrs_kjb.sh $EMRSDATADIR/ETL_Load_PROVIDERS_ADHOC.kjb ETL_Load_PROVIDERS_ADHOC >> $MAXDAT_ETL_LOGS/$MODULE_EMRS/ETL_Load_PROVIDERS_ADHOC_$(date +%Y%m%d_%H%M%S).log &
     $MAXDAT_ETL_PATH/run_emrs_ktr.sh $EMRSDATADIR/DIM_Load_ALERTS_ONETIME.ktr DIM_Load_ALERTS_ONETIME  >> $MAXDAT_ETL_LOGS/$MODULE_EMRS/DIM_Load_ALERTS_ONETIME_$(date +%Y%m%d_%H%M%S).log &
     rc=$?
     if [[ $rc != 0 ]] ; then

	     echo "EMRS_Load_Providers_Alert_adhoc failed in ILEB, review error log for additional detail." >> $EMAIL_MESSAGE
	     rm -f $INIT_OK
	     mail -s "$EMAIL_SUBJECT" "$EMAIL" < $EMAIL_MESSAGE
	     cat $EMAIL_MESSAGE
