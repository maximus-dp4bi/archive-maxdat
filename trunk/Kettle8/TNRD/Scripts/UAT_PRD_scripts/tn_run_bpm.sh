#!/bin/bash
#set -x
source /u01/maximus/maxdat-prd/TNRD8/scripts/.set_env
...
# $Revision: 25076 $
# $Date: 2018-10-01 13:59:04 -0400 (Mon, 01 Oct 2018) $
# $Author: aa24065 $
# =================================================================================
...
$MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AgencyReports/RedeterminationProcessing/TN_Redet_Processing_Supplemental.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/TN_Redet_Processing_Supplemental_$(date +%Y%m%d_%H%M%S).log &

wait
$MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AgencyReports/RedeterminationProcessing/TN_Load_App_Recon_Indicator.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/TN_Load_App_Recon_Indicator_$(date +%Y%m%d_%H%M%S).log &
...
$MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AgencyReports/RedeterminationProcessing/TN_Load_Processing_Timeliness.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/TN_Load_Processing_Timeliness_$(date +%Y%m%d_%H%M%S).log &

$MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/DocumentMonitoring/TN_Populate_MFD_Job.kjb $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/TN_Populate_MFD_Job_$(date +%Y%m%d_%H%M%S).log &
