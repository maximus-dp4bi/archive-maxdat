...
#   then
      $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/ProcessLetters/ProcessLetters_RUNALL_ONCE_A_DAY.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/ProcessLetters_RUNALL_ONCE_A_DAY_$(date +%Y%m%d_%H%M%S).log &
       # wait
	$MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AgencyReports/RedeterminationProcessing/TN_Load_App_Billable_Outcomes.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/TN_Load_App_Billable_Outcomes_$(date +%Y%m%d_%H%M%S).log &
        #wait  
