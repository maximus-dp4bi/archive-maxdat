...
if [[ -e $INIT_HOURLY_OK ]] ; then
   echo "Run Aborted - $INIT_HOURLY_OK exists"
   exit;
else
   echo "Starting ${STCODE}_run_hourly.sh in ${ENV_CODE}."
   rm -f $EMAIL_MESSAGE $CHILD_FAIL
   touch $INIT_HOURLY_OK
fi

# Run the Init Check to see if resources are available
$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/bpm_Init_check.kjb" -level="$KJB_LOG_LEVEL"  > $MAXDAT_ETL_LOGS/${STCODE}_hourly_init_check_$(date +%Y%m%d_%H%M%S).log
#
