...
find $MAXDAT_ETL_LOGS \( -ctime +$LOG_LIFE_DAYS -o -size 0 \) -and \(  -name '*.log' \) -exec rm -f '{}' \;
find /opt/maximus/maxdat-prd/TNRD/ReportArchive \( -ctime +30 -o -size 0 \) -and \(  -name '*.zip' \)  -exec rm -f '{}' \;
