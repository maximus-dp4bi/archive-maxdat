...
#---------------------------------
CUSTOM_DIR=$MAXDAT_ETL_PATH/AgencyReports/RedetDailyStatusFile
LOG_DIR=$MAXDAT_ETL_LOGS
LOG_NAME=`echo $0|awk -F"/" '{print substr($NF,1,length($NF)-3)}'`_$(date +%Y%m%d_%H%M%S)
LOG_FILE="${LOG_DIR}/${LOG_NAME}.log"

$MAXDAT_KETTLE_DIR/kitchen.sh -file="$CUSTOM_DIR/TN_RedetDailyStatus_Job.kjb" -log="$LOG_FILE" -level="$KTR_LOG_LEVEL"