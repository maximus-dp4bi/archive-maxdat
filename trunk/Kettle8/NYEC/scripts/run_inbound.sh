...
then
  ...
  touch $INIT_INB_OK
  $MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_DIR/$MODULE_MII/ManageInbound_Job_Start.kjb" -level="$LOG_LEVEL" >> $MAXDAT_LOG_DIR/$MODULE_MII/ManageInbound_Job_Start_$(date +%Y%m%d_%H%M%S).log
  rc=$?
  if [[ $rc != 0 ]] ; then
        rm -f $INIT_INB_OK
