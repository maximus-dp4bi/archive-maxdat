...
EMAIL_SUBJECT="${ENV_CODE}-${STCODE} Errors With run_bpm.sh"
...
if [[ $rc != 0 ]] ; then
  ...
  $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/$MODULE_INIT/Run_Initialization.kjb $KTR_LOG_LEVEL >> $MAXDAT_ETL_LOGS/$MODULE_INIT/Run_Initialization_$(date +%Y%m%d_%H%M%S).log
	if [[ $? -eq 0 ]]
	then 
...
#       $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/StateReview/ProcessStateReview_RUNALL.kjb $KTR_LOG_LEVEL >> $MAXDAT_ETL_LOGS/ProcessStateReview_RUNALL_$(date +%Y%m%d_%H%M%S).log & 
        $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/$MODULE_PC/Process_Complaints_RUN_ALL.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/$MODULE_PC/Process_Complaints_RUN_ALL_$(date +%Y%m%d_%H%M%S).log &
        $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/$MODULE_MW_V2/MW_V2_RUNALL_TIMER.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/$MODULE_MW_V2/MW_V2_RUNALL_TIMER_RUN_ALL_$(date +%Y%m%d_%H%M%S).log &
        wait
