...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/NYEC/ETL/scripts/ny_run_bpm.sh $
# $Revision: 9779 $
# $Date: 2014-04-28 14:51:03 -0600 (Mon, 28 Apr 2014) $
# $Author: mc34742 $
  ...
  $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/$MODULE_INIT/Run_Initialization.kjb $KTR_LOG_LEVEL >> $MAXDAT_ETL_LOGS/$MODULE_INIT/Run_Initialization_$(date +%Y%m%d_%H%M%S).log
	...
	then 
	  ...
	  $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/$MODULE_MW/ManageWork_RUNALL.kjb $KTR_LOG_LEVEL >> $MAXDAT_ETL_LOGS/$MODULE_MW/ManageWork_RUNALL_$(date +%Y%m%d_%H%M%S).log &
        ...
        $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/$MODULE_MI/ProcessMI_RUNALL.kjb $KTR_LOG_LEVEL >> $MAXDAT_ETL_LOGS/$MODULE_MI/ProcessMI_RUNALL_$(date +%Y%m%d_%H%M%S).log & 
