...
SELECT /*+ parallel(5) */
      ...
      he.MD_DETERMINATION
      FROM PAIEB_UAT.app_header  ah
           LEFT JOIN PAIEB_UAT.app_header_ext he
               ON (ah.application_id = he.application_id)
           LEFT JOIN PAIEB_UAT.app_elig_outcome o
           ...
           (SELECT *
              FROM (SELECT dd.application_id,
                      ...
                      FROM PAIEB_UAT.app_doc_data dd
              ...
              FROM (SELECT sh.application_id,
                           ...
                           LEAD (sh.status_cd, 1)
                               OVER (PARTITION BY sh.application_id
                                     ORDER BY sh.app_status_history_id ASC)
                               next_status
                      FROM PAIEB_UAT.app_status_history sh
  ...
  FROM (SELECT sh.application_id,
   ...
   GROUP BY T.Application_id, T.rn     ) ash
           ...
           (  SELECT i.application_id,
                     ...
                     SUM (
                         CASE
                             ...
                             ELSE 0
                         END)    mi_data_count
                FROM PAIEB_UAT.app_missing_info i
                     JOIN PAIEB_UAT.enum_app_mi_type t ON (i.mi_type_cd = t.VALUE)
               WHERE satisfied_date IS NULL
            GROUP BY application_id) mi_type
               ON (ah.application_id = mi_type.application_id)
           LEFT JOIN PAIEB_UAT.staff stf ON (TO_CHAR (stf.staff_id) = ah.created_by)
           ...
           (SELECT lr.lmreq_id, lr.sent_on sent_date, lrl.reference_id
              FROM PAIEB_UAT.letter_request  lr
                   JOIN PAIEB_UAT.letter_request_link lrl
                       ON (lr.lmreq_id = lrl.lmreq_id)
                   JOIN PAIEB_UAT.letter_definition ld ON (lr.lmdef_id = ld.lmdef_id)
           ...
           (SELECT *
              FROM (SELECT lr.lmreq_id,
                      ...
                      FROM PAIEB_UAT.letter_request  lr
                           JOIN PAIEB_UAT.letter_request_link lrl
                               ON (lr.lmreq_id = lrl.lmreq_id)
                           JOIN PAIEB_UAT.letter_definition ld
           ...
           (SELECT *
              FROM (SELECT notification_request_id,
                      ...
                      FROM PAIEB_UAT.notification_request
           ...
           (SELECT i.client_id,
              ...
              FROM PAIEB_UAT.app_header  ah
                   LEFT JOIN PAIEB_UAT.app_individual i
                   ...
                   (SELECT lr.lmreq_id, lrl.client_id, lr.sent_on sent_date
                      FROM PAIEB_UAT.letter_request_link  lrl
                           JOIN PAIEB_UAT.letter_request lr
                               ON (    lr.lmreq_id = lrl.lmreq_id
                                   AND lr.request_type = 'L'
           ...
           (  SELECT link_ref_id,
                ...
                FROM (SELECT link_ref_id,
                        ...
                        FROM (SELECT link_ref_id,
                                ...
                                FROM PAIEB_UAT.document d
                                     JOIN PAIEB_UAT.document_set s
                                         ON (d.document_set_id =
                                             s.document_set_id)
                                     JOIN PAIEB_UAT.doc_link dl
                                         ON (d.document_id = dl.document_id)
                                     JOIN PAIEB_UAT.enum_document_source ds
           ...
           /*  LEFT JOIN (SELECT link_ref_id, received_date FROM (
                           ...
                           FROM PAIEB_UAT.document d
                           JOIN PAIEB_UAT.document_set s ON (d.document_set_id = s.document_set_id)
                           JOIN PAIEB_UAT.doc_link dl ON (d.document_id = dl.document_id)
                           JOIN PAIEB_UAT.enum_document_source ds ON (s.doc_source_cd = ds.value)
                           )
           ...
           */
           ...
           (  SELECT application_id,
                ...
                FROM (SELECT mi.application_id,
                        ...
                        FROM PAIEB_UAT.app_missing_info mi
...
####################################################
 ...
 SELECT /*+ parallel(8) */ i.application_id AS app_id
  ...
  ,i.mi_ind
 FROM PAIEB_UAT.app_header ah
 JOIN PAIEB_UAT.app_individual i ON (ah.application_id = i.application_id)
 JOIN PAIEB_UAT.client c ON (i.client_id = c.clnt_client_id)
 JOIN PAIEB_UAT.app_case_link ac ON (ah.application_id = ac.application_id)
 LEFT JOIN PAIEB_UAT.app_header_ext he ON ah.application_id = he.application_id
...
FROM PAIEB_UAT.app_header ah
JOIN PAIEB_UAT.app_header_ext he ON ah.application_id = he.application_id
--LEFT JOIN harmony_conv.openclose oc ON oc.openid = ah.application_id
...
LEFT JOIN (SELECT COALESCE(a.application_id, b.application_id) as application_id, a.start_date, b.end_date, a.start_cd, b.end_cd, c.pending_start_dt, c.pending_end_dt
            FROM (SELECT l.last, l.app_status_history_id, l.application_id, l.status_dt as start_date, l.status_cd as start_cd
                  FROM (SELECT FIRST_VALUE(app_status_history_id) OVER(PARTITION BY sh.application_id, aps.inventory_indicator ORDER BY app_status_history_id) last
                    ...
                    FROM PAIEB_UAT.app_status_history sh
                    JOIN PAIEB_UAT.app_status aps ON (sh.status_cd = aps.app_status_code)
...
JOIN PAIEB_UAT.d_dates bdd ON bdd.d_date >= cast(ah.create_ts as date) 
...
FROM  PAIEB_UAT.app_case_link ac
...
LEFT JOIN (SELECT f.addr_case_id, f.addr_type_cd,f.addr_street_1,f.addr_street_2,f.addr_city,f.addr_state_cd,f.addr_zip,f.addr_ctlk_id, 'Y' as fa_active
           ...
           AND NOT EXISTS (SELECT 1 FROM PAIEB_UAT.address 
           ...
           AND EXISTS (SELECT 1 FROM PAIEB_UAT.address 
...
FROM PAIEB_UAT.ENUM_ENROLL_DELAYED_REASON r
...
FROM PAIEB_UAT.enum_length_care
WHERE effective_end_date IS NULL
UNION ALL
...
FROM PAIEB_UAT.enum_level_care
WHERE effective_end_date IS NULL
UNION ALL
...
FROM PAIEB_UAT.enum_reason_delay_code r
WHERE effective_end_date IS NULL
UNION ALL
...
FROM PAIEB_UAT.enum_subprogram_type
WHERE effective_end_date IS NULL
UNION ALL
...
FROM paieb_uat.assessment ss
 INNER JOIN paieb_uat.calendar_item ci ON (ss.calendar_item_id = ci.calendar_item_id)
 left join paieb_uat.staff stc on (stc.staff_id = ci.created_by)
 LEFT JOIN paieb_uat.staff stf ON (ci.ref_id = stf.staff_id AND ci.ref_type = 'STAFF')
 LEFT JOIN paieb_uat.groups g ON (g.group_id = stf.default_group_id AND g.type_cd = 'TEAM' AND g.group_name like ('Field EB Unit%'))
 ...
 LEFT JOIN paieb_uat.staff st_sup ON (g.supervisor_staff_id = st_sup.staff_id)
 LEFT JOIN paieb_uat.enum_calendar_item_status cst ON (ci.item_status_cd = cst.value)
 LEFT JOIN paieb_uat.enum_calendar_status_reason cir ON (ci.item_status_reason_cd = cir.value)
 LEFT JOIN paieb_uat.enum_calendar_item_reason eci ON (ci.ref_id_2 = eci.value AND ci.ref_type_2 = 'REASON')
...
LEFT JOIN paieb_uat.phone_number p ON (ac.case_id = p.phon_case_id)
                          AND phon_type_cd = 'CH'
...
FROM PAIEB_UAT.APP_ADV_PLAN_SELECTION ASEL
