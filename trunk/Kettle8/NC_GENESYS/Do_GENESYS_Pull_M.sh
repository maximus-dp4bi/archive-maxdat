...
EMAIL_PULL_SUBJECT="NC MONTHLY GENESYS CLOUD DOWNLOAD LINKS";
...
rc2=0
   rm -f $EMAIL_PULL_MESSAGE	
   ($MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/GENESYS_Pull/GC_Pull_Main_M.kjb" -param:media_type_param=voice -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/GC_Pull_Main_M_voice_$(date +%Y%m%d_%H%M%S).log)  
rc2=$(expr "$rc2" + "$?")
...
rc2=$(expr "$rc2" + "$?")
   ...
   ($MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/GENESYS_Pull/GC_Pull_Main_M.kjb" -param:media_type_param=chat -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/GC_Pull_Main_M_chat_$(date +%Y%m%d_%H%M%S).log) 
rc2=$(expr "$rc2" + "$?")
...
rc2=$(expr "$rc2" + "$?")
   ...
   ($MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/GENESYS_Pull/GC_Pull_Main_M.kjb" -param:media_type_param=callback -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/GC_Pull_Main_M_callback_$(date +%Y%m%d_%H%M%S).log) 
rc2=$(expr "$rc2" + "$?")
