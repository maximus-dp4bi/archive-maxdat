...
then
   # run the sequence of scripts dependent on each one finishing successfully
         $EMRSSRIPTS/run_emrs_kjb.sh $EMRSDATADIR/ETL_Load_Cases_HIST_ONETIMERUN.kjb ETL_Load_Cases_HIST_ONETIMERUN >> $LOG_DIR/ETL_Load_Cases_HIST_ONETIMERUN_$(date +%Y%m%d_%H%M%S).log
else
