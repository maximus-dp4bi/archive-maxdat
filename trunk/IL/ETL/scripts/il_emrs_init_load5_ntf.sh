...
then
   # run the sequence of scripts dependent on each one finishing successfully
               $EMRSSRIPTS/run_emrs_ktr.sh $EMRSDATADIR/DIM_Load_NOTIFICATIONS_HIST.ktr DIM_Load_NOTIFICATIONS_HIST >> $LOG_DIR/DIM_Load_NOTIFICATIONS_HIST_$(date +%Y%m%d_%H%M%S).log
else
