...
if [[ $rc != 0 ]] ; then
     ...
     $EMRSSCRIPTS/run_emrs_ktr.sh $EMRSDATADIR/STG_Load_S_ENROLLMENTS_adhoc.ktr STG_Load_S_ENROLLMENTS_adhoc >> $LOG_DIR/EMRS_STG_Load_S_ENROLLMENTS_adhoc_$(date +%Y%m%d_%H%M%S).log
     rc=$?
     if [[ $rc != 0 ]] ; then
