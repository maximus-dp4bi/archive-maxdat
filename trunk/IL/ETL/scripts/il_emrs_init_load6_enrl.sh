...
then
   ...
   # run the sequence of scripts dependent on each one finishing successfully
    $EMRSSRIPTS/run_emrs_kjb.sh $EMRSDATADIR/ETL_Load_Enrollment_Fact_ONETIME.kjb ETL_Load_Enrollment_Fact >> $LOG_DIR/ETL_Load_Enrollment_Fact_$(date +%Y%m%d_%H%M%S).log
   fi
else
