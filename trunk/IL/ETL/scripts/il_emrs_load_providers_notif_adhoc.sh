...
if [[ $rc != 0 ]] ; then
     ...
     $EMRSSCRIPTS/run_emrs_kjb.sh $EMRSDATADIR/ETL_Load_Enroll_Notifications_adhoc.kjb  ETL_Load_Enroll_Notifications_adhoc  >> $LOG_DIR/ETL_Load_Enroll_Notifications_adhoc_$(date +%Y%m%d_%H%M%S).log &
     rc=$?
     if [[ $rc != 0 ]] ; then
