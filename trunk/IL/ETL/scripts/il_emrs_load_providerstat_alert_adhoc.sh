...
if [[ $rc != 0 ]] ; then
     ...
     $EMRSSCRIPTS/run_emrs_kjb.sh $EMRSDATADIR/ETL_Load_PROVIDERS_ADHOC.kjb ETL_Load_PROVIDERS_ADHOC >> $LOG_DIR/ETL_Load_PROVIDERS_ADHOC_$(date +%Y%m%d_%H%M%S).log &
     $EMRSSCRIPTS/run_emrs_ktr.sh $EMRSDATADIR/DIM_Load_ALERTS_ONETIME.ktr DIM_Load_ALERTS_ONETIME  >> $LOG_DIR/DIM_Load_ALERTS_ONETIME_$(date +%Y%m%d_%H%M%S).log &
     rc=$?
     if [[ $rc != 0 ]] ; then
