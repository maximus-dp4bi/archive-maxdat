...
export MAXDAT_KETTLE_DIR=/u01/app/appadmin/product/pdi-ce-7.0.0.0-25/data-integration
export KETTLE_IL_HOME="/u01/maximus/maxdat-$ENV/$STCODE/config_7"
...
EMAIL_SUBJECT="Errors With ${STCODE}_run_bpm_7.sh in $ENV_CODE"

echo "Starting ${STCODE}_run_bpm_7.sh in ${ENV_CODE}."
# BPM init check
 $MAXDAT_KETTLE_DIR/kitchen.sh -file="$SCRIPTS_DIR/bpm_Init_check.kjb" -level="$KJB_LOG_LEVEL_DEBUG"  >> $LOGS_DIR/${STCODE}_bpm_init_check_PDI7_$(date +%Y%m%d_%H%M%S).log
...
if [[ $rc != 0 ]] ; then
	...
	$SCRIPTS_DIR/run_kjb_7.sh $SCRIPTS_DIR/ProcessLetters/ProcessLetters_RUNALL_ONCE_A_DAY.kjb $KJB_LOG_LEVEL >> $LOGS_DIR/Process_Letters_runall_PDI7_$(date +%Y%m%d_%H%M%S).log &
  
fi

echo "Finished ${STCODE}_run_bpm_7.sh in ${ENV_CODE}."
exit 0