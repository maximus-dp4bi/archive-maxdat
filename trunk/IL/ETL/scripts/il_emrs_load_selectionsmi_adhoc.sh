...
if [[ $rc != 0 ]] ; then
     ...
     $EMRSSCRIPTS/run_emrs_ktr.sh $EMRSDATADIR/DIM_Load_SELECTION_MISSING_INFO_ADHOC.ktr DIM_Load_SELECTION_MISSING_INFO_ADHOC >> $LOG_DIR/DIM_Load_SELECTION_MISSING_INFO_ADHOC_$(date +%Y%m%d_%H%M%S).log &
     rc=$?
     if [[ $rc != 0 ]] ; then
