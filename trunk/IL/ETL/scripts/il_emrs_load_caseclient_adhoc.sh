...
if [[ $rc != 0 ]] ; then
     ...
     $EMRSSCRIPTS/run_emrs_ktr.sh $EMRSDATADIR/DIM_Load_CASE_CLIENTS_ONETIME.ktr DIM_Load_CASE_CLIENTS_ONETIME >> $LOG_DIR/DIM_Load_CASE_CLIENTS_ONETIME_$(date +%Y%m%d_%H%M%S).log &
     rc=$?
     if [[ $rc != 0 ]] ; then
