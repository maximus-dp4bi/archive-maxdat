...
then
   ...
   $EMRSSRIPTS/run_emrs_kjb.sh $EMRSDATADIR/ETL_Load_Dimension_Lookups.kjb ETL_Load_Dimension_Lookups >> $LOG_DIR/ETL_Load_Dimension_Lookups_$(date +%Y%m%d_%H%M%S).log
            rc=$?
            if [[ $rc == 0 ]] ; then
