...
then
   ...
   $EMRSSRIPTS/run_emrs_kjb.sh $EMRSDATADIR/ETL_Load_Dimension_Lookups.kjb ETL_Load_Dimension_Lookups >> $LOG_DIR/EMRS_Load_Dimension_Lookups_$(date +%Y%m%d_%H%M%S).log
            rc=$?
            if [[ $rc == 0 ]] ; then
               $EMRSSRIPTS/run_emrs_ktr.sh $EMRSDATADIR/FACT_Load_ENROLLMENTS_HIST.ktr DIM_Load_NOTIFICATIONS >> $LOG_DIR/EMRS_Load_ENROLLMENTS_HIST_$(date +%Y%m%d_%H%M%S).log 
                  rc=$?
                  if [[ $rc == 0 ]] ; then
