...
then
   ...
   # run the sequence of scripts dependent on each one finishing successfully
                     $EMRSSRIPTS/run_emrs_ktr.sh $EMRSDATADIR/DIM_Load_ENROLLMENT_NOTIFICATION.ktr DIM_Load_ENROLLMENT_NOTIFICATION >> $LOG_DIR/DIM_Load_ENROLLMENT_NOTIFICATION_$(date +%Y%m%d_%H%M%S).log
   fi
else
