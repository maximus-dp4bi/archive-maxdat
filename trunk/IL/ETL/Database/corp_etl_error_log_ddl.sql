create table MAXDAT.CORP_ETL_ERROR_LOG
(
...
comment on column MAXDAT.CORP_ETL_ERROR_LOG.ERR_DATE
  is 'Date of Error, Defaults to System date' ;
comment on column MAXDAT.CORP_ETL_ERROR_LOG.err_level
  is 'Level or error - ABORT,CRITICAL,LOG Defaults to CRITICAL' ;
comment on column MAXDAT.CORP_ETL_ERROR_LOG.PROCESS_NAME
  is 'Name of process, this should identify what workbook the error came from';  
comment on column MAXDAT.CORP_ETL_ERROR_LOG.JOB_NAME
  is 'Name of Job or Transformation';  
comment on column MAXDAT.CORP_ETL_ERROR_LOG.NR_OF_ERROR
  is 'Corresponds to the Kettle Error filed of the same name';
  comment on column MAXDAT.CORP_ETL_ERROR_LOG.ERROR_DESC
  is 'Corresponds to the Kettle Error filed of the same name';
  comment on column MAXDAT.CORP_ETL_ERROR_LOG.ERROR_CODES
  is 'Corresponds to the Kettle Error filed of the same name';
  comment on column MAXDAT.CORP_ETL_ERROR_LOG.ERROR_FIELD
  is 'Corresponds to the Kettle Error filed of the same name';
  comment on column "MAXDAT"."CORP_ETL_ERROR_LOG"."DRIVER_TABLE_NAME" is 'Documents';
   COMMENT ON COLUMN "MAXDAT"."CORP_ETL_ERROR_LOG"."DRIVER_KEY_NUMBER" IS 'DCN';
   
  
  
-- Create/Recreate primary, unique and foreign key constraints 
alter table MAXDAT.CORP_ETL_ERROR_LOG
...
create or replace public synonym CORP_ETL_ERROR_LOG
  for MAXDAT.CORP_ETL_ERROR_LOG;

-- Create sequence 
create sequence MAXDAT.SEQ_CEEL_ID
minvalue 1
...
CREATE OR REPLACE TRIGGER MAXDAT.TRG_BIUR_CORP_ETL_ERROR_LOG
