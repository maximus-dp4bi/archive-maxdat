...
SELECT *
FROM PUREINSIGHTS_PRD."PUBLIC".D_PI_EVALUATIONS_VW
WHERE EVALUATIONID ='dff3d9d5-93c1-41ce-8c84-fee95c1da371'
AND PROJECTID ='2001';

SELECT RELEASEDATE,*
FROM PUREINSIGHTS_PRD."PUBLIC".D_PI_EVALUATIONS_VW
WHERE EVALUATIONID ='fe2028d5-5ced-4fa5-a235-ed4094708ff0'
AND PROJECTID ='2001';

SELECT DISTINCT projectid, projectname, evaluationid, RELEASEDATE, projectreleasedate,
FROM PUREINSIGHTS_PRD."PUBLIC".D_PI_EVALUATIONS_VW
WHERE EVALUATIONID IN ('4c4796e6-14c7-4023-a933-599f62c64b67', 'fe2028d5-5ced-4fa5-a235-ed4094708ff0', 'dff3d9d5-93c1-41ce-8c84-fee95c1da371') 
AND PROJECTID ='2001';


SELECT DISTINCT projectid, projectname, to_char(raw:id) AS evalutaion_id , to_char(raw:releaseDate) AS release_date FROM raw.evaluations WHERE raw:id IN ('4c4796e6-14c7-4023-a933-599f62c64b67', 'fe2028d5-5ced-4fa5-a235-ed4094708ff0', 'dff3d9d5-93c1-41ce-8c84-fee95c1da371', '798f7c05-e805-43ad-b4c3-addc5530b791', '797cdff1-4088-4498-933a-839360f52d98'); 
...
SELECT projectid, tablename, filename, file_timestamp, ingest_start_timestamp, ingest_end_timestamp, datediff(SECOND, ingest_start_timestamp, ingest_end_timestamp) ingest_time_seconds FROM raw.pi_files_to_ingest WHERE ingest_start_timestamp >= '2022-09-19';
SELECT count(*) FROM (SELECT projectid, tablename, filename, ingest_start_timestamp, ingest_end_timestamp, datediff(SECOND, ingest_start_timestamp, ingest_end_timestamp) ingest_time_seconds FROM raw.pi_files_to_ingest WHERE ingest_start_timestamp >= '2022-09-19');
