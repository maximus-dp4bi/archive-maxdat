...
#merge file list log tables data into main log table in case they weren't merged in the previous run
#${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=tsv -q "call raw.MergeProjectFileListsForIngestion(${REINGEST_FLAG});"

#recreate project files-to-ingest tables
${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=tsv -q "call raw.createProjectFilesListTables(${REINGEST_FLAG});"

#get projects currently configured as active for this timezone and kick off ingestion scripts

project_array=()
configured_project_count=0
project_array=($(${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=tsv -q "select projectid from ${DB}.public.D_PI_INGESTION_PROJECTS_VW;")) 
...
${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=plain -q " call ${DB}.raw.ingestUningestedPIData_V2(${PROJECTID},${REINGEST_FLAG},${INGESTALL_FLAG},false,true,'${AFTERTIMESTRING}','${BEFORETIMESTRING}','${LOOKBACKDAYS}','${TABLENAME}');"&
pid_list+=" $!"
done
...
then 
echo -e "${failed_rc_count} PI data ingestion processes failed to complete on initial run. Please check the log for details. \n" >> $INGESTION_LOG
fi


#merge project log tables data into main log table
${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=tsv -q "call raw.MergeProjectLogsForIngestion(${REINGEST_FLAG},null);"

#merge file list log tables data into main log table
${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=tsv -q "call raw.MergeProjectFileListsForIngestion(${REINGEST_FLAG});"
...
missing_table_array=($(${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=plain -q " with tables_and_sps as (
       select table_name as object_name, s3_sourced as s3_sourced,active as active
       from ${DB}.public.d_pi_tables
       union
       select sp_name as object_name, to_boolean('true') as s3_sourced, active as active
       from ${DB}.public.d_pi_ingestion_sps
       )
  select distinct (pr.projectid)
from ${DB}.public.D_PI_INGESTION_PROJECTS_VW pr
left outer join tables_and_sps ta
where ta.s3_sourced = true and ta.active = true
and not exists (select 1 from ${DB}.public.d_pi_project_unavailable_tables ua
               where ua.project_id = pr.projectid
               and ua.table_name = ta.object_name
...
and not exists (select 1 from ${DB}.raw.ingest_pi_data_det_log dl
               ...
               and dl.object_category in ('TABLE','TABLE REINGEST','SP','SP REINGEST')
               and dl.object_name = ta.object_name
...
${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=plain -q " call ${DB}.raw.ingestUningestedPIData_V2(${PROJECTID},${REINGEST_FLAG},false,false,true,'${AFTERTIMESTRING}','${BEFORETIMESTRING}','${LOOKBACKDAYS}','${TABLENAME}');"&
pid_list+=" $!"
done
...
echo -e "${failed_rc_count_subs} PI data ingestion processes failed to complete on subsequent run. Please check the log for details. \n" >> $INGESTION_LOG
fi
...
from ${DB}.public.D_PI_INGESTION_PROJECTS_VW pr
where not exists (select 1 from ${DB}.raw.ingest_pi_data_det_log dl
...
failed_count=0
failed_count=$(${SNOW_DIR}/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=plain -q "select count(*) from (
  with  max_table_ingest_ts as (
...
group by dl2.projectid, dl2.object_name
 ...
 max_sp_ingest_ts as (
  select dl2.projectid, dl2.object_name, max(dl2.ts) as ts from raw.ingest_pi_data_det_log dl2
               where dl2.object_category in ('SP','SP REINGEST')
               and dl2.status_string = 'SUCCEEDED'
                and date(convert_timezone('UTC','America/New_York',to_timestamp(to_varchar(dl2.ts)))) = date(current_timestamp())
group by dl2.projectid, dl2.object_name
)
select projectname, convert_timezone('UTC','America/New_York',to_timestamp(to_varchar(dl.ts))) as ingestion_time, dl.msg
...
left outer join public.D_PI_INGESTION_PROJECTS_VW pr
on dl.projectid = pr.projectid
where (pr.active = true or dl.projectid = 'ALL')
and upper(dl.status_string) = 'FAILED'
and date(convert_timezone('UTC','America/New_York',to_timestamp(to_varchar(dl.ts)))) = date(current_timestamp())
and object_category not in ('SP','SP REINGEST')
  and ((max_table_ts.ts is null) or (dl.ts > max_table_ts.ts) 
     or (object_category not in  ('TABLE','TABLE REINGEST')))
and not exists (select 1 from public.d_pi_project_unavailable_tables ut
 ...
 select projectname, convert_timezone('UTC','America/New_York',to_timestamp(to_varchar(dl.ts))) as ingestion_time, dl.msg
from raw.ingest_pi_data_det_log dl
 left outer join max_sp_ingest_ts max_sp_ts
 on dl.projectid = max_sp_ts.projectid
 and dl.object_name = max_sp_ts.object_name
left outer join public.D_PI_INGESTION_PROJECTS_VW pr
on dl.projectid = pr.projectid
where (pr.active = true or dl.projectid = 'ALL')
and upper(dl.status_string) = 'FAILED'
and object_category not in ('TABLE','TABLE REINGEST')
and date(convert_timezone('UTC','America/New_York',to_timestamp(to_varchar(dl.ts)))) = date(current_timestamp())
and ((max_sp_ts.ts is null) or (dl.ts > max_sp_ts.ts) or (object_category not in  ('SP','SP REINGEST')))
...
and ((max_sp_ts.ts is null) or (dl.ts > max_sp_ts.ts) or (object_category not in  ('SP','SP REINGEST')))
 ...
 order by 1,2 desc; " >> $INGESTION_LOG 
fi
...
left outer join ${DB}.public.D_PI_INGESTION_PROJECTS_VW pr
on dl.projectid = pr.projectid
where upper(dl.status_string) = 'SUCCEEDED'
...
left outer join ${DB}.public.D_PI_INGESTION_PROJECTS_VW pr
on dl.projectid = pr.projectid
where upper(dl.status_string) = 'WARNING'
and date(convert_timezone('UTC','America/New_York',to_timestamp(to_varchar(dl.ts)))) = date(current_timestamp()) ; ")
