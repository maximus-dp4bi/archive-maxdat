...
project_array=($(/snowsql/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=tsv -q "select projectid from ${DB}.public.d_pi_projects where projectid = ${PROJECTID};")) 
...
/snowsql/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=plain -q " call ${DB}.raw.ingestUningestedPIData(${PROJECTIDS},false,false,true);"&
pid_list+=" $!"
done
...
/snowsql/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=plain -q " call ${DB}.stage.S_PI_PRJ_SESSION_CONTACT_SUMMARY_LOAD(${PROJECTID});"
...
/snowsql/snowsql -c ${INGEST_C} -o exit_on_error=true -o friendly=false -o header=false -o timing=false -o output_format=plain -q " call ${DB}.stage.update_groups_membership_daily_snapshot(${PROJECTID},${REINGEST_FLAG});"
...
from ${DB}.raw.ingest_pi_data_det_log dl
                where dl.projectid = '${PROJECTID}' and upper(dl.object_name) = 'S_PI_PRJ_SESSION_CONTACT_SUMMARY_LOAD' and upper(dl.status_string) = 'SUCCEEDED'
                and date(convert_timezone('UTC','America/New_York',to_timestamp(to_varchar(dl.ts)))) = date(current_timestamp());")
...
from ${DB}.raw.ingest_pi_data_det_log dl
                where dl.projectid = '${PROJECTID}' and upper(dl.object_name) = 'UPDATE_GROUPS_MEMBERSHIP_DAILY_SNAPSHOT' and upper(dl.status_string) = 'SUCCEEDED'
                and date(convert_timezone('UTC','America/New_York',to_timestamp(to_varchar(dl.ts)))) = date(current_timestamp());")
...
then
echo -e "\nThe groups membership daily snapshot process for misc project does not have a success indicator in the PI data ingestion log: \n" >> $INGESTION_LOG
fi
...
then
echo -e "\nGroups membership daily snapshot procedure completed successfully for misc project. \n" >> $INGESTION_LOG_SUCCESS
fi
