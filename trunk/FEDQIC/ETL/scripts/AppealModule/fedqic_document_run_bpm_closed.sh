...
# $URL: svn://svn-staging.maximus.com/dev1d/maxdat/trunk/FEDQIC/ETL/scripts/corp_run_bpm.sh $
# $Revision: 10349 $
...
function error_exit
{
   ...
   rm -f $INIT_OK_DOCUMENT
   exit 1
}

# Checking for run file - Abort if it exists, otherwise create it
if [[ -e $INIT_OK_DOCUMENT ]] ; then
   echo "Run Aborted - $INIT_OK_DOCUMENT exists"
   exit;
else
   echo "Starting ${STCODE}_document_run_bpm.sh in ${ENV_CODE}."
   rm -f $EMAIL_MESSAGE $CHILD_FAIL
   touch $INIT_OK_DOCUMENT
fi
...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/Initialize_Documents_Closed.kjb $KJB_LOG_LEVEL > $MAXDAT_ETL_LOGS/${STCODE}_Initialize_Documents_$(date +%Y%m%d_%H%M%S).log
...
#       the lines you do not want entirely. Do not leave any "->" without a "#" or the script will error.
       $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/ProcessDocuments_RUNALL.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/ProcessDocuments_RUNALL_$(date +%Y%m%d_%H%M%S).log &
