...
if [[ -e $INIT_OK_APPEAL ]] ; then
   echo "Run Aborted - $INIT_OK_APPEAL exists"
   exit;
else
   echo "Starting ${STCODE}_appeal_run_bpm.sh in ${ENV_CODE}."
   rm -f $EMAIL_MESSAGE $CHILD_FAIL
   touch $INIT_OK_APPEAL
fi
...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/Initialize_Appeals.kjb $KJB_LOG_LEVEL > $MAXDAT_ETL_LOGS/${STCODE}_Initialize_Appeals_$(date +%Y%m%d_%H%M%S).log
...
#       the lines you do not want entirely. Do not leave any "->" without a "#" or the script will error.
       $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/ProcessAppeals_RUNALL.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/ProcessAppeals_RUNALL_$(date +%Y%m%d_%H%M%S).log &
