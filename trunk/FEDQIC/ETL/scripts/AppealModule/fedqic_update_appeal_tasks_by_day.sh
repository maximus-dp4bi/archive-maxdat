...
if [[ -e $init_ok_atbd ]] ; then
   echo "Run Aborted - $init_ok_atbd exists"
   exit;
else
...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/daily_update_appeal_tasks_by_day.kjb $KJB_LOG_LEVEL > $MAXDAT_ETL_LOGS/${STCODE}_update_appeal_tasks_by_date_$(date +%Y%m%d_%H%M%S).log
...
#   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/load_lookup_tables.kjb $KJB_LOG_LEVEL > $MAXDAT_ETL_LOGS/${STCODE}_load_lookup_tables_$(date +%Y%m%d_%H%M%S).log
