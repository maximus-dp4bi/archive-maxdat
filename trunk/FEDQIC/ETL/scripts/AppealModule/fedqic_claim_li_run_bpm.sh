...
function error_exit
{
   ...
   rm -f $INIT_OK_CLAIM_LI
   exit 1
}

# Checking for run file - Abort if it exists, otherwise create it
if [[ -e $INIT_OK_CLAIM_LI ]] ; then
   echo "Run Aborted - $INIT_OK_CLAIM_LI exists"
   exit;
else
   echo "Starting ${STCODE}_claim_run_bpm.sh in ${ENV_CODE}."
   rm -f $EMAIL_MESSAGE $CHILD_FAIL
   touch $INIT_OK_CLAIM_LI
fi
...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/Initialize_Claims_LI.kjb $KJB_LOG_LEVEL > $MAXDAT_ETL_LOGS/${STCODE}_Initialize_Claims_LI_$(date +%Y%m%d_%H%M%S).log
...
#       the lines you do not want entirely. Do not leave any "->" without a "#" or the script will error.
       $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/ProcessClaims_LI_RUNALL.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/ProcessClaims_LI_RUNALL_$(date +%Y%m%d_%H%M%S).log &
