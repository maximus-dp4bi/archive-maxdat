...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/daily_update_appeals_by_day_by_part.kjb $KJB_LOG_LEVEL > $MAXDAT_ETL_LOGS/${STCODE}_update_appeals_by_date_by_art_$(date +%Y%m%d_%H%M%S).log
