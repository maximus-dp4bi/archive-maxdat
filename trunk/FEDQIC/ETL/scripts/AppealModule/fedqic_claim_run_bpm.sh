...
if [[ -e $INIT_OK_CLAIM ]] ; then
   echo "Run Aborted - $INIT_OK_CLAIM exists"
   exit;
else
...
if [[ $rc != 0 ]] ; then
   ...
   $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/Initialize_Claims.kjb $KJB_LOG_LEVEL > $MAXDAT_ETL_LOGS/${STCODE}_Initialize_Claims_$(date +%Y%m%d_%H%M%S).log
...
#       the lines you do not want entirely. Do not leave any "->" without a "#" or the script will error.
       $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/AppealModule/ProcessClaims_RUNALL.kjb  $KJB_LOG_LEVEL   > $MAXDAT_ETL_LOGS/ProcessClaims_RUNALL_$(date +%Y%m%d_%H%M%S).log &
