...
strTargetColumn = ReadExcelFile_GetCellValueByColumnNameAndRowIndex_2(cDataFileURL, cTargetColumn, iIndex_Detail, strWorkbook_Detail)
strTargetWhereClause = ReadExcelFile_GetCellValueByColumnNameAndRowIndex_2(cDataFileURL, cTargetWhereClause, iIndex_Detail, strWorkbook_Detail)
strTargetDateColumn = ReadExcelFile_GetCellValueByColumnNameAndRowIndex_2(cDataFileURL, cTargetDateColumn, iIndex_Detail, strWorkbook_Detail)

If strTargetWhereClause <> "SQL" Then
strValue_Target = Snowflake_ReadDB_ReturnSingleValue_D_DATE_Dynamic(strUser, strPassword, strTargetColumn, strTargetSchema, strTargetView, strTargetProjectID, strTargetWhereClause, strTargetDateColumn, strSF_Connection)
Else
strValue_Target = Snowflake_ReadDB_ReturnSingleValue_ExternalSQL(strUser, strPassword, strWorkbook_Detail, strTargetColumn, strTargetSchema, strTargetView, strSQL_Location, strSF_Connection)
End  If
...
' Convert expected value from "#MM/DD/YYY#" to "YYYY-MM-DD"
strExpectedValue = replace(strExpectedValue, "#", "")
strIsDate = IsDate(strExpectedValue)
If strIsDate = true Then
strExpectedValue = ConvertDate_from_MMslashDDslashYYYY_to_YYYYdashMMdashDD(strExpectedValue)	
End If

' Remove leading zeros from numeric value
strExpectedValue = Number_RemoveLeadingZeros(strExpectedValue)
