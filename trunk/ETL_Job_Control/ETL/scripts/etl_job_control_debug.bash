...
echo "DB_MAXDAT_PASSWORD=${DB_MAXDAT_PASSWORD}"

if [ "${DB_MAXDAT_PASSWORD:0:9}" = "Encrypted" ]; then
	echo "DB_MAXDAT_PASSWORD IS ENCRYPTED"

	DB_MAXDAT_ENCRYPTED_PASSWORD=$(grep "^DB_MAXDAT_PASSWORD" ${KETTLE_PROPERTIES_FILE} | cut -d'=' -f2 | cut -d' ' -f2)
	echo "DB_MAXDAT_ENCRYPTED_PASSWORD=${DB_MAXDAT_ENCRYPTED_PASSWORD}"
...
echo "${DB_MAXDAT_CLIENT} -S ${DB_MAXDAT_USER}/${DB_MAXDAT_PASSWORD}@//${DB_MAXDAT_HOSTNAME}:${DB_MAXDAT_PORT}/${DB_MAXDAT_NAME} @${ETL_JOBCONTROL_SQL_PATH}/set_etl_jobs.sql > ${JOB_CONFIG_RESULTS_FILE}"
