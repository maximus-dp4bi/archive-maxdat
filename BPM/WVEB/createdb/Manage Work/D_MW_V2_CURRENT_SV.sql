...
AS
  SELECT SI.STEP_INSTANCE_ID AS MW_BI_ID
    ...
    ccbs.curr_created_by_staff_id ,
    DECODE(SI.ESCALATED_IND, 1, 'Y', 'N') AS ESCALATED_FLAG ,
    cets.curr_escalated_to_staff_id ,
    cfbs.curr_forwarded_by_staff_id ,
  ...
  LEFT JOIN
    (SELECT ext_staff_number AS curr_created_by_staff_id 
      FROM staff 
      UNION 
      SELECT TO_CHAR(staff_id) AS curr_created_by_staff_id 
      FROM staff
    ) ccbs ON SI.CREATED_BY = ccbs.curr_created_by_staff_id
  LEFT JOIN
    (SELECT ext_staff_number AS curr_escalated_to_staff_id 
      FROM staff 
      UNION 
      SELECT TO_CHAR(staff_id) AS curr_escalated_to_staff_id 
      FROM staff
    ) cets ON SI.ESCALATE_TO = cets.curr_escalated_to_staff_id
  LEFT JOIN
    (SELECT ext_staff_number AS curr_forwarded_by_staff_id 
      FROM staff 
      UNION 
      SELECT TO_CHAR(staff_id) AS curr_forwarded_by_staff_id 
      FROM staff
    ) cfbs ON SI.FORWARDED_BY = cfbs.curr_forwarded_by_staff_id
  LEFT JOIN
    (SELECT ext_staff_number AS curr_owner_staff_id 
      FROM staff 
      UNION 
      SELECT TO_CHAR(staff_id) AS curr_owner_staff_id 
      FROM staff
    ) cosi                                ON SI.OWNER = cosi.curr_owner_staff_id
  LEFT JOIN maxdat_lookup.d_task_types dt ON (dt.task_type_id = si.step_definition_id)
  LEFT JOIN
    (SELECT h.owner, 
            h.step_instance_id, 
            h.status 
    ...
    --D_MW_V2_CURRENT_SV
  WHERE si.completed_ts >= add_months(TRUNC(sysdate,'mm'),-8)
  OR si.status NOT IN('TERMINATED','COMPLETED') ;
  
  GRANT SELECT ON MAXDAT_LOOKUP.D_MW_V2_CURRENT_SV TO EB_MAXDAT_REPORTS ;
