...
,cl.clnt_fname||CASE WHEN SUBSTR(cl.clnt_mi,1,1) IS NULL THEN ' ' ELSE ' '||SUBSTR(cl.clnt_mi,1,1)||' ' END ||cl.clnt_lname AS FULL_NAME
...
,CASE WHEN EXTRACT(YEAR FROM(TRUNC(sysdate) - clnt_dob) YEAR TO MONTH) < 1 THEN 'Y' ELSE 'N' END AS NEWBORN
,CASE WHEN EXTRACT(YEAR FROM(TRUNC(sysdate) - clnt_dob) YEAR TO MONTH) < 19 THEN 'C' ELSE 'A' END AS ADULT_OR_CHILD
,CASE WHEN (SELECT COUNT(1) FROM eb.other_health_care ohc WHERE ohc.ohc_client_id = cl.clnt_client_id AND (ohc.ohc_end_date > SYSDATE OR ohc.ohc_end_date IS NULL)) > 0 THEN 'Y' ELSE 'N' END AS THIRD_PARTY_RESOURCES_AVAIL
