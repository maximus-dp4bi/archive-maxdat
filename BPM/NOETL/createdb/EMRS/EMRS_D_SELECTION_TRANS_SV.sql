...
--    COALESCE(st.CLIENT_AID_CATEGORY_CD, ss.CLIENT_AID_CATEGORY_CD, '0') AS SUBPROGRAM_CODE, --on Aid Category, this field is not used
    ...
    con.plan_service_type_cd prior_plan_service_type
  ...
  left join eb.contract con on (st.prior_contract_id = con.contract_id and st.prior_plan_id = con.plan_id)
  WHERE (st.create_Ts >= ADD_MONTHS(TRUNC(sysdate, 'mm'), -3)) --and COALESCE(ss.end_nd, st.end_nd) > COALESCE(ss.start_nd, st.start_nd) );
