...
FROM
(
SELECT /*+ parallel(10) */ ---INDEX_DESC(CR) INDEX_desc(SELC) INDEX_DESC(ST) index_desc(crl) index_desc(CSCL) index_desc(SMID) */
CR.CALL_RECORD_ID
, CR.CALL_TYPE_CD
...
WHERE 1=1
AND CR.CALL_RECORD_ID >= 26310000
...
WHERE CA.ROWN = 1
AND CA.CALL_DATE >= GREATEST(TO_DATE('1/1/2022','MM/DD/YYYY'), ADD_MONTHS(TRUNC(SYSDATE), -13))
;
