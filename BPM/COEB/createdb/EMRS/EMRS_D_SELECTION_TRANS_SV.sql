CREATE OR REPLACE VIEW EMRS_D_SELECTION_TRANS_SV AS
  SELECT /*+ parallel(10) */ ----INDEX_DESC(st) INDEX_DESC(ss) INDEX_DESC(ev) */
...
--    COALESCE(st.CLIENT_AID_CATEGORY_CD, ss.CLIENT_AID_CATEGORY_CD, '0') AS SUBPROGRAM_CODE, --on Aid Category, this field is not used
  ...
  left JOIN &schema_name..selection_segment ss on (st.client_id = ss.client_id and st.selection_segment_id = ss.selection_segment_id)
  left join &schema_name..contract con on (st.contract_id = con.contract_id and st.plan_id = con.plan_id)
  left join &schema_name..contract conp on (st.prior_contract_id = conp.contract_id and st.prior_plan_id = conp.plan_id)
  LEFT JOIN &schema_name..event ev on (EV.REF_TYPE = 'SELECTION_TXN' 
  					AND EV.REF_ID = ST.SELECTION_TXN_ID 
  					AND EV.EVENT_TYPE_CD IN ('ENROLL_PCP_TRANSFER_SUBMITTED','ENROLL_TRANSFER_SUBMITTED','CHOICE_ENROLLMENT','INCOMPLETE_INVALID_CHOICE') 
  					AND EV.CALL_RECORD_ID IS NOT NULL)
  WHERE (st.selection_txn_id >= 16710000 and st.create_Ts >= ADD_MONTHS(TRUNC(sysdate, 'mm'), -13)) --and COALESCE(ss.end_nd, st.end_nd) > COALESCE(ss.start_nd, st.start_nd) );
