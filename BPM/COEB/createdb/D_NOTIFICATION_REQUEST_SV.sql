...
FROM eb.letter_request lr 
  ...
  LEFT JOIN (SELECT *
                   FROM (SELECT h.notification_request_id,
                                ...
                                ROW_NUMBER() OVER(PARTITION BY h.notification_request_id ORDER BY h.create_ts DESC) crn
                         FROM eb.nreq_status_history h                            
                         WHERE status_cd IN('EMAIL_FAIL','EMAIL_FAILED_IN_TRANSIT','FAILED','ERR','BLOCK BOUNCE','HARD BOUNCE','SOFT BOUNCE','UNKNOWN BOUNCE','TECHNICAL BOUNCE'))
 ...
 LEFT JOIN (SELECT *
                   FROM (SELECT h.notification_request_id,
                                ...
                                ROW_NUMBER() OVER(PARTITION BY h.notification_request_id ORDER BY h.create_ts DESC) crn
                         FROM eb.nreq_status_history h                            
                         WHERE status_cd IN('SMS_FAIL','SMS_FAILED_IN_TRANSIT','FAILED','ERR','BLOCK BOUNCE','HARD BOUNCE','SOFT BOUNCE','UNKNOWN BOUNCE','TECHNICAL BOUNCE'))
  ...
  LEFT JOIN (SELECT *
                   FROM (SELECT h.notification_request_id,
                                ...
                                ROW_NUMBER() OVER(PARTITION BY h.notification_request_id ORDER BY h.create_ts DESC) crn
                         FROM eb.nreq_status_history h                            
                         WHERE status_cd IN('CLICKED','CLICK'))
