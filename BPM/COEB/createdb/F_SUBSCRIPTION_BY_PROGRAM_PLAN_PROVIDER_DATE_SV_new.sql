...
AS
with planinfo as (
SELECT /*+ parallel(5) */ COALESCE(con.plan_service_type_cd,'UD') plan_service_type_cd, COALESCE(pl.plan_id_ext,'0')plan_id_ext ,COALESCE(pl.plan_id,0) plan_id,cc.cscl_case_id case_id
          ...
          and slct.end_nd >= to_number(to_char(ADD_MONTHS(TRUNC(sysdate, 'mm'), -8),'YYYYMMDD'))
...
SELECT cs.*
  ...
  LEFT JOIN (select * from planinfo)  slct ON slct.case_id = cs.case_id AND cs.event_date BETWEEN TRUNC(slct.start_date) AND COALESCE(TRUNC(slct.end_date),TO_DATE('12/31/2099','mm/dd/yyyy'))     
)
WHERE rown = 1   
UNION ALL
SELECT *
...
SELECT *
 ...
 SELECT cs.*
       ...
       --  ,ROW_NUMBER() OVER(PARTITION BY cs.case_id,cs.event_date ORDER BY slct.selection_segment_id ) rown
        ,ROW_NUMBER() OVER(PARTITION BY cs.case_id,cs.event_date ORDER BY slct_close_ind,selection_segment_id ) rown
    FROM(SELECT /*+ parallel(5) */ DISTINCT cl.case_id,TRUNC(cr.create_ts) event_date
          ,0 email_only
          ,0 none_email_only
          ,0 none_email_only_call
          ,0 sms_only
    ...
    FROM EB.CALL_RECORD cr
        JOIN EB.call_record_link cl ON cl.call_record_id = cr.call_record_id                                      
    WHERE cr.call_record_id >= 26350000
    AND TRUNC(cr.create_ts) >= ADD_MONTHS(TRUNC(sysdate,'mm'),-6) 
    AND NOT EXISTS(SELECT 1 FROM EB.event eprev WHERE eprev.case_id = cl.case_id AND eprev.event_type_cd IN('OPT_IN_EMAIL','OPT_IN_SMS','CASE_PREFERRED_TO_DIGITAL') AND TRUNC(eprev.create_ts) <= TRUNC(cr.create_ts)) 
    AND NOT EXISTS(SELECT 1 
                    FROM EB.EVENT e 
                      JOIN EB.cases cs ON e.case_id = cs.case_id                              
                      JOIN EB.ENUM_MANUAL_CALL_ACTION ma ON (e.context = ma.value) 
                      JOIN EB.ENUM_MAN_CALL_ACT_CATEGORIES mct ON mct.value = ma.categories
