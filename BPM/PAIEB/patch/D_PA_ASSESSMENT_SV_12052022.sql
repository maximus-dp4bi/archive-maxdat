...
FROM assessment ss
 ...
 left join staff stc on (stc.staff_id = ci.created_by)
 LEFT JOIN staff stf ON (ci.ref_id = stf.staff_id AND ci.ref_type = 'STAFF')
 LEFT JOIN groups g ON (g.group_id = stf.default_group_id AND g.type_cd = 'TEAM' AND (g.group_name like ('Field EB Unit%') or g.group_name = 'OUTREACH TEAM 1'))
...
GRANT SELECT ON MAXDAT_SUPPORT.D_PA_ASSESSMENT_SV TO MAXDATSUPPORT_PFP_READ_ONLY;
