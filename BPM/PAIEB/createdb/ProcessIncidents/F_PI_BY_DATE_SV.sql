...
AS
  SELECT i.incident_header_id PI_BI_ID ,
    ...
    CASE
      WHEN status_cd IN(SELECT OUT_VAR FROM  maxdat_support.CORP_ETL_LIST_LKUP WHERE name = 'INCIDENT_CLOSED_STATUS')
      THEN i.status_ts
      ELSE NULL
    END COMPLETE_DATE ,
    (
    ...
    CASE
      WHEN I.status_cd IN(SELECT OUT_VAR FROM  maxdat_support.CORP_ETL_LIST_LKUP WHERE name = 'INCIDENT_CLOSED_STATUS') AND D.D_DATE = TRUNC(i.status_ts)
      THEN 0
      ELSE 1
  ...
  JOIN ATS.incident_header i ON (D.D_DATE BETWEEN TRUNC(I.CREATE_TS)   
                                  AND CASE WHEN i.status_cd IN(SELECT OUT_VAR FROM  maxdat_support.CORP_ETL_LIST_LKUP WHERE name = 'INCIDENT_CLOSED_STATUS') THEN TRUNC(I.STATUS_TS)
                                      ELSE TRUNC(sysdate)
                                      END)
--  WHERE i.incident_header_type_cd IN ('COMPLAINT','LIAISON')
