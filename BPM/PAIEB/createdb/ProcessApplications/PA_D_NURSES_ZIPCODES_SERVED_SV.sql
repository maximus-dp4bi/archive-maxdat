...
AS
select nz.nurse_zipcode_id as nurse_zipcode_id
      ...
      ,CASE WHEN LENGTH(TRIM(su.LAST_NAME)) < 1 THEN TRIM(su.FIRST_NAME) 
             WHEN LENGTH(TRIM(su.FIRST_NAME)) < 1 THEN TRIM(su.LAST_NAME)
             ELSE TRIM(su.LAST_NAME) || ', ' || TRIM(su.FIRST_NAME) 
             END AS updated_by
...
left outer join   ats.staff su on (su.staff_id = nz.updated_by)
union
