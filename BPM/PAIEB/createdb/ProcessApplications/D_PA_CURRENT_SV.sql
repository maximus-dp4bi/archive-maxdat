...
SELECT ah.application_id app_id
       ...
       ,COALESCE(pa600_doc.received_date, oc.pa600receiveddate) pa600_received_date
       ...
       ,CASE WHEN COALESCE(denied_incomp_ash_id, 0) > COALESCE(denied_inelig_ash_id, 0) AND COALESCE(denied_incomp_ash_id, 0) > COALESCE(approved_ash_id, 0) 
              THEN COALESCE(hcbsden_doc.received_date,ash.approved_status_dt, ash.denied_inelig_status_dt, ash.denied_incomp_status_dt) 
...
FROM app_header ah 
  ...
  LEFT JOIN (SELECT comp.application_id, comp.status_date, comp.status_cd, comp.rn
             ...
             FROM app_status_history sh) comp
  ...
  LEFT JOIN (SELECT i.client_id
              ...
              WHERE ah.status_cd NOT IN('ENROLLED', 'CLOSED','COMPLETED')) pa600_ltr ON (ah.application_id = pa600_ltr.application_id and pa600_ltr.rn = 1)
  --letter  
 --doc received 
  ...
  LEFT JOIN (SELECT * 
              FROM (SELECT link_ref_id,s.received_date,ROW_NUMBER() OVER (PARTITION BY link_ref_id ORDER BY s.received_date) rn
                FROM document d 
                JOIN document_set s ON (d.document_set_id = s.document_set_id)
                JOIN doc_link dl ON (d.document_id = dl.document_id) 
                JOIN enum_document_source ds ON (s.doc_source_cd = ds.value)
