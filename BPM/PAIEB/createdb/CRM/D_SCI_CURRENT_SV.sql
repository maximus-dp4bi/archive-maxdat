...
LEFT JOIN ATS.staff sw ON (cr.worker_id = sw.staff_id)
LEFT JOIN ATS.staff cb ON (cr.created_by = to_char(cb.staff_id))
LEFT JOIN ATS.staff ub ON (cr.updated_by = to_char(ub.staff_id))
LEFT JOIN ATS.enum_language l ON (l.value = cr.language_cd)
LEFT JOIN ATS.enum_caller_type ct ON (ct.value = cr.caller_type_cd)
LEFT JOIN ATS.enum_call_type clt ON (clt.value = cr.call_type_cd)
...
LEFT JOIN (SELECT crl.CALL_RECORD_ID,sum(case when cs.CASE_NEED_TRANSLATOR_IND = 'Y' then 1 else 0 end) as TRANSLATION_REQ
            FROM ATS.CALL_RECORD_LINK crl 
            JOIN ATS.CASES cs ON (crl.CASE_ID = cs.CASE_ID)            
