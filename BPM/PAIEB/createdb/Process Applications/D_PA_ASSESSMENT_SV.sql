CREATE OR REPLACE VIEW D_PA_ASSESSMENT_SV
AS
...
FROM assessment ss  
 ...
 LEFT JOIN group_staff gf ON (gf.staff_id = stf.staff_id AND gf.group_id in (103100,103083,102955,102954,103055,103056,103057,103058,103059))
 LEFT JOIN groups g ON (g.group_id = gf.group_id AND g.type_cd = 'TEAM' AND g.group_name like ('Field EB%'))
...
WHERE ss.assessment_type_cd IN('INTAKE_VISIT','INTAKE_VISIT_NO_MA');

 GRANT SELECT ON D_PA_ASSESSMENT_SV TO MAXDAT_REPORTS;