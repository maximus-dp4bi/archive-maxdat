...
AS
  SELECT /*+ parallel(si,5) */ SI.STEP_INSTANCE_ID AS MW_BI_ID
    --D_MW_V2_CURRENT_SV
    ,
    CASE
  ...
  LEFT JOIN
    (SELECT /*+ parallel(h,5) */ h.step_instance_id, max(create_ts) create_ts FROM step_instance_history h WHERE h.STATUS = 'CLAIMED' group by h.step_instance_id
    ) sihc ON (sihc.step_instance_id = SI.STEP_INSTANCE_ID)
  WHERE ((si.status = 'COMPLETED' AND si.completed_ts >= add_months(TRUNC(sysdate,'mm'),-6))
