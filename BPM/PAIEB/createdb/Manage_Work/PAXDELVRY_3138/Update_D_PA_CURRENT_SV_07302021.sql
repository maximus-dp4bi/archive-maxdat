...
SELECT /*+ parallel(5) */ ah.application_id app_id
       ...
       --Application Processing Cycle Times
        ...
        -- added by James Frick, 8/15/2019
         ...
         , case when ddoc.HOW_HEARD_OTHER LIKE '%DUAL%' then 'Y' else 'N' end Dual_Household_flag 
--app
FROM app_header ah
  ...
  left join (select * from 
            ( select dd.application_id, upper(dd.how_heard_other) how_heard_other
                     , ROW_NUMBER() over (partition by dd.application_id order by dd.app_doc_data_id desc) rown
                     from app_doc_data dd
                     where upper(dd.how_heard_other) like '%DUAL%' 
                     )
                     where rown = 1) ddoc on (ah.application_id = ddoc.application_id)
