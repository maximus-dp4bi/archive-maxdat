...
CREATE OR REPLACE VIEW MAXDAT_SUPPORT.D_MW_V2_CURRENT_SV
AS SELECT /*+ parallel(si,512) */ SI.STEP_INSTANCE_ID AS MW_BI_ID
    --D_MW_V2_CURRENT_SV
    ,
    CASE
  ...
  LEFT JOIN GROUPS g                       ON (SI.TEAM_ID = g.group_id)
  LEFT JOIN
  ...
  LEFT JOIN
    (SELECT /*+ parallel(h,512) */ h.step_instance_id, max(create_ts) create_ts FROM step_instance_history h WHERE h.STATUS = 'CLAIMED' group by h.step_instance_id
    ) sihc ON (sihc.step_instance_id = SI.STEP_INSTANCE_ID)
  WHERE g.type_cd = 'TEAM' AND sd.step_type_cd IN ('VIRTUAL_HUMAN_TASK','HUMAN_TASK') and 
  ((si.status = 'COMPLETED' AND si.completed_ts >= add_months(TRUNC(sysdate,'mm'),-6))
  OR (si.status IN ('CLAIMED','UNCLAIMED','INPROCESS')))
  ;
