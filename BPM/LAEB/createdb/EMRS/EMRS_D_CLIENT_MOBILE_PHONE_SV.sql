DROP VIEW MAXDAT_SUPPORT.EMRS_D_CLIENT_MOBILE_PHONE_SV;

CREATE OR REPLACE VIEW MAXDAT_SUPPORT.EMRS_D_CLIENT_MOBILE_PHONE_SV
AS
SELECT COALESCE(p.PHON_ID, -1*c.clnt_client_id) AS PHONE_ID
  ...
  ,CASE WHEN ROW_NUMBER() OVER(PARTITION BY c.CLNT_CLIENT_ID ORDER BY COALESCE(p.PHON_ID, -1*c.clnt_client_id)) = 1 
        ...
        END AS IS_CURRENT
FROM EB.CLIENT c
LEFT JOIN EB.PHONE_NUMBER p ON (p.PHON_TYPE_CD IN ('MB', 'CM') 
                                AND c.clnt_client_id = p.clnt_client_id)
LEFT JOIN EB.ENUM_PHONE_TYPE t ON (p.PHON_TYPE_CD = t.VALUE);

GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CLIENT_MOBILE_PHONE_SV TO DP4BI_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CLIENT_MOBILE_PHONE_SV TO MAXDATSUPPORT_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CLIENT_MOBILE_PHONE_SV TO MAXDAT_SUPPORT_READ_ONLY;
