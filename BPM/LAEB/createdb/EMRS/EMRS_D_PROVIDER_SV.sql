DROP VIEW MAXDAT_SUPPORT.EMRS_D_PROVIDER_SV;

CREATE OR REPLACE VIEW MAXDAT_SUPPORT.EMRS_D_PROVIDER_SV
AS
SELECT nw.network_id AS PROVIDER_NUMBER,
  ...
  FROM EB.NETWORK nw
  LEFT JOIN EB.provider p                  ON (p.provider_id = nw.provider_id)
  LEFT JOIN EB.network_language nwl1       ON (nw.network_id = nwl1.network_id AND nwl1.language_ext_cd = '1')
  LEFT JOIN EB.network_language nwl2       ON (nw.network_id = nwl2.network_id AND nwl2.language_ext_cd = '2')
  LEFT JOIN EB.network_language nwl3       ON (nw.network_id = nwl3.network_id AND nwl3.language_ext_cd = '3')
  LEFT JOIN EB.panel_limit pl              ON (nw.network_id = pl.network_id)
  LEFT JOIN EB.network_office_hours noh    ON (nw.network_id = noh.network_id)
...
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_PROVIDER_SV TO DP4BI_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_PROVIDER_SV TO MAXDATSUPPORT_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_PROVIDER_SV TO MAXDAT_SUPPORT_READ_ONLY;
