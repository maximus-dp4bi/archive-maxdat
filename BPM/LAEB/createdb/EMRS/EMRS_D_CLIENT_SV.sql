...
CREATE OR REPLACE VIEW MAXDAT_SUPPORT.EMRS_D_CLIENT_SV
AS
SELECT /*+ parallel(10) */
  ...
  FROM EB.client cl
  LEFT JOIN MAXDAT_SUPPORT.D_CLIENT_ELIGIBILITY ce ON (cl.clnt_client_id = ce.client_id AND ce.SEQUENCE_NUM =1)
  LEFT JOIN (select /*+ parallel(10) */ e.CLIENT_ELIG_STATUS_ID
              ...
              FROM eb.CLIENT_ELIG_STATUS e) ceg ON (cl.clnt_client_id = ceg.client_id AND ceg.end_date IS NULL)
...
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CLIENT_SV TO DP4BI_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CLIENT_SV TO MAXDATSUPPORT_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CLIENT_SV TO MAXDAT_SUPPORT_READ_ONLY;
