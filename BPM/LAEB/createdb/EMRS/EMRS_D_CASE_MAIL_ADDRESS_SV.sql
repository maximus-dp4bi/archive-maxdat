DROP VIEW MAXDAT_SUPPORT.EMRS_D_CASE_MAIL_ADDRESS_SV;

CREATE OR REPLACE VIEW MAXDAT_SUPPORT.EMRS_D_CASE_MAIL_ADDRESS_SV
AS
WITH tmp AS
  (SELECT /*+ parallel(10) */ a.ADDR_ID 
    , a.ADDR_CASE_ID AS CASE_ID
    ,ROW_NUMBER() OVER(PARTITION BY a.ADDR_CASE_ID ORDER BY CASE WHEN a.ADDR_TYPE_CD = 'MM' THEN 1 
  ...
  WHERE addr.addr_case_id IS NOT NULL            
  UNION ALL
  SELECT /*+ parallel(10) */
...
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CASE_MAIL_ADDRESS_SV TO DP4BI_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CASE_MAIL_ADDRESS_SV TO MAXDATSUPPORT_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CASE_MAIL_ADDRESS_SV TO MAXDAT_SUPPORT_READ_ONLY;
