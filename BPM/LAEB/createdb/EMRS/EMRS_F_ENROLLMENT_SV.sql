...
CREATE OR REPLACE VIEW MAXDAT_SUPPORT.EMRS_F_ENROLLMENT_SV
AS
SELECT 
    ...
    st.selection_source_cd AS SELECTION_SOURCE_CODE,
  ...
  (SELECT 
    ...
    ROW_NUMBER() OVER(PARTITION BY COALESCE(txn.client_id, seg.client_id), COALESCE(seg.start_nd, txn.start_nd),  CASE WHEN txn.TRANSACTION_TYPE_CD = 'Disenrollment' THEN 1 ELSE 0 END ORDER BY txn.selection_txn_id, seg.SELECTION_SEGMENT_ID DESC) as rn,
    ...
    -- end sel txn data    
    FROM eb.selection_txn txn
    FULL OUTER JOIN eb.selection_segment seg on (txn.client_id = seg.client_id AND txn.plan_id = seg.plan_id AND txn.start_nd = seg.start_nd) 
  ...
  JOIN eb.client_supplementary_info cs ON (st.client_id = cs.client_id AND st.program_type_cd = cs.program_cd) 
  WHERE st.rn = 1;

GRANT SELECT ON MAXDAT_SUPPORT.EMRS_F_ENROLLMENT_SV TO DP4BI_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_F_ENROLLMENT_SV TO MAXDATSUPPORT_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_F_ENROLLMENT_SV TO MAXDAT_SUPPORT_READ_ONLY;
