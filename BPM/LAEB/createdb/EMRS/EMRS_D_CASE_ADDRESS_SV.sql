DROP VIEW MAXDAT_SUPPORT.EMRS_D_CASE_ADDRESS_SV;

CREATE OR REPLACE VIEW MAXDAT_SUPPORT.EMRS_D_CASE_ADDRESS_SV
AS
WITH tmp 
  AS
  (SELECT a.ADDR_ID 
    , a.ADDR_CASE_ID AS CASE_ID
    ,ROW_NUMBER() OVER(PARTITION BY a.CLNT_CLIENT_ID ORDER BY CASE WHEN a.ADDR_TYPE_CD = 'MR' THEN 1 
    ...
    AND a.addr_case_id IS NOT NULL)
    
  SELECT 
    addr.addr_id AS ADDRESS_ID ,
    addr.addr_case_id AS CASE_ID ,
    COALESCE(addr.addr_type_cd, 'UD') AS ADDR_TYPE_CD ,
    b.residence as RESIDENCE,
    b.mail as MAIL,
  ...
  LEFT JOIN tmp b ON (addr.addr_id = b.addr_id and (b.RESIDENCE = 1 OR b.MAIL = 1))
...
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CASE_ADDRESS_SV TO DP4BI_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CASE_ADDRESS_SV TO GT83345;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CASE_ADDRESS_SV TO MAXDATSUPPORT_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CASE_ADDRESS_SV TO MAXDAT_SUPPORT_READ_ONLY;
