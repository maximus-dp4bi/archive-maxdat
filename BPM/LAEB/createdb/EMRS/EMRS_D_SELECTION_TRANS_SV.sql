...
CREATE OR REPLACE VIEW MAXDAT_SUPPORT.EMRS_D_SELECTION_TRANS_SV
AS
SELECT
  ...
  /*  COALESCE(ltr.MAILED_DATE, ltr.CLNT_GENERIC_FIELD1_DATE) AS GRACE_PERIOD_START,
    (COALESCE(ltr.MAILED_DATE, ltr.CLNT_GENERIC_FIELD1_DATE) + 90) AS GRACE_PERIOD_END,
     CASE WHEN st.status_date < (COALESCE(ltr.MAILED_DATE, ltr.CLNT_GENERIC_FIELD1_DATE) + 90)
          THEN 1
          WHEN st.status_date > (COALESCE(ltr.MAILED_DATE, ltr.CLNT_GENERIC_FIELD1_DATE) + 90)
          THEN 2
          ELSE 0 END AS IN_GRACE_PERIOD*/
  ...
  LEFT JOIN eb.CLIENT_ELIG_STATUS ceg ON (st.client_id = ceg.client_id AND st.status_date >= TRUNC(ceg.start_date) AND st.status_date < TRUNC(COALESCE(ceg.end_date, sysdate)))
  LEFT JOIN (SELECT lrl.CLIENT_ID
                , c.CLNT_GENERIC_FIELD1_DATE
                , min(lr.MAILED_DATE) AS MAILED_DATE
              FROM CLIENT c
              LEFT JOIN LETTER_REQUEST_LINK lrl ON (c.CLNT_CLIENT_ID = lrl.CLIENT_ID)
              LEFT JOIN LETTER_REQUEST lr ON (lrl.LMREQ_ID = lr.LMREQ_ID
                                              AND lr.REQUESTED_ON >= (c.CLNT_GENERIC_FIELD1_DATE - 35)
                                              AND lr.MAILED_DATE IS NOT NULL
                                              AND lr.LMDEF_ID in (17, 60))
              GROUP BY lrl.CLIENT_ID
              , c.CLNT_GENERIC_FIELD1_DATE) ltr ON (st.client_id = ltr.client_id)
  LEFT JOIN (SELECT
              a.SELECTION_TXN_ID,
              ...
              a.PLAN_ID_EXT,
              a.PLAN_TYPE_CD,
              LAG(a.SELECTION_TXN_ID, 1) OVER (PARTITION BY a.client_id ORDER BY a.SEGMENT_START_ND, a.SELECTION_SEGMENT_ID ASC) PRIOR_SELECTION_TXN_ID,
              LAG(a.SEGMENT_START_ND, 1) OVER (PARTITION BY a.client_id ORDER BY a.SEGMENT_START_ND, a.SELECTION_SEGMENT_ID ASC) PRIOR_SEGMENT_START_ND,
              LAG(a.SEGMENT_END_ND, 1) OVER (PARTITION BY a.client_id ORDER BY a.SEGMENT_START_ND, a.SELECTION_SEGMENT_ID ASC) PRIOR_SEGMENT_END_ND,
              LAG(a.PLAN_ID, 1) OVER (PARTITION BY a.client_id ORDER BY a.SEGMENT_START_ND, a.SELECTION_SEGMENT_ID ASC) PRIOR_PLAN_ID,
              LAG(a.PLAN_ID_EXT, 1) OVER (PARTITION BY a.client_id ORDER BY a.SEGMENT_START_ND, a.SELECTION_SEGMENT_ID ASC) PRIOR_PLAN_ID_EXT,
              LAG(a.SELECTION_GENERIC_FIELD5_TXT, 1) OVER (PARTITION BY a.client_id ORDER BY a.SEGMENT_START_ND, a.SELECTION_SEGMENT_ID ASC) PRIOR_SLCTION_GEN_FIELD5_TXT,
              LAG(a.STATUS_DATE, 1) OVER (PARTITION BY a.client_id ORDER BY a.SEGMENT_START_ND, a.SELECTION_SEGMENT_ID ASC) PRIOR_STATUS_DATE
              FROM
              (SELECT
                ...
                ROW_NUMBER() OVER(PARTITION BY ss1.CLIENT_ID, ss1.START_ND ORDER BY ss1.START_ND DESC) as rn
                FROM eb.SELECTION_SEGMENT ss1
                LEFT JOIN eb.SELECTION_TXN st1 ON (ss1.CLIENT_ID = st1.CLIENT_ID
                ...
                WHERE RN = 1) p ON (st.CLIENT_ID = p.CLIENT_ID AND st.START_DATE = p.START_DATE AND st.TRANSACTION_TYPE_CD = 'Transfer')  --only join on transer transactions
WHERE st.rn = 1;

GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_SELECTION_TRANS_SV TO DP4BI_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_SELECTION_TRANS_SV TO MAXDATSUPPORT_READ_ONLY;
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_SELECTION_TRANS_SV TO MAXDAT_SUPPORT_READ_ONLY;
