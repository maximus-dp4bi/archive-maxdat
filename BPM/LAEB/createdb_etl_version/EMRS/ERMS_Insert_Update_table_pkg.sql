...
IS
    ...
    v_mi_count number;
    v_bak_count number;
    v_trunc_ins_yn VARCHAR2(1);
BEGIN
  BEGIN
    SELECT value
    INTO v_trunc_ins_yn
    FROM corp_etl_control
    WHERE name = 'EMRS_SELECTION_SEGMENT_TRUNC_INS_YN';
  EXCEPTION
...
--   RETURNING JOB_ID INTO v_job_id;
   ...
   IF v_trunc_ins_yn = 'Y' THEN
     
     --create temp table 
     BEGIN
      EXECUTE IMMEDIATE 'DROP TABLE emrs_s_selection_segment_'||TO_CHAR(sysdate,'yyyymmdd');
     EXCEPTION
       WHEN OTHERS THEN
        NULL;
     END;

    EXECUTE IMMEDIATE 'CREATE TABLE emrs_s_selection_segment_'||TO_CHAR(sysdate,'yyyymmdd')||' AS SELECT * FROM emrs_d_selection_segment mi WHERE NOT EXISTS(SELECT 1 FROM emrs_s_selection_segment_stg stg WHERE mi.client_id = stg.client_id AND mi.plan_type_cd = stg.plan_type_cd)';        
    --rename existing table
    EXECUTE IMMEDIATE 'ALTER TABLE EMRS_D_SELECTION_SEGMENT RENAME TO EMRS_D_SELECTION_SEGMENT_BAK_'||TO_CHAR(sysdate,'yyyymmdd');    
    --rename new table
    EXECUTE IMMEDIATE 'ALTER TABLE emrs_s_selection_segment_'||TO_CHAR(sysdate,'yyyymmdd')||' RENAME TO EMRS_D_SELECTION_SEGMENT';
    EXECUTE IMMEDIATE 'GRANT SELECT ON EMRS_D_SELECTION_SEGMENT TO MAXDAT_LAEB_READ_ONLY';
    EXECUTE IMMEDIATE 'ALTER VIEW EMRS_D_SELECTION_TRANS_SV COMPILE';
    EXECUTE IMMEDIATE 'ALTER VIEW EMRS_D_PRIOR_PROGRAM_SV COMPILE';
    EXECUTE IMMEDIATE 'ALTER VIEW EMRS_F_ENROLLMENT_SV COMPILE';
    EXECUTE IMMEDIATE 'ALTER VIEW EMRS_F_PLAN_TRANSFER_SV COMPILE';
    
     --truncate table
    -- EXECUTE IMMEDIATE 'TRUNCATE TABLE EMRS_D_SELECTION_SEGMENT';      
     --insert back records
    --  EXECUTE IMMEDIATE 'INSERT /*+ Enable_Parallel_Dml Parallel */
    --                       INTO emrs_d_selection_segment(selection_segment_id,client_id,program_type_cd,plan_type_cd,start_date,end_date,status_cd,status_date,plan_id,plan_id_ext,
	--                           network_id,provider_id_ext,provider_first_name,provider_middle_name,provider_last_name,choice_reason_cd,disenroll_reason_cd_1,disenroll_reason_cd_2,
	--                           client_aid_category_cd,county_cd,zipcode,created_by,create_ts,updated_by,update_ts,contract_id,start_nd,end_nd,ss_generic_field1_date,ss_generic_field2_date,
	--                           ss_generic_field3_num,ss_generic_field4_num,ss_generic_field5_txt,ss_generic_field6_txt,ss_generic_field7_txt,ss_generic_field8_txt,ss_generic_field9_txt,ss_generic_field10_txt) 
    --                       SELECT selection_segment_id,client_id,program_type_cd,plan_type_cd,start_date,end_date,status_cd,status_date,plan_id,plan_id_ext,
	--                           network_id,provider_id_ext,provider_first_name,provider_middle_name,provider_last_name,choice_reason_cd,disenroll_reason_cd_1,disenroll_reason_cd_2,
	--                           client_aid_category_cd,county_cd,zipcode,created_by,create_ts,updated_by,update_ts,contract_id,start_nd,end_nd,ss_generic_field1_date,ss_generic_field2_date,
	--                           ss_generic_field3_num,ss_generic_field4_num,ss_generic_field5_txt,ss_generic_field6_txt,ss_generic_field7_txt,ss_generic_field8_txt,ss_generic_field9_txt,ss_generic_field10_txt
    --                       FROM emrs_s_selection_segment_'||TO_CHAR(sysdate,'yyyymmdd');
    --  EXECUTE IMMEDIATE 'COMMIT';

    -- SELECT COUNT(*)
    -- INTO v_mi_count
    -- FROM emrs_d_selection_segment;
...
--     WHERE JOB_ID =  v_job_id;
    ...
    IF v_mi_count > 0 THEN   
      EXECUTE IMMEDIATE 'DROP TABLE EMRS_D_SELECTION_SEGMENT_BAK_'||TO_CHAR(sysdate,'yyyymmdd');
    END IF;     
    EXECUTE IMMEDIATE 'ALTER TABLE EMRS_D_SELECTION_SEGMENT add constraint PK_SELECTION_SEGMENT primary key(selection_segment_id) USING INDEX TABLESPACE MAXDAT_LAEB_DATA';    
    EXECUTE IMMEDIATE 'CREATE INDEX IDX1_SELECTION_SEGMENT ON EMRS_D_SELECTION_SEGMENT (CLIENT_ID)';    
    EXECUTE IMMEDIATE 'CREATE INDEX SELECTION_SEGMENT_IDX01	ON EMRS_D_SELECTION_SEGMENT (CLIENT_ID, START_ND, END_ND)';    
    EXECUTE IMMEDIATE 'CREATE INDEX SELECTION_SEGMENT_IDX2 ON EMRS_D_SELECTION_SEGMENT (CLIENT_ID, START_DATE, END_DATE, STATUS_CD)';
    EXECUTE IMMEDIATE 'CREATE INDEX SELECTION_SEGMENT_IDX3 ON EMRS_D_SELECTION_SEGMENT (PLAN_ID)';
    EXECUTE IMMEDIATE 'CREATE INDEX SELECTION_SEGMENT_IDX4	ON EMRS_D_SELECTION_SEGMENT (CONTRACT_ID)';
    EXECUTE IMMEDIATE 'CREATE INDEX SELECTION_SEGMENT_IDX5	ON EMRS_D_SELECTION_SEGMENT (PLAN_TYPE_CD)';
    EXECUTE IMMEDIATE 'CREATE INDEX SELECTION_SEGMENT_IDX6	ON EMRS_D_SELECTION_SEGMENT (STATUS_DATE)';

    --dbms_stats.gather_table_stats('MAXDAT_LAEB','EMRS_D_SELECTION_SEGMENT');
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      BEGIN
        ...
        EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM EMRS_D_SELECTION_SEGMENT_BAK_'||TO_CHAR(sysdate,'yyyymmdd') INTO v_bak_count;
      EXCEPTION
        WHEN OTHERS THEN
          v_bak_count := 0;
      END;    
      IF v_bak_count > 0 AND v_mi_count = 0 THEN
        --restore old data
        EXECUTE IMMEDIATE 'DROP TABLE EMRS_D_SELECTION_SEGMENT';
        EXECUTE IMMEDIATE 'ALTER TABLE EMRS_D_SELECTION_SEGMENT_BAK_'||TO_CHAR(sysdate,'yyyymmdd')||' RENAME TO EMRS_D_SELECTION_SEGMENT';   
      END IF;
...
IS
    ...
    v_stg_cnt NUMBER :=0;
    v_lookback_days NUMBER := 0;
BEGIN

   --get lookback days
   FOR l IN (SELECT TO_NUMBER(value) lookback_days
             FROM corp_etl_control
             WHERE name = 'SLCTSEGMENT_SYNCH_LOOKBACK_DAYS')
   LOOP
     v_lookback_days := l.lookback_days;    
   END LOOP;
   ...
   IF v_max_process_ts IS NULL OR v_processed_to_app_ts != v_max_process_ts THEN
    ...
    -- IF v_stg_cnt = v_processed_cnt THEN
       ...
       WHERE EXISTS(SELECT 1 FROM emrs_d_elig_segment_details sd where ss.client_id = sd.client_id and sd.segment_type_cd = 'MI' and sd.start_date = ss.start_date --and sd.start_date = sd.end_date 
                    AND TRUNC(sd.record_date) >= TRUNC(v_processed_to_app_ts) - v_lookback_days
       ...
       FROM emrs_d_selection_segment ss       
       WHERE EXISTS(SELECT 1 FROM emrs_etl_override_response r where ss.client_id = r.matched_client_id AND TRUNC(r.processed_ts) = TRUNC(v_processed_to_app_ts))
       AND NOT EXISTS(SELECT 1 FROM emrs_d_elig_segment_details sd where ss.client_id = sd.client_id and sd.segment_type_cd = 'MI' and sd.start_date = ss.start_date 
                    AND TRUNC(sd.record_date) >= TRUNC(v_processed_to_app_ts) - v_lookback_days
                    AND TO_NUMBER(TO_CHAR(ss.create_ts,'yyyymmddhh24mi')) < TO_NUMBER(TO_CHAR(sd.record_date,'yyyymmddhh24mi')) );
