CREATE OR REPLACE VIEW EMRS_D_CASE_MAIL_ADDRESS_SV
AS
WITH tmp AS
  (SELECT /*+ parallel(10) */ a.ADDR_ID 
    , a.CASE_ID AS CASE_ID
    ,ROW_NUMBER() OVER(PARTITION BY a.CASE_ID ORDER BY CASE WHEN a.ADDR_TYPE_CD = 'MM' THEN 1 
  ...
  WHERE addr.case_id IS NOT NULL            
  UNION ALL
  SELECT /*+ parallel(10) */
...
GRANT SELECT ON EMRS_D_CASE_MAIL_ADDRESS_SV TO &role_name;
