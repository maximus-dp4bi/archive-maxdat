CREATE OR REPLACE VIEW MAXDAT_LAEB.EMRS_D_PROVIDER_SV
AS
SELECT nw.network_id AS PROVIDER_NUMBER,
    ...
    CASE WHEN LENGTH(TRIM(nw.last_name)) IS NULL THEN TRIM(nw.first_name) 
          WHEN LENGTH(TRIM(nw.first_name)) IS NULL THEN TRIM(nw.last_name)
          ELSE TRIM(nw.last_name) || ', ' || TRIM(nw.first_name) 
  ...
  FROM MAXDAT_LAEB.EMRS_D_NETWORK nw
  LEFT JOIN MAXDAT_LAEB.EMRS_D_provider p                  ON (p.provider_id = nw.provider_id)
  LEFT JOIN MAXDAT_LAEB.EMRS_D_network_language nwl1       ON (nw.network_id = nwl1.network_id AND nwl1.language_ext_cd = '1')
  LEFT JOIN MAXDAT_LAEB.EMRS_D_network_language nwl2       ON (nw.network_id = nwl2.network_id AND nwl2.language_ext_cd = '2')
  LEFT JOIN MAXDAT_LAEB.EMRS_D_network_language nwl3       ON (nw.network_id = nwl3.network_id AND nwl3.language_ext_cd = '3')
  LEFT JOIN MAXDAT_LAEB.EMRS_D_panel_limit pl              ON (nw.network_id = pl.network_id)
  LEFT JOIN MAXDAT_LAEB.EMRS_D_network_office_hours noh    ON (nw.network_id = noh.network_id)
...
GRANT SELECT ON MAXDAT_LAEB.EMRS_D_PROVIDER_SV TO MAXDAT_LAEB_READ_ONLY;

DROP VIEW MAXDAT_LAEB.EMRS_D_PROVIDER_SPECIALTY_SV;

CREATE OR REPLACE VIEW MAXDAT_LAEB.EMRS_D_PROVIDER_SPECIALTY_SV
AS
SELECT network_id AS PROVIDER_NUMBER ,
  ...
  FROM MAXDAT_LAEB.EMRS_D_network_specialty
WITH READ ONLY;

GRANT SELECT ON MAXDAT_LAEB.EMRS_D_PROVIDER_SPECIALTY_SV TO MAXDAT_LAEB_READ_ONLY;

DROP VIEW MAXDAT_LAEB.EMRS_D_SELECTION_MI_SV;

CREATE OR REPLACE VIEW MAXDAT_LAEB.EMRS_D_SELECTION_MI_SV
AS
SELECT smi.missing_info_id||st.selection_txn_id||smd.missing_info_details_id AS SELECTION_MISSING_INFO_ID,
  ...
  FROM MAXDAT_LAEB.EMRS_D_selection_txn st
  LEFT JOIN MAXDAT_LAEB.EMRS_D_selectn_missing_info smi              ON st.missing_info_id = smi.missing_info_id
  LEFT JOIN MAXDAT_LAEB.EMRS_D_seltn_misng_info_detl smd ON smi.missing_info_id = smd.missing_info_id AND st.client_id = smd.client_id
WITH READ ONLY;

GRANT SELECT ON MAXDAT_LAEB.EMRS_D_SELECTION_MI_SV TO MAXDAT_LAEB_READ_ONLY;
...
SELECT
    ...
    CASE WHEN TRUNC(cl.clnt_dob) > TRUNC(st.CREATE_TS) - 365 THEN 'NE' ELSE COALESCE(st.SELECTION_SOURCE_CD, 'A') END AS SELECTION_SOURCE_CD,
    CASE
      WHEN TRUNC(cl.clnt_dob) > TRUNC(st.CREATE_TS) - 365 THEN 'Newborn'
...
SELECT 
    ...
    CASE WHEN TRUNC(cs.dob) > TRUNC(st.CREATE_TS) - 365 THEN 'Newborn' ELSE st.selection_source_cd END AS SELECTION_SOURCE_CODE,
    CASE WHEN st.selection_source_cd IN ('P','C','W')
         THEN 'Y'
         ELSE 'N'
    END AS IS_CHOICE_IND,
    CASE
      WHEN TRUNC(cs.dob) > TRUNC(st.CREATE_TS) - 365 THEN 'Newborn'
      WHEN st.SELECTION_SOURCE_CD in('P','GS','G','OP','OL') THEN 'Phone' 
      WHEN st.SELECTION_SOURCE_CD = 'A' AND st.OVERRIDE_REASON_CD IS NULL AND st.TRANSACTION_TYPE_CD IN ('NewEnrollment','Disenrollment','Transfer') THEN 'Phone'  
      WHEN st.SELECTION_SOURCE_CD = 'A' AND st.OVERRIDE_REASON_CD = '20' AND st.TRANSACTION_TYPE_CD IN ('NewEnrollment','Disenrollment','Transfer') THEN 'Phone'  
      WHEN st.SELECTION_SOURCE_CD = 'A' AND st.OVERRIDE_REASON_CD = 'M005' AND st.TRANSACTION_TYPE_CD = 'Disenrollment' THEN 'Phone' 
...
DROP VIEW MAXDAT_LAEB.EMRS_D_PRIOR_PROGRAM_SV;

CREATE OR REPLACE VIEW MAXDAT_LAEB.EMRS_D_PRIOR_PROGRAM_SV
AS
SELECT ss.SELECTION_SEGMENT_ID AS PRIOR_SELECTION_SEGMENT_ID,
  ss.Program_Type_CD AS PRIOR_PROGRAM_CODE,
  prg.program_name AS PRIOR_PROGRAM_NAME,
    CASE
      WHEN PRG.end_date IS NULL
      THEN 'A'
      ELSE 'I'
    END active_inactive ,
    PRG.end_date end_date ,
    NULL prior_program_category ,
    PROGRAM_CODE prior_managed_care_program ,
    PRG.start_date start_date
FROM MAXDAT_LAEB.EMRS_D_selection_segment ss 
LEFT JOIN MAXDAT_LAEB.EMRS_D_program prg ON prg.PROGRAM_CODE = ss.Program_Type_CD
UNION ALL
...
FROM MAXDAT_LAEB.EMRS_D_selection_txn s
WHERE s.prior_selection_segment_id not in (SELECT ss.SELECTION_SEGMENT_ID FROM MAXDAT_LAEB.EMRS_D_selection_segment ss)
UNION ALL
...
GRANT SELECT ON MAXDAT_LAEB.EMRS_D_PRIOR_PROGRAM_SV TO MAXDAT_LAEB_READ_ONLY;

DROP VIEW MAXDAT_LAEB.EMRS_F_PLAN_TRANSFER_SV;

CREATE OR REPLACE VIEW EMRS_F_PLAN_TRANSFER_SV
AS
...
JOIN (SELECT st.selection_txn_id
      ...
      LEFT JOIN (SELECT 
                  ...
                  (SELECT 
                    ...
                    FROM emrs_d_selection_txn st
                    JOIN emrs_d_selection_segment ss ON (ss.client_id = st.client_id AND ss.START_DATE = st.START_DATE AND ss.PLAN_ID_EXT = st.PLAN_ID_EXT AND st.TRANSACTION_TYPE_CD <> 'Disenrollment')
...
GRANT SELECT ON EMRS_F_PLAN_TRANSFER_SV TO MAXDAT_LAEB_READ_ONLY;