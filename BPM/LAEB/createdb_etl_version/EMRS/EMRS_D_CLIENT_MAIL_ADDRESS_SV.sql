CREATE OR REPLACE VIEW EMRS_D_CLIENT_MAIL_ADDRESS_SV
AS
WITH tmp AS
  (SELECT /*+ parallel(10) */ a.ADDR_ID 
  , a.client_id
  , ROW_NUMBER() OVER(PARTITION BY a.client_id ORDER BY CASE WHEN a.ADDR_TYPE_CD = 'MM' THEN 1 
  ...
  WHERE addr.client_id IS NOT NULL            
  UNION ALL
  SELECT /*+ parallel(10) */
    c.client_id AS ADDRESS_ID ,
  ...
  WHERE c.client_id NOT IN (SELECT /*+ parallel(10) */ client_id FROM tmp);

GRANT SELECT ON EMRS_D_CLIENT_MAIL_ADDRESS_SV TO &role_name;
