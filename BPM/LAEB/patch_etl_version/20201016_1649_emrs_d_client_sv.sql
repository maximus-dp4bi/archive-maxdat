...
SELECT /*+ parallel(10) */
  ...
  LEFT JOIN D_CLIENT_ELIGIBILITY ce ON (cl.client_id = ce.client_id AND ce.SEQUENCE_NUM =1)
  LEFT JOIN (SELECT  e.CLIENT_ELIG_STATUS_ID
              ...
              FROM emrs_d_client_eligibility e) ceg ON (cl.client_id = ceg.client_id AND ceg.end_date IS NULL)  
  LEFT JOIN emrs_d_other_health_care ohc ON (ohc.client_id = cl.client_id AND (ohc.ohc_end_date > SYSDATE OR ohc.ohc_end_date IS NULL));
