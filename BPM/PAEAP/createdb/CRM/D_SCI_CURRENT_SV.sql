...
AS
  ...
  cr.CALL_RECORD_FIELD1 AS CONTACT_RECORD_FIELD1,
...
LEFT JOIN maxdat_support.corp_etl_list_lkup ll on (ll.name = 'CLIENT_INQUIRY_CONTACT_TYPE' and ll.value = cr.CALL_TYPE_CD)
...
LEFT JOIN (SELECT REF_ID1, NOTE_ID, CATEGORY_CD, NOTE_TYPE_CD, SOURCE, RANK() OVER(PARTITION BY REF_ID1 ORDER BY NOTE_ID DESC) AS RNK
            FROM EB.NOTE
...
; 
  ...
  GRANT SELECT ON maxdat_support.D_SCI_CURRENT_SV TO EB_MAXDAT_REPORTS;  