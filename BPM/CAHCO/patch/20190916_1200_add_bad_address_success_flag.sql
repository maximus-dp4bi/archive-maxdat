ALTER TABLE MAXDAT.HCO_S_PACKET_MAILING_STG ADD BAD_ADDRESS_SUCCESS_FLAG VARCHAR2 (1);
/
ALTER TABLE MAXDAT.HCO_S_LETTER_MAILING_STG ADD BAD_ADDRESS_SUCCESS_FLAG VARCHAR2(1);
/
ALTER TABLE MAXDAT.HCO_D_LETTER_MAILING ADD BAD_ADDRESS_SUCCESS_FLAG VARCHAR2(1);
/
ALTER TABLE MAXDAT.HCO_D_PACKET_MAILING ADD BAD_ADDRESS_SUCCESS_FLAG VARCHAR2(1);
/
ALTER TABLE MAXDAT.HCO_D_ALTERNATIVE_FORMAT ADD BAD_ADDRESS_SUCCESS_FLAG VARCHAR2 (1);
/

CREATE OR REPLACE VIEW MAXDAT.HCO_D_LETTER_MAILING_SV
AS
SELECT 
...
bad_address_flag,
bad_address_success_flag
FROM hco_d_letter_mailing lm
 LEFT JOIN emrs_d_language ml ON lm.language_id = ml.language_code;
/

GRANT SELECT ON MAXDAT.HCO_D_LETTER_MAILING_SV TO MAXDAT_OLTP_SIU;
GRANT SELECT ON MAXDAT.HCO_D_LETTER_MAILING_SV TO MAXDAT_OLTP_SIUD;
GRANT SELECT ON MAXDAT.HCO_D_LETTER_MAILING_SV TO MAXDAT_READ_ONLY;



CREATE OR REPLACE PACKAGE BODY MAXDAT.CAHCO_MAILING_ETL_PKG AS
...
BEGIN
   ...
   MERGE /*+ Enable_Parallel_Dml Parallel */
    ...
    USING (SELECT lm.dp_letter_mail_id, tmp.bad_address_transaction_type,tmp.bad_address_record_date,tmp.bad_address_flag,tmp.bad_address_success_flag
           FROM hco_d_letter_mailing lm
            JOIN(
              SELECT ba.TransactionType bad_address_transaction_type,
                      ba.RecordCreateDate bad_address_record_date,
                      ba.SuccessFlag bad_address_success_flag,
            ...
            OR coalesce(lm.bad_address_success_flag,'*') != coalesce(tmp.bad_address_success_flag,'*') )            
    ...
    WHEN MATCHED THEN UPDATE
     SET s.bad_address_transaction_type = st.bad_address_transaction_type
        ...
        ,s.bad_address_success_flag = st.bad_address_success_flag
...
BEGIN
   ...
   USING (SELECT tmp.dim_letter_mailing_id,
        ...
        WHERE  COALESCE(t.vendor_received_date, TO_DATE('07/07/7777','mm/dd/yyyy')) != COALESCE(tmp.vendor_received_date, TO_DATE('07/07/7777','mm/dd/yyyy'))          
            ...
            OR coalesce(t.bad_address_success_flag,'*') !=  coalesce(tmp.bad_address_success_flag,'*')        
...
BEGIN
   ...
   MERGE /*+ Enable_Parallel_Dml Parallel */
    ...
    USING (SELECT lm.dp_packet_mail_id, tmp.bad_address_transaction_type,tmp.bad_address_record_date,tmp.bad_address_flag,tmp.bad_address_success_flag
...
GRANT DEBUG ON MAXDAT.CAHCO_MAILING_ETL_PKG TO MAXDAT_PFP_READ_ONLY;
GRANT EXECUTE ON MAXDAT.CAHCO_MAILING_ETL_PKG TO MAXDAT_READ_ONLY;
