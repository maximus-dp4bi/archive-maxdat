...
CREATE OR REPLACE VIEW emrs_f_exemptions_dcn_by_day_sv
AS
SELECT d_date
...
FROM bpm_d_dates dd 
    LEFT JOIN (SELECT TRUNC(d.record_date) record_date,
                  COUNT(DISTINCT CASE WHEN d.plan_type = 'M' THEN d.dcn ELSE NULL END) exemptions_med_received     
                 ,COUNT(DISTINCT CASE WHEN d.plan_type = 'D' THEN d.dcn ELSE NULL END) exemptions_den_received
    ...
    LEFT JOIN(SELECT processed_date
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code IN('A','B','S','T','U','R') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) exemptions_med_processed                      
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code IN('D') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) exemptions_med_pending_mmcd
           ,COUNT(DISTINCT CASE WHEN proc.enrollment_status NOT IN('U','E','F','G','I') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) exemptions_med_processed_prior
           ,COUNT(DISTINCT CASE WHEN proc.disenrollment_id > 0 AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) exemptions_med_disenrollment
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code IN('S','T','U','R') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) exemptions_med_denied       
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code IN('A','B') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) exemptions_med_approved
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code NOT IN('A','B','S','T','U','R','D') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) exemptions_med_pending_maximus
           ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process <= TO_NUMBER(cl.out_var) AND proc.plan_type = 'M' THEN proc.dcn ELSE NULL END) exemptions_med_processed_timely
           ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process > TO_NUMBER(cl.out_var) AND proc.plan_type = 'M' THEN proc.dcn ELSE NULL END) exemptions_med_processed_untimely       
       
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code IN('A','B','S','T','U','R') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) exemptions_den_processed                         
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code IN('D') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) exemptions_den_pending_mmcd
           ,COUNT(DISTINCT CASE WHEN proc.enrollment_status NOT IN('U','E','F','G','I') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) exemptions_den_processed_prior
           ,COUNT(DISTINCT CASE WHEN proc.disenrollment_id > 0 AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) exemptions_den_disenrollment
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code IN('S','T','U','R') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) exemptions_den_denied
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code IN('A','B') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) exemptions_den_approved
           ,COUNT(DISTINCT CASE WHEN proc.exemption_status_code NOT IN('A','B','S','T','U','R','D') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) exemptions_den_pending_maximus
           ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process <= TO_NUMBER(cl.out_var) AND proc.plan_type = 'D' THEN proc.dcn ELSE NULL END) exemptions_den_processed_timely
           ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process > TO_NUMBER(cl.out_var) AND proc.plan_type = 'D' THEN proc.dcn ELSE NULL END) exemptions_den_processed_untimely              
...
GRANT SELECT ON "EMRS_F_EXEMPTIONS_DCN_BY_DAY_SV" TO "MAXDAT_READ_ONLY";
