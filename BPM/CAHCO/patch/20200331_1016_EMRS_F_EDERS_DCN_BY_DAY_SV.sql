CREATE OR REPLACE VIEW EMRS_F_EDERS_DCN_BY_DAY_SV
AS
SELECT d_date  
  ,eders_med_processed  
...
FROM bpm_d_dates dd
  LEFT JOIN (SELECT TRUNC(d.record_date) record_date
               ,COUNT(DISTINCT CASE WHEN d.plan_type = 'M' THEN d.dcn ELSE NULL END) eders_med_received     
               ,COUNT(DISTINCT CASE WHEN d.plan_type = 'D' THEN d.dcn ELSE NULL END) eders_den_received
               ,COUNT(DISTINCT CASE WHEN d.retro_disenroll = 'Y' THEN d.dcn ELSE NULL END) eders_retro_received
               ,COUNT(DISTINCT CASE WHEN d.plan_type = 'M' AND d.retro_disenroll = 'Y' THEN d.dcn ELSE NULL END) eders_med_retro_received
               ,COUNT(DISTINCT CASE WHEN d.plan_type = 'D' AND d.retro_disenroll = 'Y' THEN d.dcn ELSE NULL END) eders_den_retro_received               
  ...
  LEFT JOIN(
      SELECT processed_date     
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('2','4','B','D','M') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) eders_med_processed    
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('2','4','D') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) eders_med_denied
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('B','M') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) eders_med_approved
       ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process <= TO_NUMBER(cl.out_var) AND proc.plan_type = 'M' THEN proc.dcn ELSE NULL END) eders_med_processed_timely
       ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process > TO_NUMBER(cl.out_var) AND proc.plan_type = 'M' THEN proc.dcn ELSE NULL END) eders_med_processed_untimely       
       
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('2','4','B','D','M') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) eders_den_processed          
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('2','4','D') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) eders_den_denied
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('B','M') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) eders_den_approved
       ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process <= TO_NUMBER(cl.out_var) AND proc.plan_type = 'D' THEN proc.dcn ELSE NULL END) eders_den_processed_timely
       ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process > TO_NUMBER(cl.out_var) AND proc.plan_type = 'D' THEN proc.dcn ELSE NULL END) eders_den_processed_untimely       

       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('2','4','B','D','M') AND proc.retro_disenroll = 'Y' THEN proc.dcn ELSE null END) eders_retro_processed  
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('2','4','B','D','M') AND proc.plan_type = 'M' AND proc.retro_disenroll = 'Y' THEN proc.dcn ELSE null END) eders_med_retro_processed                  
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('2','4','B','D','M') AND proc.plan_type = 'D' AND proc.retro_disenroll = 'Y' THEN proc.dcn ELSE null END) eders_den_retro_processed                         
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('2','4','D') AND proc.retro_disenroll = 'Y'  THEN proc.dcn ELSE null END) eders_retro_denied
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code IN('B','M') AND proc.retro_disenroll = 'Y'  THEN proc.dcn ELSE null END) eders_retro_approved
       ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process <= TO_NUMBER(cl.out_var) AND proc.retro_disenroll = 'Y' THEN proc.dcn ELSE NULL END) eders_retro_processed_timely
       ,COUNT(DISTINCT CASE WHEN proc.row_num = 1 AND proc.days_to_process > TO_NUMBER(cl.out_var) AND proc.retro_disenroll = 'Y' THEN proc.dcn ELSE NULL END) eders_retro_processed_untimely       
       ...
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code NOT IN('2','4','B','D','M') AND proc.plan_type = 'M' THEN proc.dcn ELSE null END) eders_med_pending
       ,COUNT(DISTINCT CASE WHEN proc.emrgcy_denr_status_code NOT IN('2','4','B','D','M') AND proc.plan_type = 'D' THEN proc.dcn ELSE null END) eders_den_pending
   ...
   LEFT JOIN (SELECT TRUNC(h.record_date) received_back_date
                    ,COUNT(DISTINCT CASE WHEN d.plan_type = 'M' THEN d.dcn ELSE NULL END) eders_med_received_back
                    ,COUNT(DISTINCT CASE WHEN d.plan_type = 'D' THEN d.dcn ELSE NULL END) eders_den_received_back
                    ,COUNT(DISTINCT CASE WHEN d.retro_disenroll = 'Y' THEN dcn ELSE NULL END) eders_retro_received_back                    
             FROM emrs_d_emrgcy_denr_hist h
 ...
 GRANT SELECT ON "EMRS_F_EDERS_DCN_BY_DAY_SV" TO "MAXDAT_READ_ONLY";
