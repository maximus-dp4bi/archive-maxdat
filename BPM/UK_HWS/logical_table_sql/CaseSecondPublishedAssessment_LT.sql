select
c.Id as CaseId
, pa.AssessmentSequenceType
, pa.AssessmentSequence
, pa.AssessmentRequiredDT
, pa.FinishDate AssessmentFinishDate
, pa.CreatedDate AssessmentCreatedDate
, pa.DateOfAssessment DateOfAssessment
, pa.AssessmentType
, pa.FirstPublishDate
, pa.AppointmentBookingDate
, pa.SentForEmployeeReviewDate
, pa.PublishToEmployeeDate
, pa.PublishToEmployerDate
, pa.PublishToGPDate
FROM [Case].[Case] c
left outer join
(
...
from [HWSInternal].[Assessment].[Assessment] aa
  ...
  LEFT OUTER JOIN HWSInternal.Appointment.Appointment ap on aa.CaseId = ap.CaseId
               AND CAST(aa.DateOfAssessment as date) = cast(ap.AppointmentDate as date)
...
on c.Id = pa.CaseId and pa.AssessmentSequence = 2
