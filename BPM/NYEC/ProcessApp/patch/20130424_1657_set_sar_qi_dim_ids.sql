...
declare
  ...
  where facts.DNPASAR_ID is null;
...
select 
  facts.NYEC_PA_BI_ID,
  cur."Application ID",
  cur."Cur Reactivation Number",
  facts.D_DATE,
  coalesce(cur."Complete Date",cur."Cancel Date") complete_date,
  cur."Cur QC Indicator",
  nepa.QC_IND
from F_NYEC_PA_BY_DATE facts
inner join D_NYEC_PA_CURRENT cur on (facts.NYEC_PA_BI_ID = cur.NYEC_PA_BI_ID)
inner join NYEC_ETL_PROCESS_APP nepa on (cur."Application ID" = nepa.APP_ID and cur."Cur Reactivation Number" = nepa.REACTIVATION_NBR)
where 
  facts.DNPAQI_ID is null
  and nepa.QC_IND is not null 
  and nepa.QC_IND != 'Y'
  and nepa.QC_IND != 'N'
  --and cur."Cur Reactivation Number" is not null)
  --and coalesce(cur."Complete Date",cur."Cancel Date") is null;
order by   
  facts.NYEC_PA_BI_ID asc,
  cur."Application ID" asc,
  cur."Cur Reactivation Number" asc,
  facts.D_DATE asc;
