CREATE OR REPLACE VIEW EMRS_D_MAXSTAR_ADDRESS_SV
AS
...
LEFT JOIN eb.address ac ON (ac.addr_type_cd IN ('MR', 'MM') 
                                AND ac.END_NDT > to_number(to_char(sysdate, 'yyyymmddHH24misssss')) 
                                AND c.client_id = ac.clnt_client_id)
LEFT JOIN eb.address acs ON (acs.clnt_client_id IS NULL AND acs.addr_type_cd IN ('MR', 'MM') 
...
GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_MAXSTAR_ADDRESS_SV TO MAXDAT_REPORTS;
