...
AS
  ...
  TRIM(e.CONTEXT) AS SUPP_EVENT_CONTEXT,
  CASE WHEN e.EVENT_TYPE_CD = 'MANUAL_ACTION' THEN TRIM(ma.report_label) ELSE TRIM(bz.report_label) END AS EVENT_ACTION,
  CASE WHEN e.EVENT_TYPE_CD = 'MANUAL_ACTION' THEN TRIM(e.context) ELSE TRIM(e.event_type_cd) END AS EVENT_ACTION_CODE,
  TRIM(mct.report_label) AS MANUAL_ACTION_CATEGORY,
  TRIM(ma.categories) MANUAL_ACTION_CATEGORY_CODE,
...
LEFT JOIN ENUM_MANUAL_CALL_ACTION ma ON (e.context = ma.value) 
