...
AS
  ...
  JOIN eb.incident_header i ON (D.D_DATE BETWEEN TRUNC(I.CREATE_TS)   
                                  AND CASE WHEN i.status_cd IN('REFERRED_TO_STATE','REFERRED_TO_PLAN','CLOSED','WITHDRAWN') THEN TRUNC(I.STATUS_TS)
                                      ELSE TRUNC(sysdate)
                                      END)
