...
AS
  SELECT i.incident_header_id PI_BI_ID ,
    ...
    --rt.report_label RESOLUTION_TYPE , --replaced with NULL for now because enum table does not exist
    ...
    --opt.report_label OTHER_PARTY_NAME ,  --replaced with NULL for now because enum table does not exist
...
--  LEFT JOIN eb.enum_resolution_type rt         ON (rt.value=i.resolution_type_cd)
  ...
  LEFT JOIN eb.staff cs                        ON (cs.staff_id = TO_CHAR(i.created_by))
  LEFT JOIN
