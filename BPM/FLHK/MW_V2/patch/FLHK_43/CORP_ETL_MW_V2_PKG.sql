  ...
  begin
    for new_cur in (
                     SELECT v.*
                          ...
                          , sysdate system_date
  ------ Get data from MW2
                      FROM D_MW_V2_CURRENT V
                     WHERE v.CANCEL_WORK_DATE is null
                       AND v.stage_done_date is null
                     ORDER BY task_id, DECODE(curr_task_status,'COMPLETED',2,'TERMINATED',3,1),curr_status_date
    )
    loop
       ...
       SELECT * INTO v_stage_upd_rec from corp_etl_mw_v2_wip where task_id = new_cur.task_id;
        ...
        v_step := 'UPD1_20 Cancel';
         if v_stage_upd_rec.task_status = 'TERMINATED' then
            ...
            v_stage_upd_rec.last_update_by_staff_id := coalesce(new_cur.curr_last_upd_by_staff_id,0);
            --v_stage_upd_rec.last_update_date := new_cur.last_update_date;
            v_upd := 'Y';
         end if;
         --*******************************************************
        v_step := 'UPD1_30 -  Last_Update (NVL not needed)';
         if new_cur.curr_last_update_date <> v_stage_upd_rec.last_update_date then
            v_stage_upd_rec.last_update_by_staff_id := coalesce(new_cur.curr_last_upd_by_staff_id,0);
            v_stage_upd_rec.last_update_date := new_cur.curr_last_update_date;
            v_stage_upd_rec.last_event_date := new_cur.curr_last_update_date;
            v_upd := 'Y';
         end if;
         --*******************************************************
        v_step := 'UPD1_40 - Escalated (NVL not needed)';
         if new_cur.escalated_flag <> v_stage_upd_rec.escalated_flag then
            v_stage_upd_rec.escalated_flag := new_cur.escalated_flag;
             v_stage_upd_rec.escalated_to_staff_id := coalesce(new_cur.curr_escalated_to_staff_id,0);
...
--dbms_output.put_line ('UPD1_60 - task_histID :'||new_cur.step_instance_history_id||' NEW Owner: '||new_cur.owner_name||' Old : '||v_stage_upd_rec.owner_name);
         if nvl(new_cur.curr_owner_staff_id,1) <> nvl(v_stage_upd_rec.owner_staff_id,1) then
            v_stage_upd_rec.owner_staff_id := coalesce(new_cur.curr_owner_staff_id,0);
            v_upd := 'Y';
--dbms_output.put_line ('IN - UPD1_60 - task_histID :'||new_cur.step_instance_history_id||' NEW Owner: '||new_cur.owner_name||' Old : '||v_stage_upd_rec.owner_name) ;
        ...
        v_step := 'UPD1_70 - Group';
         if nvl(new_cur.curr_business_unit_id,1) <> nvl(v_stage_upd_rec.business_unit_id,1) then
            v_stage_upd_rec.business_unit_id := coalesce(new_cur.curr_business_unit_id,0);
            v_upd := 'Y';
         end if;
         --*******************************************************
        v_step := 'UPD1_80 - Team';
         if nvl(new_cur.curr_team_id,1) <> nvl(v_stage_upd_rec.team_id,1) then
            v_stage_upd_rec.team_id := coalesce(new_cur.curr_team_id,0);
            v_upd := 'Y';
         end if;
         --*******************************************************
        ...
        v_step := 'UPD1_100 - Status_Date';
         if nvl(new_cur.curr_task_status,'mm') <> nvl(v_stage_upd_rec.task_status,'mm') OR
            nvl(new_cur.curr_status_date,trunc(sysdate+200)) <> nvl(v_stage_upd_rec.status_date,trunc(sysdate+200))then
            v_stage_upd_rec.task_status := new_cur.curr_task_status;
            v_stage_upd_rec.status_date := new_cur.curr_status_date;
        ...
        v_step := 'Update Cancel Work ';
         if v_stage_upd_rec.cancel_work_date is not null THEN
            ...
            v_stage_upd_rec.cancelled_by_staff_id := coalesce(new_cur.curr_last_upd_by_staff_id,0);
            v_upd := 'Y';
         end if;
        --*******************************************************
