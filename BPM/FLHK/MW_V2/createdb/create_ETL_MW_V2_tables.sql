...
create unique index  MW_V2_TASK_ID on  CORP_ETL_MW_V2 (TASK_ID);
create index MW_V2_CASE_ID on CORP_ETL_MW_V2 (case_id);
create index MW_V2_CLIENT_ID on CORP_ETL_MW_V2 (client_id);

-- Create/Recreate primary, unique and foreign key constraints 
alter table  CORP_ETL_MW_V2  add primary key (CEMW_ID)  using index;
...
-- Create/Recreate indexes 
create unique index  MW_V2_TMP_TASK_ID on CORP_ETL_MW_V2_WIP (TASK_ID);
create unique index  MW_V2_TMP_CEMW_ID on CORP_ETL_MW_V2_WIP (CEMW_ID);
create index MW_V2_TMP_CASE_ID         on CORP_ETL_MW_V2_WIP (CASE_ID);
create index MW_V2_TMP_CLIENT_ID       on CORP_ETL_MW_V2_WIP (CLIENT_ID);
...
create index staff_key_lkup_idx1 on STAFF_KEY_LKUP(staff_key);
