...
create or replace trigger TRG_BIU_CORP_ETL_MW_V2
before insert or update on CORP_ETL_MW_V2_WIP
for each row
declare
...
begin
  ...
  if inserting then
    if
	  :new.CEMW_ID is null then  :new.CEMW_ID := SEQ_CEMW_V2_ID.nextval;
    end if;
    ...
    Then when you are inserting new records (MW_V2_Insert.ktr) you force it to â€˜Iâ€™
...
create or replace trigger TRG_AI_CORP_ETL_MW_V2_Q
after insert on CORP_ETL_MW_V2
for each row
...
create or replace trigger MAXDAT.TRG_AU_CORP_ETL_MW_V2_Q
after update on CORP_ETL_MW_V2
for each row
  ...
  v_xml_string_new :=
    ...
    <ROWSET>
      <ROW>
        ...
        <SOURCE_PROCESS_ID><![CDATA[' || :new.SOURCE_PROCESS_ID || ']]></SOURCE_PROCESS_ID>
        <SOURCE_PROCESS_INSTANCE_ID><![CDATA[' || :new.SOURCE_PROCESS_INSTANCE_ID || ']]></SOURCE_PROCESS_INSTANCE_ID>  
        <CHANNEL><![CDATA[' || :new.CHANNEL' || :new.CHANNEL || ']]></CHANNEL>
     </ROW>
    </ROWSET>
    ';
