...
create or replace PACKAGE BODY "BPM_SEMANTIC_PROJECT" as
  ...
  begin
   ...
   if p_bueq_row.BSL_ID = 2001 then 
    ...
    elsif p_bueq_row.BSL_ID = 35 then 
    FLHK_DOCUMENTS.INSERT_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.NEW_DATA);   
...
--      PROCESS_COMPLAINTS_INCIDENTS.INSERT_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.NEW_DATA);
  ...
  begin
    ...
    if p_bueq_row.BSL_ID = 2001 then 
      ...
      FLHK_DOCUMENTS.UPDATE_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.OLD_DATA,p_bueq_row.NEW_DATA);
