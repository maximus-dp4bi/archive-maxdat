...
Raj A. 05/12/2015 Changed D_Task_Types.Task_Name from 50 Bytes to 100. NYHIX has 50, NYEC has more than 50 bytes for step_definition.Display_name
Raj A. 10/08/2015 Removed MAXDAT prefix while creating F_MW_V2_BY_DATE_SV as TN project has a different schema name. 
*/
...
alter table D_MW_V2_CURRENT add constraint DMWCUR_V2_PK primary key (MW_BI_ID) using index tablespace MAXDAT_INDX;

create unique index DMWCUR_V2_UNIQ_TASK_ID on D_MW_V2_CURRENT (TASK_ID) online tablespace MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_CANCELLED_STF_ID on D_MW_V2_CURRENT (CANCELLED_BY_STAFF_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_CREATED_STF_ID on D_MW_V2_CURRENT (CURR_CREATED_BY_STAFF_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_ESCALATED_STF_ID on D_MW_V2_CURRENT (CURR_ESCALATED_TO_STAFF_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_FORWARDED_STF_ID on D_MW_V2_CURRENT (CURR_FORWARDED_BY_STAFF_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_LAST_UPD_BY_STF_ID on D_MW_V2_CURRENT (CURR_LAST_UPD_BY_STAFF_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_OWNER_STF_ID on D_MW_V2_CURRENT (CURR_OWNER_STAFF_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_CURR_TASK_TYPE_ID on D_MW_V2_CURRENT (TASK_TYPE_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_CURR_BUSUNIT_ID on D_MW_V2_CURRENT (CURR_BUSINESS_UNIT_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
create index DMWCUR_V2_CURR_TEAM_ID on D_MW_V2_CURRENT (CURR_TEAM_ID) TABLESPACE  MAXDAT_INDX parallel 4 compute statistics;
...
alter table D_TEAMS add constraint PK_TEAM_ID primary key (TEAM_ID) using index tablespace MAXDAT_INDX;
...
alter table D_MW_V2_HISTORY add constraint DMWBD_ID_PK primary key (DMWBD_ID) using index tablespace MAXDAT_INDX;
...
create unique index DMWBD_UIX2 on D_MW_V2_HISTORY (MW_BI_ID, BUCKET_START_DATE) online tablespace MAXDAT_INDX parallel 4 compute statistics;
create index DMWBD_IXL2 on D_MW_V2_HISTORY (BUCKET_START_DATE,BUCKET_END_DATE) local online tablespace MAXDAT_INDX parallel 4 compute statistics;
create index DMWBD_IXL3 on D_MW_V2_HISTORY (MW_BI_ID,BUCKET_START_DATE,BUCKET_END_DATE) local online tablespace MAXDAT_INDX parallel 4 compute statistics;
...
--Replace fact view 
CREATE OR REPLACE VIEW F_MW_V2_TASKS_BY_DAY_SV AS 
SELECT        d.MW_BI_ID,
              bdd.D_DATE,
...
grant select on F_MW_V2_TASKS_BY_DAY_SV to MAXDAT_READ_ONLY;