...
create or replace trigger TRG_BIU_CORP_ETL_COMM_OUTREACH
...
begin
  if inserting then
    if :new.CECOM_ID is null then
      :new.CECOM_ID := SEQ_CECOM_ID.nextval;
    end if;
...
if :new.EVENT_DATE < :new.REQUEST_DATE then
  v_instance_start_date := :new.EVENT_DATE;
else
    v_instance_start_date := :new.REQUEST_DATE;
  end if; 

--if :new.COMPLETE_DT is not null and :new.COMPLETE_DT >= v_instance_start_date then
if :new.COMPLETE_DT is not null and :new.EVENT_DATE > :new.COMPLETE_DT and :new.EVENT_DATE >= v_instance_start_date then 
v_instance_end_date := :new.EVENT_DATE;  
elsif  :new.COMPLETE_DT is not null and :new.EVENT_DATE <= :new.COMPLETE_DT and :new.COMPLETE_DT >= v_instance_start_date then
v_instance_end_date := :new.COMPLETE_DT; 
elsif :new.COMPLETE_DT is null then 
...
create or replace 
trigger TRG_AI_CORP_ETL_COMM_OUTRCH_Q
...
create or replace 
trigger TRG_AU_CORP_ETL_COMM_OUTRCH_Q 
