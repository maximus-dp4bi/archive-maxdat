...
create or replace trigger TRG_R_CORP_ETL_COMPLAINTS 
before insert or update on CORP_ETL_COMPLAINTS_INCIDENTS 
for each row
begin
  if inserting then
    if :new.CECI_ID is null then
      :new.CECI_ID := SEQ_CECI_ID.nextval;
    end if;
...
create or replace trigger TRG_AI_CORP_ETL_COMPLAINTS_Q
after insert on CORP_ETL_COMPLAINTS_INCIDENTS
for each row
  ...
  v_xml_string_new := 
    ...
    <ROWSET>  
      <ROW>
        ...
        <CASE_CIN>' || :new.CASE_CIN || '</CASE_CIN>
        ...
        <CREATED_BY_EID><![CDATA[' || :new.CREATED_BY_EID || ']]></CREATED_BY_EID>
        <CREATED_BY_SUP><![CDATA[' || :new.CREATED_BY_SUP || ']]></CREATED_BY_SUP>
        <CREATED_BY_SUP_NAME><![CDATA[' || :new.CREATED_BY_SUP_NAME || ']]></CREATED_BY_SUP_NAME>
        ...
        <GWF_ESCALATE_TO_STATE><![CDATA[' || :new.GWF_ESCALATE_TO_STATE || ']]></GWF_ESCALATE_TO_STATE>
...
create or replace trigger TRG_AU_CORP_ETL_COMPLAINTS_Q
after update on CORP_ETL_COMPLAINTS_INCIDENTS
for each row
  ...
  v_xml_string_old := 
    ...
    <ROWSET>  
      <ROW>
        ...
        <CASE_CIN>' || :old.CASE_CIN || '</CASE_CIN>
