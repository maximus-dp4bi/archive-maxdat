...
/*
select '     '|| 'CEPI_ID varchar2(100),' attr_element from dual  
  union 
...
create or replace package body DPY_PROCESS_INCIDENTS as
  ...
  v_bem_id number := 10; -- 'ILEB Process Incidents'
...
then
  ...
  if (p_instance_status           ='Active') then
    if ((sysdate-(p_receipt_date)>=0.5) and p_priority='1') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=1) and p_priority='2') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=2) and p_priority='3') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=3) and p_priority='4') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=4) and p_priority='5') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=5) and p_priority='6') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=6) and p_priority='7') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=7) and p_priority='8') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=8) and p_priority='9') then
      return 'Y';
    elsif (((trunc(sysdate)-trunc(p_receipt_date))>=9) and p_priority='10') then
      return 'Y';
    else
      return 'N';
    end if;
...
return varchar2
  ...
  if (p_instance_status='Complete') then
     ...
     if (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=1) and p_priority='1') then 
     return 'Processed Timely'; 
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=2) and p_priority='2') then 
     return 'Processed Timely';  
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=3) and p_priority='3') then 
     return 'Processed Timely';  
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=4) and p_priority='4') then 
     return 'Processed Timely';  
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=5) and p_priority='5') then 
     return 'Processed Timely';  
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=6) and p_priority='6') then 
     return 'Processed Timely';  
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=7) and p_priority='7') then 
     return 'Processed Timely';  
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=8) and p_priority='8') then 
     return 'Processed Timely';  
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=9) and p_priority='9') then 
     return 'Processed Timely';  
     elsif (((trunc(p_instance_status_dt)-trunc(p_receipt_date))<=10) and p_priority='10') then 
     return 'Processed Timely';  
    else
...
-- Get record for Process Incidents insert XML.
  ...
  begin  
/*
select '      extractValue(p_data_xml,''/ROWSET/ROW/CEPI_ID'') "' || 'CEPI_ID'|| '",' attr_element from dual
    union 
...
*/ 
 ...
 -- Insert BPM Event model data.
  ...
  if p_data_version != 1 then
      v_log_message := 'Unsupported BPM_UPDATE_EVENT_QUEUE.DATA_VERSION value "' || p_data_version || '" for ILEB DPY Process Incidents in procedure ' || v_procedure_name || '.';
  ...
  insert into BPM_INSTANCE 
    ...
    values
      (v_bi_id,v_bem_id,v_identifier,v_bil_id,v_bsl_id,
       v_start_date,v_end_date,to_char(v_new_data.CEPI_ID),to_date(v_new_data.STG_LAST_UPDATE_DATE,BPM_COMMON.DATE_FMT),to_date(v_new_data.STG_LAST_UPDATE_DATE,BPM_COMMON.DATE_FMT));
  ...
  v_bue_id := SEQ_BUE_ID.nextval;
    ...
    values
     ...
     BPM_EVENT.INSERT_BIA(v_bi_id,565,1,v_new_data.INCIDENT_ID,v_start_date,v_bue_id);    
     BPM_EVENT.INSERT_BIA(v_bi_id,566,1,v_new_data.TRACKING_NUMBER,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,567,2,v_new_data.CREATED_BY_GROUP,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,568,1,v_new_data.ORIGIN_ID,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,569,1,v_new_data.PRIORITY,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,570,2,v_new_data.INCIDENT_STATUS,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,571,3,v_new_data.INCIDENT_STATUS_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,572,2,v_new_data.ACTION_TYPE,v_start_date,v_bue_id);
    -- BPM_EVENT.INSERT_BIA(v_bi_id,573,4,v_new_data.ACTION_COMMENTS,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,574,2,v_new_data.RESOLUTION_TYPE,v_start_date,v_bue_id);
    -- BPM_EVENT.INSERT_BIA(v_bi_id,575,4,v_new_data.RESOLUTION_DESCRIPTION,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,576,2,v_new_data.INCIDENT_TYPE,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,577,2,v_new_data.INCIDENT_ABOUT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,578,2,v_new_data.INCIDENT_REASON,v_start_date,v_bue_id);
    -- BPM_EVENT.INSERT_BIA(v_bi_id,579,2,v_new_data.INCIDENT_DESCRIPTION,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,580,1,v_new_data.ABOUT_PROVIDER_ID,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,581,2,v_new_data.ABOUT_PLAN_CODE,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,582,2,v_new_data.REPORTED_BY,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,583,2,v_new_data.REPORTER_RELATIONSHIP,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,584,3,v_new_data.STATE_RECD_APPEAL_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,585,3,v_new_data.HEARING_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,586,1,v_new_data.SELECTION_ID,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,587,2,v_new_data.EB_FOLLOWUP_NEEDED_IND,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,588,2,v_new_data.OTHER_PARTY_NAME,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,589,2,v_new_data.ENROLLMENT_STATUS,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,590,2,v_new_data.PROGRAM_TYPE,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,591,2,v_new_data.PROGRAM_SUBTYPE,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,592,2,v_new_data.PLAN_CODE,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,593,1,v_new_data.CASE_ID,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,594,1,v_new_data.PROVIDER_ID,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,595,3,v_new_data.ASSD_IDENTIFY_RSRCH_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,596,3,v_new_data.ASED_IDENTIFY_RSRCH_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,597,2,v_new_data.ASF_IDENTIFY_RSRCH_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,598,2,v_new_data.GWF_ESCALATE_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,599,3,v_new_data.ESCALATE_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,600,3,v_new_data.ASSD_PROCESS_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,601,3,v_new_data.ASED_PROCESS_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,602,2,v_new_data.ASF_PROCESS_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,603,2,v_new_data.GWF_RETURN_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,604,3,v_new_data.RETURNED_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,605,3,v_new_data.ASSD_RESOLVE_CMPLT_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,606,3,v_new_data.ASED_RESOLVE_CMPLT_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,607,2,v_new_data.ASF_RESOLVE_CMPLT_INCIDENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,608,2,v_new_data.GWF_NOTIFY_CLIENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,609,1,v_new_data.CLIENT_NOTICE_ID,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,610,3,v_new_data.ASSD_NOTIFY_CLIENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,611,3,v_new_data.ASED_NOTIFY_CLIENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,612,2,v_new_data.ASF_NOTIFY_CLIENT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,613,2,v_new_data.GENERIC_FIELD1,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,614,2,v_new_data.GENERIC_FIELD2,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,615,2,v_new_data.GENERIC_FIELD3,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,616,2,v_new_data.GENERIC_FIELD4,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,617,2,v_new_data.GENERIC_FIELD5,v_start_date,v_bue_id);
   --  BPM_EVENT.INSERT_BIA(v_bi_id,618,2,v_new_data.ENROLLEE_RIN,v_start_date,v_bue_id);
   --  BPM_EVENT.INSERT_BIA(v_bi_id,619,2,v_new_data.REPORTER_NAME,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,620,3,v_new_data.RECEIPT_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,621,3,v_new_data.CREATE_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,622,2,v_new_data.CREATED_BY_NAME,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,623,2,v_new_data.CHANNEL,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,624,2,v_new_data.INSTANCE_STATUS,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,625,2,v_new_data.LAST_UPDATE_BY_NAME,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,626,3,v_new_data.LAST_UPDATE_BY_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,627,1,v_new_data.CURRENT_TASK_ID,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,628,3,v_new_data.COMPLETE_DT,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,629,3,v_new_data.CANCEL_DT,v_start_date,v_bue_id);
     
     BPM_EVENT.INSERT_BIA(v_bi_id,1767,2,v_new_data.CANCEL_BY,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,1768,2,v_new_data.CANCEL_METHOD,v_start_date,v_bue_id);
     BPM_EVENT.INSERT_BIA(v_bi_id,1769,2,v_new_data.CANCEL_REASON,v_start_date,v_bue_id);
 ...
 --  p_cur_incident_description	 in	varchar2, 
  ...
  -- p_status_age_in_business_days in	number,
  -- p_status_age_in_calendar_days in	number,
  -- p_cur_jeopardy_status	 in	varchar2, 
  -- p_jeopardy_status_date	 in	varchar2,
 ...
 -- r_dpicur."Cur Incident Description" 	      :=  p_cur_incident_description;	 
...
--  r_dpicur."Enrollee RIN" 		      :=  p_enrollee_rin;			 
--  r_dpicur."Reporter Name" 		      :=  p_reporter_name;			 
 ...
 -- Insert current dimension and fact as a single transaction.
   ...
   SET_DPICUR
	 ...
	 v_new_data.INCIDENT_REASON,/*v_new_data.INCIDENT_DESCRIPTION,*/v_new_data.ABOUT_PROVIDER_ID,v_new_data.ABOUT_PLAN_CODE,v_new_data.INCIDENT_STATUS,
	 ...
	 v_new_data.GENERIC_FIELD5,/*v_new_data.ENROLLEE_RIN,v_new_data.REPORTER_NAME,*/v_new_data.ASF_IDENTIFY_RSRCH_INCIDENT, v_new_data.cancel_by, v_new_data.cancel_method,v_new_data.cancel_reason
	); 
...
-- Update BPM Event model data.
 ...
 if p_data_version != 1 then
      v_log_message := 'Unsupported BPM_UPDATE_EVENT_QUEUE.DATA_VERSION value "' || p_data_version || '" with ILEB DPY Process Incidents in procedure ' || v_procedure_name || '.';
      RAISE_APPLICATION_ERROR(-20011,v_log_message);        
    end if; 
...
if v_end_date is not null then
  ...
  insert into BPM_UPDATE_EVENT(BUE_ID,BI_ID,BUTL_ID,EVENT_DATE,BPMS_PROCESSED)
    ...
    BPM_EVENT.UPDATE_BIA(v_bi_id,565,1,'N',v_old_data.INCIDENT_ID,v_new_data.INCIDENT_ID,v_bue_id,v_stg_last_update_date);      
    BPM_EVENT.UPDATE_BIA(v_bi_id,566,1,'N',v_old_data.TRACKING_NUMBER,v_new_data.TRACKING_NUMBER,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,567,2,'N',v_old_data.CREATED_BY_GROUP,v_new_data.CREATED_BY_GROUP,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,568,1,'N',v_old_data.ORIGIN_ID,v_new_data.ORIGIN_ID,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,569,1,'N',v_old_data.PRIORITY,v_new_data.PRIORITY,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,570,2,'Y',v_old_data.INCIDENT_STATUS,v_new_data.INCIDENT_STATUS,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,571,3,'Y',v_old_data.INCIDENT_STATUS_DT,v_new_data.INCIDENT_STATUS_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,572,2,'N',v_old_data.ACTION_TYPE,v_new_data.ACTION_TYPE,v_bue_id,v_stg_last_update_date);
  --  BPM_EVENT.UPDATE_BIA(v_bi_id,573,4,'Y',v_old_data.ACTION_COMMENTS,v_new_data.ACTION_COMMENTS,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,574,2,'N',v_old_data.RESOLUTION_TYPE,v_new_data.RESOLUTION_TYPE,v_bue_id,v_stg_last_update_date);
  --  BPM_EVENT.UPDATE_BIA(v_bi_id,575,4,'Y',v_old_data.RESOLUTION_DESCRIPTION,v_new_data.RESOLUTION_DESCRIPTION,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,576,2,'N',v_old_data.INCIDENT_TYPE,v_new_data.INCIDENT_TYPE,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,577,2,'Y',v_old_data.INCIDENT_ABOUT,v_new_data.INCIDENT_ABOUT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,578,2,'Y',v_old_data.INCIDENT_REASON,v_new_data.INCIDENT_REASON,v_bue_id,v_stg_last_update_date);
  --  BPM_EVENT.UPDATE_BIA(v_bi_id,579,2,'Y',v_old_data.INCIDENT_DESCRIPTION,v_new_data.INCIDENT_DESCRIPTION,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,580,1,'N',v_old_data.ABOUT_PROVIDER_ID,v_new_data.ABOUT_PROVIDER_ID,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,581,2,'N',v_old_data.ABOUT_PLAN_CODE,v_new_data.ABOUT_PLAN_CODE,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,582,2,'N',v_old_data.REPORTED_BY,v_new_data.REPORTED_BY,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,583,2,'N',v_old_data.REPORTER_RELATIONSHIP,v_new_data.REPORTER_RELATIONSHIP,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,584,3,'N',v_old_data.STATE_RECD_APPEAL_DT,v_new_data.STATE_RECD_APPEAL_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,585,3,'N',v_old_data.HEARING_DT,v_new_data.HEARING_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,586,1,'N',v_old_data.SELECTION_ID,v_new_data.SELECTION_ID,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,587,2,'N',v_old_data.EB_FOLLOWUP_NEEDED_IND,v_new_data.EB_FOLLOWUP_NEEDED_IND,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,588,2,'N',v_old_data.OTHER_PARTY_NAME,v_new_data.OTHER_PARTY_NAME,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,589,2,'Y',v_old_data.ENROLLMENT_STATUS,v_new_data.ENROLLMENT_STATUS,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,590,2,'N',v_old_data.PROGRAM_TYPE,v_new_data.PROGRAM_TYPE,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,591,2,'N',v_old_data.PROGRAM_SUBTYPE,v_new_data.PROGRAM_SUBTYPE,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,592,2,'N',v_old_data.PLAN_CODE,v_new_data.PLAN_CODE,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,593,1,'N',v_old_data.CASE_ID,v_new_data.CASE_ID,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,594,1,'N',v_old_data.PROVIDER_ID,v_new_data.PROVIDER_ID,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,595,3,'N',v_old_data.ASSD_IDENTIFY_RSRCH_INCIDENT,v_new_data.ASSD_IDENTIFY_RSRCH_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,596,3,'N',v_old_data.ASED_IDENTIFY_RSRCH_INCIDENT,v_new_data.ASED_IDENTIFY_RSRCH_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,597,2,'N',v_old_data.ASF_IDENTIFY_RSRCH_INCIDENT,v_new_data.ASF_IDENTIFY_RSRCH_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,598,2,'N',v_old_data.GWF_ESCALATE_INCIDENT,v_new_data.GWF_ESCALATE_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,599,3,'N',v_old_data.ESCALATE_DT,v_new_data.ESCALATE_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,600,3,'N',v_old_data.ASSD_PROCESS_INCIDENT,v_new_data.ASSD_PROCESS_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,601,3,'N',v_old_data.ASED_PROCESS_INCIDENT,v_new_data.ASED_PROCESS_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,602,2,'N',v_old_data.ASF_PROCESS_INCIDENT,v_new_data.ASF_PROCESS_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,603,2,'N',v_old_data.GWF_RETURN_INCIDENT,v_new_data.GWF_RETURN_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,604,3,'N',v_old_data.RETURNED_DT,v_new_data.RETURNED_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,605,3,'N',v_old_data.ASSD_RESOLVE_CMPLT_INCIDENT,v_new_data.ASSD_RESOLVE_CMPLT_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,606,3,'N',v_old_data.ASED_RESOLVE_CMPLT_INCIDENT,v_new_data.ASED_RESOLVE_CMPLT_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,607,2,'N',v_old_data.ASF_RESOLVE_CMPLT_INCIDENT,v_new_data.ASF_RESOLVE_CMPLT_INCIDENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,608,2,'N',v_old_data.GWF_NOTIFY_CLIENT,v_new_data.GWF_NOTIFY_CLIENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,609,1,'N',v_old_data.CLIENT_NOTICE_ID,v_new_data.CLIENT_NOTICE_ID,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,610,3,'N',v_old_data.ASSD_NOTIFY_CLIENT,v_new_data.ASSD_NOTIFY_CLIENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,611,3,'N',v_old_data.ASED_NOTIFY_CLIENT,v_new_data.ASED_NOTIFY_CLIENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,612,2,'N',v_old_data.ASF_NOTIFY_CLIENT,v_new_data.ASF_NOTIFY_CLIENT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,613,2,'N',v_old_data.GENERIC_FIELD1,v_new_data.GENERIC_FIELD1,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,614,2,'N',v_old_data.GENERIC_FIELD2,v_new_data.GENERIC_FIELD2,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,615,2,'N',v_old_data.GENERIC_FIELD3,v_new_data.GENERIC_FIELD3,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,616,2,'N',v_old_data.GENERIC_FIELD4,v_new_data.GENERIC_FIELD4,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,617,2,'N',v_old_data.GENERIC_FIELD5,v_new_data.GENERIC_FIELD5,v_bue_id,v_stg_last_update_date);
  --  BPM_EVENT.UPDATE_BIA(v_bi_id,618,2,'N',v_old_data.ENROLLEE_RIN,v_new_data.ENROLLEE_RIN,v_bue_id,v_stg_last_update_date);
  --  BPM_EVENT.UPDATE_BIA(v_bi_id,619,2,'N',v_old_data.REPORTER_NAME,v_new_data.REPORTER_NAME,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,620,3,'N',v_old_data.RECEIPT_DT,v_new_data.RECEIPT_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,621,3,'N',v_old_data.CREATE_DT,v_new_data.CREATE_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,622,2,'N',v_old_data.CREATED_BY_NAME,v_new_data.CREATED_BY_NAME,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,623,2,'N',v_old_data.CHANNEL,v_new_data.CHANNEL,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,624,2,'Y',v_old_data.INSTANCE_STATUS,v_new_data.INSTANCE_STATUS,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,625,2,'Y',v_old_data.LAST_UPDATE_BY_NAME,v_new_data.LAST_UPDATE_BY_NAME,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,626,3,'Y',v_old_data.LAST_UPDATE_BY_DT,v_new_data.LAST_UPDATE_BY_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,627,1,'Y',v_old_data.CURRENT_TASK_ID,v_new_data.CURRENT_TASK_ID,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,628,3,'N',v_old_data.COMPLETE_DT,v_new_data.COMPLETE_DT,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,629,3,'N',v_old_data.CANCEL_DT,v_new_data.CANCEL_DT,v_bue_id,v_stg_last_update_date);
    
    BPM_EVENT.UPDATE_BIA(v_bi_id,1767,2,'N',v_old_data.CANCEL_BY,v_new_data.CANCEL_BY,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,1768,2,'N',v_old_data.CANCEL_METHOD,v_new_data.CANCEL_METHOD,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,1769,2,'N',v_old_data.CANCEL_REASON,v_new_data.CANCEL_REASON,v_bue_id,v_stg_last_update_date);
