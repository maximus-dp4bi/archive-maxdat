...
package body PROCESS_LETTERS as
  ...
  BEGIN
     ...
     if SUBSTR(p_letter_type,1,2) <> 'IA' 
        then v_sla_category := 'Other';
     elsif 
        SUBSTR(p_letter_type,1,2) = 'IA' and NVL(p_newborn_flag,'N') = 'N' 
        then v_sla_category := 'New Enrollment';
     elsif 
        SUBSTR(p_letter_type,1,2) = 'IA' and NVL(p_newborn_flag,'N') = 'Y' 
        then v_sla_category := 'Newly Eligible Newborn';
     else 
        v_sla_category := 'Other';
     end if;
 ...
 FUNCTION GET_JEOPARDY_DAYS(
  ...
  as
    ...
    v_sid number := null;
    v_serial_number number := null;
    v_logon_time varchar2(17) := null;
  ...
  begin
    ...
    begin
      ...
      v_log_message := 'V$SESSION: USERNAME=' || v_username || ' SID=' || v_sid || ' SERIAL#=' || v_serial_number || ' LOGIN_TIME=' || v_logon_time;
    ...
    update /*+ PARALLEL(D_PL_CURRENT,6) */ D_PL_CURRENT
    set
