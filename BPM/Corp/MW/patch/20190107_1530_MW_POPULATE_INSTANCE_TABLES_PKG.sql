...
-- Do not edit these four SVN_* variable values.  They are populated when you commit code to SVN and used later to identify deployed code.
    SVN_FILE_URL varchar2(200) := '$URL: svn://svn-staging.maximus.com/dev1d/maxdat/BPM/Corp/MW/patch/20181031_1530_MW_POPULATE_INSTANCE_TABLES_PKG_UAT.sql $';
    SVN_REVISION varchar2(20) := '$Revision: 25826 $';
    SVN_REVISION_DATE varchar2(60) := '$Date: 2018-12-21 19:00:13 -0500 (Fri, 21 Dec 2018) $';
...
, p.PARENT_PROCESS_ID
, (SELECT min(m11.create_date) FROM GTT_MW_TASK_INSTANCE m11 WHERE m11.source_process_instance_id = m1.source_process_instance_id
and m11.task_type_id = tte.task_type_id and e.is_starting_entity = 'Y') process_start_date
, (SELECT max(m11.COMPLETE_DATE) FROM GTT_MW_TASK_INSTANCE m11 WHERE m11.source_process_instance_id = m1.source_process_instance_id
      and m11.task_type_id = tte.task_type_id and e.is_terminating_entity = 'Y'
      AND m11.CREATE_DATE = (SELECT max(create_date) FROM GTT_MW_TASK_INSTANCE m12 WHERE m12.source_process_instance_id = m11.source_process_instance_id
      and m12.task_type_id = tte.task_type_id and e.is_terminating_entity = 'Y')) process_complete_date
...
, (SELECT max(m11.COMPLETE_DATE) FROM D_MW_TASK_INSTANCE m11, d_bpm_entity en, d_bpm_task_type_entity tte WHERE m11.source_process_instance_id = mw.source_process_instance_id
      and en.entity_id = tte.entity_id and m11.task_type_id = tte.task_type_id and en.is_terminating_entity = 'Y'
      AND m11.CREATE_DATE = (SELECT max(create_date) FROM D_MW_TASK_INSTANCE m12, d_bpm_entity en, d_bpm_task_type_entity tte 
      WHERE m12.source_process_instance_id = m11.source_process_instance_id
      and en.entity_id = tte.entity_id and m12.task_type_id = tte.task_type_id and en.is_terminating_entity = 'Y'
...
AND (SELECT max(m11.COMPLETE_DATE) FROM D_MW_TASK_INSTANCE m11, d_bpm_entity en, d_bpm_task_type_entity tte WHERE m11.source_process_instance_id = mw.source_process_instance_id
      and en.entity_id = tte.entity_id and m11.task_type_id = tte.task_type_id and en.is_terminating_entity = 'Y'
      AND m11.CREATE_DATE = (SELECT max(create_date) FROM D_MW_TASK_INSTANCE m12, d_bpm_entity en, d_bpm_task_type_entity tte 
      WHERE m12.source_process_instance_id = m11.source_process_instance_id
      and en.entity_id = tte.entity_id and m12.task_type_id = tte.task_type_id and en.is_terminating_entity = 'Y'
...
FROM 
--(SELECT source_entity_id entity_id, source_task_id task_id FROM new_flow_instance
--      UNION
