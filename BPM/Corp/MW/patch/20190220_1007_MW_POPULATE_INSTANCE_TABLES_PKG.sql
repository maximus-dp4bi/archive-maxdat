...
select tte.task_type_id, tte.entity_id, e.is_starting_entity, e.is_terminating_entity 
FROM D_BPM_TASK_TYPE_ENTITY tte, d_bpm_entity e
where tte.entity_id = e.entity_id
)
...
, p.PARENT_PROCESS_ID
, (SELECT min(m11.create_date) FROM GTT_MW_TASK_INSTANCE m11, tte_start_terminator tte_st WHERE m11.source_process_instance_id = m1.source_process_instance_id
and m11.task_type_id = tte_st.task_type_id and tte_st.is_starting_entity = 'Y') process_start_date
, (SELECT max(m11.COMPLETE_DATE) FROM GTT_MW_TASK_INSTANCE m11, tte_start_terminator tte_st WHERE m11.source_process_instance_id = m1.source_process_instance_id
      and m11.task_type_id = tte_st.task_type_id and tte_st.is_terminating_entity = 'Y'
      AND m11.CREATE_DATE = (SELECT max(create_date) FROM GTT_MW_TASK_INSTANCE m12, tte_start_terminator tte_st WHERE m12.source_process_instance_id = m11.source_process_instance_id
      and m12.task_type_id = tte_st.task_type_id and tte_st.is_terminating_entity = 'Y')) process_complete_date
...
, nvl(pi.process_start_date,(SELECT min(m10.create_date) FROM D_MW_TASK_INSTANCE m10, tte_start_terminator tte_st WHERE m10.source_process_instance_id = mw.source_process_instance_id
and m10.task_type_id = tte_st.task_type_id and tte_st.is_starting_entity = 'Y')) process_start_date
, nvl(pi.process_complete_date, (SELECT max(m11.COMPLETE_DATE) FROM D_MW_TASK_INSTANCE m11, tte_start_terminator tte_st WHERE m11.source_process_instance_id = mw.source_process_instance_id
      and m11.task_type_id = tte_st.task_type_id and tte_st.is_terminating_entity = 'Y'
      AND m11.CREATE_DATE = (SELECT max(create_date) FROM D_MW_TASK_INSTANCE m12, tte_start_terminator tte_st WHERE m12.source_process_instance_id = m11.source_process_instance_id
      and m12.task_type_id = tte_st.task_type_id and tte_st.is_terminating_entity = 'Y'))) process_complete_date
, 'UNKN' timeliness_status
...
AND exists (SELECT m11.COMPLETE_DATE FROM D_MW_TASK_INSTANCE m11, tte_start_terminator tte_st WHERE m11.source_process_instance_id = mw.source_process_instance_id
      and m11.task_type_id = tte_st.task_type_id and tte_st.is_terminating_entity = 'Y' and m11.complete_date is not null))
OR ( pi.PROCESS_START_DATE IS NULL AND exists (SELECT m10.create_date FROM D_MW_TASK_INSTANCE m10, tte_start_terminator tte_st WHERE m10.source_process_instance_id = mw.source_process_instance_id
and m10.task_type_id = tte_st.task_type_id and tte_st.is_starting_entity = 'Y' and m10.create_date is not null))
...
sELECT DISTINCT ti.task_id ENTITY_INSTANCE_ID, e.entity_id, e.entity_type_id, ti.source_process_instance_id PROCESS_INSTANCE_ID, e.entity_name, e.entity_description, ti.create_date start_date, ti.complete_date, 'UNKN' timeliness_status,
CASE WHEN TI.complete_date IS NULL THEN 'Y' ELSE 'N' END is_active, e.is_starting_entity, e.is_terminating_entity
FROM GTT_MW_TASK_INSTANCE ti
JOIN d_bpm_process_instance pi
ON ti.source_process_instance_id = pi.PROCESS_INSTANCE_ID
join d_bpm_task_type_entity te
on ti.task_type_id = te.task_type_id
join d_bpm_entity e 
on te.entity_id = e.entity_id
and e.process_id = pi.process_id
)
...
JOIN d_bpm_process_instance pi
ON pi.process_instance_id = utl1.source_process_instance_id
JOIN d_bpm_entity_instance ei
ON ei.entity_instance_id = utl1.task_id
JOIN d_bpm_entity_instance ei2
ON ei2.entity_instance_id = utl2.task_id
