  use maxdat;
  SVN_FILE_URL varchar(200) = '$URL$'; 
  SVN_REVISION varchar(20) = '$Revision$'; 
  SVN_REVISION_DATE varchar(60) = '$Date$'; 
  SVN_REVISION_AUTHOR varchar(20) = '$Author$';

  JOB_STATUS_FAILED     varchar(10) = 'FAILED';
  JOB_STATUS_LOCKING    varchar(10) = 'LOCKING';
  JOB_STATUS_PROCESSING varchar(10) = 'PROCESSING';
  JOB_STATUS_RESERVING  varchar(10) = 'RESERVING';
  JOB_STATUS_SLEEPING   varchar(10) = 'SLEEPING';
  JOB_STATUS_STARTED    varchar(10) = 'STARTED';
  JOB_STATUS_STOPPED    varchar(10) = 'STOPPED';


  @v_batch_sleep_seconds integer =   1;
  @v_process_sleep_seconds integer = 120;

  @v_bdm_name_bpm_semantic varchar(12) = 'BPM_SEMANTIC';
  @v_bdm_num_bpm_semantic  integer = 2;

  -- Handle Numbers can be anything unique.
  @v_lock_handle_prefix integer = 120000; -- random #

  @v_lock_timeout_seconds  integer = 300;
  



  -- Get data model name.
  create function PROCESS_BPM_QUEUE_GET_DATA_MODEL_NAME (@p_bdm_id integer) 
  returns varchar
  as begin
    declare @v_procedure_name varchar= OBJECT_NAME(@@PROCID) + '.' + 'GET_DATA_MODEL_NAME',
    @v_log_message varchar = null,
    @v_error_number integer = null,
    @v_bdm_name varchar(12) = null,

  @v_bdm_name_bpm_semantic varchar(12) = 'BPM_SEMANTIC',
  @v_bdm_num_bpm_semantic  integer = 2;


    if @p_bdm_id = @v_bdm_num_bpm_semantic
      select @v_bdm_name = @v_bdm_name_bpm_semantic;
    else begin
      select @v_log_message = 'Unsupported data model number "' + @p_bdm_id + '".';
      select @v_error_number = 50023;
      exec dbo.BPM_COMMON_LOGGER(dbo.BPM_COMMON_LOG_LEVEL_SEVERE(),null,@v_procedure_name,null,null,null,null,@v_log_message,null);
  ...
  function PROCESS_BPM_QUEUE_GET_BDM_ID (p_data_model_name varchar) 
  return number result_cache
  as
    @v_procedure_name varchar(61) = OBJECT_NAME(@@PROCID) + '.' + 'GET_BDM_ID';
  ...
  function PROCESS_BPM_QUEUE_GET_LOCK_HANDLE_NUM
    (p_bsl_id number,
     p_bdm_id number) 
  return number result_cache
  as
  ...
  function PROCESS_BPM_QUEUE_GET_RELEASE_LOCK_STATUS_NAME (p_status integer) 
  return varchar result_cache
  as
    @v_status_name varchar(40) = null;
  begin
    case
  ...
  function PROCESS_BPM_QUEUE_GET_REQUEST_LOCK_STATUS_NAME (p_status integer)  
  return varchar result_cache
  as
    @v_status_name varchar(40) = null;
  begin
  ...
  -- Release a lock.
  procedure PROCESS_BPM_QUEUE_RELEASE_LOCK
    (p_bsl_id in number,
     p_bdm_id in number,
     p_status out number)
  as
  ...
  -- Request a lock.
  procedure PROCESS_BPM_QUEUE_REQUEST_LOCK
    (p_bsl_id in number,
     p_bdm_id in number,
  ...
  procedure PROCESS_BPM_QUEUE_RELEASE_QUEUE_ROW_ARCHIVE
    (p_bueq_id in number)
  as
  begin
  ...
  procedure PROCESS_BPM_QUEUE_ARCHIVE_PROCESSED_ROW
    (p_bueq_id in number)
  as
  begin
  ...
  procedure PROCESS_BPM_QUEUE_RELEASE_QUEUE_ROW
    (p_bueq_id in number)
  as
  begin
    if p_bueq_id is null then
  ...
  procedure PROCESS_BPM_QUEUE_RESERVE_QUEUE_BY_BSL
  ...
  as
    @v_procedure_name varchar(61) = OBJECT_NAME(@@PROCID) + '.' + 'RESERVE_QUEUE_BY_BSL';
    @v_log_message clob = null;
    @v_num_rows_updated number = null;
    @v_status number = null;
    @v_enabled varchar(1) = null;
  ...
  procedure PROCESS_BPM_QUEUE_PROCESS_RESERVED_ROWS
    (p_bdm_id in number,
     p_job_id in number,
     p_job_batch_id in number,
     p_process_bueq_id in number)
  as 
    @v_procedure_name varchar(61) = OBJECT_NAME(@@PROCID) + '.' + 'PROCESS_RESERVED_ROWS';
    @v_sql_code number = null;
    @v_log_message clob = null;
    @v_enabled varchar(1) = null;
    @v_prev_identifier varchar(100) = null;
    @v_block_identifier varchar(100) = null;
  ...
  procedure PROCESS_BPM_QUEUE_PROCESS_ALL_ROWS_BY_BSL
    (p_bsl_id in number,
     p_bdm_id in number,
     p_batch_size in number,
     p_job_id in number)
  as
    @v_procedure_name varchar(61) = OBJECT_NAME(@@PROCID) + '.' + 'PROCESS_ALL_ROWS_BY_BSL';
    @v_log_message clob = null;
    @v_sql_code number = null;
    @v_bdm_num number = null;
