...
AS
  ...
  FROM ats.enum_county ec
  LEFT OUTER JOIN
    (SELECT DISTINCT c.plan_service_type_cd, 
      cc.county_cd 
      FROM ats.contract_amendment ca
      JOIN ats.county_contract cc ON (cc.contract_amendment_id = ca.contract_amendment_id )
      JOIN ats.contract c ON (ca.contract_id = c.contract_id )
