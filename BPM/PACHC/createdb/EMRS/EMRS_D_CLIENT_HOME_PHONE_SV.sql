...
SELECT COALESCE(p.PHON_ID, -1*c.clnt_client_id) AS PHONE_ID
  ...
  ,CASE WHEN ROW_NUMBER() OVER(PARTITION BY c.CLNT_CLIENT_ID ORDER BY COALESCE(p.PHON_ID, -1*c.clnt_client_id)) = 1 
        ...
        END AS IS_CURRENT
FROM ATS.CLIENT c
LEFT JOIN ATS.PHONE_NUMBER p ON (p.PHON_TYPE_CD IN ('HM','CH', 'H2', 'PR') 
                                AND c.clnt_client_id = p.clnt_client_id)
LEFT JOIN ATS.ENUM_PHONE_TYPE t ON (p.PHON_TYPE_CD = t.VALUE);

GRANT SELECT ON EMRS_D_CLIENT_HOME_PHONE_SV TO MAXDAT_REPORTS;