CREATE OR REPLACE VIEW EMRS_F_ENROLLMENT_SV
 ...
 SELECT 
    ...
    COALESCE(s.selection_txn_id, ss.SELECTION_SEGMENT_ID*-1) AS SELECTION_TRANSACTION_ID,
    ...
    COALESCE(s.selection_source_cd,'A') AS SELECTION_SOURCE_CODE,
...
--    COALESCE(ac.subprogram_codes, '0') AS SUBPROGRAM_CODE , --on Aid Category, this field is not used
    ...
    CASE WHEN ss.PROVIDER_ID_EXT IS NULL THEN 'N'
          ELSE 'Y'
    END AS PCP_SELECTED,
