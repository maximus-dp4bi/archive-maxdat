...
--------------------------------------------------------
  ...
  begin
    ...
    if p_bueq_row.BSL_ID = 2003 then 
      ...
      null;
    elsif p_bueq_row.BSL_ID = 2005 then
      Process_Documents.INSERT_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.NEW_DATA);
      null;
    elsif p_bueq_row.BSL_ID = 2006 then
      Process_Claims.INSERT_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.NEW_DATA);
      null;
    elsif p_bueq_row.BSL_ID = 2007 then
      Process_Claims_LI.INSERT_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.NEW_DATA);
      null;
    else
  ...
  begin
      ...
      Process_Documents.UPDATE_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.OLD_DATA,p_bueq_row.NEW_DATA);
      elsif p_bueq_row.BSL_ID = 2006 then 
      Process_Claims.UPDATE_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.OLD_DATA,p_bueq_row.NEW_DATA);
      elsif p_bueq_row.BSL_ID = 2007 then 
      Process_Claims_LI.UPDATE_BPM_SEMANTIC(p_bueq_row.DATA_VERSION,p_bueq_row.OLD_DATA,p_bueq_row.NEW_DATA);
    else
      v_log_message := 'Unexpected BPM_UPDATE_EVENT_QUEUE.BSL_ID value "' || p_bueq_row.BSL_ID || '" in procedure ' || v_procedure_name || '.';
      v_error_number := -20012;
