...
AS SELECT d_date
       ...
       ,SUM(withdrawn_count) withdrawn_count
       ,SUM(closed_count) + SUM(cancellation_count) + SUM(withdrawn_count) termination_count
       ,SUM(timely_appeals_count) timely_appeals_count
       ,SUM(untimely_appeals_count) untimely_appeals_count
FROM (
  SELECT TRUNC(create_date) d_date, COUNT(1) creation_count, 0 inventory_count, 0 sla_inventory_count, 0 completion_count, 0 closed_count, 0 cancellation_count, 0 withdrawn_count, 
0 timely_appeals_count, 0 untimely_appeals_count
  FROM D_MW_APPEAL_INSTANCE
  GROUP BY TRUNC(create_date)
  UNION ALL
  SELECT TRUNC(complete_date) d_date, 0 creation_count, 0 inventory_count, 0 sla_inventory_count, COUNT(1) completion_count,0 closed_count, 0 cancellation_count, 0 withdrawn_count,
sum(case when complete_date <= deadline_date then 1 else 0 end) timely_appeals_count, 
sum(case when complete_date > deadline_date then 1 else 0 end) untimely_appeals_count
  FROM D_MW_APPEAL_INSTANCE
  WHERE complete_date IS NOT NULL
  GROUP BY TRUNC(complete_date)
  UNION ALL
  SELECT TRUNC(closed_date) d_date, 0 creation_count, 0 inventory_count, 0 sla_inventory_count, 0 completion_count,count(1) closed_count, 0 cancellation_count, 0 withdrawn_count,
0 timely_appeals_count, 0 untimely_appeals_count
  FROM D_MW_APPEAL_INSTANCE
  WHERE closed_date IS NOT NULL
  GROUP BY TRUNC(closed_date)
  UNION ALL
  SELECT TRUNC(cancelled_date) d_date, 0 creation_count, 0 inventory_count, 0 sla_inventory_count, 0 completion_count, 0 closed_count, COUNT(1) cancellation_count, 0 withdrawn_count,
0 timely_appeals_count, 0 untimely_appeals_count
  FROM D_MW_APPEAL_INSTANCE
  WHERE cancelled_date IS NOT NULL
  GROUP BY TRUNC(cancelled_date)
  UNION ALL
  SELECT TRUNC(withdrawn_date) d_date, 0 creation_count, 0 inventory_count, 0 sla_inventory_count, 0 completion_count, 0 closed_count, 0 cancellation_count, count(1) withdrawn_count,
0 timely_appeals_count, 0 untimely_appeals_count
  FROM D_MW_APPEAL_INSTANCE
  WHERE withdrawn_date IS NOT NULL
  GROUP BY TRUNC(withdrawn_date)
  UNION ALL
  SELECT d_date, 0 creation_count, COUNT(1) inventory_count, 0 sla_inventory_count, 0 completion_count, 0 closed_count, 0 cancellation_count, 0 withdrawn_count,
0 timely_appeals_count, 0 untimely_appeals_count
  FROM D_MW_APPEAL_INSTANCE
    JOIN d_dates bdd
  ...
  GROUP BY d_date
  UNION ALL
  SELECT d_date, 0 creation_count, 0 inventory_count, count(1) sla_inventory_count, 0 completion_count, 0 closed_count, 0 cancellation_count, 0 withdrawn_count,
0 timely_appeals_count, 0 untimely_appeals_count
  FROM D_MW_APPEAL_INSTANCE
    JOIN d_dates bdd
      ON  ((create_date is null) OR (bdd.D_DATE >= TRUNC(create_date)))
     AND ( 
            	((closed_date is null) OR (bdd.d_date < trunc(closed_date))) AND
            	((complete_date is null) OR (bdd.d_date < trunc(complete_date))) AND
 		((cancelled_date is null) OR (bdd.d_date < trunc(cancelled_date))) AND
     		((withdrawn_date is null) OR (bdd.d_date < trunc(withdrawn_date)))            
        )     
  GROUP BY d_date
