...
create or replace package ETL_COMMON as
  ...
  SVN_FILE_URL varchar2(200) := '$URL: svn://svn-staging.maximus.com/dev1d/maxdat/BPM/Core/createdb/ETL_COMMON_pkg.sql $';
  SVN_REVISION varchar2(20) := '$Revision: 17631 $';
  SVN_REVISION_DATE varchar2(60) := '$Date: 2016-06-30 17:46:37 -0400 (Thu, 30 Jun 2016) $';
...
create or replace package body ETL_COMMON as
  ...
  begin
    ...
    loop
      ...
      if v_daynum between 2 and 7 then
  ...
  begin
     v_curdate := TRUNC(p_start_date);
      ...
      if (to_char(v_curdate,'D') IN(1) OR v_holiday != 0) and p_number_days = 0  then      
        v_businessdays := 1;        
      end if;
    ...
    loop
       ...
       v_daynum  := to_char(v_curdate,'D');
       if (v_daynum between 2 and 7) AND v_holiday = 0 then
          v_counter := v_counter + 1;
       else
         v_skipcntr := v_skipcntr + 1;
       end if;
