CREATE OR REPLACE package MAXDAT.MW_POPULATE_INSTANCE_TABLES as

-- Do not edit these four SVN_* variable values.  They are populated when you commit code to SVN and used later to identify deployed code.
    SVN_FILE_URL varchar2(200) := '$URL: svn://svn-staging.maximus.com/dev1d/maxdat/BPM/FEDQIC/ManageWork/createdb/MW_POPULATE_INSTANCE_TABLES_PKG.sql $';
    SVN_REVISION varchar2(20) := '$Revision: 22763 $';
    SVN_REVISION_DATE varchar2(60) := '$Date: 2018-03-15 17:08:10 -0400 (Thu, 15 Mar 2018) $';
...
CREATE OR REPLACE package body MAXDAT.MW_POPULATE_INSTANCE_TABLES as
...
SELECT coalesce((MAX(job_end_date) - 2), SYSDATE) INTO v_last_mw_run_date FROM CORP_ETL_JOB_STATISTICS WHERE JOB_NAME ='ManageWork_RUNALL' AND JOB_STATUS_CD = 'COMPLETED';
-----------------------------------------------------------------------------
...
, updated_task_list AS ( -- Generate a list of open, new and updated task records that are used in a process
SELECT ti.* 
...
, p.PARENT_PROCESS_ID
, (SELECT min(m11.create_date) FROM updated_task_list m11 WHERE m11.source_process_instance_id = m1.source_process_instance_id) process_start_date
, (SELECT m11.COMPLETE_DATE FROM updated_task_list m11 WHERE m11.source_process_instance_id = m1.source_process_instance_id 
      AND m11.CREATE_DATE = (SELECT max(create_date) FROM updated_task_list m12 WHERE m12.source_process_instance_id = m11.source_process_instance_id)) process_complete_date
, 'UNKN' timeliness_status
FROM updated_task_list m1
...
, (SELECT m11.COMPLETE_DATE FROM D_MW_TASK_INSTANCE m11 WHERE m11.source_process_instance_id = mw.source_process_instance_id 
...
AND (SELECT m11.COMPLETE_DATE FROM D_MW_TASK_INSTANCE m11 WHERE m11.source_process_instance_id = mw.source_process_instance_id 
...
JOIN updated_task_list utl1
ON cfl.source_task_type_id = utl1.task_type_id
JOIN updated_task_list utl2
ON cfl.destination_task_type_id = utl2.task_type_id
WHERE ((utl1.complete_date = utl2.create_date ) OR (utl1.complete_date <= utl2.create_date AND (utl2.create_date - utl1.complete_date) <.0002))
...
FROM (SELECT source_entity_id entity_id, source_task_id task_id FROM new_flow_instance
      UNION 
      SELECT destination_entity_id entity_id, dest_task_id task_id  FROM new_flow_instance) a
JOIN updated_task_list ti
...
GRANT EXECUTE ON MAXDAT.MW_POPULATE_INSTANCE_TABLES TO MAXDAT_PFP_E;
GRANT DEBUG ON MAXDAT.MW_POPULATE_INSTANCE_TABLES TO MAXDAT_PFP_READ_ONLY;
GRANT EXECUTE ON MAXDAT.MW_POPULATE_INSTANCE_TABLES TO MAXDAT_READ_ONLY;
