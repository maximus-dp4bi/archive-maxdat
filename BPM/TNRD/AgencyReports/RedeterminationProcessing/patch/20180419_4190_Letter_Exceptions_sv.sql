-- Unable to render VIEW DDL for object MAXDAT.LETTER_EXCEPTIONS_SV with DBMS_METADATA attempting internal generator.
...
OR
--check 406 later
(S.LAST_NAME = ('System') AND LR.LETTER_STATUS_CD IN ('SENT') AND LD.LMDEF_ID IN ('62', '63', '64','65', '66', '67', '68', '69', '70', '71', '86', '87', '81', '82')
AND TRUNC (LR.LETTER_REQUESTED_ON) = MI.NOTIFICATION_DATE AND LR.LETTER_REQUEST_TYPE IN ('L', 'S')
--AND TRUNC(LR.LETTER_REQUESTED_ON) = (select trunc(decode( to_char( sysdate, 'DY' ), 'MON', sysdate -3, sysdate -1 )) from dual)
AND TRUNC(LR.LETTER_SENT_ON) = (select max(d_date) from d_dates
where business_day_flag = 'Y'
...
SELECT distinct letter_req_id,
        ...
        -- receipt_date,
         ...
         || no_msp_threshold
         || ' '
         || msp_inc_less_than_thresh exceptions
