...
--, SI.CLIENT_ID
--, SI.CASE_ID
, COALESCE(SI.CLIENT_ID,ai.client_id,dl.client_id,dls.client_id) CLIENT_ID
...
FROM STEP_INSTANCE_stg SI
     ...
     LEFT JOIN app_individual_stg ai ON ai.application_id = SI.ref_id AND SI.ref_type = 'APP_HEADER' AND ai.applicant_ind = 1
     LEFT JOIN app_case_link_stg al ON al.application_id = SI.ref_id AND SI.ref_type = 'APP_HEADER'     
     LEFT JOIN document_set_stg ds ON ds.document_set_id = si.ref_id AND si.ref_type = 'DOCUMENT_SET_ID'
     ...
     LEFT JOIN app_doc_data_stg ad ON ad.app_doc_data_id = si.ref_id AND si.ref_type = 'APP_DOC_DATA'
