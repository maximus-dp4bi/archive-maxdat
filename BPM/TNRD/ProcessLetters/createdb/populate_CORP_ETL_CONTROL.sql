/*
Raj A. 10/14/2015  Creation.
                   Modified the original file, BPM\NYHIX\ProcessLetters\createdb\populate_CORP_ETL_CONTROL.sql, to change to TNRD where-ever it says NYHIX.
				   All global controls are the same as in NYHIX script, version-13868.
Raj A. 10/21/2015  Added letter type of 'Other' for temporarily loading data into dev. Deploying the same to UAT too.
Raj A. 10/29/2015  ********  PROJECT-SPECIFIC file. ********
				   As the other letter types in NYHIX do not make sense for ILEB or TNRD. 
*/
...
values (Seq_cell_Id.Nextval,'ProcLetters_SLA_Days','TASK_TYPE','Other','N/A','LETTER_TYPE',null,SYSDATE,to_date('07-JUL-77','DD-MON-RR'),'The number of days after which the letter request is determined to be processed untimely.',SYSDATE,SYSDATE);      
Insert into CORP_ETL_LIST_LKUP (CELL_ID,NAME,LIST_TYPE,VALUE,OUT_VAR,REF_TYPE,REF_ID,START_DATE,END_DATE,COMMENTS,CREATED_TS,UPDATED_TS) 
values (Seq_cell_Id.Nextval,'ProcLetters_SLA_Jeopardy_Days','TASK_TYPE','Other','0','LETTER_TYPE',null,SYSDATE,to_date('07-JUL-77','DD-MON-RR'),'Age at which time the letter request is determined to be in jeopardy of becoming untimely',SYSDATE,SYSDATE);      
Insert into CORP_ETL_LIST_LKUP (CELL_ID,NAME,LIST_TYPE,VALUE,OUT_VAR,REF_TYPE,REF_ID,START_DATE,END_DATE,COMMENTS,CREATED_TS,UPDATED_TS) 
values (Seq_cell_Id.Nextval,'ProcLetters_SLA_Days_Type','TASK_TYPE','Other','B','LETTER_TYPE',null,SYSDATE,to_date('07-JUL-77','DD-MON-RR'),'Indicates if the SLA DAYS should be measured in Business Days or Calendar Days.',SYSDATE,SYSDATE);      
Insert into CORP_ETL_LIST_LKUP (CELL_ID,NAME,LIST_TYPE,VALUE,OUT_VAR,REF_TYPE,REF_ID,START_DATE,END_DATE,COMMENTS,CREATED_TS,UPDATED_TS) 
values (Seq_cell_Id.Nextval,'ProcLetters_SLA_Target_Days','TASK_TYPE','Other','0','LETTER_TYPE',null,SYSDATE,to_date('07-JUL-77','DD-MON-RR'),'The number of days in which the letter request should be completed, as defined by the project',SYSDATE,SYSDATE);
...
values ('PL_SCHEDULE_START','V','09:00:00','Process Letters ETL to run once daily on TNRD. Represents the hour & minute value on 24-hour format.', sysdate, sysdate);
insert into CORP_ETL_CONTROL
values ('PL_SCHEDULE_END','V','22:00:00','Process Letters ETL to run once daily on TNRD. Represents the hour & minute value on 24-hour format.', sysdate, sysdate);
