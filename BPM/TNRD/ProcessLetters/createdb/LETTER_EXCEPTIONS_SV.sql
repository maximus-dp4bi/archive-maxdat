...
CREATE OR REPLACE VIEW LETTER_EXCEPTIONS_SV
AS 
WITH tn
    AS
       (SELECT DISTINCT  '1'                    AS REPORTNUM,
...
--AND TRUNC(LR.LETTER_REQUESTED_ON) = (select trunc(decode( to_char( sysdate, 'DY' ), 'MON', sysdate -3, sysdate -1 )) from dual)
AND TRUNC(LR.LETTER_SENT_ON) = (select max(d_date) from d_dates
where business_day_flag = 'Y'
and today = 'N')
)
OR
(S1.STAFF_TYPE_CD IN ('STATE') AND LR.LETTER_STATUS_CD IN ('SENT') AND LD.LMDEF_ID IN ('62', '63', '64','65', '66', '67', '68', '69', '70', '71', '86', '87', '81', '82','91')
...
OR
(
LR.LETTER_REQUEST_TYPE IN ('L', 'S') AND LR.LETTER_STATUS_CD IN ('SENT') AND LD.LMDEF_ID IN ('79','80')
--AND TRUNC(LR.LETTER_REQUESTED_ON) = (select trunc(decode( to_char( sysdate, 'DY' ), 'MON', sysdate -3, sysdate -1 )) from dual)
AND TRUNC(LR.LETTER_SENT_ON) = (select max(d_date) from d_dates
where business_day_flag = 'Y'
and today = 'N')
...
where business_day_flag = 'Y'
and today = 'N')
/*remove line below to include all CK kids*/
--AND  csi.clnt_cin LIKE '456%'
    AND floor(months_between(receipt_date,clnt_dob)/12) < 19
    --remove line below to include all 408 letter population
...
INNER JOIN d_letter_definition_sv ld
  ...
  LEFT OUTER JOIN letter_request_link_stg ll
    ...
    AND ll.reference_type = 'APP_HEADER'
LEFT JOIN  APP_HEADER_STG ah ON ll.reference_id = ah.application_id
--LEFT JOIN APP_MISSING_INFO_STG MI ON MI.APPLICATION_ID = AC.APPLICATION_ID --remove 5/2 and pull in 406 letters
...
OR
--check 406 later
(S.LAST_NAME = ('System') AND LR.LETTER_STATUS_CD IN ('SENT') AND LD.LMDEF_ID IN ('62', '63', '64','65', '66', '67', '68', '69', '70', '71', '86', '87', '81', '82','91')
AND TRUNC (LR.LETTER_REQUESTED_ON) = MI.NOTIFICATION_DATE AND LR.LETTER_REQUEST_TYPE IN ('L', 'S')
--AND TRUNC(LR.LETTER_REQUESTED_ON) = (select trunc(decode( to_char( sysdate, 'DY' ), 'MON', sysdate -3, sysdate -1 )) from dual)
AND TRUNC(LR.LETTER_SENT_ON) = (select max(d_date) from d_dates
where business_day_flag = 'Y'
...
SELECT letter_req_id,
        ...
        -- receipt_date,
         ...
         || ' '
         ||
         CASE WHEN DENIAL_REASONS IS NULL
         THEN no_msp_denial || ' ' || no_med_denial  || ' ' || no_std_denial || ' ' || no_chip_denial
         ELSE DENIAL_REASONS END
         || ' '
         || no_qmb_denial
         || ' '
         ...
         || no_msp_threshold
         || ' '
         || msp_inc_less_than_thresh
         || ' '
         || response_date_error exceptions,
         client_id,
         state_case_id,
         previous_aid_category
...
--                 receipt_date,
                 ...
                 ( CASE
                     WHEN letter_type IN ( 'TN 404', 'TN 405', 'TN 405a',
                                           'TN 408'
                                           ,
                                           'TN 409', 'TN 413', 'TN 413a' )
                          AND outcomereasoncdvalue = ('OVER_INCOME_MED')
                          AND Nvl(magimonthlythresholdmedvalue, 0) = 0 THEN  --ROW 64
                     'NO Threshold'
                   END ) NO_MEDICAID_THRESHOLD,
                 ( CASE
                     WHEN letter_type IN ( 'TN 404', 'TN 405', 'TN 405a',
                                           'TN 408'
                                           ,
                 ...
                 ( CASE
                   ...
                   END ) QI1_INC_LESS_THAN_THRESH
                        ,
 /*5/10/2018 TN410mspOnly exception for OVER_INCOME_MSP uses miqi1 threshold*/
                  ( CASE
                     WHEN letter_type = 'TN 410msp'
                          AND outcomereasoncdmsponlyvalue = 'MSP_OVER_INCOME'
                          AND Nvl(magimonthlythresholdqi1value, 0) = 0 THEN
                     'NO Threshold'
                   END ) NO_MSP_THRESHOLD,
                 ( CASE
                     WHEN letter_type = 'TN 410msp'
                          AND outcomereasoncdmsponlyvalue = 'MSP_OVER_INCOME'
                          AND Nvl(magimonthlythresholdqi1value, 0) >
                 ...
                 END EARLY_NR_LETTER
                ,
                   (CASE WHEN letter_type in ('TN 408', 'TN 409') AND (OUTCOMEREASONCDSTDVALUE IS NULL AND OUTCOMEREASONCDCHIPVALUE IS NULL AND OUTCOMEREASONCDVALUE IS NULL AND outcomereasoncdmspvalue IS NULL) THEN 'No Denial Reasons'
                    WHEN letter_type in ('TN 408', 'TN 409') AND (OUTCOMEREASONCDSTDVALUE IS NULL AND OUTCOMEREASONCDCHIPVALUE IS NULL AND OUTCOMEREASONCDVALUE IS NULL AND outcomereasoncdmspvalue IS NOT NULL) THEN 'Only MSP Denial'
                    ELSE NULL END) DENIAL_REASONS,
                /*add exceptions for 406 letters 7-3-2018'*/
                                 CASE
                   WHEN Round(( respondbydate - dateofletter )) < 20
                        AND letter_type IN ('TN 406a', 'TN 406b', 'TN 406c', 'TN 406d', 'TN 406e', 'TN 406f', 'TN 406g', 'TN 406h', 'TN 406j', 'TN 406l', 'TN 406m', 'TN 406n', 'TN 406o', 'TN 406p')
                        THEN 'Response Date Error'
                   WHEN Round(( respondbydate - dateofletter )) < 60
                        AND letter_type = 'TN 406i'
                        THEN
                   'Response Date Error'
                 END                                             RESPONSE_DATE_ERROR,
                 client_id,
                 state_case_id,
                 previous_aid_category                 
          FROM   tnreview);
