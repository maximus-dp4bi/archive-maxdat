...
*/
create or replace trigger TRG_NYHX_ETL_IDR_INCI 
before insert or update on NYHX_ETL_IDR_INCIDENTS 
for each row
begin
  if inserting then
    if :new.NEII_ID is null then
      :new.NEII_ID := SEQ_NEII_ID.nextval;
    end if;
...
-- TODO - Determine if list of columns is actually required for this trigger 
-- and if so why it isn't required for insert trigger and document or change.
...
begin 
  if inserting then
    if :new.NEIIR_ID is null then
      :new.NEIIR_ID := SEQ_NEIIR_ID.nextval;
    end if;    
  end if;
