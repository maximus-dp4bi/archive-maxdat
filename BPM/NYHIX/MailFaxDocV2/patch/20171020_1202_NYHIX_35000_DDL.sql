...
alter table F_NYHIX_MFD_BY_DATE  add  RECEIVED_COUNT number;
alter table D_NYHIX_MFD_CURRENT_V2 add RECEIVED_COUNT number;
...
JOIN D_NYHIX_MFD_CURRENT_V2 d on (bdd.D_DATE >= TRUNC(D.INSTANCE_START_DATE) 
     and bdd.D_DATE <= nvl(TRUNC(D.INSTANCE_END_DATE),sysdate));
