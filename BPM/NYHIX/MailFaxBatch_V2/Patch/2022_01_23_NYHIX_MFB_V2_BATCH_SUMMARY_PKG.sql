...
BEGIN
		...
		GV_PARENT_JOB_ID := P_PARENT_JOB_ID;
			...
			(SOURCE_SERVER, BATCH_GUID, MFB_V2_PARENT_JOB_ID)
			SELECT DISTINCT
				SOURCE_SERVER,
				BATCH_GUID,
				P_PARENT_JOB_ID
...
IS
	...
	BEGIN
	    ...
	    LV_RECORD_COUNT    := GV_RECORD_COUNT;
			...
			if lv_RECORD_COUNT < 10 -- DEBUG
			then
			DBMS_OUTPUT.PUT_LINE('proces one parm BATCH_GUID: '||P_SOURCE_SERVER||' '||P_BATCH_GUID);
			end if;
			...
			--if lv_RECORD_COUNT < 10 -- DEBUG
			--then
			--DBMS_OUTPUT.PUT_LINE('proces one BATCH_GUID: '||GV_SOURCE_SERVER||' '||GV_BATCH_GUID);
			--end if;

			--if gv_batch_guid  = '{960afa66-bbd6-4b4f-b285-886ed74c790e}'
			--then 
			--	DBMS_OUTPUT.PUT_LINE('PROCESS ONE BATCH '||GV_SOURCE_SERVER||' '||GV_BATCH_GUID);
			--end if;
			
			--GV_SRC_REC_SUMMARY := NULL;
			--GV_TARGET_REC := NULL;

			-- DBMS_OUTPUT.PUT_LINE('PROCESS_ONE_BATCH  GV_SOURCE_SERVER: '||GV_SOURCE_SERVER);  -- DEBUG
			-- DBMS_OUTPUT.PUT_LINE('PROCESS_ONE_BATCH  GV_BATCH_GUID: '||GV_BATCH_GUID);			-- DEBUG
            ...
            ELSE
				...
				IF NVL(GV_AGE_IN_BUSINESS_DAYS,0) = 0
				OR NVL(GV_SRC_REC_SUMMARY.AGE_IN_BUSINESS_DAYS,0) = 0
				THEN
					GV_AGE_IN_BUSINESS_DAYS := BPM_COMMON.BUS_DAYS_BETWEEN(
									GV_SRC_REC_SUMMARY.CREATE_DT,
									nvl(GV_SRC_REC_SUMMARY.batch_complete_dt,sysdate)
