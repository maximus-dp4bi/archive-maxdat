...
-----------------------------------------------------
	...
	BEGIN
		...
		IF LENGTH(P_JOB_ID) > 10
		THEN -- iTS A BATCH_NAME NOT A JOB
			-- GET A PARENT_JOB_ID
			GV_PARENT_JOB_ID := SEQ_JOB_ID.NEXTVAL;
		END IF;

        GV_JOB_NAME	:= substr(GV_PROCESS_NAME||' Parent ID: '||GV_PARENT_JOB_ID||' - '||'Step NYHIX_MFB_V2_BATCH_SUMMARY',1,80);
		...
		THEN -- iTS A BATCH_NAME NOT A JOB
			...
			(SOURCE_SERVER, BATCH_GUID)
			SELECT SOURCE_SERVER, BATCH_GUID
			FROM NYHIX_MFB_V2_BATCH_SUMMARY
			WHERE BATCH_NAME = P_JOB_ID;  --<<< BATCH_NAME

			COMMIT;	

		ELSE -- ITS NOT A BATCH_GUID IS A JOD ID
		...
		LOOP  -- Main "Driving" Loop
			...
			--DBMS_OUTPUT.PUT_LINE('Processing BATCH_GUID: '||GV_BATCH_GUID);
			...
			Extract_Target(GV_SOURCE_SERVER, GV_BATCH_GUID);

			Extract_Stats_Batch(GV_SOURCE_SERVER, GV_BATCH_GUID);

            -- THE EXTRACTS MUST BE IN THE PROPER ORDER
            -- ALL EXTRAXTS MAY NOT BE NEEDED
            Extract_Document(GV_SOURCE_SERVER, GV_BATCH_GUID);
            Extract_Envelope(GV_SOURCE_SERVER, GV_BATCH_GUID);
            -- Extract_Stats_Batch_Module(GV_SOURCE_SERVER, GV_BATCH_GUID);
            Extract_Maxdat_Reporting(GV_SOURCE_SERVER, GV_BATCH_GUID);
            Extract_Stats_Form_Type(GV_SOURCE_SERVER, GV_BATCH_GUID);
            Extract_Batch_Event(GV_SOURCE_SERVER, GV_BATCH_GUID);
...
--									);
				...
				GV_SRC_REC_SUMMARY.AGE_IN_BUSINESS_DAYS :=  BPM_COMMON.BUS_DAYS_BETWEEN(
									GV_SRC_REC_SUMMARY.CREATE_DT,
									nvl(GV_SRC_REC_SUMMARY.batch_complete_dt,sysdate)
									);
            ...
            IF GV_TARGET_ROWID IS NULL
				...
				THEN
					...
					LOAD_F_BY_DAY(NVL(GV_SRC_REC_SUMMARY.BATCH_GUID,GV_TARGET_REC.BATCH_GUID));
	...
	EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Load_BATCH_SUMMARY';
...
BEGIN
	...
	---
		...
		--	DBMS_OUTPUT.PUT_LINE('Processing ETL_CONTROL_CSR: '||GV_BATCH_GUID);
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			GV_ERR_LEVEL		  	:= 'CRITICAL';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Extract_CORP_ETL_CONTROL';
...
EXCEPTION
    WHEN OTHERS
		THEN
			...
			GV_DRIVER_TABLE_NAME  	:= 'NYHIX_MFB_V2_BATCH_SUMMARY';
			GV_ERR_LEVEL		  	:= 'CRITICAL';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.INITIALIZE_GV_SRC_REC_SUMMARY';
...
--	LV_DATE_BPM_COMMON_DATE_FMT 		VARCHAR2(21)	:= 'YYYY-MM-DD HH24:MI:SS'; -- << BPM_COMMON.Date_FMT
	...
	BEGIN
		...
		EXCEPTION
            ...
            THEN
				...
				GV_ERR_LEVEL		  	:= 'LOG';
				GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.INSERT_F_MFB_V2_BY_HOUR';
	   ...
	   -- completed or cancelled.
		...
		-------------------------------------------------
			...
			LV_FACT_BY_HOUR_REC.F_MFB_V2_FBH_ID := MAXDAT.SEQ_NYHIX_MFB_BATCH_BY_HOUR_ID.NEXTVAL;
			LV_FACT_BY_HOUR_REC.MFB_V2_BI_ID    := P_MFB_V2_BI_ID;
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			GV_DRIVER_TABLE_NAME  	:= 'F_MFB_V2_BY_HOUR';
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.LOAD_F_MFB_V2_BY_HOUR';
...
--			CEJS_JOB_ID 				= GV_SRC_REC_SUMMARY.CEJS_JOB_ID,    				--	73
	...
	EXCEPTION
		...
		GV_DRIVER_TABLE_NAME  	:= 'NYHIX_MFB_V2_BATCH_SUMMARY';
		GV_ERR_LEVEL		  	:= 'LOG';
		GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.UPDATE_BATCH_SUMMARY';
...
--    GV_PARENT_JOB_ID := 0;
	...
	EXCEPTION
		...
		GV_DRIVER_TABLE_NAME  	:= 'NYHIX_MFB_V2_BATCH_SUMMARY';
		GV_ERR_LEVEL		  	:= 'LOG';
		GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.INSERT_BATCH_SUMMARY';
...
-----------------------------------------------------
	...
	EXCEPTION
		...
		GV_DRIVER_TABLE_NAME  	:= 'NYHIX_MFB_V2_BATCH_SUMMARY';
		GV_ERR_LEVEL		  	:= 'LOG';
		GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.DELETE_BATCH_SUMMARY';
...
EXCEPTION
	WHEN OTHERS THEN
		...
		GV_ERR_LEVEL		  	:= 'CRITICAL';
		GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Insert_Corp_ETL_Job_Statistics';
...
EXCEPTION
	WHEN OTHERS THEN
		...
		GV_DRIVER_TABLE_NAME  	:= 'Corp_ETL_Job_Statistics';
		GV_ERR_LEVEL		  	:= 'CRITICAL';
		GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Update_Corp_ETL_Job_Statistics';
...
EXCEPTION
	...
	When Others then
		...
		GV_ERR_LEVEL		  	:= 'CRITICAL';
		GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Post_Error';
...
--		GV_TARGET_REC.CEJS_JOB_ID,
	...
	EXCEPTION
		...
		GV_DRIVER_TABLE_NAME  	:= 'NYHIX_MFB_V2_BATCH_SUMMARY';
		GV_ERR_LEVEL		  	:= 'LOG';
		GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Extract_Target';
...
Procedure Extract_Stats_Batch ( P_SOURCE_SERVER VARCHAR DEFAULT 'CENTRAL', p_Batch_GUID varchar default null) IS
	BEGIN
		...
		SELECT
			...
			SOURCE_SERVER,
			BATCH_DESCRIPTION,
			CASE WHEN SOURCE_SERVER = 'CENTRAL' 
				THEN NVL(REPROCESSED_FLAG,'N') --<<  The flag could be null especially from REMOTE
				ELSE 'N' 
				END 						AS REPROCESSED_FLAG,
			REPROCESSED_DATE,	
			CREATION_STATION_ID,
			CREATION_USER_NAME,
			CREATION_USER_ID,
	...
	EXCEPTION
		...
		THEN
			...
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Extract_Stats_Batch';
...
Procedure Extract_Document ( P_SOURCE_SERVER varchar DEFAULT 'CENTRAL', p_Batch_GUID varchar default null) IS
	...
	EXCEPTION
		...
		WHEN OTHERS
			...
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Extract_Document';
...
--GV_SRC_REC_MAXDAT_REPORTING
    ...
    Exception
        ...
        THEN
			...
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Extract_Maxdat_Reporting';
...
Procedure Extract_Batch_Event ( P_SOURCE_SERVER VARCHAR DEFAULT 'CENTRAL', P_BATCH_GUID varchar default null) IS
	...
	and batch_guid = p_Batch_GUID
	-- this could be used to filter out events which are after the cancel or complete dates 
	-- and nvl(end_date_time,sysdate)  
	-- <= least(nvl(gv_rec_rec.batch_complete_dt,sysdate), 
	--			nvl(gv_src_rec.candcel_dt) 
	--			)
...
EXCEPTION
		...
		WHEN OTHERS THEN
			...
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.Extract_Batch_Event';
...
-----------------------------------------------------
	...
	EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.UPDATE_F_BY_DAY';
...
-----------------------------------------------------
	...
	BEGIN
		...
		VALUES ( -- SQL FROM SECTION 8
         ...
         --   GV_FACT_JOIN_REC.SRC_MFB_V2_UPDATE_DATE,			-- 8 	5
            GV_FACT_JOIN_REC.SRC_CREATE_DT,						-- 8 	6
            GV_FACT_JOIN_REC.SRC_FAX_BATCH_SOURCE,				-- 8 	7
            GV_FACT_JOIN_REC.SRC_BATCH_CLASS,					-- 8 	8
            GV_FACT_JOIN_REC.SRC_CANCEL_DT,						-- 8 	9
            GV_FACT_JOIN_REC.SRC_REPROCESSED_FLAG,				-- 8 	10
            GV_FACT_JOIN_REC.SRC_REPROCESSED_DATE,				-- 8 	10
            GV_FACT_JOIN_REC.SRC_BATCH_COMPLETE_DT,				-- 8 	11
	...
	EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			GV_DRIVER_TABLE_NAME  	:= 'F_MFB_V2_BY_DAY';
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.INSERT_F_BY_DAY';
...
-----------------------------------------------------
	...
	EXCEPTION
		...
		THEN
			...
			GV_DRIVER_TABLE_NAME  	:= 'F_MFB_V2_BY_DAY';
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.LOAD_F_BY_DAY DELETES';
			POST_ERROR;
			RETURN;
...
--		END IF;
            ...
            IF GV_FACT_JOIN_REC.SRC_BUSINESS_DAY_FLAG = 'Y'
				...
				GV_FACT_JOIN_REC.SRC_AGE_IN_BUSINESS_HOURS 
					:= MAXDAT.BUS_HRS_BETWEEN
						(
						GV_FACT_JOIN_REC.SRC_CREATE_DT, 							-- << START_DATE ******
						LEAST(SYSDATE,												-- << END_DATE  *******
								NVL(GV_FACT_JOIN_REC.SRC_BATCH_COMPLETE_DT,
									GV_FACT_JOIN_REC.SRC_D_DATE+19/24
									) 
							 ),  
							LV_AGE_IN_BUSINESS_DAYS									-- <<AGE IN bUSINESS DAYS
							);	
	...
	EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			GV_DRIVER_TABLE_NAME  	:= 'F_MFB_V2_BY_DAY';
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.LOAD_F_BY_DAY';
			POST_ERROR;
			...
			GV_DRIVER_TABLE_NAME  	:= 'F_MFB_V2_BY_DAY';
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.LOAD_F_BY_DAY CANCEL_DT';
			POST_ERROR;
			RETURN;
...
ALTER VIEW MAXDAT.F_MFB_V2_BY_HOUR_SV COMPILE;