...
-- 512 - locked.
	...
	-- NOTE these need to be derrived only once a day
...
--		LV_LAST_UPDATE_DATE DATE		:= NULL;
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
	        GV_ERROR_CODE 			:= SQLCODE;
            GV_ERROR_MESSAGE 		:= SUBSTR(SQLERRM, 1, 3000);
			GV_DRIVER_KEY_NUMBER  	:= 'MFB_V2_BI_ID: '||P_MFB_V2_BI_ID;
	...
	END IF;
      ...
      EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			GV_ERROR_CODE 			:= SQLCODE;
			GV_ERROR_MESSAGE 		:= SUBSTR(SQLERRM, 1, 3000);
			GV_DRIVER_KEY_NUMBER  	:= 'F_MFB_V2_FBH_ID: '||LV_FACT_REC.F_MFB_V2_FBH_ID;
			GV_DRIVER_TABLE_NAME  	:= 'F_MFB_V2_BY_HOUR';
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.UPDATE_F_MFB_V2_BY_HOUR';
...
--			CEJS_JOB_ID 				= GV_SRC_REC_SUMMARY.CEJS_JOB_ID,    				--	73
		...
		AND BATCH_GUID = GV_BATCH_GUID
		AND BATCH_GUID = GV_SRC_REC_SUMMARY.BATCH_GUID;
...
-----------------------------------------------------
 ...
 --           AGE_IN_BUSINESS_DAYS                      =  JOIN_REC.SRC_AGE_IN_BUSINESS_DAYS,		-- 6 	13
		...
		and batch_guid = JOIN_REC.SRC_BATCH_GUID;
...
-----------------------------------------------------
	...
	EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			GV_SQL_CODE 			:= SQLCODE;
			GV_LOG_MESSAGE 			:= SQLERRM;
			GV_DRIVER_KEY_NUMBER  	:= 'BATCH_GUID : '||JOIN_REC.SRC_BATCH_GUID;
			GV_DRIVER_TABLE_NAME  	:= 'F_MFB_V2_BY_DAY';
			GV_ERR_LEVEL		  	:= 'LOG';
...
-----------------------------------------------------
	...
	EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			GV_DRIVER_TABLE_NAME  	:= 'F_MFB_V2_BY_DAY';
			GV_ERR_LEVEL		  	:= 'LOG';
			GV_PROCESS_NAME 		:= 'NYHIX_MFB_V2_BATCH_SUMMARY_PKG.DELETE_F_BY_DAY';
...
--		COMMIT;
	...
	EXCEPTION
        ...
        WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE('fAILED IN  Load_F_BY_DAY for '||
