...
-- UPD1_600 - SET BATCH_COMPLETE_DATE AND CURRENT_BATCH_MODULE_ID
...
-- UPD1_610 - SET COMPLETE_DT AND CURRENT_BATCH_MODULE_ID ( FOR INCOMPLETE BATCHES )
...
AND GV_SRC_REC_EVENT.MODULE_CLOSE_NAME <> '${QC_MODULE_NAME}'
...
THEN GV_SRC_REC_SUMMARY.ASSD_CLASSIFICATION 	:= GV_SRC_REC_SUMMARY.ASED_SCAN_BATCH;
	GV_SRC_REC_SUMMARY.ASSD_RECOGNITION			:= GV_SRC_REC_SUMMARY.ASED_SCAN_BATCH;
	GV_SRC_REC_SUMMARY.GWF_QC_REQUIRED 			:= 'N';
END IF;
...
IF NVL(GV_SRC_REC_EVENT.SML_MODULE_NAME,'?') = 'KCN Server'
AND GV_SRC_REC_EVENT.END_DATE_TIME IS NOT NULL
THEN  
