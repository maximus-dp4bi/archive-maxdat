...
--		LV_LAST_UPDATE_DATE DATE		:= NULL;
    ...
    BEGIN
		...
		IF P_CREATE_DT >  NVL(P_BATCH_COMPLETED_DT,TO_DATE(SYSDATE))
        OR P_CREATE_DT >  NVL(P_CANCELLED_DT,TO_DATE(SYSDATE))
		OR P_CREATE_DT >  BPM_COMMON.MAX_DATE
			THEN
				LV_SQL_CODE := -20030;
				LV_LOG_MESSAGE := 'ATTEMPTED TO INSERT INVALID FACT DATE RANGE.  ' || 
                'D_DATE = ' || P_CREATE_DT || 
                ' BUCKET_START_DATE = ' || LV_BUCKET_START_DATE ||
                ' BUCKET_END_DATE = ' || LV_BUCKET_END_DATE;
    ...
    ELSE    
		...
		INSERT INTO F_MFB_V2_BY_HOUR
            ...
            (
				...
				LV_FACT_REC.CREATION_COUNT,
				LV_FACT_REC.INVENTORY_COUNT,
				LV_FACT_REC.COMPLETION_COUNT
			);	
