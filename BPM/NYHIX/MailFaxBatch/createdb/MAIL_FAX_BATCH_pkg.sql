...
v_new_received_counts_id     maxdat.f_mfd_received_counts.f_received_counts_id%TYPE;
v_old_received_counts_id     maxdat.f_mfd_received_counts.f_received_counts_id%TYPE;

BEGIN

  BEGIN  -- Get the F_RECEIVED_COUNTS_ID for the P_NEW_RECEIVED_DT

    SELECT f_received_counts_id
    INTO   v_new_received_counts_id
    FROM   maxdat.f_mfd_received_counts
    WHERE  received_date = p_new_received_dt;

  EXCEPTION WHEN NO_DATA_FOUND
  THEN
    v_new_received_counts_id := -1;
  
  END;

  BEGIN  -- Get the F_RECEIVED_COUNTS_ID for the P_OLD_RECEIVED_DT

    SELECT f_received_counts_id
    INTO   v_old_received_counts_id
    FROM   maxdat.f_mfd_received_counts
    WHERE  received_date = p_old_received_dt;

  EXCEPTION WHEN NO_DATA_FOUND
  THEN
    v_old_received_counts_id := -1;
...
-- * if the count increases by 1 (received date
...
-- **********************************************
  ...
  THEN
    ...
    THEN
      ...
      IF v_new_received_counts_id = -1
      THEN

        INSERT INTO maxdat.f_mfd_received_counts
        (received_date, received_count)
        VALUES
        (p_new_received_dt, 1);
      ...
      ELSE
        ...
        WHERE  f_received_counts_id = v_new_received_counts_id;
  ...
  THEN
    ...
    THEN
--***************** OLD Received Date processing first
      IF p_old_received_dt <= p_create_dt  -- If the OLD received date counted originally then it would have to be reversed for that date
      THEN

        IF v_old_received_counts_id != -1  -- If the received date already exists reduce the count by 1
                                           -- (if the received date doesn't exist then do not add it since record was not counted in the first place)
        THEN

          UPDATE maxdat.f_mfd_received_counts
          SET    received_count = received_count - 1
          WHERE  f_received_counts_id = v_old_received_counts_id;
    
        END IF;  -- v_old_received_counts_id != -1
  
      END IF;  -- p_old_received_dt <= p_create_dt
...
--***************** NEW Received Date processing next
      ...
      THEN
        ...
        IF v_new_received_counts_id = -1  -- if received date record does not exist then add it with a starting count of 1
        THEN
        ...
        ELSIF v_new_received_counts_id != -1  -- if received date record does exist then add 1 to the count
        THEN
...
.SCAN_DT);
  ...
  begin
    ...
    UPDATE_RECEIVED_COUNTS('INSERT', v_new_data.CREATE_DT, v_new_data.RECEIVED_DT, NULL);
...
-- Update BPM Semantic model data.
  ...
  begin
    ...
    UPDATE_RECEIVED_COUNTS('UPDATE', v_new_data.CREATE_DT, v_new_data.RECEIVED_DT, v_old_data.RECEIVED_DT);
