create or replace package                      MAIL_FAX_BATCH as
  ...
  SVN_REVISION varchar2(20) := '$Revision: 17633 $'; 
  SVN_REVISION_DATE varchar2(60) := '$Date: 2016-06-30 15:21:46 -0700 (Thu, 30 Jun 2016) $'; 
...
update D_MFB_CURRENT
 ...
 commit;
  ...
  begin  
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/FAX_BATCH_SOURCE') "FAX_BATCH_SOURCE"
    into p_data_record
       from dual;
...
-- Insert or update dimension for BPM Semantic model - Process Mail Fax Batch process - Current.
  ...
  (p_set_type               in           varchar2,
   ...
   p_fax_batch_source				in			varchar2
   )
...
r_dmfbcur.FAX_BATCH_SOURCE           :=         p_fax_batch_source;       
...
if p_set_type = 'INSERT' then
 ...
 -- Insert current dimension and fact as a single transaction.
   ...
   ('INSERT',v_identifier,v_bi_id, 
     ...
     ,v_new_data.BATCH_GUID,v_new_data.SOURCE_SERVER,v_new_data.BATCH_DESCRIPTION,v_new_data.REPROCESSED_FLAG,v_new_data.FAX_BATCH_SOURCE
   );
