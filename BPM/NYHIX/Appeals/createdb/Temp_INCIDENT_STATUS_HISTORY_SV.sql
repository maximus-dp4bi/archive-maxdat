CREATE OR REPLACE FORCE EDITIONABLE VIEW "MAXDAT"."TEMP_INCIDENT_STATUS_HISTORY_SV" ("INCIDENT_HEADER_STAT_HIST_ID", "INCIDENT_ID", "STATUS_CD", "INCIDENT_STATUS", "INCIDENT_STATUS_DT", "CREATED_BY", "CREATED_BY_STAFF_ID", "END_DATE", "AGE_IN_BUSINESS_DAYS", "AGE_IN_CALENDAR_DAYS", "AGE_IN_BUSINESS_DAYS_FLOW", "AGE_IN_CALENDAR_DAYS_FLOW","AGE_IN_BUSINESS_DAYS_FLOW_NEW", "AGE_IN_CALENDAR_DAYS_FLOW_NEW") AS 
WITH get_basic_range AS
(
SELECT distinct
...
where INCIDENT_HEADER_STAT_HIST_ID > (select value from corp_etl_control where name='INCIDENT_HEADER_STAT_HIST_ID_START') and
      INCIDENT_HEADER_STAT_HIST_ID < (select value from corp_etl_control where name='INCIDENT_HEADER_STAT_HIST_ID_END')
), get_basic AS
(
SELECT 
INCIDENT_HEADER_STAT_HIST_ID,
...
0   as  AGE_IN_BUSINESS_DAYS_FLOW_NEW,
0   as AGE_IN_CALENDAR_DAYS_FLOW_NEW
FROM get_basic_range r
LEFT JOIN get_basic b 
ON r.INCIDENT_HEADER_ID = b.INCIDENT_ID
