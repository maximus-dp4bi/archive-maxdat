...
type T_INS_APL_XML is record 
    (
     ...
     APL_PRIMARY_MEMBER_ID varchar2(100),
    ...
    -- INCIDENT_REASON varchar2(80),
	 ...
	 APL_OUTRCH_IND varchar2(1),
	 APL_OUTRCH_DATE varchar(19),
	 APL_OUTRCH_OUTCM varchar2(80),
	 APL_RSN_FR_WTHDRWL varchar2(80),
     OUTRCH_OUTCM_DATE varchar2(19),
	 SPECIALIZED_FLAG varchar2(1),
	 UNBL_CNTCT_START_DATE varchar2(19)
     );
...
/
  ...
  CREATE OR REPLACE PACKAGE BODY "MAXDAT"."NYHIX_PROCESS_APPEALS" as
...
-- Get record for Process Incidents insert XML.
  ...
  begin
    ...
    select
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/APL_PRIMARY_MEMBER_ID') "APL_PRIMARY_MEMBER_ID",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/FAIR_HEARING_TRACKING_NBR') "FAIR_HEARING_TRACKING_NBR",
     ...
     -- extractValue(p_data_xml,'/ROWSET/ROW/INCIDENT_REASON') "INCIDENT_REASON",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/NYHIX_ID') "NYHIX_ID",
	  ...
	  extractValue(p_data_xml,'/ROWSET/ROW/DECISION_DETAILS') "DECISION_DETAILS",
	  extractValue(p_data_xml,'/ROWSET/ROW/APL_OUTRCH_IND') "APL_OUTRCH_IND",
	  extractValue(p_data_xml,'/ROWSET/ROW/APL_OUTRCH_DATE') "APL_OUTRCH_DATE",
	  extractValue(p_data_xml,'/ROWSET/ROW/APL_OUTRCH_OUTCM') "APL_OUTRCH_OUTCM",
	  extractValue(p_data_xml,'/ROWSET/ROW/APL_RSN_FR_WTHDRWL') "APL_RSN_FR_WTHDRWL",
	  extractValue(p_data_xml,'/ROWSET/ROW/OUTRCH_OUTCM_DATE') "OUTRCH_OUTCM_DATE",
	  extractValue(p_data_xml,'/ROWSET/ROW/SPECIALIZED_FLAG') "SPECIALIZED_FLAG",
	  extractValue(p_data_xml,'/ROWSET/ROW/UNBL_CNTCT_START_DATE') "UNBL_CNTCT_START_DATE"
      into p_data_record
       from dual;
...
--    p_timeliness_status                 in  varchar2,
    ...
    p_fair_hearing_tracking_nbr         in varchar2,
    p_apl_primary_member_id             in number,
    p_account_id                        in varchar2,
    p_nyhix_id                          in varchar2,
    p_pref_language                     in varchar2,
    p_decision_details                  in varchar2,
	p_apl_outrch_ind                    in number,
    p_apl_outrch_date                   in varchar2,
    p_apl_outrch_outcm                  in varchar2,
	p_apl_rsn_fr_wthdrwl                in varchar2,
	p_outrch_outcm_date	                in varchar2,
	p_specialized_flag                  in number,
	p_unbl_cntct_start_date             in varchar2
    )
  as
  ...
  begin
    ...
    r_aplcur.FAIR_HEARING_TRACKING_NBR            :=      p_fair_hearing_tracking_nbr;
    r_aplcur.APL_PRIMARY_MEMBER_ID                :=      p_apl_primary_member_id;
    r_aplcur.ACCOUNT_ID                           :=      p_account_id;
    r_aplcur.NYHIX_ID                             :=      p_nyhix_id;
    r_aplcur.PREF_LANGUAGE                        :=      p_pref_language;
    r_aplcur.DECISION_DETAILS                     :=      p_decision_details;
	r_aplcur.APL_OUTRCH_IND                       :=      p_apl_outrch_ind;
	r_aplcur.APL_OUTRCH_DATE                      :=      to_date(p_apl_outrch_date, bpm_common.date_fmt);
	r_aplcur.APL_OUTRCH_OUTCM                     :=      p_apl_outrch_outcm;
	r_aplcur.APL_RSN_FR_WTHDRWL                   :=      p_apl_rsn_fr_wthdrwl;
	r_aplcur.OUTRCH_OUTCM_DATE                    :=      to_date(p_outrch_outcm_date, bpm_common.date_fmt);
	r_aplcur.SPECIALIZED_FLAG                     :=      p_specialized_flag;
	r_aplcur.UNBL_CNTCT_START_DATE                :=      to_date(p_unbl_cntct_start_date, bpm_common.date_fmt);
    ...
    begin
      ...
      SET_DAPLCUR
        ('INSERT',v_identifier,v_bi_id,
          ...
          v_new_data.fair_hearing_tracking_nbr,
          v_new_data.apl_primary_member_id,
          v_new_data.account_id,
          v_new_data.nyhix_id,
          v_new_data.pref_language,
          v_new_data.decision_details,
		  v_new_data.apl_outrch_ind,
	      v_new_data.apl_outrch_date,
	      v_new_data.apl_outrch_outcm,
		  v_new_data.apl_rsn_fr_wthdrwl,
		  v_new_data.outrch_outcm_date,
	      v_new_data.specialized_flag,
		  v_new_data.unbl_cntct_start_date);
