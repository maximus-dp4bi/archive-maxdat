DROP VIEW MAXDAT.D_NYHIX_APPEALS_SV;

--------------------------------------------------------
--  DDL for View D_NYHIX_APPEALS_SV
--------------------------------------------------------

  CREATE OR REPLACE FORCE EDITIONABLE VIEW "MAXDAT"."D_NYHIX_APPEALS_SV" ("PROGRAM_TYPE", "DATE_INITIATED", "APPEAL_ID", "ACCOUNT_ID", "INCIDENT_REASON", "INITIATED_COUNT") AS 
  with appeals_excluded as
       (  
       select distinct FAIR_HEARING_TRACKING_NBR , a11.apl_bi_id --added by AWS
...
FROM 
      (
        ...
        and a11.FAIR_HEARING_TRACKING_NBR not like 'APM%'
           AND apl_bi_id not in ( select apl_bi_id from appeals_excluded where FAIR_HEARING_TRACKING_NBR  is not null ) --changed criteria from appeal_id to instance_id AWS
