...
SELECT
	...
	--
	FROM HDR
	left outer join LIVE_OBSRV_COMPLTD on 1=1
	left outer join REVIEW_SESSIONS_COMPLTD on 1=1
	left outer join QPP_CHECKLIST_COMPLTD on 1=1
	left outer join QPP_QUALITY_SCORE_1 on 1=1
	left outer join QPP_QUALITY_SCORE_2 on 1=1;
...
BEGIN
    ...
    -- validate the parms
    -- the following cannot be null
    if IN_DATES_MONTH_NUM               is null
    or IN_Team_lead_AGENT_STAFF_ID      is null
    or IN_TEAM_LEAD_QLTY_CREATE_USER  	is null
    or IN_REVIEW_SESSIONS_COMPL_RTNG	is null
    or IN_LIVE_OBSRV_COMPL_RTNG		    is null
    or IN_QPP_CHECKLIST_COMPL_RTNG 	    is null
    then 
        Return;
    end if;    

    -- Per request these 3 can be null
    --  IN_QPP_QUALITY_SCORE_1_RTNG 	
    --  IN_QPP_QUALITY_SCORE_2_RTNG		
