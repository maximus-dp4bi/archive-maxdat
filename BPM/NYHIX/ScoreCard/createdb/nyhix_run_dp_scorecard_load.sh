...
if [[ -e $INIT_OK ]] ; then
   ...
   echo "Starting ${STCODE}_run_dp_scorecard.sh in ${ENV_CODE}."
...
if [[ $rc != 0 ]] ; then
	...
	$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/DP_Scorecard/Process_DP_SCORECARD_Files.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/Process_DP_SCORECARD_Files_$(date +%Y%m%d_%H%M%S).log &
	$MAXDAT_KETTLE_DIR/kitchen.sh -file="$MAXDAT_ETL_PATH/DP_Scorecard/Load_PP_WFM_Tables.kjb" -level="$KJB_LOG_LEVEL"  >> $MAXDAT_ETL_LOGS/Load_PP_WFM_Tables_$(date +%Y%m%d_%H%M%S).log &
	 wait
		if [[ -e $CHILD_FAIL ]]
		then
