...
BEGIN
  ...
  SELECT COUNT(0)    INTO   v_before_count  FROM DP_SCORECARD.Sc_Lag_Time  WHERE (AGENT_ID IN ('258879') AND TRUNC(LAG_DATE) in ('01-NOV-19')) OR (AGENT_ID IN ('138384') AND TRUNC(LAG_DATE) in ('06-NOV-19')) ;
  dbms_output.put_line('Before record count: '||v_before_count);

  IF v_before_count > 0
  THEN

    DELETE FROM DP_SCORECARD.Sc_Lag_Time WHERE (AGENT_ID IN ('258879') AND TRUNC(LAG_DATE) in ('01-NOV-19')) OR (AGENT_ID IN ('138384') AND TRUNC(LAG_DATE) in ('06-NOV-19'));
  ...
  INTO   v_after_count   FROM DP_SCORECARD.Sc_Lag_Time
  WHERE (AGENT_ID IN ('258879') AND TRUNC(LAG_DATE) in ('01-NOV-19')) OR (AGENT_ID IN ('138384') AND TRUNC(LAG_DATE) in ('06-NOV-19')) ;
