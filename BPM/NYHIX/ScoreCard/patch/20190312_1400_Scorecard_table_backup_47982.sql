...
declare  c int;
begin
   select count(*) into c from user_tables where table_name ='SC_LAG_TIME_47982_20190314';
   if c = 1 then
      execute immediate 'drop table DP_SCORECARD.SC_LAG_TIME_47982_20190314 cascade constraints';
   end if;
end;
/

create table DP_SCORECARD.SC_LAG_TIME_47982_20190314
(
...
begin
  ...
  INSERT INTO DP_SCORECARD.SC_LAG_TIME_47982_20190314
  SELECT lag_date, agent_id, supervisor_id, tot_sched_productive_time, create_by, create_date, adherence_flag
  from   DP_SCORECARD.SC_LAG_TIME;

  v_insert_count := SQL%ROWCOUNT;

  dbms_output.put_line('INSERT INTO Table DP_SCORECARD.SC_LAG_TIME_47982_20190314 record count: '||v_insert_count);

  SELECT COUNT(*)
  INTO   v_aft_record_count
  FROM   DP_SCORECARD.SC_LAG_TIME_47982_20190314;

  dbms_output.put_line('Table DP_SCORECARD.SC_LAG_TIME_47982_20190314 record count: '||v_aft_record_count);
...
DECLARE  c int;
begin
   select count(*) into c from user_tables where table_name ='SC_AGENT_STAT_47982_20190314';
   if c = 1 then
      execute immediate 'drop table DP_SCORECARD.SC_AGENT_STAT_47982_20190314 cascade constraints';
   end if;
end;
/

create table DP_SCORECARD.SC_AGENT_STAT_47982_20190314
(
...
begin
  ...
  INSERT INTO DP_SCORECARD.SC_AGENT_STAT_47982_20190314
  ...
  dbms_output.put_line('INSERT INTO Table DP_SCORECARD.SC_AGENT_STAT_47982_20190314 record count: '||v_insert_count);

  SELECT COUNT(*)
  INTO   v_aft_record_count
  FROM   DP_SCORECARD.SC_AGENT_STAT_47982_20190314;

  dbms_output.put_line('Table DP_SCORECARD.SC_AGENT_STAT_47982_20190314 record count: '||v_aft_record_count);
