...
-- Do not edit these four SVN_* variable values.  They are populated when you commit code to SVN and used later to identify deployed code.
 	SVN_FILE_URL varchar2(200) := '$URL: svn://svn-staging.maximus.com/dev1d/maxdat/BPM/NYHIX/ScoreCard/patch/20180702_1005_CR1886_PP_WFM_BACK_OFFICE_LOAD_PKG.SQL $';
  	SVN_REVISION varchar2(20) := '$Revision: 24040 $';
 	SVN_REVISION_DATE varchar2(60) := '$Date: 2018-07-02 10:41:17 -0500 (Mon, 02 Jul 2018) $';
...
-- method using EE_ADHERENCE.  See procedure 4.1 and 4.2
...
PROCEDURE STEP4_lOAD_SC_PRDUCTION_BO_P41(idx in VARCHAR2) IS
	...
	( SELECT WRK.STAFF_ID, WRK.EVENT_DATE,
			...
			SUM(CASE WHEN LKP.EE_ADHERENCE = 'Y' -- NOTE THESE ROWS ARE GENERATED IF EITHER EE_ADHERENCE = 'Y'
				OR LKP.EE_ADHERENCE_V2 = 'Y'      -- OR EE_ADHERENCE_V2 = 'Y' THIS WILL ALLOW FOR COMPARISON BETWEEN THE METHODS
				THEN HANDLE_TIME_IN_SECONDS ELSE 0
			END)/(60*60) AS ACTUAL_PRODUCTIVE_HOURS
		FROM ( SELECT * FROM DP_SCORECARD.SCORECARD_HIERARCHY ) H
		JOIN
			( SELECT * FROM DP_SCORECARD.PP_WFM_DAILY_SUMMARY_WRK
		...
		JOIN ( SELECT * FROM MAXDAT.PP_BO_EVENT_TARGET_LKUP ) LKP
	...
	(
            ...
            LEFT OUTER JOIN ( SELECT * FROM MAXDAT.PP_BO_EVENT_TARGET_LKUP ) LKP
				ON LKP.EVENT_ID = BO.EVENT_ID
				AND TRUNC(BO.TASK_START) BETWEEN TRUNC(LKP.START_DATE) AND TRUNC(NVL(LKP.END_DATE,SYSDATE))
			WHERE TRUNC(TASK_START) = TRUNC(TASK_END)
...
(
	...
	where event_date >= TRUNC(ADD_MONTHS(SYSDATE, -12), 'MM')
	and trunc(nvl(STAFF_TERMINATION_DATE,sysdate)) >= event_date
	and ( EE_ADHERENCE = 'Y' OR EE_ADHERENCE_V2 = 'Y' )
	and department is not null
	and building is not null
...
SELECT STAFF_ID,
  ...
  WHERE ( EE_ADHERENCE = 'Y' OR EE_ADHERENCE_V2 = 'Y' )
  and EVENT_DATE >= TRUNC(ADD_MONTHS(SYSDATE, -12), 'MM')
GROUP BY STAFF_ID, EVENT_DATE
),
SCHEDULED_DAILY as
