...
-- Do not edit these four SVN_* variable values.  They are populated when you commit code to SVN and used later to identify deployed code.
 	SVN_FILE_URL varchar2(200) := '$URL: svn://svn-staging.maximus.com/dev1d/maxdat/BPM/NYHIX/ScoreCard/patch/20190805_022_LOAD_SC_SUMMARY_CC_ROLLUP.sql $';
  	SVN_REVISION varchar2(20) := '$Revision: 27757 $';
 	SVN_REVISION_DATE varchar2(60) := '$Date: 2019-08-05 15:17:18 -0400 (Mon, 05 Aug 2019) $';
...
INSERT INTO DP_SCORECARD.SC_SUMMARY_CC_ROLLUP
    ...
    ,AGENT_DISCONNECTED_SHORT_CALLS_DT_AGG
    ,TOT_CALLS_ANSWERED_DT_AGG
    ,TOT_TOT_RETURN_TO_QUEUE_DT_AGG
    ,CALLS_OFFERED_DT_AGG
    ...
    ,SUM_QC_SCORE_DT_AGG
    ,COUNT_QC_SCORE_DT_AGG
    ,FIRST_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG
    ,FIRST_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG
    ,SECOND_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG
    ,SECOND_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG
    ,ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG
    ,ADHERENCE_TOT_NOT_READY_TIME_DT_AGG
    ,LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG
    ,FIRST_PRIOR_MONTH_ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG
    ,FIRST_PRIOR_MONTH_ADHERENCE_TOT_NOT_READY_TIME_DT_AGG
    ,FIRST_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG
    ,SECOND_PRIOR_MONTH_ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG
    ,SECOND_PRIOR_MONTH_ADHERENCE_TOT_NOT_READY_TIME_DT_AGG
    ,SECOND_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG
    --THREE_MONTH_SHORT_CALLS_REQ_MISSED_COUNT
    --THREE_MONTH_RTQ_REQ_MISSED_COUNT
...
SELECT
	SUPERVISOR_STAFF_ID,
    ------------------
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.AGENT_DISCONNECTED_SHORT_CALLS,0)
			ELSE 0 END)                                                     AS AGENT_DISCONNECTED_SHORT_CALLS_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.TOT_CALLS_ANSWERED,0)
			ELSE 0 END)                                                     AS TOT_CALLS_ANSWERED_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.TOT_TOT_RETURN_TO_QUEUE,0)
			ELSE 0 END)                                                     AS TOT_TOT_RETURN_TO_QUEUE_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.CALLS_OFFERED,0)
			ELSE 0 END)                                                     AS CALLS_OFFERED_DT_AGG,
    sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_FIRST_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.AGENT_DISCONNECTED_SHORT_CALLS,0)
    ...
    sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_SECOND_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.AGENT_DISCONNECTED_SHORT_CALLS,0)
  ...
  --TEAM QUALITY
    ,sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.SUM_QC_SCORE,0)
			ELSE 0 END)                                                     AS SUM_QC_SCORE_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.COUNT_QC_SCORE,0)
			ELSE 0 END)                                                     AS COUNT_QC_SCORE_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_FIRST_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.SUM_QC_SCORE,0)
			ELSE 0 END)                                                     AS FIRST_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_FIRST_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.COUNT_QC_SCORE,0)
			ELSE 0 END)                                                     AS FIRST_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,
    sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_SECOND_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.SUM_QC_SCORE,0)
			ELSE 0 END)                                                     AS SECOND_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_SECOND_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.COUNT_QC_SCORE,0)
			ELSE 0 END)                                                     AS SECOND_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,
  --TEAM ADHERENCE
    sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.ADHERENCE_TOT_LOGGED_IN_TIME,0)
			ELSE 0 END)                                                     AS ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.ADHERENCE_TOT_NOT_READY_TIME,0)
			ELSE 0 END)                                                     AS ADHERENCE_TOT_NOT_READY_TIME_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.LAG_TIME_TOT_SCHED_PROD_TIME,0)
			ELSE 0 END)                                                     AS LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_FIRST_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.ADHERENCE_TOT_LOGGED_IN_TIME,0)
			ELSE 0 END)                                                     AS FIRST_PRIOR_MONTH_ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_FIRST_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.ADHERENCE_TOT_NOT_READY_TIME,0)
			ELSE 0 END)                                                     AS FIRST_PRIOR_MONTH_ADHERENCE_TOT_NOT_READY_TIME_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_FIRST_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.LAG_TIME_TOT_SCHED_PROD_TIME,0)
			ELSE 0 END)                                                     AS FIRST_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,
    sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_SECOND_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.ADHERENCE_TOT_LOGGED_IN_TIME,0)
			ELSE 0 END)                                                     AS SECOND_PRIOR_MONTH_ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG,
    sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_SECOND_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.ADHERENCE_TOT_NOT_READY_TIME,0)
			ELSE 0 END)                                                     AS SECOND_PRIOR_MONTH_ADHERENCE_TOT_NOT_READY_TIME_DT_AGG,
	sum(CASE WHEN SC_SUMMARY_CC.DATES_MONTH_NUM = LV_SECOND_PRIOR_MONTH_DATES_MONTH_NUM
		THEN NVL(SC_SUMMARY_CC.LAG_TIME_TOT_SCHED_PROD_TIME,0)
			ELSE 0 END)                                                     AS SECOND_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG
  --
FROM DP_SCORECARD.SC_SUMMARY_CC
where SC_SUMMARY_CC.DATES_MONTH_NUM = LV_DATES_MONTH_NUM
        OR SC_SUMMARY_CC.DATES_MONTH_NUM = LV_FIRST_PRIOR_MONTH_DATES_MONTH_NUM
        OR  SC_SUMMARY_CC.DATES_MONTH_NUM = LV_SECOND_PRIOR_MONTH_DATES_MONTH_NUM
GROUP BY
	SUPERVISOR_STAFF_ID
)
---------------
...
--  CC_SUMMARY.DAYS_CALLS_ANSWERED,
	...
	CC_SUMMARY.SECOND_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME
     -- Added as part of JIRA48260 under DP1.7
    ,RTQSC_DT_AGG.AGENT_DISCONNECTED_SHORT_CALLS_DT_AGG
    ,RTQSC_DT_AGG.TOT_CALLS_ANSWERED_DT_AGG
    ,RTQSC_DT_AGG.TOT_TOT_RETURN_TO_QUEUE_DT_AGG
    ,RTQSC_DT_AGG.CALLS_OFFERED_DT_AGG
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_SC_AGENT_DISCONNECTED_SHORT_CALLS
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_SC_TOT_CALLS_ANSWERED
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_SC_AGENT_DISCONNECTED_SHORT_CALLS
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_SC_TOT_CALLS_ANSWERED
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_RTQ_CALLS_OFFERED
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_RTQ_CALLS_OFFERED
    ,RTQSC_DT_AGG.SUM_QC_SCORE_DT_AGG
    ,RTQSC_DT_AGG.COUNT_QC_SCORE_DT_AGG
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG
    ,RTQSC_DT_AGG.ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG
    ,RTQSC_DT_AGG.ADHERENCE_TOT_NOT_READY_TIME_DT_AGG
    ,RTQSC_DT_AGG.LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_ADHERENCE_TOT_NOT_READY_TIME_DT_AGG
    ,RTQSC_DT_AGG.FIRST_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_ADHERENCE_TOT_NOT_READY_TIME_DT_AGG
    ,RTQSC_DT_AGG.SECOND_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG
...
LEFT OUTER JOIN RTQSC_DT_AGG ON RTQSC_DT_AGG.SUPERVISOR_STAFF_ID = CC_SUMMARY.SUPERVISOR_STAFF_ID
    ;
...
FOR MTH_LOOP IN ADD_MTH_START .. ADD_MTH_STOP
    ...
    ------------------------------------------------------------------
    -- THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT
    ------------------------------------------------------------------
    SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT :=
			(
            --
            case
            when
                case
                when NVL(SUMMARY_REC.COUNT_QC_SCORE_DT_AGG,0) > 0
                then (NVL(SUMMARY_REC.SUM_QC_SCORE_DT_AGG,0) / NVL(SUMMARY_REC.COUNT_QC_SCORE_DT_AGG,0))
                else 1 end < 94.5
            then 1 else 0
            end
            --
            ...
            when
                case
                when NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,0) > 0
                then (NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG,0) / NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,0))
                else 1 end < 94.5
            then 1 else 0
            end
            --
            ...
            when
                case
                when NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,0) > 0
                then (NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG,0) / NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,0))
                else 1 end < 94.5
            then 1 else 0
            end
            --
    ...
    ------------------------------------------------------------------
    -- THREE_MONTH_AVG_TEAM_QUALITY
    ------------------------------------------------------------------
    SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY :=
			CASE WHEN
                (NVL(SUMMARY_REC.COUNT_QC_SCORE_DT_AGG,0)
				+NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,0)
				+NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,0)
                )
				> 0
				THEN
					( NVL(SUMMARY_REC.SUM_QC_SCORE_DT_AGG,0)
					+NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG,0)
					+NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_SUM_QC_SCORE_DT_AGG,0)
					) /
					(
					NVL(SUMMARY_REC.COUNT_QC_SCORE_DT_AGG,0)
					+NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_COUNT_QC_SCORE_DT_AGG,0)
    ...
    ------------------------------------------------------------------
    -- THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT
    ------------------------------------------------------------------
    SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT :=
			(
            --
            case
            when
                case
                when NVL(SUMMARY_REC.LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0) > 0
                then (
                    (NVL(SUMMARY_REC.ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG,0)
					-
					NVL(SUMMARY_REC.ADHERENCE_TOT_NOT_READY_TIME_DT_AGG,0)
					)  / NVL(SUMMARY_REC.LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0)
                    )
                else 0 end < .965
            then 1 else 0
            end
            ...
            when
                case
                when NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0) > 0
                then (
                    (NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG,0)
					-
					NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_ADHERENCE_TOT_NOT_READY_TIME_DT_AGG,0)
					)  / NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0)
                    )
                else 0 end < .965
            then 1 else 0
            end
            ...
            when
                case
                when NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0) > 0
                then (
                    (NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_ADHERENCE_TOT_LOGGED_IN_TIME_DT_AGG,0)
					-
					NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_ADHERENCE_TOT_NOT_READY_TIME_DT_AGG,0)
					)  / NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0)
                    )
                else 0 end < .965
            then 1 else 0
            end
    ...
    ------------------------------------------------------------------
    -- THREE_MONTH_AVG_TEAM_ADHERENCE
    ------------------------------------------------------------------
    SUMMARY_REC.THREE_MONTH_AVG_TEAM_ADHERENCE :=
			CASE WHEN
                (NVL(SUMMARY_REC.LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0)
				+NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0)
				+NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_ADHERENCE_LAG_TIME_TOT_SCHED_PROD_TIME_DT_AGG,0)
...
------------------------------------------------------------------
    -- THREE_MONTH_SHORT_CALLS_REQ_MISSED_COUNT --Created as part of JIRA48260 under DP1.7
    ------------------------------------------------------------------
    SUMMARY_REC.THREE_MONTH_SHORT_CALLS_REQ_MISSED_COUNT :=
            ...
            when
                case
                when NVL(SUMMARY_REC.TOT_CALLS_ANSWERED_DT_AGG,0) > 0
                then (
                NVL(SUMMARY_REC.AGENT_DISCONNECTED_SHORT_CALLS_DT_AGG,0) / NVL(SUMMARY_REC.TOT_CALLS_ANSWERED_DT_AGG,0)
                    )
                else 1 end >= .015
            then 1 else 0
            end
...
------------------------------------------------------------------
    -- THREE_MONTH_RTQ_REQ_MISSED_COUNT --Created as part of JIRA48260 under DP1.7
    ------------------------------------------------------------------
    SUMMARY_REC.THREE_MONTH_RTQ_REQ_MISSED_COUNT :=
			(
            --
            case
            when
                case
                when NVL(SUMMARY_REC.CALLS_OFFERED_DT_AGG,0) > 0
                then (
                NVL(SUMMARY_REC.TOT_TOT_RETURN_TO_QUEUE_DT_AGG,0) / NVL(SUMMARY_REC.CALLS_OFFERED_DT_AGG,0)
                    )
                else 1 end >= .035
            then 1 else 0
            end
            ...
            when
                case
                when NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) > 0
                then (
                 NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_CALLS_OFFERED,0)
                )
                else 1 end >= .035
            then 1 else 0
            end
            ...
            when
                case
                when NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) > 0
                then (
                 NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_CALLS_OFFERED,0)
                )
                else 1 end >= .035
            then 1 else 0
            end
...
------------------------------------------------------------------
    -- THREE_MONTH_RTQ_REQ_MET_COUNT --Created as part of JIRA57545 under DP1.7
    ------------------------------------------------------------------
    SUMMARY_REC.THREE_MONTH_RTQ_REQ_MET_COUNT :=
			(
            --
            case
            when
...
---------------                    
 ------------------------------------------------------------------
    -- THREE_MONTH_TRTQ_TSC_FLAG
	-- Created as part of JIRA48260 under DP1.7
	--
	-- 1-Unacceptable
	-- If Team's RTQ (< or = to 3%) and Short Calls (< or = to 1%) 
    -- meets for 1 month of the reporting period. Result will 'Unacceptable
	-- Result = 1
	--
	-- 2 - Below Expectations
	-- If Team's  RTQ (< or = to 3%) and Short Calls (< or = to 1%)
    -- were met for 2 months of the reporting period. Result will 'Below Expectations'.
	-- 1 Month Missed
	-- Result = 2
	--
	-- 3 - Meets Expectations
	-- If Team's  RTQ (< or = to 3%) and Short Calls (< or = to 1%) 
    -- meets for each month of the reporting period.  Result will 'Meets Expectations'.
	-- Result = 3
	--
	-- 4 - Above Expectations
	-- If Team's RTQ (< or = to 2%) for each month 
    -- in the reporting period AND Short Calls was 0% 
    -- for two months in the reporting period and 
    -- less than or equal to 1% for one month 
    -- of the reporting period. Result will 'Above Expectations'.
	-- Result = 4
	--
	-- 5 - Superior
	-- If Team's RTQ (0%) and Short Calls (0%) 
    -- for each month in the reporting period. Result will 'Superior'.
	-- Result = 5
    ------------------------------------------------------------------

			SUMMARY_REC.THREE_MONTH_TRTQ_TSC_FLAG :=
				CASE
                WHEN
                ((NVL(SUMMARY_REC.TOT_CALLS_ANSWERED_DT_AGG,0)
				+NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_SC_TOT_CALLS_ANSWERED,0)
				+NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_SC_TOT_CALLS_ANSWERED,0)
                ) = 0
                AND (NVL(SUMMARY_REC.CALLS_OFFERED_DT_AGG,0)
				+NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_CALLS_OFFERED,0)
				+NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_CALLS_OFFERED,0)
                ) = 0
                )
                    THEN 0
                ----------------
				WHEN NVL(SUMMARY_REC.THREE_MONTH_SHORT_CALLS_REQ_MISSED_COUNT,0) > 1
                OR NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MISSED_COUNT,0) > 1
					THEN 1
				----------------
				WHEN NVL(SUMMARY_REC.THREE_MONTH_SHORT_CALLS_REQ_MISSED_COUNT,0) = 1
                OR NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MISSED_COUNT,0) = 1
				...
				AND NVL(SUMMARY_REC.AGENT_DISCONNECTED_SHORT_CALLS_DT_AGG,0) / NVL(SUMMARY_REC.TOT_CALLS_ANSWERED_DT_AGG,0) >= .005
                AND NVL(SUMMARY_REC.AGENT_DISCONNECTED_SHORT_CALLS_DT_AGG,0) / NVL(SUMMARY_REC.TOT_CALLS_ANSWERED_DT_AGG,0) < .015
                ...
                AND NVL(SUMMARY_REC.THREE_MONTH_SHORT_CALLS_REQ_MET_COUNT,0) >= 2
				--
                AND NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MET_COUNT,0) = 3
				AND NVL(SUMMARY_REC.TOT_TOT_RETURN_TO_QUEUE_DT_AGG,0) / NVL(SUMMARY_REC.CALLS_OFFERED_DT_AGG,0) >= .005
                AND NVL(SUMMARY_REC.TOT_TOT_RETURN_TO_QUEUE_DT_AGG,0) / NVL(SUMMARY_REC.CALLS_OFFERED_DT_AGG,0) < .035
                AND NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) >= .005
                AND NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) < .035
                AND NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) >= .005
                AND NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) < .035
                	THEN 3
				------------------
				WHEN NVL(SUMMARY_REC.THREE_MONTH_SHORT_CALLS_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.AGENT_DISCONNECTED_SHORT_CALLS_DT_AGG,0) / NVL(SUMMARY_REC.TOT_CALLS_ANSWERED_DT_AGG,0) < .015
                AND NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_SC_AGENT_DISCONNECTED_SHORT_CALLS,0) / NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_SC_TOT_CALLS_ANSWERED,0) < .015
                AND NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_SC_AGENT_DISCONNECTED_SHORT_CALLS,0) / NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_SC_TOT_CALLS_ANSWERED,0) < .015
                AND NVL(SUMMARY_REC.THREE_MONTH_SHORT_CALLS_REQ_MET_COUNT,0) <= 1
				--
                AND NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MET_COUNT,0) <= 3
				AND NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MET_COUNT,0) > 0
				AND NVL(SUMMARY_REC.TOT_TOT_RETURN_TO_QUEUE_DT_AGG,0) / NVL(SUMMARY_REC.CALLS_OFFERED_DT_AGG,0) <= .02499
                AND NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) <= .02499
                AND NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) <= .02499
                	THEN 4
				------------------
				WHEN NVL(SUMMARY_REC.THREE_MONTH_SHORT_CALLS_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.AGENT_DISCONNECTED_SHORT_CALLS_DT_AGG,0) / NVL(SUMMARY_REC.TOT_CALLS_ANSWERED_DT_AGG,0) <= .00499
                ...
                AND NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.THREE_MONTH_RTQ_REQ_MET_COUNT,0) = 0
				AND NVL(SUMMARY_REC.TOT_TOT_RETURN_TO_QUEUE_DT_AGG,0) / NVL(SUMMARY_REC.CALLS_OFFERED_DT_AGG,0) <= .00499
                AND NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.FIRST_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) <= .00499
                AND NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_TOT_TOT_RETURN_TO_QUEUE,0) / NVL(SUMMARY_REC.SECOND_PRIOR_MONTH_RTQ_CALLS_OFFERED,0) <= .00499
                	THEN 5
				------------------
				ELSE 3 END;
   ...
   ------------------------------------------------------------------
    ...
    --5		If overall quality average is 100% for the reporting period, result should be 'Superior'(5)
    --4  	If requirement is met for all months in the reporting period and overall quality average is  99%, result should be 'Above Expectations'(4)
    --3  	If requirement is met for all months in the reporting period and overall quality average is between 95%-98%, result should be 'Meets Expectations'(3)
    --2  	If Requirement not met for 1 month of the reporting period, result  should be ' Below Expectations'(2)
    --1  	If Requirement not met for 2 months in the reporting period, result  should be 'Unacceptable'(1)
    --0  	Reps with No Quality details for all three months will be set to Null(0)
    ------------------------------------------------------------------
			SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_DT_AGG_FLAG :=
				CASE
				WHEN NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY,0) = 0
				THEN 0
				-----
				WHEN NVL(SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT,0) > 1
					THEN 1
				-----
				WHEN (NVL(SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT,0) = 1
				OR NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY,0) < 94.5)
					THEN 2
				-----
				WHEN (NVL(SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY,0) >= 94.5
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY,0) < 98.5)
					THEN 3
				-----
				WHEN (NVL(SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY,0) >= 98.5
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY,0) < 99.5)
					THEN 4
				-----
				WHEN (NVL(SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY,0) >= 99.5)
    ...
    --5		If overall adherence score is 100%, result should be 'Superior'(5)
    --4  	If requirement is met for all months in the reporting period and overall adherence score is  99%, result should be 'Above Expectations'(4)
    --3  	If requirement is met for all months in the reporting period and overall adherence score is between 97%-98%, result should be 'Meets Expectations'(3)
    --2  	If Requirement not met for 1 month of the reporting period, result  should be ' Below Expectations'(2)
    --1  	If Requirement not met for 2 months in the reporting period, result  should be 'Unacceptable'(1)
    --0  	Reps with No Adherence details for all three months will be set to Null(0)
    ------------------------------------------------------------------
			SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_DT_AGG_FLAG :=
				CASE
				WHEN NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_ADHERENCE,0) = 0
				THEN 0
				-----
				WHEN NVL(SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT,0) > 1
					THEN 1
				-----
				WHEN (NVL(SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT,0) = 1
				OR NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_ADHERENCE,0) <= .96499)
					THEN 2
				-----
				WHEN (NVL(SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_ADHERENCE,0) >= .965
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_ADHERENCE,0) < .985)
					THEN 3
				-----
				WHEN (NVL(SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT,0) = 0
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_ADHERENCE,0) < .995
				AND NVL(SUMMARY_REC.THREE_MONTH_AVG_TEAM_ADHERENCE,0) >= .985)
					THEN 4
				-----
				WHEN (NVL(SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT,0) = 0
    ...
    ------------------------------------------------------------------
            ...
            --
				...
				SET
                    ...
                    --Added as part of JIRA48260 under DP1.7
                  ,THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT = SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_REQ_MISSED_COUNT
                  ,THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT = SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_REQ_MISSED_COUNT
                  ,THREE_MONTH_AVG_TEAM_QUALITY             =   SUMMARY_REC.THREE_MONTH_AVG_TEAM_QUALITY
                  ,THREE_MONTH_AVG_TEAM_ADHERENCE           =   SUMMARY_REC.THREE_MONTH_AVG_TEAM_ADHERENCE
                  ...
                  ,THREE_MONTH_RTQ_REQ_MISSED_COUNT         =  SUMMARY_REC.THREE_MONTH_RTQ_REQ_MISSED_COUNT
                  ,THREE_MONTH_RTQ_REQ_MET_COUNT            =  SUMMARY_REC.THREE_MONTH_RTQ_REQ_MET_COUNT
                  ,THREE_MONTH_TRTQ_TSC_FLAG                =   SUMMARY_REC.THREE_MONTH_TRTQ_TSC_FLAG
                  ,THREE_MONTH_TEAM_QUALITY_DT_AGG_FLAG     = SUMMARY_REC.THREE_MONTH_TEAM_QUALITY_DT_AGG_FLAG
                  ,THREE_MONTH_TEAM_ADHERENCE_DT_AGG_FLAG   = SUMMARY_REC.THREE_MONTH_TEAM_ADHERENCE_DT_AGG_FLAG
                  --Added as part of JIRA48260 under DP1.7                    
...
grant execute, debug on "DP_SCORECARD"."LOAD_SC_SUMMARY_CC_ROLLUP" to MAXDAT;
grant execute, debug on "DP_SCORECARD"."LOAD_SC_SUMMARY_CC_ROLLUP" to MAXDAT_REPORTS;
grant execute, debug on "DP_SCORECARD"."LOAD_SC_SUMMARY_CC_ROLLUP" to MAXDAT_READ_ONLY;
grant execute on "DP_SCORECARD"."LOAD_SC_SUMMARY_CC_ROLLUP" to DP_SCORECARD_READ_ONLY;
GRANT execute ON "DP_SCORECARD"."LOAD_SC_SUMMARY_CC_ROLLUP" to MAXDAT_MSTR_TRX_RPT;
GRANT EXECUTE ON "DP_SCORECARD"."LOAD_SC_SUMMARY_CC_ROLLUP" to DP_SCORECARD_PFP_E;
GRANT DEBUG ON "DP_SCORECARD"."LOAD_SC_SUMMARY_CC_ROLLUP" to DP_SCORECARD_PFP_READ_ONLY;
