...
AND Case when (e.call_date is null
		...
		then E.EVALUATION_DATE_TIME
		else e.call_date end >= TRUNC(ADD_MONTHS(SYSDATE, -12), 'MM')
;
...
create table dp_scorecard.wfl_sc_summary_cc_47593_test4 
as select * from dp_scorecard.sc_summary_cc;

grant select on dp_scorecard.wfl_sc_summary_cc_47593_test4 to dp_scorecard_read_only;

grant select on dp_scorecard.wfl_sc_summary_cc_47593_test4 to maxdat_read_only;
