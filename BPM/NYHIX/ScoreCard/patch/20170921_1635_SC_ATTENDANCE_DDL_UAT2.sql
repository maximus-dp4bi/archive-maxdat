...
CREATE OR REPLACE PACKAGE BODY DP_SCORECARD.Calc_Attendance_Score_PKG AS 
	...
	GV_ROLLING_MONTHS   			NUMBER(3) 		:= -6;           --- << Change to -12 for Production USE -3 month rolling FOR TESTING
	GV_STARTING_POINT_DATE          DATE      		:= TO_DATE('21070701','YYYYMMDD');  
...
--	GV_HOLD_EOM_FLAG              	VARCHAR2(1 BYTE) := NULL;
	...
	WITH 
		...
		---------------------------------------------	
	HIRE AS 
		( SELECT MIN(TO_CHAR(NVL(HIRE_DATE,SYSDATE),'YYYYMM'))      AS HIRE_MONTH_NUM 
		FROM DP_SCORECARD.SCORECARD_HIERARCHY
		WHERE STAFF_STAFF_ID = IN_STAFF_ID
		),
	TERM AS
		( SELECT MAX(TO_CHAR(NVL(TERMINATION_DATE,SYSDATE),'YYYYMM')) AS TERM_MONTH_NUM
		FROM DP_SCORECARD.SCORECARD_HIERARCHY
		WHERE STAFF_STAFF_ID = IN_STAFF_ID
		),
	ATND AS		
		( SELECT MIN(TO_CHAR(NVL(ENTRY_DATE,SYSDATE),'YYYYMM')) AS ATTD_MONTH_NUM
				FROM DP_SCORECARD.SC_ATTENDANCE
				WHERE STAFF_ID = IN_STAFF_ID 
		),
	INIT AS		
		( SELECT MIN(TO_CHAR(NVL(START_DATE,SYSDATE),'YYYYMM')) AS INITIAL_DATE
          FROM DP_SCORECARD.SC_ATTENDANCE_INITIAL_SCORE      
				WHERE STAFF_ID = IN_STAFF_ID
		),
	STAFF_START_STOP AS
		(		 
		SELECT  GREATEST(HIRE_MONTH_NUM, 
                            LEAST( NVL(ATTD_MONTH_NUM,TO_CHAR(SYSDATE,'YYYYMM')), 
                                    NVL(INITIAL_DATE,TO_CHAR(SYSDATE,'YYYYMM')) 
                                 )
                        ) AS ATTND_DATES_MONTH_NUM, 
				TERM_MONTH_NUM  
		FROM HIRE		
...
IS
	...
	BEGIN
            ...
            ELSE
				...
				Raise_Application_Error (-20343, 'COUNT > 1 FOR : '||GV_HOLD_STAFF_ID||' '||GV_HOLD_DATES_MONTH_NUM); 
...
GRANT EXECUTE ON DP_SCORECARD.Calc_Attendance_Score_PKG TO MAXDAT;
GRANT EXECUTE ON DP_SCORECARD.Calc_Attendance_Score_PKG TO MAXDAT_READ_ONLY;
GRANT EXECUTE ON DP_SCORECARD.Calc_Attendance_Score_PKG TO MAXDAT_REPORTS;
