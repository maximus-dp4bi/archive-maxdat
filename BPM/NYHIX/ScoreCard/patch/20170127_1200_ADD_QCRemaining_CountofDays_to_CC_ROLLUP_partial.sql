--select * from dp_scorecard.scorecard_hierarchy_sv order by manager_name 1378

/* ALTER TABLE dp_scorecard.SC_SUMMARY_CC_ROLLUP
ADD (qcs_remaining NUMBER,
     sum_qc_score   NUMBER,
...
CREATE OR REPLACE VIEW DP_SCORECARD.SCORECARD_QUALITY_MTHLY_RU_SV AS
with just_months as
(
      SELECT
...
grant select on SCORECARD_QUALITY_MTHLY_RU_SV to MAXDAT;
grant select on SCORECARD_QUALITY_MTHLY_RU_SV to MAXDAT_READ_ONLY;
...
grant select on scorecard_summary_cc_rollup_sv to MAXDAT;
grant select on scorecard_summary_cc_rollup_sv to MAXDAT_READ_ONLY;
...
CALL_metrics AS
 ...
 SELECT
  ...
  WHERE not exists (select 1 from sc_exclusion_yes_sv b where a21.agent_id = b.agent_id and trunc(a21.as_date) = trunc(b.exclusion_date))
        and TRUNC(a21.AS_DATE) >= TRUNC(ADD_MONTHS(SYSDATE, -11), 'MM')
   GROUP BY a20.staff_staff_id, a21.AS_DATE, a21.EXCLUSION_FLAG
  ) a11 on a10.staff_staff_id = a11.staff_staff_id
   GROUP BY a10.supervisor_staff_id/*a10.staff_staff_id*/, /*a11.AS_DATE*/ dates_month_num, dates_year, a11.EXCLUSION_FLAG
