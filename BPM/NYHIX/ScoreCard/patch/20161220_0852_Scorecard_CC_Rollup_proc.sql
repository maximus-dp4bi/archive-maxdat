...
SELECT distinct h.supervisor_staff_id,
       ...
       avg(SCORE_TOTAL) over (partition by h.supervisor_staff_id ,to_char(TRUNC(EVAL_DATE), 'YYYYMM')) as avg_qc_score,
       count(*) over (partition by h.supervisor_staff_id ,to_char(TRUNC(EVAL_DATE), 'YYYYMM')) as qcs_performed
  from dp_scorecard.SCORECARD_QUALITY_SV q
