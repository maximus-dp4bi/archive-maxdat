...
Referred to supervisor
Checked application'
    ,incident_description = CAST(q'[Account Number:AC0000986945  HX Number: HX0001295206 Caller's First and Last Name:  Masoumeh V Tehrani  Caller Phone Number: 6312553105 SSN: 072784252 DOB: 12/1/1949 Eligibility Determination date: 4/1/2014 Is caller enrolled in a QHP/Dental Plan in the Individual Marketplace or Small Business Marketplace? Yes If SBM, employer group name: N/A Did the caller receive a Plan Card (QHP/ MMC)?: Yes Did the caller receive a CBIC (if yes, provide CIN)?: N/A What coverage appears in eMedNY (FFS/MMC/None)?: N/A What did the plan tell the caller?: N/A Description of call: Consumer payed her  monthly Premium on 3/28/2014 for it to take effect 4/1/2014 also payed  she selected her plan after the 2/15/2014 shouldnt have pay for march Is the caller being denied an urgent medical service/prescription? N/A Date & Time the medical appointment is scheduled? N/A  How many days of medication does the caller have left? N/A Provider Name:Health Republic Provider Phone Number: 1888-990-5702 ]' as VARCHAR2(4000))
where incident_id =  26100074;


begin
for cur in(select a.cmpl_bi_id, b.DCMPLIA_ID,c.DCMPLID_ID, d.DCMPLIR_ID, e.DCMPLAC_ID
           from d_complaint_current a, D_COMPLAINT_INCIDENT_ABOUT b,D_COMPLAINT_INCIDENT_DESC c , D_COMPLAINT_INCIDENT_REASON d,D_COMPLAINT_ACTION_COMMENTS e
           where a.cur_incident_about =  b.incident_about
           and a.cur_incident_reason = d.incident_reason
           and a.cur_incident_description = c.incident_description
           and a.cur_action_comments = e.action_comments           
           and incident_id in(26060635,26091200,26097670,26094445,26094424,26103692,26127886,26142179,26100074))
loop
  update f_complaint_by_date
  set DCMPLIA_ID = cur.DCMPLIA_ID
      ,DCMPLID_ID = cur.DCMPLID_ID
      ,DCMPLIR_ID = cur.DCMPLIR_ID
      ,DCMPLAC_ID = cur.DCMPLAC_ID
  where cmpl_bi_id = cur.cmpl_bi_id
