...
update d_mw_v2_current
 ...
 where trunc(INSTANCE_END_DATE) >= '1-JUL-2015'
 and trunc(INSTANCE_END_DATE) <= '31-JUL-2015';
commit;
...
-- update d_mw_current
   -- set "Age in Business Days" = manage_work.GET_AGE_IN_BUSINESS_DAYS("Create Date","Complete Date"),
       -- "Timeliness Status" = manage_work.GET_TIMELINESS_STATUS("Complete Date",
															   ...
															   -- "SLA Days")
 -- where trunc("Complete Date") >= '1-JUL-2015'
 -- and trunc("Complete Date") <= '31-JUL-2015';
-- commit;
...
update D_APPEALS_CURRENT
   set AGE_IN_BUSINESS_DAYS = nyhix_process_appeals.GET_AGE_IN_BUSINESS_DAYS(CREATE_DATE,CUR_COMPLETE_DATE),
	   APPEAL_TIMELINESS_STATUS = nyhix_process_appeals.GET_TIMELINESS_STATUS(CREATE_DATE,
														                      ...
														                      INCIDENT_TYPE)
   where trunc(CUR_COMPLETE_DATE) >= '1-JUL-2015'
   and trunc(CUR_COMPLETE_DATE) <= '31-JUL-2015';
commit;
...
update d_idr_current
  ...
  where trunc(instance_complete_dt) >= '1-JUL-2015'
  and trunc(instance_complete_dt) <= '31-JUL-2015';
commit;
  ...
  where trunc(BATCH_COMPLETE_DT) >= '1-JUL-2015'
  and trunc(BATCH_COMPLETE_DT) <= '31-JUL-2015';   
commit;
...
-- update D_NYHIX_MFD_CURRENT
 ...
 -- where trunc(complete_dt) >=  '1-JUL-2015'
 -- and trunc(complete_dt) <=  '31-JUL-2015';
-- commit;
...
WHERE trunc(complete_date) >=  '1-JUL-2015'
and trunc(complete_date) <=  '31-JUL-2015';
commit;
