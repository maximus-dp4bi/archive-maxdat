...
CREATE OR REPLACE VIEW F_NYHIX_NOTIFICATIONS_BY_TYPE_SV_OLD AS 
WITH Web AS
...
group by  a11.DOCUMENT_TYPE,a11.COMPLETE_DATE), Applications  AS
...
(SELECT 'Identity Proof Forms' as document_type,ENV_STATUS_DT as complete_date,
              COUNT((CASE WHEN (a11.FORM_TYPE in ('Identity Proof Form','Identity Proofing Form') OR (a11.DOC_TYPE in ('Other') AND a11.FORM_TYPE in ('Nav/CAC ID Proofing Form'))) THEN a11.DCN ELSE NULL END))  completed_count
 FROM     maxdat.D_NYHIX_MFD_CURRENT_V2            a11
 WHERE   (TRUNC(a11.ENV_STATUS_DT) BETWEEN  TRUNC(ADD_MONTHS(SYSDATE, -12),'MM')  AND TRUNC(sysdate)
 AND a11.ENV_STATUS_CD in ('COMPLETEDRELEASED')
 AND ((a11.FORM_TYPE in ('Identity Proof Form','Identity Proofing Form') OR (a11.DOC_TYPE in ('Other') AND a11.FORM_TYPE in ('Nav/CAC ID Proofing Form'))))) 
 group by ENV_STATUS_DT
)
...
GRANT   SELECT ON   MAXDAT.F_NYHIX_NOTIFICATIONS_BY_TYPE_SV_OLD TO   MAXDAT_READ_ONLY;