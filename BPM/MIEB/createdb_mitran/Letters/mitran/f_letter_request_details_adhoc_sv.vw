create or replace force view f_letter_request_details_adhoc_sv as
with lr as (
select
...
, (case when lmreq.eyr_mailed_date is not null then trunc(lmreq.mailed_date) else null end) eyr_mailed_date
...
from lall
;
grant select on F_LETTER_REQUEST_DETAILS_ADHOC_SV to MAXDAT_MITRAN_READ_ONLY;
