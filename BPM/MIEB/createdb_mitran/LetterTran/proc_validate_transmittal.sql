create or replace procedure validate_transmittal (
...
v_date_limit date;
verrstart varchar2(100) :='                    Please fix below errors:' || chr(10);
...
--first level pool errors
    ...
    if IN_FORMNAME = 'QA' AND IN_USER_ID IS NULL THEN
      verror := verror+1;
      verrmsg := verrmsg || 'QA User Signature required;  ' || chr(10);
    end if;
...
---second level
    ...
    begin
      select tran_detail_id, invoiced into v_tran_detail_id, v_tran_detail_invoiced from d_tran_detail_sv td where td.tran_header_id = v_tran_header_id and td.LMDEF_ID = v_lmdef_id and upper(td.LTR_FILENAME) = upper(IN_TRAN_FILENAME) and rownum = 1;
    exception when no_Data_found then
              null;
     when others then
       null;
    ...
    if v_tran_detail_id is not null AND IN_FORMNAME = 'ADD' then
       verror := verror + 1;
      verrmsg := verrmsg || 'Transmittal entry for this letter and letter filename already exists, please use Update;  ' || chr(10);
    elsif v_tran_detail_id is null and IN_FORMNAME IN ( 'UPD','QA') THEN
       verror := verror + 1;
      verrmsg := verrmsg || 'Transmittal entry for this letter not found, please use Add;  ' || chr(10);
    elsif v_tran_detail_id is not null and IN_FORMNAME IN ('UPD','QA') and (v_tran_detail_invoiced is not null or v_tran_detail_invoiced = 'Y' ) then
       verror := verror + 1;
    ...
    if (in_qa_abort is not null or in_qa_split is not null or in_qa_reject is not null) and in_user_id is null then
       verror := verror + 1;
