CREATE OR REPLACE Procedure TRAN_ADJUST_INSERT
    (
...
BEGIN
  ...
  verrmsg := '';
V_TRAN_DATE := TRUNC(IN_TRAN_DATE);
...
if verrmsg is not null then
  ...
  return;
    ...
    BEGIN
      select tran_header_id into v_tran_header_id from d_tran_header th where th.tran_date = V_TRAN_DATE and rownum = 1;
    ...
    begin
      select tran_detail_id into v_tran_detail_id from d_tran_detail_sv td where td.tran_header_id = v_tran_header_id and td.LMDEF_ID = v_lmdef_id and upper(td.LTR_FILENAME) = upper(IN_TRAN_FILENAME) and rownum = 1;
    exception when no_Data_found then
              null;
     when others then
       null;
    ...
    if v_tran_detail_id is not null then
      raise_application_error(-20106, 'Transmittal entry for this letter and letter filename already exists, please use Update');
    end if;
    ...
    IF V_LMDEF_ID > 0 THEN
      ...
      if v_tran_header_id is null and v_project_code is not null and v_program_code is not null then
           ...
           values
             ( v_project_code, v_program_code, V_TRAN_DATE)
...
grant execute on TRAN_ADJUST_INSERT to MAXDAT_MITRAN_PFP_E;
grant execute on TRAN_ADJUST_INSERT to MAXDAT_MITRAN_READ_ONLY;
grant execute on TRAN_ADJUST_INSERT to MAXDAT_REPORTS;
