...
begin
select count(*) into v_count_spchar from dual
where translate (in_qa_user_id, 'x!#$%&()*+/:;<=>?@[\]^`{}|~', 'x') != in_qa_user_id;
end;

    if v_count_spchar > 0 THEN
       /*verror := verror + 1;*/
         verrmsg := verrmsg || 'Action User cannot contain special characters; ' ||CHR(10);
...
validate_transmittal_update(
 ...
 , IN_UPDATED_BY
  , OUT_ERRORS => verrmsg
    ...
    begin
      select tran_detail_id into v_tran_detail_id_exists from d_tran_detail_sv td where td.tran_header_id = v_tran_header_id and td.LMDEF_ID = v_lmdef_id and td.tran_detail_id <> v_tran_detail_id and rownum = 1;
    exception when no_Data_found then
              null;
     when others then
       null;
    ...
    IF V_LMDEF_ID > 0 THEN
      ...
      if v_tran_header_id is not null and v_project_code is not null and v_program_code is not null and v_tran_detail_id is not null then
            update d_tran_detail
               set subprogram_code = v_subprogram_code,
                   lmdef_id = v_lmdef_id,
