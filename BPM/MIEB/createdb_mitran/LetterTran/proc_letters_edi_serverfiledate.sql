...
for filerec in (select xml_filename, max(trunc(server_file_date)) server_file_date from etl_l_letters_edi where job_ctrl_id = jobrec.job_ctrl_id group by xml_filename)
  loop

    begin
      vtrandate := filerec.server_file_date;
...
for drec in (select xml_filename, xml_lettertype, xml_materialcd, max(trunc(server_file_date)) server_file_date, count(1) qty from etl_l_letters_edi where job_Ctrl_id = jobrec.job_Ctrl_id and tran_date = trec.tran_Date group by xml_filename, xml_lettertype, xml_materialcd order by xml_lettertype, xml_materialcd)
loop
    v_lmdef_id := null;
   ...
   if v_lmdef_id is not null then
     v_tran_detail_id := null;
      ...
      update d_tran_detail set requested_qty = drec.qty, server_file_date = drec.server_file_date where tran_detail_id = v_tran_detail_id;

      exception when no_data_found then
        begin

        insert into d_tran_detail
          (tran_detail_id, tran_header_id, subprogram_code, lmdef_id, ltr_filename, requested_qty, server_file_date)
        values
          (v_tran_detail_id, v_tran_header_id, v_subprogram_code, v_lmdef_id, drec.xml_filename, drec.qty, drec.server_file_date);
        exception when others then
            v_code := SQLCODE;
            v_desc := SQLERRM;
              corp_etl_stage_pkg.log_etl_msg(in_job_name => 'PROCESS_LETTERS_EDI'
