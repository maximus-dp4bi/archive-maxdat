drop procedure MITRAN_TRAN_COMM_FORM_INSERT;
...
exception when others then
 ...
 then
   ...
   if in_action_date < v_date_limit then
    ...
    if IN_TRAN_DATE > trunc(sysdate)  then
       verror := verror + 1;
   ...
   if in_action_date > trunc(sysdate) then
       verror := verror + 1;
...
grant execute on TRAN_COMM_INSERT to MAXDAT_MITRAN_PFP_E;
grant execute on TRAN_COMM_INSERT to MAXDAT_MITRAN_READ_ONLY;
grant execute on TRAN_COMM_INSERT to MAXDAT_REPORTS;
