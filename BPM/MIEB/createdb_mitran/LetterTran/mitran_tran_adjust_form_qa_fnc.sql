CREATE OR REPLACE Procedure TRAN_ADJUST_QA
  ...
  , IN_LETTER_ADJUST_REASON_LABEL IN OUT VARCHAR2
  , IN_COMMENTS IN OUT VARCHAR2
   )
...
BEGIN
  v_driver_key := nvl(to_char(IN_TRAN_JOB_ID),'null');
  v_err_field := 'IN_JOBID';

  verrmsg := ' ';

if in_tran_job_id is null and in_tran_detail_id is null then
          verrmsg := verrmsg + chr(10) || 'Transmittal Letter record cannot be identified';
end if;

if in_tran_job_id is not null AND in_tran_detail_id is null then
  --return the next in line QA
  begin
        select tran_detail_id
         into v_tran_detail_id
          from d_tran_detail
         where tran_job_id = in_tran_job_id
         and rownum = 1;
         return;
...
if in_qa_signed is null and (in_qa_split is not null or in_qa_reject is not null or in_qa_abort is not null or in_qa_comments is not null) then
  verrmsg := verrmsg || CHR(10) || 'QA needs approver Signature';
end if;
    ...
    EXCEPTION WHEN no_data_found then
              verrmsg := verrmsg || 'Adjustment Reason not found';
