create or replace view f_letter_counts_by_day_sv as
with lr as
(
select /*+ parallel(10)*/
...
, null requested_on
, null sent_on
, td.eyr_mailed_date mailed_date
, th.tran_date  REPORT_DATE
, td.eyr_mailed_qty mailed_count
, 0 confirmed_count
, 0 adjustment_count
, 0 adjustment_flag
, lmdef.letter_invoice_group_code
...
from
d_tran_header th
left join d_tran_detail td on td.tran_header_id = th.tran_header_id
left join d_letter_definition lmdef on lmdef.lmdef_id = td.lmdef_id
left join D_LETTER_INVOICE_GROUP lig on lig.letter_invoice_group_code = lmdef.letter_invoice_group_code
where tran_date >= TO_DATE('11/1/2021','MM/DD/YYYY')
)
, eyrl as
(
select /*+ parallel(10)*/
