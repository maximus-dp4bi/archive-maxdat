...
SELECT /*+ parallel(10) */
    ...
    COALESCE(CASE WHEN (cr.caller_type_cd = 'FEC' OR (ss.ref_source_id IS NOT NULL AND ss.ref_source_type = 'OFFICE')) THEN 'F' ELSE ss.selection_source_cd END,'0') selection_source_code,
    ...
    '0' risk_group_code,    
    COALESCE(ac.value,COALESCE(ss.client_aid_category_cd, '0')) aid_category_code,
    '0' reason_code, --term reason code    
    '0' rejection_code,
    COALESCE(case when ss.plan_type_cd = 'DENTAL' then 'DEN'
                   when ac.subprogram_codes = 'MC' then 'MED' 
                  else ac.subprogram_codes end,'0') subprogram_code ,
    case when ac.subprogram_codes = 'MC' then 'Y' else 'N' end is_mc,
    ...
    coalesce((SELECT MIN(TRUNC(sh.create_ts))
      ...
      WHERE sh.selection_txn_id = ss.selection_txn_id AND sh.status_cd = (case when ac.subprogram_codes = 'ICO' then 'uploadedToIFC' else 'uploadedToState' end)
  ...
  inner join case_client cc on cc.cscl_id = cs.case_client_id 
  LEFT JOIN enum_aid_category ac ON cc.cscl_adlk_id = ac.value  
--subprogram comes from contract
  LEFT JOIN contract ca ON ss.contract_id = ca.contract_id
  LEFT JOIN event e ON (e.ref_id = ss.selection_txn_id AND e.ref_type = 'SELECTION_TXN' AND e.event_type_cd IN('PHONE_ENROLLMENT','CHOICE_ENROLLMENT'))
