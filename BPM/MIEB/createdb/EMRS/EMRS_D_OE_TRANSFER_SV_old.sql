CREATE OR REPLACE VIEW EMRS_D_OE_TRANSFER_SV_1
 AS
 select program_type_cd
       , subprogram_code to_subprogram_code
       , plan_id to_plan_id
--       , curr_selection_segment_id to_selection_segment_id
--       , curr_start_date to_start_date
--       , curr_end_date to_end_date
       , prior_subprogram_code from_subprogram_code
       , prior_plan_id from_plan_id
--       , prior_selection_segment_id from_selection_segment_id
--       , prior_start_date from_start_date
--       , prior_end_Date from_End_date
       , record_date
...
from (
   ...
   COALESCE(s.prior_plan_id, 0) AS prior_plan_id,
    s.prior_plan_id_ext ,
    COALESCE(ca.subprogram_codes,COALESCE(ca.subprogram_codes,'0')) AS prior_subprogram_code,
    s.status_cd ,
    ...
    csi.case_id, 
    csi.case_cin,
--    ss.start_date curr_start_date,
--    ss.end_date curr_end_Date,
--    ssp.start_date prior_start_date,
--    ssp.end_date prior_end_date,
--    ss.selection_segment_id curr_selection_segment_id,
--    ssp.selection_segment_id prior_selection_segment_id,
    (
          select max(sent_on) oe_letter
          from letter_request r
          join letter_definition d on d.lmdef_id = r.lmdef_id
          and d.name in ('OE','OEC','OECM' ,'OEHC','OEHM','OEM')
...
--  INNER JOIN SELECTION_SEGMENT ssp ON ssp.client_id = s.client_id and ssp.plan_id = s.prior_plan_id and ssp.plan_type_cd = s.plan_type_cd and ssp.end_date = ss.start_date-1 and ssp.start_date < nvl(ssp.end_date, ssp.start_date + 1)
  ...
  where ca.subprogram_codes in ('MED','MC','CSHCS','HMP')
)
where oe_letter_sent_date is not null;

--GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_OE_TRANSFER_SV TO MAXDAT_REPORTS; 
