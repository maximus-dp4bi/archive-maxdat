CREATE OR REPLACE VIEW "MAXDAT_SUPPORT"."EMRS_F_AA_TO_VOL_SV" AS 
  
  SELECT trunc(d.D_date,'MM') AS RECORD_DATE
       , p.PLAN_ID TO_PLAN_ID
       ,pp.plan_id FROM_PLAN_ID
       ,sp.TO_CLIENT_AID_CATEGORY_CD
       ,sp.FROM_CLIENT_AID_CATEGORY_CD
...
left join (
      ...
      (
            ...
            ,E1.CLIENT_AID_CATEGORY_CD FROM_CLIENT_AID_CATEGORY_CD
            ,E2_PRIOR.CLIENT_AID_CATEGORY_CD TO_CLIENT_AID_CATEGORY_CD
...
--                 and e1.prior_plan_id = 800
        ...
        and sp.from_plan_id = pp.plan_id
        )
where d.D_DATE BETWEEN ADD_MONTHS(TRUNC(sysdate, 'mm'), -5) AND TRUNC(sysdate)
