...
CREATE OR REPLACE PACKAGE ETL_JOB AS
	...
	PROCEDURE ENABLE_ETL_JOB (p_job_id IN NUMBER, 
                              p_apply_to_childs IN VARCHAR2 DEFAULT 'N');
	PROCEDURE DISABLE_ETL_JOB (p_job_id IN NUMBER, 
                               p_apply_to_childs IN VARCHAR2 DEFAULT 'N');
	PROCEDURE COMPLETE_ETL_JOB (p_job_id IN NUMBER);
	PROCEDURE RESET_ETL_JOB (p_job_id IN NUMBER, 
                             p_apply_to_childs IN VARCHAR2 DEFAULT 'N');
	PROCEDURE CANCEL_ETL_JOB (p_job_id IN NUMBER, 
                              p_apply_to_childs IN VARCHAR2 DEFAULT 'N');
	PROCEDURE ADD_ETL_JOB_HOLD (p_job_id_to_hold IN NUMBER,
                                p_job_id_running IN NUMBER,	
                                p_skip_run IN VARCHAR2 DEFAULT 'N');
	PROCEDURE DELETE_ETL_JOB_HOLD (p_job_id_held IN NUMBER,
                                   p_job_id_running IN NUMBER);	
END;
/
-- END PACKAGE ETL_JOB
...
CREATE OR REPLACE PACKAGE BODY ETL_JOB AS
    ...
    AS
        ...
        v_i_mi_str VARCHAR2(50);
        v_i_hr_str VARCHAR2(50);
        v_i_dy_str VARCHAR2(50);
        v_i_mn_str VARCHAR2(50);
        v_i_dw_str VARCHAR2(50);
    ...
    BEGIN
		...
		-- STEP 70: Skip the execution for hold job when corresponding running job is current being executed   
        v_step_id := 70;
        v_step_desc := 'Skip the execution for hold job when corresponding running job is current being executed';
        ...
        SET 
            JOB_NEXT_EXEC_DT = (SELECT CASE WHEN COALESCE(JOB_SCHEDULE,'*') = '*' THEN TO_DATE(gv_DEFAULT_HIGH_DATE,gv_DATE_FORMAT)
                                ...
                                WHERE jc.JOB_ID = ETL_JOB_STATUS.JOB_ID),
            LAST_UPD_DT = SYSDATE,
            LAST_UPD_USER = USER
        WHERE
            JOB_ID IN (SELECT jl.JOB_ID
                       FROM ETL_JOB_LIST jl
					        JOIN ETL_JOB_HOLD_SKIP jhs
							  ON jl.JOB_ID = jhs.JOB_ID_TO_HOLD
							 AND jhs.SKIP_RUN = 'Y'
			  	            JOIN ETL_JOB_STATUS js
							  ON js.JOB_ID = jhs.JOB_ID_RUNNING
							 AND js.JOB_RUNNABLE = 'N');	 
							   
		-- STEP 80: Ignore the hold job when corresponding running job is current being executed
        v_step_id := 80;
        v_step_desc := 'Ignore the hold job when corresponding running job is current being executed';
		
        DELETE FROM ETL_JOB_LIST
		WHERE JOB_ID IN (SELECT jl.JOB_ID
                         FROM ETL_JOB_LIST jl
					          JOIN ETL_JOB_HOLD_SKIP jhs
							    ON jl.JOB_ID = jhs.JOB_ID_TO_HOLD
						      JOIN ETL_JOB_STATUS js
		...
		-- STEP 90: Skip the execution for hold job when running job is also ready for execution in the same batch   
        v_step_id := 90;
        v_step_desc := 'Skip the execution for hold job when running job is also ready for execution in the same batch';
            ...
            JOB_ID IN (SELECT jl.JOB_ID
                       FROM ETL_JOB_LIST jl
					        JOIN ETL_JOB_HOLD_SKIP jhs
							  ON jl.JOB_ID = jhs.JOB_ID_TO_HOLD
							 AND jhs.SKIP_RUN = 'Y'
			  	            JOIN ETL_JOB_LIST jl2
							  ON jl2.JOB_ID = jhs.JOB_ID_RUNNING);	 
							   
		-- STEP 100: Ignore the hold job when corresponding running job is also ready for execution in the same batch
        v_step_id := 100;
        v_step_desc := 'Ignore the hold job when corresponding running job is also ready for execution in the same batch';
        ...
        -- STEP 110: delete job if another job with the same script name is already running     
        v_step_id := 110;
        v_step_desc := 'delete job if another job with the same script name is already running';

        DELETE FROM ETL_JOB_LIST lst
		WHERE JOB_ID IN (SELECT cfg1.JOB_ID
                         FROM ETL_JOB_CONFIG cfg1
					          JOIN ETL_JOB_CONFIG cfg2
					            ON cfg1.JOB_SCRIPT_NAME = cfg2.JOB_SCRIPT_NAME
                               AND cfg1.JOB_ID <> cfg2.JOB_ID
                               AND cfg1.JOB_ID = lst.JOB_ID
						      JOIN ETL_JOB_STATUS js
							    ON js.JOB_ID = cfg2.JOB_ID
							   AND js.JOB_RUNNABLE = 'N');	

        -- STEP 120: just execute one job when multiple jobs with the same script name are ready for execution
        v_step_id := 120;
        v_step_desc := 'just execute one job when multiple jobs with the same script name are ready for execution';
        
        DELETE FROM ETL_JOB_LIST
		WHERE JOB_ID IN (SELECT JOB_ID 
                         FROM (SELECT cfg.JOB_SCRIPT_NAME, cfg.JOB_ID, RANK() over (PARTITION BY cfg.JOB_SCRIPT_NAME ORDER BY cfg.JOB_ID) rank_num    
                               FROM ETL_JOB_CONFIG cfg
                                    JOIN ETL_JOB_LIST lst
                                    ON lst.JOB_ID = cfg.JOB_ID) a 
                         WHERE rank_num >= 2);
        
        -- STEP 130: Commit all changes
        v_step_id := 130;
    ...
    -- BEGIN PROCEDURE ENABLE_ETL_JOB
    PROCEDURE ENABLE_ETL_JOB (p_job_id IN NUMBER, 
                              p_apply_to_childs IN VARCHAR2 DEFAULT 'N')
    AS
        -- variable declaration
    ...
    BEGIN
        ...
        -- STEP 10: update JOB_ENABLED with Y
        v_step_id := 10;
        v_step_desc := 'update JOB_ENABLED with Y when the job should be Enabled';
		...
		-- STEP 20:  Commit the updates
		v_step_id := 20;
		v_step_desc := 'Commit the updates';
		
		COMMIT;
        
        -- STEP 30: Cascade the ENABLE status for all the children
        v_step_id := 30;
        v_step_desc := 'Cascade the ENABLE status for all the children';
        
        IF (COALESCE(p_apply_to_childs,'N') = 'Y') THEN
            FOR child_job_rec IN CURSOR_CHILD_JOBS
            LOOP
                ENABLE_ETL_JOB(child_job_rec.JOB_ID, 'Y');
            END LOOP;
        END IF;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ADD_ETL_LOG ($$PLSQL_UNIT || '.ENABLE_ETL_JOB procedure : ' || SUBSTR(SQLERRM,1,200), 'ERROR', SQLCODE, NULL, NULL, v_step_id, v_step_desc);
                RAISE; 
    END;
    -- END PROCEDURE ENABLE_ETL_JOB
 
    -- BEGIN PROCEDURE DISABLE_ETL_JOB
    PROCEDURE DISABLE_ETL_JOB (p_job_id IN NUMBER, 
                               p_apply_to_childs IN VARCHAR2 DEFAULT 'N')
    AS
        -- variable declaration
    ...
    BEGIN
        ...
        -- STEP 10: update JOB_ENABLED with N
        v_step_id := 10;
        v_step_desc := 'update JOB_ENABLED with N when the job should be Disabled';
        ...
        -- STEP 30: Cascade the DISABLE status for all the children
        v_step_id := 30;
        v_step_desc := 'Cascade the DISABLE status for all the children';
        
        IF (COALESCE(p_apply_to_childs,'N') = 'Y') THEN
            FOR child_job_rec IN CURSOR_CHILD_JOBS
            LOOP
                DISABLE_ETL_JOB (child_job_rec.JOB_ID, 'Y');
            END LOOP;
        END IF;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ADD_ETL_LOG ($$PLSQL_UNIT || '.DISABLE_ETL_JOB procedure : ' || SUBSTR(SQLERRM,1,200), 'ERROR', SQLCODE, NULL, NULL, v_step_id, v_step_desc);
                RAISE; 
    END;
    -- END PROCEDURE DISABLE_ETL_JOB

    -- BEGIN PROCEDURE COMPLETE_ETL_JOB
    PROCEDURE COMPLETE_ETL_JOB (p_job_id IN NUMBER)
    AS
        -- variable declaration
        v_step_id NUMBER;
	...
	BEGIN
		...
		-- STEP 10:  check how many active childs are waiting for a run
		v_step_id := 10;
		...
		-- STEP 20: get the job type and parent job id for later processing
		v_step_id := 20;
		...
		-- STEP 30:  update the job run with WAITING (if active child EXISTS waiting run) or COMPLETED (if no active childs need run)
		v_step_id := 30;
		...
		-- STEP 40: update the job status with WAITING (if active child EXISTS waiting run) or COMPLETED (if no active childs need run)               
		v_step_id := 40;
		...
		-- STEP 50:  Commit the updates
		v_step_id := 50;
		v_step_desc := 'Commit the updates';
		
		COMMIT;
		
		-- STEP 60:  When a child is set to COMPLETED, check if parent status can be updated from WAITING to COMPLETED
		v_step_id := 60; 
		...
		IF (v_job_type = 'CHILD' AND v_active_childs_waiting = 0 AND v_parent_job_id <> 0) THEN
			COMPLETE_ETL_JOB(v_parent_job_id);
		END IF;        
                    
        -- STEP 1000: Exception Handling
        EXCEPTION
            WHEN OTHERS THEN
                ADD_ETL_LOG ($$PLSQL_UNIT || '.COMPLETE_ETL_JOB procedure : ' || SUBSTR(SQLERRM,1,200), 'ERROR', SQLCODE, NULL, NULL, v_step_id, v_step_desc);
                RAISE; 
    END;
    -- END PROCEDURE COMPLETE_ETL_JOB

    -- BEGIN PROCEDURE RESET_ETL_JOB
    PROCEDURE RESET_ETL_JOB (p_job_id IN NUMBER, 
                             p_apply_to_childs IN VARCHAR2 DEFAULT 'N')
    AS
        -- variable declaration
    ...
    BEGIN
		...
		-- STEP 10: Enable the job if it is disabled
		v_step_id := 10;
		v_step_desc := 'Enable the job if it is disabled';
		...
		-- STEP 20: update the job run with COMPLETED if job is in WAITING status, if not set to RESET
		v_step_id := 20;
		v_step_desc := 'update the job run with COMPLETED if job is in WAITING status, if not set to RESET';
		...
		SET 
			RUN_END_DT = CASE WHEN RUN_STATUS = 'STARTED' THEN SYSDATE
							  ELSE RUN_END_DT
						 END,
			RUN_STATUS = CASE WHEN RUN_STATUS = 'STARTED' THEN 'RESET'
							  ELSE 'COMPLETED'
						 END,
			LAST_UPD_DT = SYSDATE,
			LAST_UPD_USER = USER 
		...
		-- STEP 30:  update the job status with COMPLETED if job is in WAITING status, if not set to RESET
		-- In case of RESET, JOB_NEXT_EXEC_DT will be set to SYSDATE to execute it in the next job execution
		v_step_id := 30;
		v_step_desc := 'update the job status with COMPLETED if job is in WAITING status, if not set to RESET';
		
		UPDATE ETL_JOB_STATUS
		SET 
			JOB_RUNNABLE = 'Y',
			JOB_STATUS = CASE WHEN JOB_STATUS = 'STARTED' THEN 'RESET'
							  ELSE 'COMPLETED'
						 END,
			JOB_NEXT_EXEC_DT = SYSDATE,
			LAST_UPD_DT = SYSDATE,
			LAST_UPD_USER = USER
		WHERE 
			JOB_ID = p_job_id
		...
		-- STEP 40:  Commit the updates
		v_step_id := 40;
		v_step_desc := 'Commit the updates';
		
		COMMIT;
		
        -- STEP 50: Cascade the RESET status for all the children
        v_step_id := 50;
        v_step_desc := 'Cascade the RESET status for all the children';
        
        IF (COALESCE(p_apply_to_childs,'N') = 'Y') THEN
            FOR child_job_rec IN CURSOR_CHILD_JOBS
            LOOP
                RESET_ETL_JOB(child_job_rec.JOB_ID, 'Y');
            END LOOP;
        END IF;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ADD_ETL_LOG ($$PLSQL_UNIT || '.RESET_ETL_JOB procedure : ' || SUBSTR(SQLERRM,1,200), 'ERROR', SQLCODE, NULL, NULL, v_step_id, v_step_desc);
                RAISE; 
    END;
    -- END PROCEDURE RESET_ETL_JOB
    
    -- BEGIN PROCEDURE CANCEL_ETL_JOB
    PROCEDURE CANCEL_ETL_JOB (p_job_id IN NUMBER, 
                              p_apply_to_childs IN VARCHAR2 DEFAULT 'N')
    AS
        -- variable declaration
    ...
    BEGIN
		...
		-- STEP 10: update the job run with COMPLETED if job is in WAITING status, if not set to CANCELLED
		v_step_id := 10;
		v_step_desc := 'update the job run with COMPLETED if job is in WAITING status, if not set to CANCELLED';
		...
		SET 
			RUN_END_DT = CASE WHEN RUN_STATUS = 'STARTED' THEN SYSDATE
							  ELSE RUN_END_DT
						 END,
			RUN_STATUS = CASE WHEN RUN_STATUS = 'STARTED' THEN 'CANCELLED'
							  ELSE 'COMPLETED'
						 END,
			LAST_UPD_DT = SYSDATE,
			LAST_UPD_USER = USER 
		...
		-- STEP 20:  update the job status with COMPLETED if job is in WAITING status, if not set to CANCELLED
		v_step_id := 20;
		v_step_desc := 'update the job status with COMPLETED if job is in WAITING status, if not set to CANCELLED';
		
		UPDATE ETL_JOB_STATUS
		SET 
			JOB_RUNNABLE = 'Y',
			JOB_STATUS = CASE WHEN JOB_STATUS = 'STARTED' THEN 'CANCELLED'
							  ELSE 'COMPLETED'
						 END,
			LAST_UPD_DT = SYSDATE,
			LAST_UPD_USER = USER
		...
		-- STEP 30:  Commit the updates
		v_step_id := 30;
		v_step_desc := 'Commit the updates';
		
		COMMIT;
		
        -- STEP 40: Cascade the CANCELLED status for all the children
        v_step_id := 40;
        v_step_desc := 'Cascade the CANCELLED status for all the children';
        
        IF (COALESCE(p_apply_to_childs,'N') = 'Y') THEN
            FOR child_job_rec IN CURSOR_CHILD_JOBS
            LOOP
                CANCEL_ETL_JOB(child_job_rec.JOB_ID, 'Y');
            END LOOP;
        END IF;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ADD_ETL_LOG ($$PLSQL_UNIT || '.CANCEL_ETL_JOB procedure : ' || SUBSTR(SQLERRM,1,200), 'ERROR', SQLCODE, NULL, NULL, v_step_id, v_step_desc);
                RAISE; 
    END;
    -- END PROCEDURE CANCEL_ETL_JOB

	-- BEGIN PROCEDURE ADD_ETL_JOB_HOLD
    PROCEDURE ADD_ETL_JOB_HOLD (p_job_id_to_hold IN NUMBER,
                                p_job_id_running IN NUMBER,	
                                p_skip_run IN VARCHAR2 DEFAULT 'N')
    AS
    ...
    BEGIN
		...
		-- STEP 10: check if the hold job request already exists for the given jobs 
		v_step_id := 10;
		v_step_desc := 'check if the hold job request already exists for the given jobs';
		
		SELECT COUNT(*) INTO v_rec_count
        FROM ETL_JOB_HOLD_SKIP
        WHERE JOB_ID_TO_HOLD = p_job_id_to_hold
		  AND JOB_ID_RUNNING = p_job_id_running;

        IF (v_rec_count > 0) THEN
		
			-- STEP 20: update the request if the hold job request already exists 
			v_step_id := 20;
			v_step_desc := 'update the request if the hold job request already exists';
            ...
            SET
				...
				LAST_UPD_USER = USER
            WHERE
                JOB_ID_TO_HOLD = p_job_id_to_hold
		    AND JOB_ID_RUNNING = p_job_id_running;

		ELSE
		
			-- STEP 30: add a new hold job request 
			v_step_id := 30;
			v_step_desc := 'add a new hold job request';

			INSERT INTO ETL_JOB_HOLD_SKIP (
				JOB_ID_TO_HOLD,
				JOB_ID_RUNNING,
				SKIP_RUN,
                LAST_UPD_DT,
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ADD_ETL_LOG ($$PLSQL_UNIT || '.ADD_ETL_JOB_HOLD procedure : ' || SUBSTR(SQLERRM,1,200), 'ERROR', SQLCODE, NULL, NULL, v_step_id, v_step_desc);
                RAISE; 
    END;
    -- END PROCEDURE ADD_ETL_JOB_HOLD
	
	-- BEGIN PROCEDURE DELETE_ETL_JOB_HOLD
    PROCEDURE DELETE_ETL_JOB_HOLD (p_job_id_held IN NUMBER,
                                   p_job_id_running IN NUMBER)
    AS
        -- variable declaration
    ...
    BEGIN
		...
		-- STEP 10: delete the hold job request for the given job 
		v_step_id := 10;
		v_step_desc := 'delete the hold job request for the given job';
		
        DELETE FROM ETL_JOB_HOLD_SKIP
        WHERE JOB_ID_TO_HOLD = p_job_id_held
		  AND JOB_ID_RUNNING = p_job_id_running;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ADD_ETL_LOG ($$PLSQL_UNIT || '.DELETE_ETL_JOB_HOLD procedure : ' || SUBSTR(SQLERRM,1,200), 'ERROR', SQLCODE, NULL, NULL, v_step_id, v_step_desc);
                RAISE; 
    END;
    -- END PROCEDURE DELETE_ETL_JOB_HOLD
