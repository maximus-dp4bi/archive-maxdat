  ...
  */
  type T_INS_MW_XML is record
    (
     ...
     ASSIGNED_DATE varchar2(19),
     ORIGINAL_CREATION_DATE varchar2(19),
...
create or replace package body MANAGE_WORK as
  ...
  begin
    ...
    set
      ...
      Age_From_Assigned_Date = GET_AGE_IN_CALENDAR_DAYS(Assigned_Date,nvl("Cancel Work Date","Complete Date") )
    where
  ...
  begin
   ...
   select
        ...
        extractValue(p_data_xml,'/ROWSET/ROW/ASSIGNED_DATE') "ASSIGNED_DATE",
        extractValue(p_data_xml,'/ROWSET/ROW/ORIGINAL_CREATION_DATE') "ORIGINAL_CREATION_DATE",
        extractValue(p_data_xml,'/ROWSET/ROW/CASE_NUMBER') "CASE_NUMBER",
        extractValue(p_data_xml,'/ROWSET/ROW/PARENT_TASK_ID') "PARENT_TASK_ID",
        extractValue(p_data_xml,'/ROWSET/ROW/ROLE_NAME') "ROLE_NAME",
  ...
  as
    ...
    v_assigned_date date := null;
    v_received_date date := null;

  begin
    v_last_update_date := to_date(p_last_update_date,BPM_COMMON.DATE_FMT);
    v_status_date := to_date(p_status_date,BPM_COMMON.DATE_FMT);
    v_received_date := to_date(p_received_date,BPM_COMMON.DATE_FMT);
    v_assigned_date := to_date(p_assigned_date,BPM_COMMON.DATE_FMT);
  ...
  procedure SET_DMWCUR
      (p_set_type in varchar2,
       ...
       p_received_date in varchar2,
       p_assigned_date in varchar2,
       p_original_creation_date in varchar2,
  ...
  begin
    ...
    r_dmwcur.ASSIGNED_DATE    :=  to_date(p_assigned_date,BPM_COMMON.DATE_FMT);
    r_dmwcur.ORIGINAL_CREATE_DATE    :=  to_date(p_original_creation_date,BPM_COMMON.DATE_FMT);
    r_dmwcur.CASE_NUMBER    := p_case_number;
    r_dmwcur.PARENT_TASK_ID := p_parent_task_id;
    r_dmwcur.AGE_FROM_ASSIGNED_DATE := GET_AGE_IN_CALENDAR_DAYS(r_dmwcur.Assigned_Date,nvl(r_dmwcur."Cancel Work Date",r_dmwcur."Complete Date"));

    r_dmwcur.ROLE_NAME  := p_role_name;
    r_dmwcur.CLAIM_DATE := to_date(p_claim_date,BPM_COMMON.DATE_FMT);
  ...
  begin
    ...
    r_fmwbd.received_date := v_received_date;
    r_fmwbd.assigned_date := v_assigned_date;
