...
AS
 ...
 where employee_id = in_employee_id
   ...
   cursor c2 is
   select (sum(entered_hours) + in_entered_hours) as hrs
      from d_deltek_hours dh
     where dh.employee_id = in_employee_id
       and trunc(dh.hours_date) = trunc(in_date);
...
BEGIN
   ...
   open c2;
   fetch c2 into v_sum_hours;

   if  v_ddh_id > 0 or v_sum_hours > 24 then
     /*do nothing*/
      null;
