...
create or replace package PROCESS_LETTERS as
  ...
  function GET_EPA_MAIL_DT_FOR_CASE
     (p_case_id in number)
     return date;
  ...
  function GET_JEOPARDY_DAYS (p_program in varchar2,p_letter_type in varchar2) return number result_cache;

  function GET_SLA_CATEGORY (p_program in varchar2,p_letter_type in varchar2) return varchar2 result_cache;

  function GET_PL_SLA_CATEGORY (p_program in varchar2,p_letter_type in varchar2) return varchar2 result_cache;

  function GET_SLA_DAYS (p_program in varchar2,p_letter_type in varchar2) return varchar2 result_cache;

  function GET_SLA_DAYS_TYPE (p_program in varchar2,p_letter_type in varchar2) return varchar2 result_cache;

  function GET_SLA_TARGET_DAYS (p_program in varchar2,p_letter_type in varchar2) return varchar2 result_cache;

  function GET_TIMELINE_STATUS
    (p_program in varchar2,
     p_create_date in date,
     p_complete_date in date,
     p_mailed_date in date,
     p_letter_type in varchar2,
     p_case_id in number,
     p_epm_mail_date_for_case in date)
    return varchar2;
...
*/
  ...
  type T_INS_PL_XML is record
    (
     ...
     EPM_MAIL_DT_FOR_CASE varchar2(19),
     ...
     NEW_LETTER_REQUEST_ID varchar2(100),
...
/
create or replace package body PROCESS_LETTERS as

  v_bem_id number := 12; -- 'TXEB Process Letters'
  ...
  begin
    ...
    select  coalesce(max(mailed_date),to_date('07-JUL-7777','DD-MON-YYYY'))
    into    v_epa_mail_dt
    from    d_pl_current
    where   case_id             =   p_case_id
    and     letter_type         =   'Enrollment Letter'
    and     letter_status       =   'Mailed';

    commit;
    return v_epa_mail_dt;
  end;


  function GET_SLA_CATEGORY (p_program in varchar2,p_letter_type in varchar2)
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
    v_special_letter varchar2(500);
  begin
  /*
  Fetching SLA_Category for TX PL is a two-step (two functions used) process because for some letter types, i.e., 'H10','H11','E1R','XMR','IAM','VLM', SLA is different for each individual letter;

  SLA_Category Calculation:
  If it is a CHIP program related letter, then sla_category =  'CHIP Letter';
  If it is a Medicaid program related letter, then sla_category =  'Medicaid Letter'; Letter types, 'H10','H11','E1R','XMR','IAM','VLM', can be a CHIP/Medicaid/Other depending on program type.
  If it is one of the Dual Demo letter types (though program is Medicaid) , then sla_category = 'Dual Eligibility Center';
  If the program = null , then 'Other Letter'.
  */
    begin
      select OUT_VAR
      into v_special_letter
      from CORP_ETL_LIST_LKUP
      where NAME = 'ProcLetters_SpecialSLA_Letters'
      and VALUE = p_letter_type;
    exception
      when no_data_found then
        v_special_letter := null;
    end;
    /*if  p_letter_type = 'Checkup Due Letter' then
      v_sla_category := 'Checkup Due Letter';
    elsif  p_letter_type = 'Checkup Reminder Letter' then
      v_sla_category := 'Checkup Reminder Letter';
    elsif  p_letter_type = 'Enrollment Reminder' then
      v_sla_category := 'Enrollment Reminder';
    elsif  p_letter_type = 'StarPlus Expansion Reminder Letter' then
      v_sla_category := 'StarPlus Expansion Reminder Letter';  */
    if v_special_letter is not null then
       v_sla_category := v_special_letter;
    elsif p_program = 'Medicaid'  then
      v_sla_category := 'Medicaid Letter';
   -- elsif  p_letter_type = 'MDCP Reminder letter' then
   --   v_sla_category := 'MDCP Reminder letter';
    --elsif  trim(p_program) = 'Children' || '''s Health Insurance' then
    elsif p_program = 'CHIP' then
      v_sla_category := 'CHIP Letter';
    else
      v_sla_category := 'Other Letter';
    end if;
  ...
  function GET_PL_SLA_CATEGORY (p_program in varchar2,p_letter_type in varchar2)
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
  begin

    v_sla_category := GET_SLA_CATEGORY(p_program,p_letter_type);

    --IF INSTR(v_sla_category,'CHIP') = 0 AND INSTR(v_sla_category,'Medicaid') = 0 THEN
    IF v_sla_category = 'Dual Eligibility Letter' THEN
      return v_sla_category;
    ELSE
      IF NVL(p_program,'X') != 'CHIP' AND NVL(p_program,'X') != 'Medicaid' THEN
        v_sla_category := 'Other Letter';
      ELSE
        v_sla_category := p_program||' Letter';
      END IF;
    END IF;
  ...
  function GET_JEOPARDY_DAYS (p_program in varchar2,p_letter_type in varchar2)
  ...
  begin
    ...
    from CORP_ETL_LIST_LKUP
    where
      NAME = 'ProcLetters_SLA_Jeopardy_Days'
  ...
  as
    v_jeopardy_days number := null;
  begin
    return p_create_date + GET_JEOPARDY_DAYS(p_program,p_letter_type);
  end;
  ...
  as
    v_sla_category varchar2(100) := null;
    v_jeopardy_days number := null;
    v_jeopardy varchar2(10):= null;
    v_last_epa_mail_date date;
    v_sla_type varchar2(10);
  begin

    v_sla_category := GET_SLA_CATEGORY(p_program, p_letter_type);
    v_sla_type := GET_SLA_DAYS_TYPE(p_program,p_letter_type);
    ...
    where
      ...
      /*If p_complete_date is null and p_letter_type in(  'Checkup Due Letter','Checkup Reminder Letter','Enrollment Reminder','StarPlus Expansion Reminder Letter')  then
        If  trunc(sysdate) - trunc(p_create_date)  > v_jeopardy_days then
          v_jeopardy := 'Y';
        else
          v_jeopardy := 'N';
        end if;
     elsif p_complete_date is null and BPM_COMMON.BUS_DAYS_BETWEEN(p_create_date,sysdate) > v_jeopardy_days then
          v_jeopardy := 'Y';
       else
          v_jeopardy := 'N';
      end if;*/
  ...
  function GET_SLA_DAYS (p_program in varchar2,p_letter_type in varchar2)
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
  ...
  function GET_SLA_DAYS_TYPE (p_program in varchar2,p_letter_type in varchar2)
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
  ...
  begin
    ...
    from CORP_ETL_LIST_LKUP
    where
      NAME = 'ProcLetters_SLA_Days_Type'
      and VALUE = v_sla_category
      and REF_TYPE = 'LETTER_TYPE';
  ...
  function GET_SLA_TARGET_DAYS (p_program in varchar2,p_letter_type in varchar2)
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
    v_sla_target_days varchar(10) := null;
  begin
    ...
    from CORP_ETL_LIST_LKUP
    where
      NAME = 'ProcLetters_SLA_Target_Days'
      and VALUE = v_sla_category
      and REF_TYPE = 'LETTER_TYPE';
 ...
 --Mailed_dt is used while calling GET_TIMELINE_STATUS for complete date
  function GET_TIMELINE_STATUS
    (p_program in varchar2,
     p_create_date in date,
     p_complete_date in date,
  ...
  begin
    ...
    v_sla_category := GET_SLA_CATEGORY(p_program,p_letter_type);
    v_sla_days := GET_SLA_DAYS(p_program,p_letter_type);
    v_sla_type := GET_SLA_DAYS_TYPE(p_program,p_letter_type);

  if p_mailed_date is not null then
    if v_sla_category = 'Checkup Due Letter' then
      if BPM_COMMON.BUS_DAYS_BETWEEN(last_day(p_create_date),p_mailed_date) <= v_sla_days then
        return 'Timely';
      else
        return 'Untimely';
      end if;
    elsif v_sla_category = 'Checkup Reminder Letter' then
      if BPM_COMMON.BUS_DAYS_BETWEEN(last_day(add_months(p_create_date, v_sla_days)), p_mailed_date) = 0 then
        return 'Timely';
      else
        return 'Untimely';
      end if;
    elsif v_sla_category = 'Enrollment Reminder'  then
        --v_last_epa_mail_date := GET_EPA_MAIL_DT_FOR_CASE(p_case_id);
        v_last_epa_mail_date := COALESCE(p_epm_mail_date_for_case,to_date('07-JUL-7777','DD-MON-YYYY'));  -- AA 6/16/15 TXEB-2903
        If v_last_epa_mail_date + v_sla_days > p_create_date then
         return 'Timely';
        else
         return 'Untimely';
       end if;
   /* elsif  v_sla_category = 'Other Letter' and BPM_COMMON.BUS_DAYS_BETWEEN(p_create_date,p_mailed_date) <= v_sla_days then
      return 'Timely';
    elsif v_sla_category = 'Medicaid Letter' and BPM_COMMON.BUS_DAYS_BETWEEN(p_create_date,p_mailed_date) <= v_sla_days then
      return 'Timely';
    elsif v_sla_category = 'CHIP Letter' and BPM_COMMON.BUS_DAYS_BETWEEN(p_create_date,p_mailed_date) <= v_sla_days then
      return 'Timely';
    elsif  v_sla_category = 'STAR+PLUS Manadatory Reminder letter' and (p_mailed_date - p_create_date) <= v_sla_days then
      return 'Timely';
  --  elsif  v_sla_category = 'MDCP Reminder letter' and (nvl(p_complete_date,sysdate) - p_create_date) <= v_sla_days then
   --   return 'Timely';
    else
      return 'Untimely';*/
    else
      if v_sla_type = 'B' THEN
        if BPM_COMMON.BUS_DAYS_BETWEEN(p_create_date,p_mailed_date) <= v_sla_days then
          return 'Timely';
        else
          return 'Untimely';
        end if;
      else
        if p_mailed_date - p_create_date <= v_sla_days then
          return 'Timely';
        else
          return 'Untimely';
       end if;
  ...
  begin
    ...
    set
      ...
      TIMELINESS_STATUS = GET_TIMELINE_STATUS(PROGRAM,CREATE_DATE,COMPLETE_DATE,MAILED_DATE,LETTER_TYPE,CASE_ID,EPM_MAIL_DATE_FOR_CASE),
      JEOPARDY_STATUS = GET_JEOPARDY_STATUS(PROGRAM,CREATE_DATE,COMPLETE_DATE,LETTER_TYPE),
      SLA_CATEGORY = GET_PL_SLA_CATEGORY(PROGRAM,LETTER_TYPE),
      SLA_DAYS = GET_SLA_DAYS(PROGRAM,LETTER_TYPE),
      SLA_DAYS_TYPE = GET_SLA_DAYS_TYPE(PROGRAM,LETTER_TYPE),
      SLA_JEOPARDY_DATE = GET_JEOPARDY_DATE(PROGRAM,CREATE_DATE,LETTER_TYPE),
      SLA_JEOPARDY_DAYS = GET_JEOPARDY_DAYS(PROGRAM,LETTER_TYPE),
      SLA_TARGET_DAYS = GET_SLA_TARGET_DAYS(PROGRAM,LETTER_TYPE)
    where
  ...
  begin
  /*
select '      extractValue(p_data_xml,''/ROWSET/ROW/CEPN_ID'') "' || 'CEPN_ID'|| '",' attr_element from dual
    union
...
*/
    select
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/ASED_RESOLVE_RESP') "ASED_RESOLVE_RESP",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/ASF_RESOLVE_RESP') "ASF_RESOLVE_RESP",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/ASSD_RESOLVE_RESP') "ASSD_RESOLVE_RESP",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/CEPN_ID') "CEPN_ID",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/EPM_MAIL_DT_FOR_CASE') "EPM_MAIL_DT_FOR_CASE",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/LAST_ERRORED_DATE') "LAST_ERRORED_DATE",
      extractValue(p_data_xml,'/ROWSET/ROW/LAST_ERROR_FIXED_BY') "LAST_ERROR_FIXED_BY",
      extractValue(p_data_xml,'/ROWSET/ROW/LAST_UPDATED_BY_ID') "LAST_UPDATED_BY_ID",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/LETTER_DEFINITION_ID') "LETTER_DEFINITION_ID",
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/NEW_LETTER_REQUEST_ID') "NEW_LETTER_REQUEST_ID",
      extractValue(p_data_xml,'/ROWSET/ROW/PRINT_DT') "PRINT_DT",
      extractValue(p_data_xml,'/ROWSET/ROW/PROGRAM') "PROGRAM",
      extractValue(p_data_xml,'/ROWSET/ROW/REJECT_FLAG') "REJECT_FLAG",
...
*/
  ...
  procedure SET_DPLCUR
    (p_set_type in varchar2,
     ...
     p_resolve_resp_start_dt in varchar2,
     p_resolve_resp_end_dt in varchar2,
     p_resolve_resp_flag in varchar2,
     p_new_letter_request_id in varchar2,
	 p_created_by_id in varchar2,
     p_last_updated_by_id in varchar2,
	 p_cancel_by_id in varchar2,
	 p_error_count in varchar2,
	 p_last_errored_date in varchar2,
   p_last_error_fixed_by in varchar2,
	 p_letter_rejected_flag in varchar2,
   p_epm_mail_dt_for_case in varchar2,
   p_letter_definition_id	in number
    )
  as
  ...
  begin
    ...
    r_dplcur.CANCEL_METHOD := p_cancel_method;
    r_dplcur.CANCEL_REASON := p_cancel_reason;
    ...
    r_dplcur.AGE_IN_BUSINESS_DAYS := GET_AGE_IN_BUSINESS_DAYS(to_date(p_create_date,BPM_COMMON.DATE_FMT),to_date(p_complete_date,BPM_COMMON.DATE_FMT));
    r_dplcur.AGE_IN_CALENDAR_DAYS := GET_AGE_IN_CALENDAR_DAYS(to_date(p_create_date,BPM_COMMON.DATE_FMT),to_date(p_complete_date,BPM_COMMON.DATE_FMT));
    r_dplcur.TIMELINESS_STATUS := GET_TIMELINE_STATUS(p_program,to_date(p_create_date,BPM_COMMON.DATE_FMT),to_date(p_complete_date,BPM_COMMON.DATE_FMT),to_date(p_mailed_date,BPM_COMMON.DATE_FMT),p_letter_type,p_case_id,to_date(p_epm_mail_dt_for_case,BPM_COMMON.DATE_FMT));
    r_dplcur.JEOPARDY_STATUS := GET_JEOPARDY_STATUS(p_program,to_date(p_create_date,BPM_COMMON.DATE_FMT),to_date(p_complete_date,BPM_COMMON.DATE_FMT),p_letter_type);
    r_dplcur.SLA_CATEGORY := GET_PL_SLA_CATEGORY(p_program,p_letter_type);
    r_dplcur.SLA_DAYS := GET_SLA_DAYS(p_program,p_letter_type);
    r_dplcur.SLA_DAYS_TYPE := GET_SLA_DAYS_TYPE(p_program,p_letter_type);
    r_dplcur.SLA_JEOPARDY_DATE := GET_JEOPARDY_DATE(p_program,to_date(p_create_date,BPM_COMMON.DATE_FMT),p_letter_type);
    r_dplcur.SLA_JEOPARDY_DAYS := GET_JEOPARDY_DAYS(p_program,p_letter_type);
    r_dplcur.SLA_TARGET_DAYS := GET_SLA_TARGET_DAYS(p_program,p_letter_type);
    ...
    r_dplcur.RESOLVE_RESP_START_DATE := to_date(p_resolve_resp_start_dt,BPM_COMMON.DATE_FMT);
    r_dplcur.RESOLVE_RESP_END_DATE := to_date(p_resolve_resp_end_dt,BPM_COMMON.DATE_FMT);
    r_dplcur.RESOLVE_RESP_FLAG := p_resolve_resp_flag;
    r_dplcur.NEW_LETTER_REQUEST_ID := p_new_letter_request_id;
    r_dplcur.CREATED_BY_ID := p_created_by_id;
    r_dplcur.LAST_UPDATED_BY_ID := p_last_updated_by_id;
    r_dplcur.CANCEL_BY_ID := p_cancel_by_id;
    r_dplcur.ERROR_COUNT := p_error_count;
    r_dplcur.LAST_ERRORED_DATE := to_date(p_last_errored_date,BPM_COMMON.DATE_FMT);
    r_dplcur.LAST_ERROR_FIXED_BY := p_last_error_fixed_by;    
    r_dplcur.LETTER_REJECTED_FLAG := p_letter_rejected_flag;
    r_dplcur.EPM_MAIL_DATE_FOR_CASE := to_date(p_epm_mail_dt_for_case,BPM_COMMON.DATE_FMT);
    r_dplcur.LETTER_DEFINITION_ID := p_letter_definition_id;
  ...
  begin
    ...
    --v_end_date := to_date(v_new_data.COMPLETE_DT,BPM_COMMON.DATE_FMT);
    ...
    begin
      ...
      SET_DPLCUR
        ('INSERT',
         ...
         v_new_data.ASSD_RESOLVE_RESP,
         v_new_data.ASED_RESOLVE_RESP,
         v_new_data.ASF_RESOLVE_RESP,
         v_new_data.NEW_LETTER_REQUEST_ID,
		 v_new_data.CREATED_BY_ID,
		 v_new_data.LAST_UPDATED_BY_ID,
		 v_new_data.CANCEL_BY_ID,
		 v_new_data.ERROR_COUNT,
		 v_new_data.LAST_ERRORED_DATE,
		 v_new_data.LAST_ERROR_FIXED_BY,
		 v_new_data.REJECT_FLAG,
     v_new_data.EPM_MAIL_DT_FOR_CASE,
     v_new_data.LETTER_DEFINITION_ID
         );
