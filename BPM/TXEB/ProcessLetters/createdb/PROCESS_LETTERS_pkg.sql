...
create or replace package body PROCESS_LETTERS as
 ...
 --Mailed_dt is used while calling GET_TIMELINE_STATUS for complete date 
  ...
  if p_mailed_date is not null then
    if v_sla_category = 'Checkup Due Letter' and BPM_COMMON.BUS_DAYS_BETWEEN(last_day(p_create_date),p_mailed_date) <= v_sla_days then
       return 'Timely';
    elsif v_sla_category = 'Checkup Reminder Letter' and BPM_COMMON.BUS_DAYS_BETWEEN(last_day(add_months(p_create_date, v_sla_days)), p_mailed_date) = 0 then
       return 'Timely';
  ...
  begin
    ...
    set
      ...
      TIMELINESS_STATUS = GET_TIMELINE_STATUS(PROGRAM,CREATE_DATE,COMPLETE_DATE,MAILED_DATE,LETTER_TYPE,CASE_ID),
  ...
  begin
    ...
    r_dplcur.TIMELINESS_STATUS := GET_TIMELINE_STATUS(p_program,to_date(p_create_date,BPM_COMMON.DATE_FMT),to_date(p_complete_date,BPM_COMMON.DATE_FMT),to_date(p_mailed_date,BPM_COMMON.DATE_FMT),p_letter_type,p_case_id);
