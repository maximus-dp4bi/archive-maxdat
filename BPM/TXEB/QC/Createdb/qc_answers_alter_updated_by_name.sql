...
, max(qscr.first_name || ' '|| nvl(qscr.middle_name,'') ||' '|| qscr.last_name) response_created_by_name
, max(qsup.first_name || ' '|| nvl(qsup.middle_name,'') ||' '|| qsup.last_name) response_updated_by_name
...
left join qc_staff qscr on qscr.staff_id = sur.created_by
left join qc_staff qsup on qsup.staff_id = sur.updated_by
join (select survey_id, template_question_id, LISTAGG(nvl(sur1.comments,''), '; ') WITHIN GROUP (ORDER BY sur1.template_question_id) AS survey_Comments
 from maxdat.survey_response sur1
