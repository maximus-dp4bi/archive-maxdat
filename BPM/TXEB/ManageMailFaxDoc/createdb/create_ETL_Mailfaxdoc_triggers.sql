...
create or replace trigger TRG_BIU_CORP_ETL_MAILFAXDOC 
before insert or update on CORP_ETL_MAILFAXDOC 
for each row
begin
  if inserting then
    if :new.CEMFD_ID is null then
      :new.CEMFD_ID := SEQ_CEMFD_ID.nextval;
    end if;
  ...
  v_xml_string_new := 
    ...
    <ROWSET>  
      <ROW>
        ...
        <GWF_CHANNEL_ONLINE><![CDATA[' || :new.GWF_CHANNEL_ONLINE || ']]></GWF_CHANNEL_ONLINE>
    ...
    <ROWSET>  
      <ROW>
        ...
        <GWF_AUTOLINK_OUTCOME><![CDATA[' || :old.GWF_AUTOLINK_OUTCOME || ']]></GWF_AUTOLINK_OUTCOME>
	      <GWF_CHANNEL_ONLINE><![CDATA[' || :old.GWF_CHANNEL_ONLINE || ']]></GWF_CHANNEL_ONLINE>
