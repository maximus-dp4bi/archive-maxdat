...
create or replace package MANAGE_JOBS as
  ...
  SVN_FILE_URL varchar2(200) := '$URL: svn://rcmxapp1d.maximus.com/maxdat/BPM/ILEB/ManageJobs/createdb/MANAGE_JOBS_pkg.sql $'; 
  SVN_REVISION varchar2(20) := '$Revision: 8899 $'; 
  SVN_REVISION_DATE varchar2(60) := '$Date: 2014-03-13 07:27:33 -0700 (Thu, 13 Mar 2014) $'; 
...
IS
  v_max NUMBER;
BEGIN
  SELECT DISTINCT LK.MAXIMUM_RECORDS_EXPECTED
  INTO v_max
  FROM corp_etl_manage_jobs jb,
    CORP_MJ_FILE_CAL_LKUP lk
  WHERE jb.file_name  = lk.file_name(+)
...
update D_MJ_CURRENT
    set
      ...
      RECORD_COUNT_MIN_THRESHOLD = RECORD_CNT_MIN_THRE(file_name),
      RECORD_COUNT_MAX_THRESHOLD = RECORD_CNT_MAX_THRE(file_name),
      PER_ERR_ALERT = GET_PER_ERR_ALERT(file_name)
...
BPM_EVENT.INSERT_BIA(v_bi_id, 2179,2,v_new_data.CANCEL_BY,v_start_date,v_bue_id);
BPM_EVENT.INSERT_BIA(v_bi_id, 2180,2,v_new_data.CANCEL_METHOD,v_start_date,v_bue_id);
...
commit;
 ...
 -- Insert or update dimension for BPM Semantic model - Manage Jobs process - Current.
  ...
  r_dmjcur.RECORD_COUNT_MIN_THRESHOLD := RECORD_CNT_MIN_THRE(r_dmjcur.file_name);
  r_dmjcur.RECORD_COUNT_MAX_THRESHOLD := RECORD_CNT_MAX_THRE(r_dmjcur.file_name);
  r_dmjcur.PER_ERR_ALERT := GET_PER_ERR_ALERT(r_dmjcur.file_name);
...
BPM_EVENT.UPDATE_BIA(v_bi_id,2179,2,'N',v_old_data.CANCEL_BY,v_new_data.CANCEL_BY,v_bue_id,v_stg_last_update_date);
BPM_EVENT.UPDATE_BIA(v_bi_id,2180,2,'N',v_old_data.CANCEL_METHOD,v_new_data.CANCEL_METHOD,v_bue_id,v_stg_last_update_date);  
commit;
