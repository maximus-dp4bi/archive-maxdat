...
begin
  ...
  /* 
  select '        <' || 'CEPN_ID' || '>'' || :new.'  || 'CEPN_ID' || ' || ''</' || 'CEPN_ID' || '>'  attr_element from dual
  union
  ...
  where 
    ...
    and COLUMN_NAME not in ('AGE_IN_BUSINESS_DAYS','AGE_IN_CALENDAR_DAYS','APP_CYCLE_BUS_DAYS',
      'APP_CYCLE_CAL_DAYS','JEOPARDY_FLAG','TIMELINESS_STATUS','DAYS_UNTIL_TIMEOUT','LETTER_RESP_FILE_NAME','ERROR_REASON','TRANSMIT_FILE_NAME')
    
   union 
   select '        <' || 'STG_LAST_UPDATE_DATE' || '>'' || to_char(:new.'  || 'STG_LAST_UPDATE_DATE' || ',BPM_COMMON.GET_DATE_FMT) || ''</' || 'STG_LAST_UPDATE_DATE' || '>'  attr_element from dual
  union
  select 
  '        <' || atc.COLUMN_NAME || '><![CDATA['' || substr(:new.'  || atc.COLUMN_NAME || ',1,100) || '']]></' || atc.COLUMN_NAME || '>'    attr_element
  from BPM_ATTRIBUTE_STAGING_TABLE bast
  inner join ALL_TAB_COLUMNS atc on (bast.STAGING_TABLE_COLUMN = atc.COLUMN_NAME)
  where 
    bast.BSL_ID = 12
    and atc.TABLE_NAME = 'CORP_ETL_PROC_LETTERS'
    and COLUMN_NAME in ('LETTER_RESP_FILE_NAME','ERROR_REASON','TRANSMIT_FILE_NAME')
  ...
  v_xml_string_new := 
    ...
    <ROWSET>  
      <ROW> 
        ...
        <ERROR_REASON><![CDATA[' || substr(:new.ERROR_REASON,1,100) || ']]></ERROR_REASON>
        ...
        <LETTER_RESP_FILE_NAME><![CDATA[' || substr(:new.LETTER_RESP_FILE_NAME,1,100) || ']]></LETTER_RESP_FILE_NAME>
        ...
        <TRANSMIT_FILE_NAME><![CDATA[' || substr(:new.TRANSMIT_FILE_NAME,1,100) || ']]></TRANSMIT_FILE_NAME>
...
begin
  ...
  union
  select 
  '        <' || atc.COLUMN_NAME || '><![CDATA['' || substr(:old.'  || atc.COLUMN_NAME || ',1,100) || '']]></' || atc.COLUMN_NAME || '>'    attr_element
  from BPM_ATTRIBUTE_STAGING_TABLE bast
  inner join ALL_TAB_COLUMNS atc on (bast.STAGING_TABLE_COLUMN = atc.COLUMN_NAME)
  where 
    bast.BSL_ID = 12
  ...
  v_xml_string_old := 
    ...
    <ROWSET>  
      <ROW>
        ...
        <ERROR_REASON><![CDATA[' || substr(:old.ERROR_REASON,1,100) || ']]></ERROR_REASON>
        ...
        <LETTER_RESP_FILE_NAME><![CDATA[' || substr(:old.LETTER_RESP_FILE_NAME,1,100) || ']]></LETTER_RESP_FILE_NAME>
        ...
        <TRANSMIT_FILE_NAME><![CDATA[' || substr(:old.TRANSMIT_FILE_NAME,1,100) || ']]></TRANSMIT_FILE_NAME>
