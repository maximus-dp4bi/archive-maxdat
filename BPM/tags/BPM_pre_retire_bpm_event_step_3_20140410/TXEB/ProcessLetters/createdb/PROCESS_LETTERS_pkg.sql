...
create or replace package PROCESS_LETTERS as
  ...
  function GET_JEOPARDY_DAYS (p_program in varchar2) return number result_cache;
  
  function GET_SLA_CATEGORY (p_program in varchar2) return varchar2 result_cache;
  
  function GET_SLA_DAYS (p_program in varchar2) return varchar2 result_cache;
    
  function GET_SLA_DAYS_TYPE (p_program in varchar2) return varchar2 result_cache;
    
  function GET_SLA_TARGET_DAYS (p_program in varchar2) return varchar2 result_cache;
...
create or replace package body PROCESS_LETTERS as
  ...
  function GET_SLA_CATEGORY (p_program in varchar2) 
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
  begin
  
    if p_program = 'Medicaid Program'  then
  ...
  function GET_JEOPARDY_DAYS (p_program in varchar2) 
  ...
  begin
    ...
    v_sla_category := GET_SLA_CATEGORY(p_program);
  ...
  as
    v_jeopardy_days number := null;
  begin
    return p_create_date + GET_JEOPARDY_DAYS(p_program);
  end;
  ...
  function GET_SLA_DAYS (p_program in varchar2) 
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
  ...
  function GET_SLA_DAYS_TYPE (p_program in varchar2) 
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
  ...
  function GET_SLA_TARGET_DAYS (p_program in varchar2) 
  return varchar2 result_cache
  as
    v_sla_category varchar2(100) := null;
  ...
  begin
    ...
    v_sla_category := GET_SLA_CATEGORY(p_program);
    v_sla_days := GET_SLA_DAYS(p_program);

    if v_sla_category = 'Other Letter' and BPM_COMMON.BUS_DAYS_BETWEEN(p_create_date,nvl(p_complete_date,sysdate)) <= v_sla_days then
      return 'Timely';
    elsif v_sla_category = 'Medicaid Letter' and BPM_COMMON.BUS_DAYS_BETWEEN(p_create_date,nvl(p_complete_date,sysdate)) <= v_sla_days then
      return 'Timely';
    elsif v_sla_category = 'CHIP Letter' and BPM_COMMON.BUS_DAYS_BETWEEN(p_create_date,nvl(p_complete_date,sysdate)) <= v_sla_days then
      return 'Timely';
    else
      return 'Untimely';
    end if;
  ...
  begin
    ...
    set
      ...
      TIMELINESS_STATUS = GET_TIMELINE_STATUS(PROGRAM,CREATE_DATE,COMPLETE_DATE),
      JEOPARDY_STATUS = GET_JEOPARDY_STATUS(PROGRAM,CREATE_DATE,COMPLETE_DATE),
      SLA_CATEGORY = GET_SLA_CATEGORY(PROGRAM),
      SLA_DAYS = GET_SLA_DAYS(PROGRAM),
      SLA_DAYS_TYPE = GET_SLA_DAYS_TYPE(PROGRAM),
      SLA_JEOPARDY_DATE = GET_JEOPARDY_DATE(PROGRAM,CREATE_DATE),
      SLA_JEOPARDY_DAYS = GET_JEOPARDY_DAYS(PROGRAM),
      SLA_TARGET_DAYS = GET_SLA_TARGET_DAYS(PROGRAM)
    where 
  ...
  begin
    ...
    BPM_EVENT.INSERT_BIA(v_bi_id,1591,2,v_new_data.CANCEL_METHOD,v_start_date,v_bue_id);
    BPM_EVENT.INSERT_BIA(v_bi_id,1592,2,v_new_data.CANCEL_REASON,v_start_date,v_bue_id);
  ...
  begin
    ...
    r_dplcur.TIMELINESS_STATUS := GET_TIMELINE_STATUS(r_dplcur.PROGRAM,r_dplcur.CREATE_DATE,r_dplcur.COMPLETE_DATE);
    r_dplcur.JEOPARDY_STATUS := GET_JEOPARDY_STATUS(r_dplcur.PROGRAM,r_dplcur.CREATE_DATE,r_dplcur.COMPLETE_DATE);
    r_dplcur.SLA_CATEGORY := GET_SLA_CATEGORY(r_dplcur.PROGRAM);
    r_dplcur.SLA_DAYS := GET_SLA_DAYS(r_dplcur.PROGRAM);
    r_dplcur.SLA_DAYS_TYPE := GET_SLA_DAYS_TYPE(r_dplcur.PROGRAM);
    r_dplcur.SLA_JEOPARDY_DATE := GET_JEOPARDY_DATE(r_dplcur.PROGRAM,r_dplcur.CREATE_DATE);
    r_dplcur.SLA_JEOPARDY_DAYS := GET_JEOPARDY_DAYS(r_dplcur.PROGRAM);
    r_dplcur.SLA_TARGET_DAYS := GET_SLA_TARGET_DAYS(r_dplcur.PROGRAM);
 ...
 -- Update BPM Event model data.
  ...
  begin
    ...
    BPM_EVENT.UPDATE_BIA(v_bi_id,1562,2,'N',v_old_data.CANCEL_METHOD,v_new_data.CANCEL_METHOD,v_bue_id,v_stg_last_update_date);
    BPM_EVENT.UPDATE_BIA(v_bi_id,1562,2,'N',v_old_data.CANCEL_REASON,v_new_data.CANCEL_REASON,v_bue_id,v_stg_last_update_date);
