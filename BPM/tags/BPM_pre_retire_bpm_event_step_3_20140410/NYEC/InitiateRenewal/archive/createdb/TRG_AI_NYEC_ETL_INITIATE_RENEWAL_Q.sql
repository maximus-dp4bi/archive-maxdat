...
create or replace
trigger TRG_AI_NYEC_ETL_INIT_RENEWAL_Q
after insert on NYEC_ETL_MONITOR_RENEWAL
for each row

declare

  v_bsl_id number := 6; -- 'NYEC_ETL_INITIATE_RENEWAL'  
...
begin
  ...
  /*
  select '        <' || 'CEMR_ID' || '>'' || :new.'  || 'CEMR_ID' || ' || ''</' || 'CEMR_ID' || '>' from dual 
  union
  ...
  */
  ...
  values (SEQ_BUEQ_ID.nextval,v_bsl_id,v_bil_id,v_identifier,:new.STG_LAST_UPDATE_DATE,sysdate,v_data_version,xmltype(v_xml_string_new));
