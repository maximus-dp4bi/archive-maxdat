CREATE OR REPLACE VIEW EMRS_D_CASE_ADDRESS_SV
AS
  SELECT 
  ...
  LEFT JOIN (SELECT a.ADDR_ID 
              ...
              FROM &schema_name..ADDRESS a
              WHERE a.ADDR_TYPE_CD IN ('MR','RS','FA', 'MM', 'ML', 'GM', 'FR', 'BL', 'SP')
              AND a.ADDR_END_DATE IS NULL
              AND a.addr_case_id IS NOT NULL) b ON (addr.addr_id = b.addr_id and (b.RESIDENCE = 1 OR b.MAIL = 1))
  ...
  WHERE NOT EXISTS (SELECT 1 FROM &schema_name..address a WHERE a.ADDR_END_DATE IS NULL 
                                              AND a.ADDR_TYPE_CD IN ('MR','RS','FA', 'MM', 'ML', 'GM', 'FR', 'BL', 'SP') 
                                              AND c.case_id = a.addr_case_id);
  
  GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CASE_ADDRESS_SV TO MAXDAT_REPORTS;
