...
--    COALESCE(ac.subprogram_codes, '0') AS SUBPROGRAM_CODE , --on Aid Category, this field is not used
  ...
  JOIN &schema_name..client_supplementary_info cs ON (ss.client_id = cs.client_id AND ss.program_type_cd = cs.program_cd)
  left join &schema_name..contract con on (con.contract_id = ss.contract_id and con.plan_id = ss.plan_id)
  LEFT JOIN (SELECT s1.selection_segment_id,
              ...
              s1.status_cd
              FROM &schema_name..selection_txn s1
