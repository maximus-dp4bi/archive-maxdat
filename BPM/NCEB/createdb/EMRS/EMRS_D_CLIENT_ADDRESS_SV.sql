CREATE OR REPLACE VIEW EMRS_D_CLIENT_ADDRESS_SV
AS
  SELECT 
  ...
  LEFT JOIN (SELECT a.ADDR_ID 
              ...
              AND a.clnt_client_id IS NOT NULL) b ON (addr.addr_id = b.addr_id and (b.RESIDENCE = 1 OR b.MAIL = 1))
  ...
  WHERE NOT EXISTS (SELECT 1 FROM &schema_name..address a WHERE a.ADDR_END_DATE IS NULL 
                                              ...
                                              AND c.CLNT_CLIENT_ID = a.CLNT_CLIENT_ID);
  
  GRANT SELECT ON MAXDAT_SUPPORT.EMRS_D_CLIENT_ADDRESS_SV TO MAXDAT_REPORTS;
