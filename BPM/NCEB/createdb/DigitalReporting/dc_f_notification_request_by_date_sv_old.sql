DROP VIEW DC_F_NOTIFICATION_REQUEST_BY_DATE_SV;

CREATE OR REPLACE VIEW DC_F_NOTIFICATION_REQUEST_BY_DATE_SV
AS
...
, row_number() over(partition by lr.lmreq_id order by slct.selection_segment_id) rown
FROM (SELECT lmreq_id,case_id,lmdef_id,requested_on
  ...
  LEFT JOIN (SELECT *
                   FROM (SELECT h.notification_request_id,
                                ...
                                ROW_NUMBER() OVER(PARTITION BY h.notification_request_id ORDER BY h.create_ts DESC) crn
                         FROM eb.nreq_status_history h                            
                         WHERE status_cd IN('BLOCK BOUNCE','HARD BOUNCE','SOFT BOUNCE','UNKNOWN BOUNCE','TECHNICAL BOUNCE','ERR','PARTIAL'))
...
) es ON es.sent_date = dd.d_date OR es.email_fail_date = dd.d_date OR es.sms_fail_date = dd.d_date
where es.rown = 1
UNION
SELECT dd.d_date sent_date
  ,null lmreq_id
