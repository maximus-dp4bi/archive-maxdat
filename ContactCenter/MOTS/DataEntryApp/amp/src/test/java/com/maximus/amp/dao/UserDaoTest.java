...
import com.maximus.amp.model.Address;
...
public class UserDaoTest extends BaseDaoTestCase {
    ...
    public void testUserSearch() throws Exception {
        ...
        assertEquals("Amp", user.getFirstName());

        // test mirroring
        user = dao.get(-2L);
        user.setFirstName("AmpX");
        dao.saveAndFlush(user);
        flush();
        flushSearchIndexes();

        // now verify it is reflected in the index
        found = dao.search("AmpX");
        assertEquals(1, found.size());
        user = found.get(0);
        assertEquals("AmpX", user.getFirstName());
    }
}
