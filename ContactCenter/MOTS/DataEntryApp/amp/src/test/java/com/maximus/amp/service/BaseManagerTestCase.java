...
import com.maximus.amp.util.ConvertUtil;
...
public abstract class BaseManagerTestCase {
    ...
    /**
     * Utility method to assert that a collection has an expected size
     *
     * @param coll the collection whose size is to be validated
     * @param size the expected size of the collection
     */
    protected void assertThatCollectionSizeEqualTo(Collection coll, int size) {
		assertThat(coll.size(), is(equalTo(size)));
	}
