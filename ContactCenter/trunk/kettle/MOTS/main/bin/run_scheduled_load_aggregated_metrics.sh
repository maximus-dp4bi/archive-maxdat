...
# This program will run the Kettle job necessary to aggregate daily MAXDAT data on a scheduled basis (weekly and monthly)
...
then
	...
	JOB=execute_scheduled_load_aggregated_Contact_Center_Metrics_By_Reporting_Period
	...
	sh $MAXDAT_KETTLE_DIR/kitchen.sh -file="$JOBS_DIR/$JOB.kjb" -level="$LOG_LEVEL" -param:reportingPeriodType="$REPORTTYPE" >> "$LOG_DIR/$JOB$(date +%Y%m%d_%H%M%S).log"
