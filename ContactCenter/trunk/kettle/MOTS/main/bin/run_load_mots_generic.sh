...
# This program will run the Kettle job to load the MOTS generic database with historical data
...
JOBS_DIR=$MOTS_ETL_PATH/ContactCenter/main/jobs/initialization
JOB=load_MOTS_Generic
...
sh $MAXDAT_KETTLE_DIR/kitchen.sh -file="$JOBS_DIR/$JOB.kjb" $PARAMS -level="$LOG_LEVEL" >> "$LOG_DIR/$JOB$(date +%Y%m%d_%H%M%S).log"
