...
and project_name in ('IL EB', 'IL EEV')
--and actual_value_provided_by <> 'AUTO_LOAD'
and (trunc(smet.create_date) >= to_date('5/31/2016','mm/dd/yyyy') or trunc(smet.last_modified_date) >= to_date('5/31/2016','mm/dd/yyyy'))
and end_date > to_date('5/31/2016','mm/dd/yyyy')
and name in ('AT_WORK_NOT_HANDLING_CONTACTS_PERCENTAGE', 'MAX_NUMBER_OF_AGENTS_IN_TRAINING', 'FTE Count', 'NUMBER_OF_SKILLED_AGENTS_ATTRITTED', 'Utilization')
...
and project_name in ('IL EB', 'IL EEV')
--and actual_value_provided_by <> 'AUTO_LOAD'
and (trunc(fmet.create_date) >= to_date('5/31/2016','mm/dd/yyyy') or trunc(fmet.last_modified_date) >= to_date('5/31/2016','mm/dd/yyyy'))
and end_date > to_date('5/31/2016','mm/dd/yyyy')
and name in ('AT_WORK_NOT_HANDLING_CONTACTS_PERCENTAGE', 'MAX_NUMBER_OF_AGENTS_IN_TRAINING', 'FTE Count', 'NUMBER_OF_SKILLED_AGENTS_ATTRITTED', 'Utilization')
