...
START WITH TO_DATE(SYSDATE) NEXT SYSDATE + 1/24
  AS
select * from cc_f_dialer_by_date_sv@MAXDAT.SHRDMXDU_cisco_enterprise_cc ;
...
START WITH TO_DATE(SYSDATE) NEXT SYSDATE + 1/24
  AS
select * from HCO_F_VM_BY_HR_TYPE_BY_DAY_SV@MAXDAT.SHRDMXDU_cisco_enterprise_cc ;
...
START WITH TO_DATE(SYSDATE) NEXT SYSDATE + 1/24
  AS
select * from cc_f_v2_call_sv@MAXDAT.SHRDMXDU_cisco_enterprise_cc ;
