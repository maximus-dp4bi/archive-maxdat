...
--  delete extranneous agent staging record
DELETE CC_S_AGENT WHERE agent_id = 310;

-- create clean agent record for 71101
INSERT INTO CC_D_AGENT (
  SITE_NAME ,
  LOGIN_ID ,
  LANGUAGE ,
  JOB_TITLE ,
  ...
  RATE_CURRENCY
FROM CC_S_AGENT 
WHERE LOGIN_ID = '71101'
;

--  update existing fact records to use clean record
UPDATE CC_F_AGENT_BY_DATE
SET D_AGENT_ID = (SELECT D_AGENT_ID FROM CC_D_AGENT WHERE LOGIN_ID = '71101' AND VERSION = -1000)
WHERE D_AGENT_ID IN (SELECT DISTINCT D_AGENT_ID FROM CC_D_AGENT WHERE LOGIN_ID = '71101' AND VERSION != -1000)
;
...
-- delete extranneous 71101 records
DELETE FROM CC_D_AGENT
WHERE LOGIN_ID = '71101'
AND VERSION != -1000
;

--  update version # to non-negative number
UPDATE CC_D_AGENT
SET VERSION = 10
WHERE LOGIN_ID = '71101';
...
using ( 
  SELECT * 
	...
	WHERE (
    RECORD_END_DT > NEXT_EFF_DT 
    OR RECORD_EFF_DT = RECORD_END_DT 
    OR RECORD_EFF_DT = TO_DATE('2999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS')
  )
	ORDER BY LOGIN_ID, VERSION, RECORD_EFF_DT, RECORD_END_DT
) b
...
SET 
  a.RECORD_EFF_DT = TO_DATE('2999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS')
  , a.RECORD_END_DT = TO_DATE('2999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS');
...
using ( 
  SELECT * 
    ...
    SELECT
	  ...
	  FROM CC_D_AGENT a
    WHERE (
      RECORD_EFF_DT != TO_DATE('2999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS') 
      AND RECORD_EFF_DT != RECORD_END_DT
    )
	  ORDER BY LOGIN_ID, VERSION, RECORD_EFF_DT, RECORD_END_DT
  )sub 
	WHERE RECORD_END_DT != NEXT_EFF_DT  
...
VALUES ('1.3.1','101','101_TXEB_DUPLICATED_AGENT_CLEANUP');
