...
create or replace procedure TS_AUDIT_INSERT
    (in_call_date in DATE,
     ...
     in_call_hour_am_pm in NUMBER,
     in_call_type_id in NUMBER,
     in_call_duration_ss in NUMBER,
	 in_call_duration_mm in NUMBER,
	 in_call_duration_hh in NUMBER,
...
begin
    ...
    if  in_call_date is null or in_call_hour is null or in_call_minute is null 
     or in_call_hour_am_pm is null or in_call_duration_ss is null 
     or in_agent_id is null  /* or trunc(in_call_date) > trunc(sysdate) 
     or (in_audit_date is not null and trunc(in_audit_date) > trunc(sysdate))
     or (in_audit_date is not null and trunc(in_call_date) > trunc(in_audit_date)) */ then
        return;
    end if;
    ...
    INSERT INTO ts_audit (
            ...
            end from dual), considering the end of the week on Sunday */
            (to_number(to_char(in_call_date, 'W'))),
            ...
            in_call_hour_am_pm,
            in_call_type_id,
            (nvl(in_call_duration_hh, 0)*60*60) + (nvl(in_call_duration_mm, 0)*60) + in_call_duration_ss,
			in_call_duration_ss,
			nvl(in_call_duration_mm, 0),
			nvl(in_call_duration_hh, 0),
            in_call_language,
            in_agent_id,
            in_qa_analyst,
            in_audit_date,
            ...
            (to_number(to_char(in_audit_date, 'W'))),
...
begin
    ...
    SET
        ...
        audit_call_hour_am_pm = in_call_hour_am_pm,
        call_type_id = in_call_type_id,
        audit_call_duration = (nvl(in_call_duration_hh, 0)*60*60) + (nvl(in_call_duration_mm, 0)*60) + in_call_duration_ss,
		audit_call_duration_ss = in_call_duration_ss,
		audit_call_duration_mm = nvl(in_call_duration_mm, 0),
		audit_call_duration_hh = nvl(in_call_duration_hh, 0),
...
begin
    ...
    if  (in_question_answer_id is null) then
        return;
    end if;

    /* validates the score */
    if (in_qa_question_max_score is not null and nvl(in_answer_score, 0) > in_qa_question_max_score) then
    ...
    /* If a Yes/No score is empty, the score is 0 for no and Max for yes */
    if (in_answer_yn is not null and in_answer_yn = 0 and (in_answer_score is null or in_answer_score = in_qa_question_max_score)) then
        v_answer_score := 0;
    elsif (in_answer_yn is not null and in_answer_yn = 1 and (in_answer_score is null or in_answer_score = 0)) then
        v_answer_score := in_qa_question_max_score;
    end if;
...
create or replace procedure TS_QUEST_ANSWER_DET_INS_UPD
    (in_question_answer_id in number,
     in_qa_question_id in number,
     in_question_detail_id in number,
     in_quest_answer_det_chk in number,
     in_qa_analyst in varchar2)
as
    v_answer_detail_count number;
begin

    if  (in_question_answer_id is null or in_qa_question_id is null or
         in_question_detail_id is null or in_quest_answer_det_chk is null) then
        return;
    end if;
    
    /*Verifies if the record exists */
    select 
        count(*) into v_answer_detail_count
    from
        TS_QUEST_ANSWER_DETAIL
    where
        audit_question_answer_id = in_question_answer_id and
        question_detail_id = in_question_detail_id and
        qa_question_id = in_qa_question_id;
        
    /* If it is 0 = Insert */
    if (v_answer_detail_count = 0) then
        INSERT INTO ts_quest_answer_detail (
            audit_question_answer_id,
            question_detail_id,
            qa_question_id,
            ...
            in_question_answer_id,
            in_question_detail_id,
            in_qa_question_id,
            in_quest_answer_det_chk,
            in_qa_analyst,
            sysdate,
            in_qa_analyst,
            sysdate
        ...
        UPDATE ts_quest_answer_detail
        SET
            question_answer_detail_chk = in_quest_answer_det_chk,
            last_updated_by = in_qa_analyst,
            last_updated_datetime = sysdate
        WHERE
            audit_question_answer_id = in_question_answer_id
            AND question_detail_id = in_question_detail_id
            AND qa_question_id = in_qa_question_id;
    end if;
    commit;
end;
/

create or replace procedure TS_QA_SUPERVISOR_INS_UPD
    (in_qa_supervisor_id in number,
     in_qa_supervisor_login_id in nvarchar2,
     in_qa_supervisor_first_name in varchar2,
     in_qa_supervisor_middle_initial in varchar2,
     in_qa_supervisor_last_name in varchar2,
     in_qa_supervisor_active in number,
     in_qa_analyst in varchar2
    )
as
    v_supervisor_login_count number;
begin

    if  in_qa_supervisor_login_id is null or in_qa_supervisor_first_name is null or in_qa_supervisor_last_name is null or
        in_qa_supervisor_active is null then
        return;
    end if;
    
    /* Verifies if the login exists */
    select 
        count(*) into v_supervisor_login_count
    from
        ts_qa_supervisor
    WHERE
        qa_supervisor_login_id = in_qa_supervisor_login_id;
    
    /* Inactivate all supervisors with the same login id case the current is active */
    /*if (in_qa_supervisor_active = 1) then
        UPDATE ts_qa_supervisor
        SET
            qa_supervisor_active = 0,
            last_updated_by = in_qa_analyst,
            last_updated_datetime = sysdate
        WHERE
            qa_supervisor_login_id = in_qa_supervisor_login_id AND
            qa_supervisor_active = 1;    
    end if;*/
    
    /* If in_qa_supervisor_id is null -> Insert operation */
    if (in_qa_supervisor_id is null and v_supervisor_login_count = 0) then

        INSERT INTO ts_qa_supervisor (
            qa_supervisor_id,
            ...
            SEQ_TS_QA_SUPERVISOR.NEXTVAL,
            in_qa_supervisor_login_id,
            in_qa_supervisor_first_name,
            in_qa_supervisor_middle_initial,
            in_qa_supervisor_last_name,
            in_qa_supervisor_active,
            in_qa_analyst,
            sysdate,
            in_qa_analyst,
            sysdate
        ...
        UPDATE dp_scorecard_dchix.ts_qa_supervisor
        SET
            qa_supervisor_first_name = in_qa_supervisor_first_name,
            qa_supervisor_middle_initial = in_qa_supervisor_middle_initial,
            qa_supervisor_last_name = in_qa_supervisor_last_name,
            qa_supervisor_active = in_qa_supervisor_active,        
            last_updated_by = in_qa_analyst,
            last_updated_datetime = sysdate
        WHERE
             qa_supervisor_login_id = in_qa_supervisor_login_id;
