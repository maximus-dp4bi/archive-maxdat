...
using (
  ...
  FROM CC_F_AGENT_BY_DATE abd
  INNER JOIN CC_D_DATES d on abd.D_DATE_ID = d.D_DATE_ID
  INNER JOIN CC_D_AGENT a on abd.MANAGER_D_AGENT_ID = a.D_AGENT_ID
  WHERE abd.MANAGER_D_AGENT_ID IN (
    SELECT sub.D_AGENT_ID
    FROM CC_D_AGENT sub
    WHERE sub.RECORD_EFF_DT = TO_DATE('2999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'))
) b
ON (a.F_AGENT_BY_DATE_ID = b.F_AGENT_BY_DATE_ID)
WHEN matched THEN UPDATE SET a.MANAGER_D_AGENT_ID = (SELECT sub2.D_AGENT_ID from CC_D_AGENT sub2 WHERE (b.D_DATE BETWEEN sub2.RECORD_EFF_DT AND sub2.RECORD_END_DT) AND (sub2.LOGIN_ID = b.LOGIN_ID));
...
using (
  ...
  FROM CC_F_AGENT_BY_DATE abd
  INNER JOIN CC_D_DATES d on abd.D_DATE_ID = d.D_DATE_ID
  INNER JOIN CC_D_AGENT a on abd.SUPERVISOR_D_AGENT_ID = a.D_AGENT_ID
  WHERE abd.SUPERVISOR_D_AGENT_ID IN (
    SELECT sub.D_AGENT_ID
    FROM CC_D_AGENT sub
    WHERE sub.RECORD_EFF_DT = TO_DATE('2999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS'))
) b
ON (a.F_AGENT_BY_DATE_ID = b.F_AGENT_BY_DATE_ID)
WHEN matched THEN UPDATE SET a.SUPERVISOR_D_AGENT_ID = (SELECT sub2.D_AGENT_ID from CC_D_AGENT sub2 WHERE (b.D_DATE BETWEEN sub2.RECORD_EFF_DT AND sub2.RECORD_END_DT) AND (sub2.LOGIN_ID = b.LOGIN_ID));
...
VALUES ('0.3.1','201','201_DUPLICATED_D_AGENT_RECORD_CLEANUP');
