...
#version 7 upgrade, moved set_env to cronfile
...
then
	...
	sh $MAXDAT_KETTLE_DIR/kitchen.sh -file="$JOBS_DIR/$JOB.kjb" -level="$LOG_LEVEL" -param:reportingPeriodType="$REPORTINGPERIODTYPE" >> "$LOG_DIR/$JOB${PDI_VERSION}_$(date +%Y%m%d_%H%M%S).log"
