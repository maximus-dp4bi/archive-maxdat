...
CREATE OR REPLACE VIEW CISCO_ENTERPRISE_CC.CC_F_QINTERVAL_MASSHEALTH_SV
AS 
select
   dt.d_date as "Date"
   ...
   , (case when project_name = 'Mass Health' then
   f.calls_abandoned_period_5+ f.calls_abandoned_period_6+ f.calls_abandoned_period_7+ f.calls_abandoned_period_8+ f.calls_abandoned_period_9
   else
   NULL
   end )
   as "Contacts_Aband>180secs"
   ,(case when project_name = 'Mass Health' and q.queue_type like 'Callback%' then return_release else NULL end ) as Count_of_Callbacks_declined_by_Customer 
 from
 ...
 where
   pj.project_name in ('Mass Health', 'NH Medicaid Service Center');
