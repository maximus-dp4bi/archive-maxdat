...
JOBS_DIR=$MAXDAT_ETL_PATH/ContactCenter/implementation/HIHIX/jobs/initialization
JOB=recover_initialization
...
$SCRIPTS_DIR/kitchen.sh -file="$JOBS_DIR/$JOB.kjb" -level="$DETAIL_LOG_LEVEL" > $LOG_DIR/$JOB$(date +%Y%m%d_%H%M%S).log
