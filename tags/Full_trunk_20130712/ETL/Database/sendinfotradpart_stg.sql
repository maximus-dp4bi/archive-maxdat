...
CREATE UNIQUE INDEX sendinfotradpart_stg_uk_idx ON sendinfotradpart_stg
...
CONSTRAINT sendinfotradpart_stg_C01
 CHECK (GWF_REQUEST_TYPE IN ('C', 'I',  'R', 'L'))
 ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C02
  CHECK (GWF_RETRY_CALL IN ('C', 'D', 'R'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C03
  CHECK (GWF_LTR_MAILED IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C04
  CHECK (ASF_RECEIVE_INFO_REQ IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C05
  CHECK (ASF_PROCESS_IMAGE IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C06
  CHECK (ASF_PERFORM_OUTBOUND_CALL IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C07
  CHECK (ASF_CREATE_NEW_CALL_REQ IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C08
  CHECK (ASF_MAIL_LETTER_REQ IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C09
  CHECK (ASF_CREATE_NEW_LETTER_REQ IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C10
  CHECK (INSTANCE_STATUS IN ( 'Active','Complete'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C11
  CHECK (INFO_REQ_SOURCE IN ('Letter Req ID','Event ID','DCN','IEDR','CIN'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C12
  CHECK (INFO_REQ_GROUP IN ('Letter','Material Request','Outbound Call','Image','Referral'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C13
  CHECK (CALL_FLAG IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C14
  CHECK (CALL_RESULT IN ('V', 'T', 'E', 'B', 'F', 'H', 'I', 'R'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C15
  CHECK (MAN_LETTER_FLAG IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C16
  CHECK (IEDR_ERROR_FLAG IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C17
  CHECK (SLA_DAYS_TYPE IN ('B','C'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C18
  CHECK (JEOPARDY_FLAG IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C19
  CHECK (NEW_IND IN ('Y','N'))
  ENABLE
 VALIDATE;
 
 ALTER TABLE MAXDAT.sendinfotradpart_stg ADD 
 CONSTRAINT sendinfotradpart_stg_C20
...
GRANT SELECT ON sendinfotradpart_stg to MAXDAT_READ_ONLY;
...
/* Trigger on Table */
CREATE OR REPLACE TRIGGER TRG_R_SENDINFOTP_STG 
  BEFORE INSERT OR
  UPDATE ON sendinfotradpart_stg FOR EACH ROW
BEGIN
  IF INSERTING THEN
    IF :new.inreq_id IS NULL THEN
      SELECT seq_inreq.NEXTVAL 
        INTO :new.inreq_Id 
        FROM DUAL;
    END IF;
  END IF;
