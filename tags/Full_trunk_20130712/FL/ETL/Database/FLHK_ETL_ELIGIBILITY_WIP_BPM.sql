...
--------------------------------------------------------
  ...
  CREATE UNIQUE INDEX "FLHK_ETL_MEMBER_ELIGIBILI_PK" ON "FLHK_ETL_ELIGIBILITY_WIP_BPM" ("ME_ID") ;
/
--------------------------------------------------------
...
--------------------------------------------------------
  ...
  ALTER TABLE "FLHK_ETL_ELIGIBILITY_WIP_BPM" ADD CONSTRAINT "FLHK_ETL_MEMBER_ELIGIBILI_PK" PRIMARY KEY ("ME_ID") ENABLE;
...
--------------------------------------------------------
--  DDL for Trigger ETL_ELIBILITY_WIP_BPM_TRG
--------------------------------------------------------
CREATE OR REPLACE TRIGGER "ETL_ELIBILITY_WIP_BPM_TRG" 
   before insert on "FLHK_ETL_ELIGIBILITY_WIP_BPM" 
   for each row 
begin  
   if inserting then 
      if :NEW."ME_ID" is null then 
         select ETL_ELIGIBILITY_WIP_BPM_SEQ.nextval into :NEW."ME_ID" from dual; 
      end if; 
   end if; 
end;
/
ALTER TRIGGER "ETL_ELIBILITY_WIP_BPM_TRG" ENABLE;
