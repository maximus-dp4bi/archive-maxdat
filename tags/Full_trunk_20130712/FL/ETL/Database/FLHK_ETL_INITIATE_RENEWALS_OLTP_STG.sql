...
--------------------------------------------------------
  ...
  "WAITING_FOR_DISENROLLMENT" NUMBER DEFAULT 0)) ;
   ...
   COMMENT ON COLUMN "FLHK_INITIATE_RENEWAL_OLTP_STG"."INSTANCE_COMPLETE_DATE" IS 'System date when the renewal is finally completed.';
   COMMENT ON COLUMN "FLHK_INITIATE_RENEWAL_OLTP_STG"."COUNT_OF_ELIGIBLE_MEMBERS" IS 'Count of the number of eligibile members on an account.';
   COMMENT ON COLUMN "FLHK_INITIATE_RENEWAL_OLTP_STG"."APP_RECEIVED_DATE" IS 'Date the renewal application was received. ';
   COMMENT ON COLUMN "FLHK_INITIATE_RENEWAL_OLTP_STG"."APP_ENTERED_BY" IS 'Who entered the renewal application. ';
...
/
--------------------------------------------------------
--  DDL for Trigger FLHK_INITIATE_RENEW_OLTP_TRG
--------------------------------------------------------

  CREATE OR REPLACE TRIGGER "FLHK_INITIATE_RENEW_OLTP_TRG" 
   before insert on "FLHK_INITIATE_RENEWAL_OLTP_STG" 
   for each row 
begin  
   if inserting then 
      if :NEW."IR_ID" is null then 
         select FLHK_INITIATE_RENEWAL_OLTP_SEQ.nextval into :NEW."IR_ID" from dual; 
      end if; 
   end if; 
end;
/
ALTER TRIGGER "FLHK_INITIATE_RENEW_OLTP_TRG" ENABLE;
/
