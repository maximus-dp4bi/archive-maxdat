...
create or replace package body MANAGE_MAIL_FAX_DOC as
  ...
  begin
    ...
    */
    select
      ...
      extractValue(p_data_xml,'/ROWSET/ROW/STG_LAST_UPDATE_DATE') "STG_LAST_UPDATE_DATE",
      extractValue(p_data_xml,'/ROWSET/ROW/WORK_TASK_ID') "WORK_TASK_ID",
      extractValue(p_data_xml,'/ROWSET/ROW/WORK_TASK_TYPE_CREATED') "WORK_TASK_TYPE_CREATED"      into p_data_record
    from dual;
  exception
    when OTHERS then
      v_sql_code := SQLCODE;
    ...
    begin
      ...
      SET_DPDCUR
        ('INSERT',v_identifier,v_bi_id
         ...
         -- ,v_new_data.age_bus_days,v_new_data.age_cal_days,v_new_data.dcn_jeopardy_status,v_new_data.dcn_jeopardy_status_dt,v_new_data.dcn_timeliness_status
        );
      INS_FPDBD
 ...
 -- Update BPM Event model data.
  ...
  begin
    ...
    BPM_EVENT.UPDATE_BIA(v_bi_id, 539,1,null,v_old_data.IA_MANUAL_LINK_TASK_ID,v_new_data.IA_MANUAL_LINK_TASK_ID,v_bue_id,v_stg_last_update_date);
    ...
    BPM_EVENT.UPDATE_BIA(v_bi_id, 563,3,null,v_old_data.INSTANCE_COMPLETE_DT,v_new_data.INSTANCE_COMPLETE_DT,v_bue_id,v_stg_last_update_date);
    commit;
    p_bue_id := v_bue_id;
  exception
    when NO_DATA_FOUND then
