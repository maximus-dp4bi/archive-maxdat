...
create or replace package body PROCESS_BPM_QUEUE as
  ...
  v_bdm_name_bpm_event    varchar2(9)  := 'BPM_EVENT';
  v_bdm_num_bpm_event     number := 1;
  ...
  -- Get data model name.
  function GET_DATA_MODEL_NAME (p_bdm_id number) return varchar2
  as
  ...
  begin
    case
      when p_bdm_id = v_bdm_num_bpm_event    then v_bdm_name := v_bdm_name_bpm_event;
      when p_bdm_id = v_bdm_num_bpm_semantic then v_bdm_name := v_bdm_name_bpm_semantic;
      else
        v_log_message := 'Unsupported data model number "' || p_bdm_id || '".';
        v_error_number := -20023;
        BPM_COMMON.LOGGER(BPM_COMMON.LOG_LEVEL_SEVERE,null,v_procedure_name,null,null,null,null,null,v_log_message,null);
  ...
  begin
    case
      when p_data_model_name = v_bdm_name_bpm_event    then v_bdm_num := v_bdm_num_bpm_event;
      when p_data_model_name = v_bdm_name_bpm_semantic then v_bdm_num := v_bdm_num_bpm_semantic;
      else
        v_log_message := 'Unsupported data model name "' || p_data_model_name || '".';
        v_error_number := -20023;
        BPM_COMMON.LOGGER(BPM_COMMON.LOG_LEVEL_SEVERE,null,v_procedure_name,null,null,null,null,null,v_log_message,v_error_number);
        RAISE_APPLICATION_ERROR(v_error_number,v_log_message);
    end case;
    return v_bdm_num;
  end;
  ...
  begin
    ...
    if v_status != 0 then
      ...
      BPM_COMMON.LOGGER(BPM_COMMON.LOG_LEVEL_WARNING,p_job_id,v_procedure_name,p_bsl_id,null,null,null,null,v_log_message,null);
    ...
    if v_status != 0 then
      ...
      BPM_COMMON.LOGGER(BPM_COMMON.LOG_LEVEL_SEVERE,p_job_id,v_procedure_name,p_bsl_id,null,null,null,null,v_log_message,null);
    end if;
    ...
    loop
      ...
      begin
        ...
        if v_prev_identifier is null 
          ...
          if r_bueq_row.OLD_DATA is null and r_bueq_row.NEW_DATA is not null then
            ...
            BPM_COMMON.LOGGER(BPM_COMMON.LOG_LEVEL_SEVERE,p_job_id,v_procedure_name,r_bueq_row.BSL_ID,r_bueq_row.BIL_ID,r_bueq_row.IDENTIFIER,null,null,v_log_message,null);
            RAISE_APPLICATION_ERROR(v_error_number,v_log_message);       
          end if;
          ...
          if p_bdm_id = v_bdm_num_bpm_event then
            RELEASE_QUEUE_ROW(r_bueq_row.BUEQ_ID);  
          elsif p_bdm_id = v_bdm_num_bpm_semantic then
      ...
      exception
        ...
        when OTHERS then
          ...
          BPM_COMMON.LOGGER(BPM_COMMON.LOG_LEVEL_SEVERE,p_job_id,v_procedure_name,r_bueq_row.BSL_ID,r_bueq_row.BIL_ID,r_bueq_row.IDENTIFIER,null,null,v_log_message,v_sql_code);   
      end;
  ...
  exception
    ...
    when OTHERS then
      v_sql_code := SQLCODE;
      v_log_message := SQLERRM;
      BPM_COMMON.LOGGER(BPM_COMMON.LOG_LEVEL_SEVERE,p_job_id,v_procedure_name,null,null,null,null,null,v_log_message,v_sql_code);   
  ...
  exception
    ...
    when OTHERS then
      ...
      v_log_message := SQLERRM;
      BPM_COMMON.LOGGER
        (BPM_COMMON.LOG_LEVEL_SEVERE,p_job_id,v_procedure_name,p_bsl_id,null,null,null,null,v_log_message,v_sql_code);
      raise;
