...
create or replace
trigger TRG_AU_NYEC_ETL_STATE_REVIEW_Q
...
begin
  ...
  /* 
  select '        <' || 'NESR_ID>'' || :old.'  || 'NESR_ID' || ' || ''</NESR_ID' || '>'  from dual 
  union
  ...
  */
  v_xml_string_old := 
    ...
    <ROWSET>
      <ROW>
        ...
        <ASED_PROCESS_DC>' || to_char(:old.ASED_PROCESS_DC,BPM_COMMON.GET_DATE_FMT) || '</ASED_PROCESS_DC>
        <ASED_RECEIVE_STATE_REVIEW>' || to_char(:old.ASED_RECEIVE_STATE_REVIEW,BPM_COMMON.GET_DATE_FMT) || '</ASED_RECEIVE_STATE_REVIEW>
        <ASED_REQUEST_MI_NOTICE>' || to_char(:old.ASED_REQUEST_MI_NOTICE,BPM_COMMON.GET_DATE_FMT) || '</ASED_REQUEST_MI_NOTICE>
        <ASED_RESEARCH>' || to_char(:old.ASED_RESEARCH,BPM_COMMON.GET_DATE_FMT) || '</ASED_RESEARCH>
        <ASF_PROCESS_DC><![CDATA[' || :old.ASF_PROCESS_DC || ']]></ASF_PROCESS_DC>
        <ASF_RECEIVE_STATE_REVIEW><![CDATA[' || :old.ASF_RECEIVE_STATE_REVIEW || ']]></ASF_RECEIVE_STATE_REVIEW>
        <ASF_REQUEST_MI_NOTICE><![CDATA[' || :old.ASF_REQUEST_MI_NOTICE || ']]></ASF_REQUEST_MI_NOTICE>
        <ASF_RESEARCH><![CDATA[' || :old.ASF_RESEARCH || ']]></ASF_RESEARCH>
        <ASPB_PROCESS_DC><![CDATA[' || :old.ASPB_PROCESS_DC || ']]></ASPB_PROCESS_DC>
        <ASPB_RECEIVE_STATE_REVIEW><![CDATA[' || :old.ASPB_RECEIVE_STATE_REVIEW || ']]></ASPB_RECEIVE_STATE_REVIEW>
        <ASPB_RESEARCH><![CDATA[' || :old.ASPB_RESEARCH || ']]></ASPB_RESEARCH>
        <ASSD_PROCESS_DC>' || to_char(:old.ASSD_PROCESS_DC,BPM_COMMON.GET_DATE_FMT) || '</ASSD_PROCESS_DC>
        <ASSD_RECEIVE_STATE_REVIEW>' || to_char(:old.ASSD_RECEIVE_STATE_REVIEW,BPM_COMMON.GET_DATE_FMT) || '</ASSD_RECEIVE_STATE_REVIEW>
        <ASSD_REQUEST_MI_NOTICE>' || to_char(:old.ASSD_REQUEST_MI_NOTICE,BPM_COMMON.GET_DATE_FMT) || '</ASSD_REQUEST_MI_NOTICE>
        <ASSD_RESEARCH>' || to_char(:old.ASSD_RESEARCH,BPM_COMMON.GET_DATE_FMT) || '</ASSD_RESEARCH>
        <AUTO_CLOSE_FLAG><![CDATA[' || :old.AUTO_CLOSE_FLAG || ']]></AUTO_CLOSE_FLAG>
        <CALL_CAMPAIGN_FLAG><![CDATA[' || :old.CALL_CAMPAIGN_FLAG || ']]></CALL_CAMPAIGN_FLAG>
        <CALL_CAMPAIGN_ID>' || :old.CALL_CAMPAIGN_ID || '</CALL_CAMPAIGN_ID>
        <CANCEL_DT>' || to_char(:old.CANCEL_DT,BPM_COMMON.GET_DATE_FMT) || '</CANCEL_DT>
        <CURRENT_TASK_ID>' || :old.CURRENT_TASK_ID || '</CURRENT_TASK_ID>
        <DATA_CORRECTION_TASK_ID>' || :old.DATA_CORRECTION_TASK_ID || '</DATA_CORRECTION_TASK_ID>
        <GWF_MI_REQUIRED><![CDATA[' || :old.GWF_MI_REQUIRED || ']]></GWF_MI_REQUIRED>
        <GWF_NEW_MI_SATISFIED><![CDATA[' || :old.GWF_NEW_MI_SATISFIED || ']]></GWF_NEW_MI_SATISFIED>
        <GWF_RESEARCH><![CDATA[' || :old.GWF_RESEARCH || ']]></GWF_RESEARCH>
        <GWF_STATE_RESULT><![CDATA[' || :old.GWF_STATE_RESULT || ']]></GWF_STATE_RESULT>
        <GWF_TASK_WORKED_BY><![CDATA[' || :old.GWF_TASK_WORKED_BY || ']]></GWF_TASK_WORKED_BY>
        ...
        <LETTER_SENT_FLAG><![CDATA[' || :old.LETTER_SENT_FLAG || ']]></LETTER_SENT_FLAG>
        <LETTER_STATUS><![CDATA[' || :old.LETTER_STATUS || ']]></LETTER_STATUS>
        <NESR_ID>' || :old.NESR_ID || '</NESR_ID>
        <NEW_MI_FLAG><![CDATA[' || :old.NEW_MI_FLAG || ']]></NEW_MI_FLAG>
        <NEW_MI_SATISFIED><![CDATA[' || :old.NEW_MI_SATISFIED || ']]></NEW_MI_SATISFIED>
        <NEW_STATE_REVIEW_TASK_ID>' || :old.NEW_STATE_REVIEW_TASK_ID || '</NEW_STATE_REVIEW_TASK_ID>
        <RESEARCH_TASK_ID>' || :old.RESEARCH_TASK_ID || '</RESEARCH_TASK_ID>
        <RFE_STATUS_FLAG><![CDATA[' || :old.RFE_STATUS_FLAG || ']]></RFE_STATUS_FLAG>
        <STATE_ACCEPT_IND><![CDATA[' || :old.STATE_ACCEPT_IND || ']]></STATE_ACCEPT_IND>
        <STATE_REVIEW_OUTCOME><![CDATA[' || :old.STATE_REVIEW_OUTCOME || ']]></STATE_REVIEW_OUTCOME>
  ...
  /* 
  select  '        <' || 'NESR_ID>'' || :new.'  || 'NESR_ID' || ' || ''</NESR_ID' || '>'  from dual 
  union
  ...
  */
  ...
  insert into BPM_UPDATE_EVENT_QUEUE
    ...
    values (SEQ_BUEQ_ID.nextval,v_bsl_id,v_bil_id,v_identifier,:new.STG_LAST_UPDATE_DATE,sysdate,v_data_version,sys.xmltype(v_xml_string_old),sys.xmltype(v_xml_string_new));
