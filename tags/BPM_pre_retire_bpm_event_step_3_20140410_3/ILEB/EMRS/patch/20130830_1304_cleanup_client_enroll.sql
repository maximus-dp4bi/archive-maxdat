DECLARE
 CURSOR client_cur IS
   ...
   WHERE EXISTS(SELECT 1 FROM emrs_d_client_plan_enrl_status s2 WHERE s1.client_number = s2.client_number
             AND s1.source_record_id = s2.source_record_id
             AND s1.client_enroll_status_id > s2.client_enroll_status_id );
...
BEGIN
   ...
   LOOP
     ...
     FOR indx IN 1 .. clnt_tab.COUNT LOOP
      DELETE FROM emrs_d_client_plan_enrl_status s1
      WHERE  EXISTS(SELECT 1 FROM emrs_d_client_plan_enrl_status s2 WHERE s1.client_number = s2.client_number
             AND s1.source_record_id = s2.source_record_id
             AND s1.client_enroll_status_id > s2.client_enroll_status_id)
      AND client_number = clnt_tab(indx).client_number;
      
      FOR e IN(SELECT client_enroll_status_id, rownum row_num
               FROM(
      ...
      LOOP
        ...
        WHERE client_enroll_status_id = e.client_enroll_status_id;
      END LOOP;
