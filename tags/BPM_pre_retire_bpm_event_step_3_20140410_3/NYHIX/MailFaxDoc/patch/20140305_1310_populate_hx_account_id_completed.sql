alter table NYHIX_ETL_MAIL_FAX_DOC add (ACCOUNT_ID number(18));
alter table NYHIX_ETL_MAIL_FAX_DOC_WIP_BPM add (ACCOUNT_ID number(18));

alter table d_nyhix_mfd_current add (ACCOUNT_ID number(18));

insert into BPM_ATTRIBUTE_LKUP (BAL_ID,BDL_ID,name,PURPOSE) values (1616,1,'Account ID','MAXe Account ID associated with the document. Attribute only applies to Verification Docs.');
insert into BPM_ATTRIBUTE values (2616,1616,18,'BOTH',sysdate,BPM_COMMON.GET_MAX_DATE,'N');
insert into BPM_ATTRIBUTE_STAGING_TABLE (BAST_ID,BA_ID,BSL_ID,STAGING_TABLE_COLUMN) values (SEQ_BAST_ID.NEXTVAL,2616,18,'ACCOUNT_ID');
commit;
...
declare
  cursor S1
  is
    select DCN, HX_ACCOUNT_ID, HXID, ACCOUNT_ID from DOCUMENT_NOTIFICATION_STG;
begin
  for C1 in S1
  LOOP
    ...
    set 
       HX_ACCOUNT_ID = C1.HX_ACCOUNT_ID,
       HXID = C1.HXID,
