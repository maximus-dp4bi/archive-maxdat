--in dev configure below MStr alert query and schedule it to run 30th min every odd hour Eastern time eg: 1:30,3:30, 4:30 and so on
select
p.projectid ProjId
...
then to_time(convert_timezone('UTC',ph.project_time_zone,sysdate())) else '24:00' end) between timeadd(min,135,to_time(project_hours_start_time)) and timeadd(min,180,to_time(project_hours_end_time))
...
and datediff(min,qia.max_project_interval_start,convert_timezone('UTC',ph.project_time_zone,sysdate())) > 120
