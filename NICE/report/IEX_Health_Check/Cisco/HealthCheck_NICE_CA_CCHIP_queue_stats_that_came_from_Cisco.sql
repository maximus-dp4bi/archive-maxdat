/* Get yesterday's NICE CA CCHIP queue stats that originated in Cisco. */
/* NICE PRD DB: 10.150.140.20 */

select 
  cast(switchoffset(qs.C_TIMESTAMP,'-07:00') as date) as Date,
  q.C_QTAG, 
  e.C_NAME,
...
inner join nice_wfm_customer1.dbo.R_ACD acd on q.C_ACD = acd.C_OID and acd.C_Name = 'Cisco Enterprise 1'
inner join nice_wfm_customer1.dbo.R_ENTITY E on Q.C_OID = E.C_OID and E.C_TYPE = 'Q'
where
  cast(switchoffset(qs.C_TIMESTAMP,'-07:00') as date) = cast(dateadd(DAY,-1,getDate()) as date)
  and cast(e.C_ID as varchar) like '22%%%'
  and e.C_DTIME is null
group by 
  q.C_QTAG,
  e.C_NAME,
  cast(switchoffset(qs.C_TIMESTAMP,'-07:00') as date),
  qs.C_QUEUE
order by 1,2,3;
