/* Get yesterday's NICE VA SOA Hampton queue stats that originated in Cisco. */
...
where
  cast(switchoffset(qs.C_TIMESTAMP,'-04:00') as date) = cast(dateadd(DAY,-0,getDate()) as date)
  and (cast(e.C_ID as varchar) like '80%%%' OR cast(e.C_ID as varchar) like '61%%%')						-- Added the PA CHC SOA Hampton Queues
