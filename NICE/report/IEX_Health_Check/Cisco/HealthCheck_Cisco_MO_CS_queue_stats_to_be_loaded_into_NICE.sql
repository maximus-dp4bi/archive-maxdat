/* Get yesterday's Cisco MO CS queue stats. */
...
where 
  cast(e.C_ID as varchar) like '70%%%'
  and e.C_DTIME is null
order by 1;
*/

select
  cast(switchoffset(todatetimeoffset(si.DateTime,-360),'-05:00') as date) as Date,
  pq.PrecisionQueueID,
...
where 
  si.PrecisionQueueID in (5110,5111,5112)
  and cast(switchoffset(todatetimeoffset(si.DateTime,-360),'-05:00') as date) = cast(dateadd(DAY,-1,getDate()) as date)
group by 
  cast(switchoffset(todatetimeoffset(si.DateTime, -360), '-05:00') as date),
  pq.EnterpriseName,
  pq.PrecisionQueueID
order by 1,2,3;
