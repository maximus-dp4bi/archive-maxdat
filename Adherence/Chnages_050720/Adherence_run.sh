...
EMAIL="soapagdatascience@maximus.com"
...
if [[ $rc != 0 ]] ; then
     ...
     $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/$MODULE_ADH/Adherence_Job.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/${STCODE}_Adherence_Enrollment_$(date +%Y%m%d_%H%M%S).log
#     $MAXDAT_ETL_PATH/run_kjb.sh $MAXDAT_ETL_PATH/$MODULE_ADH/IL_Supplemental_Data.kjb $KJB_LOG_LEVEL >> $MAXDAT_ETL_LOGS/$MODULE_EMRS/${STCODE}_IL_Supplemental_Data$(date +%Y%m%d_%H%M%S).log
     ...
     rc=$?
      if [[ $rc != 0 ]] ; then
	     ...
	     echo "Adherence_Job.kjb failed in ADH, review error log for additional detail." >> $EMAIL_MESSAGE
	     rm -f $INIT_OK
	     mail -s "$EMAIL_SUBJECT" "$EMAIL" < $EMAIL_MESSAGE
	     cat $EMAIL_MESSAGE
	     error_exit "$LINENO: An error has occurred in Adherence_run.sh"
		 else
		 ...
		 echo "Adherence_run.kjb succesful." >> $EMAIL_MESSAGE
	     rm -f $INIT_OK
	     mail -s "$EMAIL_SUBJECT" "$EMAIL" < $EMAIL_MESSAGE
	     cat $EMAIL_MESSAGE
