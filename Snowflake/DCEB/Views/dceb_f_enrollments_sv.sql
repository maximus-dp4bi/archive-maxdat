...
COUNT (DISTINCT CASE WHEN evw.transfer_or_new_enroll = 'N' AND bp.phone_id IS NOT NULL THEN evw.consumer_id ELSE NULL END ) new_enroll_bad_phone_exclusion,
COUNT (DISTINCT CASE WHEN evw.transfer_or_new_enroll = 'T' AND ccvw.core_eligibility_segments_id IS NOT NULL THEN evw.consumer_id ELSE NULL END ) transfer_newborn_exclusion,
COUNT (DISTINCT CASE WHEN evw.transfer_or_new_enroll = 'N' AND ccvw.core_eligibility_segments_id IS NOT NULL THEN evw.consumer_id ELSE NULL END ) new_enroll_newborn_exclusion,
