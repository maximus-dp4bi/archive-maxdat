...
create or replace procedure SP_POPULATE_APPLICATION_CURRENT()
  ...
  var strErrMsg = "";
   ...
   try {  
       ...
       var strSQLText = `INSERT INTO coverva_dmas.dmas_application_current(Tracking_Number,Source,App_Received_Date,Processing_Unit,Application_Type,Current_State,Initial_Review_Complete_Date,  
                            ...
                            renewal_closure_date,auto_closure_date) 
               ...
               WHERE rnk = 1 
               AND NOT EXISTS(SELECT 1 FROM coverva_dmas.dmas_application_current dac WHERE da.tracking_number = dac.tracking_number);`;           
         var strSQLStmt = snowflake.createStatement({sqlText: strSQLText});
         var ret_value = strSQLStmt.execute();
         
         strSQLText = `UPDATE coverva_dmas.dmas_application_current dac 
