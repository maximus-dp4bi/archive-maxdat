...
SELECT d_year,d_quarter,d_month_year,d_date,
 ...
 SUM(CASE WHEN medicaid_number IS NOT NULL AND REGEXP_INSTR(medicaid_number,'[^0-9]') = 0 THEN 1 ELSE 0 END) count_intake_process,
 SUM(CASE WHEN  offender_location_type = 'DOC' AND medicaid_number IS NOT NULL AND REGEXP_INSTR(medicaid_number,'[^0-9]') = 0 THEN 1 ELSE 0 END) count_doc_intake_process,
 SUM(CASE WHEN  offender_location_type = 'SCB' AND medicaid_number IS NOT NULL AND REGEXP_INSTR(medicaid_number,'[^0-9]') = 0 THEN 1 ELSE 0 END) count_scb_intake_process
FROM (SELECT d_year,
        d_month,
        TRIM(CONCAT(d_month_name,'-',d_year)) d_month_year,
        d_date, 
