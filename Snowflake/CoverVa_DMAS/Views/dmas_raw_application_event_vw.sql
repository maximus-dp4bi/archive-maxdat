...
raw_event AS(SELECT DISTINCT da.tracking_number, da.event_date 
              ...
              ,CASE WHEN (am.tracking_number IS NOT NULL AND am.ma_pg_indicator IN('YES','Y')) OR pw.tracking_number IS NOT NULL THEN 'Y' ELSE 'N' END in_app_metric_pw 
              ...
              ,CASE WHEN (ram.tracking_number IS NOT NULL AND ram.ma_pg_indicator IN('YES','Y')) OR rpw.tracking_number IS NOT NULL THEN 'Y' ELSE 'N' END in_running_pw
             ...
             ,p.ppit_vcl_due_date,COALESCE(am.case_type,'Application') case_type,am.sd_stage,am.am_processing_status,ram.running_am_processing_status,ram.running_case_type
FROM  tlist da             
--am
             LEFT JOIN (SELECT tracking_number, worker_id, pending_due_to_vcl,vcl_due_date_am, file_date,am_wid_in_ldap,sd_stage,case_type,am_processing_status,ma_pg_indicator,filename_prefix 
                        FROM(SELECT tracking_number, worker_id, pending_due_to_vcl,vcl_due_date vcl_due_date_am,CAST(f.file_date AS DATE) file_date 
                               ...
                               ,stage sd_stage,application_type case_type,application_status am_processing_status,UPPER(am.ma_pg_indicator) ma_pg_indicator 
                               ,f.filename_prefix
...
--running am
             LEFT JOIN (SELECT tracking_number, running_am_worker, file_date,running_am_wid_in_ldap,ma_pg_indicator,sd_stage,case_type running_case_type,running_am_processing_status
                        FROM(SELECT tracking_number, worker_id running_am_worker, CAST(f.file_date AS DATE) file_date 
                               ...
                               ,stage sd_stage,application_type case_type,application_status running_am_processing_status,UPPER(am.ma_pg_indicator) ma_pg_indicator
                             FROM coverva_dmas.dmas_app_metric_and_renewal_vw am 
                              JOIN coverva_dmas.dmas_file_log f ON UPPER(am.filename) = UPPER(f.filename) 
                              LEFT JOIN coverva_dmas.dmas_mms_ldap_full_load_vw ldap ON am.worker_id = ldap.ldap)
...
--cm043                           
              LEFT JOIN (SELECT tracking_number,processing_status,file_date 
                         ...
                         WHERE rnk = 1) cmd ON da.tracking_number = cmd.tracking_number AND GREATEST(da.prev_business_date,da.event_date-1) = cmd.file_date --da.event_date = cmd.file_date 
--running cm043
              LEFT JOIN (SELECT tracking_number,processing_status,file_date 
                         FROM(SELECT tracking_number,processing_status,CAST(f.file_date AS DATE) file_date 
