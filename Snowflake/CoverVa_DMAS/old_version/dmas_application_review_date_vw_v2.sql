...
FROM public.d_dates dd
 JOIN (SELECT tracking_number,file_inventory_date,current_state,inv_min_review_date,application_processing_end_date,state_app_received_date
                            FROM(SELECT da.*, irvw.inv_min_review_date,RANK() OVER (PARTITION BY da.tracking_number ORDER BY file_inventory_date DESC,dmas_application_id DESC) rnk                               
...
rvw_date AS
(SELECT da.*
     /*,CASE WHEN COALESCE(da.inv_min_review_date,LEAST(COALESCE(mio.mio_initial_review_date,cp.app_initial_review_date,mpt.mpt_initial_review_date,o.ovr_initial_review_complete_date),
                                                    ...
                                                    COALESCE(o.ovr_initial_review_complete_date,mio.mio_initial_review_date,cp.app_initial_review_date,mpt.mpt_initial_review_date))) IS NOT NULL THEN NULL ELSE 
     ...
     ,CASE WHEN da.current_state NOT IN('Waiting Initial Review','Intake') THEN
          COALESCE(COALESCE(da.inv_min_review_date,LEAST(COALESCE(mio.mio_initial_review_date,cp.app_initial_review_date,mpt.mpt_initial_review_date,o.ovr_initial_review_complete_date),
                COALESCE(cp.app_initial_review_date,mpt.mpt_initial_review_date,o.ovr_initial_review_complete_date,mio.mio_initial_review_date),
                COALESCE(mpt.mpt_initial_review_date,o.ovr_initial_review_complete_date,mio.mio_initial_review_date,cp.app_initial_review_date),
                COALESCE(o.ovr_initial_review_complete_date,mio.mio_initial_review_date,cp.app_initial_review_date,mpt.mpt_initial_review_date))), da.application_processing_end_date)
