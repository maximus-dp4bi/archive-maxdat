...
create or replace procedure SP_UPDATE_APPLICATION_END_DATE()
                   ...
                   FROM coverva_dmas.dmas_application dmas \
                      LEFT JOIN(SELECT tdh.ext_app_id,MIN(tdh.complete_task_date ) complete_task_date \
                           FROM(SELECT task_id,complete_task_date,project_id,ext_app_id \
                                FROM(SELECT tdh.task_id, th.status_date complete_task_date,tdh.project_id,tdh.selection_varchar AS ext_app_id, \
                                         RANK() OVER (PARTITION BY tdh.task_id ORDER BY tdh.task_history_id DESC,tdh.task_detail_history_id DESC) rn \
                                     FROM MARSDB.MARSDB_TASKS_HISTORY_VW th \
                                       JOIN MARSDB.MARSDB_TASK_DETAIL_HISTORY_VW tdh ON th.task_history_id = tdh.task_history_id AND th.project_id = tdh.project_id \
                      ...
                      LEFT JOIN(SELECT application_number,CAST(CONCAT(REVERSE(LEFT(REVERSE(activity_date),4)),'/',EXTRACT(YEAR FROM current_date())) AS DATE) cpu_action_date \
                                FROM(SELECT application_number,activity_date, RANK() OVER(PARTITION BY application_number ORDER BY CAST(CONCAT(REVERSE(LEFT(REVERSE(activity_date),4)),'/',EXTRACT(YEAR FROM current_date())) AS DATE) DESC) rnk \
