...
FROM public.d_dates dd 
 CROSS JOIN coverva_dmas.dmas_file_load_lkup fl
 ...
 JOIN (SELECT CAST(config_value AS DATE) config_dt FROM coverva_dmas.dmas_config_control cc WHERE cc.config_name = 'INVENTORY_FILE_DATE') cc 
...
WHERE fl.filename_prefix IN('CM_044','CM_043')
AND fl.use_in_inventory = 'Y'
...
FROM public.d_dates dd
 CROSS JOIN coverva_dmas.dmas_file_load_lkup fl
 JOIN (SELECT CAST(config_value AS DATE) config_dt FROM coverva_dmas.dmas_config_control cc WHERE cc.config_name = 'INVENTORY_FILE_DATE') cc ON dd.d_date > cc.config_dt AND dd.d_date <= current_date() 
...
WHERE fl.filename_prefix NOT IN('CM_044','CM_043','MPT') 
AND fl.use_in_inventory = 'Y'
AND dd.project_id = 117 
AND dd.business_day_flag = 'Y')
SELECT * 
