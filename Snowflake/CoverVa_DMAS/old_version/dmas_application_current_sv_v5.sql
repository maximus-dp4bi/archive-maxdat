...
WITH inv AS(SELECT da.tracking_number AS t_number
 ...
 -- ,MARSDB.get_business_difference_days(DATE_TRUNC('DAY',state_app_received_date), COALESCE(CAST(initial_review_complete_date AS DATE),current_date()),117) +1 application_age_in_business_days
  ...
  ,CASE WHEN current_state IN('Waiting Initial Review','Waiting for Verification Documents','Intake','Initial Review Complete','Non Maximus Assigned') THEN 1 ELSE 0 END pending
  ...
  ,CASE WHEN COALESCE(in_cp_indicator,'N') = 'N' AND current_state IN('Waiting Initial Review','Waiting for Verification Documents','Intake','Initial Review Complete','Non Maximus Assigned') THEN 1 ELSE 0 END inprocess_app_intake
