...
SELECT DISTINCT da.tracking_number, da.event_date 
              ...
              ,CASE WHEN rpw.tracking_number IS NOT NULL THEN 'Y' ELSE 'N' END in_running_pw
...
--am
             LEFT JOIN (SELECT tracking_number, worker_id, pending_due_to_vcl,vcl_due_date_am, file_date 
                        ...
                        WHERE rnk = 1) am ON da.tracking_number = am.tracking_number AND (CASE WHEN day_of_week IN('0','6') THEN da.event_date ELSE GREATEST(da.prev_business_date,da.ddate_last_sat) END = am.file_date) 
--running am
...
--pw                           
              LEFT JOIN (SELECT tracking_number, worker, pending_due_to_vcl,vcl_due_date_pw,file_date 
                         ...
                         WHERE rnk = 1  ) pw ON da.tracking_number = pw.tracking_number AND (CASE WHEN day_of_week IN('0','6') THEN da.event_date ELSE GREATEST(da.prev_business_date,da.ddate_last_sat) END = pw.file_date) 
--running pw                         
             LEFT JOIN (SELECT tracking_number, running_pw_worker,  file_date 
                        FROM(SELECT tracking_number, worker running_pw_worker, CAST(f.file_date AS DATE) file_date 
                               ,ROW_NUMBER() OVER(PARTITION BY pw.tracking_number ORDER BY CAST(f.file_date AS DATE) DESC,pw.appmetric_pw_data_id DESC) rnk
                             FROM coverva_dmas.app_metric_pw_full_load pw 
                              JOIN coverva_dmas.dmas_file_log f ON UPPER(pw.filename) = UPPER(f.filename) )
                        WHERE rnk = 1) rpw ON da.tracking_number = rpw.tracking_number AND da.event_date >= rpw.file_date                         
--ppit                           
...
--cm044                           
              LEFT JOIN (SELECT DISTINCT tracking_number,cm044_status,cm044_authorized_date,file_date,cm044_worker 
                         FROM(SELECT tracking_number,status cm044_status,authorized_date cm044_authorized_date,authorized_worker_id cm044_worker, CAST(f.file_date AS DATE) file_date 
                                ,ROW_NUMBER() OVER(PARTITION BY cm.tracking_number ORDER BY  CAST(f.file_date AS DATE) DESC,cm.cm044_data_id DESC) rnk 
                              FROM coverva_dmas.cm044_data_full_load cm 
                               JOIN coverva_dmas.dmas_file_log f ON UPPER(cm.filename) = UPPER(f.filename) 
...
--mio                           
LEFT JOIN (SELECT  tracking_number,mio_prod_status,mio_orig_status,mio_complete_date ,mio_last_employee ,mio_vcl_due_date,mio_update_date
                           FROM(SELECT case_number tracking_number, ldap_id mio_last_employee,task_status mio_orig_status,
                                  CASE WHEN UPPER(task_status) = 'TRANSFERRED TO LDSS' THEN 'Transferred to LDSS' 
                                       WHEN UPPER(task_status) IN('APPROVED','ABD APPROVED') THEN 'Approved'
                                       WHEN UPPER(task_status) = 'DENIED' THEN 'Denied'                                          
                                       WHEN CONCAT(UPPER(task_status),', ',UPPER(why)) = 'PENDING, EMERGENCY SERVICES' THEN 'Complete - Needs Research'
                                       WHEN CONCAT(UPPER(task_status),', ',UPPER(why)) LIKE 'PENDING%VCL%' THEN 'Waiting for Verification Documents'
                                       WHEN CONCAT(UPPER(task_status),', ',UPPER(why)) LIKE 'PENDING%HELP%' THEN 'Waiting for Verification Documents'
                                       WHEN UPPER(task_status) = 'PENDING' AND why IS NULL THEN 'Waiting for Verification Documents' 
                                       WHEN UPPER(task_status) = 'OTHER' THEN 'Waiting for Verification Documents' 
                                       WHEN CONCAT(UPPER(task_status),', ',UPPER(why)) LIKE 'PENDING%QUESTION%' THEN 'Waiting Initial Review'
                                        WHEN UPPER(task_status) = 'OTHER TRANSFER' THEN 'Waiting Initial Review'
                                     ELSE 'Waiting Initial Review' END mio_prod_status
                           ...
                           WHERE  mio_prod_status IS NOT NULL) mio ON mio.tracking_number = da.tracking_number AND da.event_date >= mio.mio_update_date;