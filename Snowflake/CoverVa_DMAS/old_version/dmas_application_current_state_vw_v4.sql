 ...
 SELECT e.tracking_number,e.event_date        
               ...
               ,e.mpt_complete_date,e.override_app_end_date,e.cm044_authorized_date,inv.latest_inventory_date, ame.last_date_in_file, e.mio_complete_date
             FROM (SELECT e.*,
...
(SELECT cs.tracking_number
  ...
  ,override_app_end_date,mpt_complete_date ,ldss_end_date,cm044_authorized_date,ad_complete_date,last_date_in_file last_date_in_any_file,mio_complete_date
 ...
 SELECT fcs.*,
   CAST(CASE WHEN current_state_2 IS NOT NULL THEN cm044_authorized_date                         
             ...
             WHEN final_current_state IN('Approved','Denied','Complete - Needs Research') THEN
               CASE WHEN current_state_4 IS NOT NULL THEN fcs.last_date_in_any_file
                    WHEN ad_complete_date IS NOT NULL THEN ad_complete_date
               ELSE NULL END                   
             WHEN current_state_9 IS NOT NULL THEN
                CASE WHEN current_state_9 IN('Approved','Denied','Transferred to LDSS') THEN mio_complete_date ELSE NULL END 
             WHEN current_state_10 IS NOT NULL THEN
                CASE WHEN current_state_10 IN('Approved','Denied','Transferred to LDSS') THEN mpt_complete_date ELSE NULL END                 
