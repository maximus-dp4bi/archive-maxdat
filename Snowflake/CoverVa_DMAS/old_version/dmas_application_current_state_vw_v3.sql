 ...
 SELECT e.tracking_number,e.event_date        
               ...
               ,CASE WHEN (NOT EXISTS(SELECT 1 FROM coverva_dmas.cp_application_inventory cpi WHERE cpi.tracking_number = e.tracking_number AND e.event_date >= cpi.sr_create_date ) 
                        ...
                        AND (mpt_orig_status IS NULL OR UPPER(mpt_orig_status) NOT LIKE 'PENDING%VCL%') THEN 'Waiting Initial Review'                        
               ...
               ,CASE WHEN e.manual_prod_status IS NOT NULL THEN e.manual_prod_status ELSE NULL END current_state_3               
               ,CASE WHEN (e.cp_inv_action_taken IS NOT NULL OR e.cp_iar_action_taken IS NOT NULL OR e.cp_inv_ad_disposition = 'REFERRED' OR cp_iar_ad_disposition = 'REFERRED'
                        OR e.manual_cpu_status = 'Transferred to LDSS' OR e.manual_cviu_status = 'Transferred to LDSS') THEN 'Transferred to LDSS' ELSE NULL END current_state_4
               /*,CASE WHEN ((in_ppit = 'N' AND in_app_metric = 'N' AND in_app_metric_pw = 'N') AND (in_running_ppit = 'Y' OR in_running_am = 'Y' OR in_running_pw = 'Y') 
                      ...
                      OR ((in_ppit = 'N' AND in_app_metric = 'N' AND in_app_metric_pw = 'N' AND in_cm043 = 'N' AND in_cviu = 'N' AND in_cpui = 'N' AND in_cpu = 'N' AND in_running_cm043 = 'Y'))   
                      OR ((in_ppit = 'N' AND in_app_metric = 'N' AND in_app_metric_pw = 'N' AND in_cm043 = 'N' AND in_cviu = 'N' AND in_cpui = 'N' AND in_cpu = 'N' AND (in_running_cviu = 'Y' OR in_running_cpui = 'Y' )))   
                  -- THEN 'current_state_6' ELSE NULL END current_state_6                           
                   THEN 'Complete - Needs Research' ELSE NULL END current_state_5 */
                ,CASE --WHEN ((in_ppit = 'N' AND in_app_metric = 'N' AND in_app_metric_pw = 'N') AND in_running_cpu = 'Y' AND (in_running_ppit = 'Y' OR in_running_am = 'Y' OR in_running_pw = 'Y') ) 
                     WHEN ((in_ppit = 'N' AND in_app_metric = 'N' AND in_app_metric_pw = 'N' AND in_cm043 = 'N' AND in_cviu = 'N' AND in_cpui = 'N') AND (in_running_ppit = 'Y' OR in_running_am = 'Y' OR in_running_pw = 'Y') )                             
                     OR (in_ppit = 'N' AND in_app_metric = 'N' AND in_app_metric_pw = 'N' AND in_cpu = 'N' AND in_running_cpu = 'Y' AND (DATEDIFF(DAY,CAST(COALESCE(cpu_app_received_date,running_cpu_app_received_date) AS DATE),cpuev.cpu_last_event) < 60))  
               ...
               ,CASE WHEN ( (in_ppit = 'N' AND in_app_metric = 'N' AND in_app_metric_pw = 'N')                 
                     AND (COALESCE(cviu_processing_status,running_cviu_status) = 'Submitted' OR COALESCE(cpui_processing_status,running_cpui_status) = 'Submitted' ) )                                                 
                   THEN 'current_state_6' ELSE NULL END current_state_6           
               ,CASE WHEN ((in_cviu = 'Y' AND cviu_worker IS NOT NULL AND REGEXP_INSTR(cviu_worker, '900') = 0)   
                             ...
                             OR (in_ppit = 'N' AND running_ppit_worker IS NOT NULL AND REGEXP_INSTR(running_ppit_worker, '900') = 0) ) THEN 'current_state_7' ELSE NULL END current_state_7 
               ,CASE WHEN ((in_cviu = 'N' AND in_cpui = 'N' AND in_ppit = 'N' AND in_app_metric = 'Y' AND worker_id IS NOT NULL AND REGEXP_INSTR(worker_id, '900') = 0 )  
                    OR (in_cviu = 'N' AND in_cpui = 'N' AND in_ppit = 'N' AND in_app_metric = 'N' AND running_am_worker IS NOT NULL AND REGEXP_INSTR(running_am_worker, '900') = 0 ) ) THEN 'current_state_8' ELSE NULL END current_state_8
               ,CASE WHEN (e.cp_inv_ad_disposition IS NOT NULL OR e.cp_iar_ad_disposition IS NOT NULL OR e.manual_cpu_status IS NOT NULL OR e.manual_cviu_status IS NOT NULL) THEN 
                 ...
                 ELSE 'Complete - Needs Research' END current_state_9
               ,CASE WHEN (NOT EXISTS(SELECT 1 FROM coverva_dmas.cp_application_inventory cpi WHERE cpi.tracking_number = e.tracking_number AND e.event_date >= cpi.sr_create_date ) 
                        OR (EXISTS(SELECT 1 FROM coverva_dmas.cp_application_inventory cpi WHERE cpi.tracking_number = e.tracking_number AND complete_task_date IS NULL) AND e.cp_inv_pend_disposition IS NULL) ) 
                        AND NOT EXISTS(SELECT 1 FROM coverva_dmas.cp_initial_application_review cpi WHERE cpi.tracking_number = e.tracking_number AND e.event_date >= cpi.status_date ) AND e.cp_iar_pend_disposition IS NULL 
                        AND in_manual_cpu = 'N' AND in_manual_cviu = 'N'
                     THEN 'Waiting Initial Review' ELSE NULL END current_state_10
               ,CASE WHEN (e.cp_inv_pend_disposition IS NOT NULL OR e.cp_iar_pend_disposition IS NOT NULL) 
                        OR (in_app_metric = 'Y' AND pending_due_vcl_am = 'Y') OR (in_app_metric_pw = 'Y' AND pending_due_vcl_pw = 'Y' ) 
                     THEN 'Waiting for Verification Documents' ELSE NULL END current_state_11
               ,CASE WHEN e.override_current_state IS NOT NULL THEN e.override_current_state ELSE NULL END current_state_12                     
               ...
               ,e.mpt_complete_date,e.override_app_end_date,e.cm044_authorized_date,inv.latest_inventory_date, ame.last_date_in_file
             FROM (SELECT e.*,
...
(SELECT cs.tracking_number
  ...
  ,current_state_1,current_state_1a,current_state_2,current_state_3,current_state_4,current_state_5,current_state_6,current_state_7,current_state_8,current_state_9,current_state_10,current_state_11,current_state_12
  ,override_app_end_date,mpt_complete_date ,ldss_end_date,cm044_authorized_date,ad_complete_date,last_date_in_file
  ,CASE WHEN current_state_1 IS NOT NULL THEN current_state_1a 
       ...
       --WHEN current_state_5 IS NOT NULL THEN current_state_5       
       WHEN current_state_5 IS NOT NULL OR current_state_6 IS NOT NULL OR current_state_7 IS NOT NULL OR current_state_8 IS NOT NULL THEN current_state_9
 ...
 FROM cs)   
 SELECT fcs.*,
   CAST(CASE WHEN current_state_2 IS NOT NULL THEN cm044_authorized_date
             WHEN current_state_3 IS NOT NULL THEN
                CASE WHEN current_state_3 IN('Approved','Denied','Transferred to LDSS') THEN mpt_complete_date ELSE NULL END             
             WHEN final_current_state = 'Transferred to LDSS' THEN CASE WHEN ldss_end_date IS NOT NULL THEN ldss_end_date ELSE NULL END
             WHEN final_current_state = 'Complete - Needs Research' AND current_state_5 = final_current_state THEN fcs.last_date_in_file
             WHEN final_current_state IN('Approved','Denied') THEN
               CASE WHEN current_state_2 IS NOT NULL THEN cm044_authorized_date 
