 ...
 SELECT e.tracking_number,e.event_date        
               ...
               ,CASE WHEN (e.cp_inv_pend_disposition IS NOT NULL OR e.cp_iar_pend_disposition IS NOT NULL)                         
                     ...
                     WHEN --(NOT EXISTS(SELECT 1 FROM coverva_dmas.cp_application_inventory cpi WHERE cpi.tracking_number = e.tracking_number AND e.event_date >= cpi.sr_create_date ) 
                        (EXISTS(SELECT 1 FROM coverva_dmas.cp_application_inventory cpi WHERE cpi.tracking_number = e.tracking_number AND complete_task_date IS NULL) AND e.cp_inv_pend_disposition IS NULL)  
               ...
               ,CASE WHEN e.mio_term_state IS NOT NULL THEN e.mio_term_state ELSE NULL END current_state_9  
               ...
               ,cpu_last_event,min_cpu_submit_date,state_app_received_date
             FROM (SELECT e.*,
              ...
              LEFT JOIN(SELECT tracking_number,MIN(event_date) last_date_in_file_mwrk
               ...
               LEFT JOIN (SELECT tracking_number,file_inventory_date latest_inventory_date,state_app_received_date 
...
(SELECT cs.tracking_number
  ...
  ,mio_complete_date,last_date_in_file_mwrk,cpu_last_event,min_cpu_submit_date
  ,CASE WHEN (current_state_1 IS NOT NULL OR current_state_0a IS NOT NULL) THEN CASE WHEN current_state_0 IS NOT NULL THEN current_state_0 ELSE current_state_1a END
 ...
 FROM cs)   
 SELECT fcs.*,
   CAST(CASE WHEN final_current_state IN('Intake','Waiting Initial Review','Waiting for Verification Documents','Initial Review Complete') THEN NULL
