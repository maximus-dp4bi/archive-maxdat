...
drop view PUBLIC.DMAS_APPLICATION_V3_CURRENT_SV;


use role MARS_DP4BI_PROD_COVERVA_DMAS_ADMIN;
CREATE OR REPLACE VIEW PUBLIC.DMAS_APPLICATION_V3_CURRENT_SV
AS
WITH inv AS(SELECT da.tracking_number AS t_number
  ...
  ,MARSDB.get_business_difference_days(DATE_TRUNC('DAY',(CASE WHEN fi.min_file_inv_date < state_app_received_date THEN state_app_received_date ELSE fi.min_file_inv_date END)),
     COALESCE(CAST(initial_review_complete_date AS DATE),current_date()),117) +1 application_age_in_business_days
  ,CASE WHEN df.filename_prefix = 'CVIU_LIAISON_SMARTSHEET' THEN state_app_received_date 
    WHEN fi.min_file_inv_date < state_app_received_date THEN state_app_received_date ELSE fi.min_file_inv_date END AS first_inventory_date
  ,fi.min_intake_date AS first_intake_date
  ,CASE WHEN application_processing_end_date IS NULL THEN NULL  ELSE  COALESCE(min_app_end_date,application_processing_end_date,max_file_inv_date) END AS first_complete_date
  ,file_inventory_date
  ,1 record_sequence 
...
FROM coverva_dmas.dmas_application_v3_current da
 ...
 LEFT JOIN (SELECT tracking_number,MIN(max_file_inv_date) max_file_inv_date
            FROM(SELECT e.tracking_number,MIN(event_date) max_file_inv_date
                 ...
                 AND NOT EXISTS(SELECT 1 FROM coverva_dmas.dmas_application_v3_event en WHERE e.tracking_number = en.tracking_number 
                                AND (en.in_app_metric = 'Y' OR en.in_cm043 = 'Y' OR en.in_rp190 = 'Y') AND en.application_event_id > e.application_event_id)
 ...
 LEFT JOIN (SELECT d.tracking_number,MIN(file_inventory_date) min_file_inv_date,MIN(CAST(intake_date AS DATE)) min_intake_date,
                ...
                MIN(CAST(application_processing_end_date AS DATE)) min_app_end_date
 ...
 LEFT JOIN (SELECT d.tracking_number,ROW_NUMBER() OVER (PARTITION BY d.tracking_number ORDER BY d.file_inventory_date DESC,d.dmas_application_id DESC) record_sequence,
            ...
            FROM coverva_dmas.dmas_application_v3_inventory d) prevcs ON da.tracking_number = prevcs.tracking_number AND prevcs.record_sequence = 2 
...
-- inv.current_state, 
 ...
 CASE WHEN inv.current_state != 'Non Maximus Assigned' THEN CASE WHEN edtfid4.end_date >= CAST(COALESCE(initial_review_date,application_processing_end_date) AS DATE) THEN 1 ELSE 0 END ELSE 0 END first_invdt_timely_5_busdays,
 CASE WHEN inv.current_state != 'Non Maximus Assigned' THEN CASE WHEN edtfid4.end_date < CAST(COALESCE(initial_review_date,application_processing_end_date) AS DATE) THEN 1 ELSE 0 END ELSE 0 END first_invdt_untimely_5_busdays,
 inv.cm044_verified,
 ...
 CASE WHEN inv.noa_generation_date IS NULL THEN NULL 
   ELSE CONCAT(EXTRACT(MONTH FROM inv.noa_generation_date),'/',EXTRACT (DAY FROM inv.noa_generation_date),'/', EXTRACT (YEAR FROM inv.noa_generation_date)) END AS noa_generation_date_char,
