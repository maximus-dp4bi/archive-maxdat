...
FROM public.d_dates dd
 CROSS JOIN (SELECT * FROM coverva_dmas.dmas_file_load_lkup ll
             ...
             WHERE CASE WHEN cc3.config_value = 'Y' THEN ll.use_in_v3_inventory WHEN cc.config_value = 'Y' THEN ll.use_in_v2_inventory ELSE ll.use_in_inventory END = 'Y') fl
 JOIN (SELECT CAST(cc.config_value AS DATE) config_dt 
       FROM coverva_dmas.dmas_config_control cc 
            JOIN coverva_dmas.dmas_config_control cc2 ON cc2.config_name = 'INVENTORY_V2_SWITCH'
            JOIN coverva_dmas.dmas_config_control cc3 ON cc3.config_name = 'INVENTORY_V3_SWITCH'
       WHERE cc.config_name = CASE WHEN cc3.config_value = 'Y' THEN 'INVENTORY_V3_FILE_DATE' WHEN cc2.config_value = 'Y' THEN 'INVENTORY_V2_FILE_DATE' ELSE 'INVENTORY_FILE_DATE' END) cc
   ON dd.d_date > cc.config_dt AND dd.d_date <= current_date() 
 LEFT JOIN(SELECT filename_prefix,filename,row_count
          ...
          WHERE f.load_file = 'Y') ) s ON dd.d_date = CAST(s.file_date AS DATE) AND s.filename_prefix = fl.filename_prefix AND s.filename_prefix NOT IN('CM_044','MPT') 
...
AND NOT EXISTS(SELECT 1 FROM coverva_dmas.dmas_file_log fl WHERE mf.filename_prefix = fl.filename_prefix AND mf.d_date = CAST(fl.file_date AS DATE) AND fl.load_status = 'COMPLETED');