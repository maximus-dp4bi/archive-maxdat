...
create or replace procedure SP_PROCESS_APPLICATION_V3_CDC()
  ...
  var vChkErr = 0;
   ...
   try {  
    ...
    strSQLText = `SELECT config_value inventory_v3_switch 
                  FROM coverva_dmas.dmas_config_control 
                  WHERE config_name = 'INVENTORY_V3_SWITCH';`;
    strSQLStmt = snowflake.createStatement({sqlText: strSQLText});
    ret_value = strSQLStmt.execute();   
    ret_value.next();
    var invV3Switch = ret_value.getColumnValue('INVENTORY_V3_SWITCH');
   
   /* STEP 1: Load files into permanent tables */
    strSQLText = "CALL SP_POPULATE_FULL_LOAD_V3_TABLE(:1);";                     
    ...
    if ( (busDay == "Y" && timeToStart == "Y") && ((errTabCount == vChkErr && warnTabCount == vChkErr) || (errTabCount == vChkErr && timeToRun == "Y") || (errTabCount == vChkErr && byPassMissingFiles == "Y") ) )
       { 
         ...
         /* STEP 4: Load CP App Inventory */
           strSQLText = "CALL SP_POPULATE_APP_V3_INVENTORY();";                     
           strSQLStmt = snowflake.createStatement({sqlText: strSQLText});
           ret_value = strSQLStmt.execute();
           
         /* STEP 5: Load CP Initial Review App */
           strSQLText = "CALL SP_POPULATE_INITIAL_APPLICATION_V3_REVIEW();";                     
           strSQLStmt = snowflake.createStatement({sqlText: strSQLText});
           ret_value = strSQLStmt.execute();
         ...
         if ( invV3Switch == "Y") 
          {
            strSQLText = "CALL SP_POPULATE_CDC_V3_TABLE();";                     
            strSQLStmt = snowflake.createStatement({sqlText: strSQLText});
            ret_value = strSQLStmt.execute();
           }
         else if (invV2Switch == "Y") 
          {
  ...
  GRANT ALL PRIVILEGES ON PROCEDURE coverva_dmas.sp_process_application_v3_cdc() TO ROLE MARS_DP4BI_PROD_ADMIN;