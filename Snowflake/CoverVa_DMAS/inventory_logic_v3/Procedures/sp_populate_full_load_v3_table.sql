...
create or replace procedure SP_POPULATE_FULL_LOAD_V3_TABLE(USE_IN_INVENTORY VARCHAR)
  ...
  strSQLText = `DELETE FROM COVERVA_DMAS.DMAS_FILE_LOG WHERE load_status IN('NOT PROCESSED','STARTED') 
                  AND filename_prefix IN(SELECT filename_prefix FROM coverva_dmas.dmas_file_load_lkup WHERE use_in_inventory = :1 OR use_in_v2_inventory = :1 OR use_in_v3_inventory = :1);`;                 
  strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [USE_IN_INVENTORY]});
  ...
  strSQLText = `INSERT INTO COVERVA_DMAS.DMAS_FILE_LOG (filename_prefix,filename,row_count,file_date,load_status) 
               ...
               FROM(SELECT f.filename_prefix,s.table_name filename,'STARTED' load_status, with_timestamp, s.row_count, 
                    ...
                    WHERE f.load_file = 'Y' 
                    AND (f.use_in_inventory = :1 OR f.use_in_v2_inventory = :1 OR use_in_v3_inventory = :1) ) 
               ORDER BY file_date;`;
  strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [USE_IN_INVENTORY]});
  ret_value = strSQLStmt.execute();
  ...
  snowflake.execute( {sqlText: "BEGIN;"} );
   try {
       ...
       strSQLText = `SELECT full_load_table_schema, t.file_id,t.filename, t.row_count, f.full_load_table_name,f.insert_fields,f.select_fields,f.where_clause 
                   FROM COVERVA_DMAS.DMAS_FILE_LOG t JOIN COVERVA_DMAS.DMAS_FILE_LOAD_LKUP f ON  t.filename_prefix = f.filename_prefix 
                   WHERE t.load_status = 'STARTED' AND (f.use_in_inventory = :1 OR f.use_in_v2_inventory = :1 OR use_in_v3_inventory = :1 ) ;`;  
       strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [USE_IN_INVENTORY]});
...
GRANT ALL PRIVILEGES ON PROCEDURE coverva_dmas.SP_POPULATE_FULL_LOAD_TABLE(VARCHAR) TO ROLE MARS_DP4BI_PROD_COVERVA_DMAS_ADMIN;
GRANT ALL PRIVILEGES ON PROCEDURE coverva_dmas.SP_POPULATE_FULL_LOAD_TABLE(VARCHAR) TO ROLE MARS_DP4BI_PROD_ADMIN;  