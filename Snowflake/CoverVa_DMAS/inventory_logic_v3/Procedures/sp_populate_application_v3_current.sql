...
create or replace procedure SP_POPULATE_APPLICATION_V3_CURRENT()
  ...
  var strErrMsg = "";
   ...
   try {  
       ...
       var strSQLText = `INSERT INTO coverva_dmas.dmas_application_v3_current(Tracking_Number,Source,App_Received_Date,Processing_Unit,Application_Type,Current_State,Initial_Review_Complete_Date,  
                            Application_Processing_End_Date,Last_Employee,Applicant_Name,Case_Number,FP_Create_Dt,FP_Update_Dt,File_ID,In_CP_Indicator, 
               ...
               WHERE rnk = 1 
               AND NOT EXISTS(SELECT 1 FROM coverva_dmas.dmas_application_v3_current dac WHERE da.tracking_number = dac.tracking_number);`;           
         var strSQLStmt = snowflake.createStatement({sqlText: strSQLText});
         var ret_value = strSQLStmt.execute();
         
         strSQLText = `UPDATE coverva_dmas.dmas_application_v3_current dac 
              SET dac.Source = da.source 
                 ...
                 ,dac.noa_generation_date = da.noa_generation_date
                 ,dac.application_incarcerated_indicator = da.application_incarcerated_indicator
                 ,dac.case_incarcerated_indicator = da.case_incarcerated_indicator
                 ,dac.status_date = da.status_date
                 ,dac.intake_state_first_date = CASE WHEN dac.intake_state_first_date IS NULL THEN da.intake_state_first_date ELSE dac.intake_state_first_date END
                 ,dac.wir_state_first_date = CASE WHEN dac.wir_state_first_date IS NULL THEN da.wir_state_first_date ELSE dac.wir_state_first_date END
                 ,dac.wfvd_state_first_date = CASE WHEN dac.wfvd_state_first_date IS NULL THEN da.wfvd_state_first_date ELSE dac.wfvd_state_first_date END
                 ,dac.irc_state_first_date = CASE WHEN dac.irc_state_first_date IS NULL THEN da.irc_state_first_date ELSE dac.irc_state_first_date END
                 ,dac.ttldss_state_first_date = CASE WHEN dac.ttldss_state_first_date IS NULL THEN da.ttldss_state_first_date ELSE dac.ttldss_state_first_date END
                 ,dac.approved_state_first_date = CASE WHEN dac.approved_state_first_date IS NULL THEN da.approved_state_first_date ELSE dac.approved_state_first_date END
                 ,dac.denied_state_first_date = CASE WHEN dac.denied_state_first_date IS NULL THEN da.denied_state_first_date ELSE dac.denied_state_first_date END
                 ,dac.complete_state_first_date = CASE WHEN dac.complete_state_first_date IS NULL THEN da.complete_state_first_date ELSE dac.complete_state_first_date END
