CREATE OR REPLACE VIEW PUBLIC.DMAS_APPLICATION_V2_CURRENT_SV
AS
WITH inv AS(SELECT da.tracking_number AS t_number
  ...
  ,CASE WHEN fi.min_file_inv_date < state_app_received_date THEN state_app_received_date ELSE fi.min_file_inv_date END AS first_inventory_date
  ...
  ,da.denial_reason
FROM coverva_dmas.dmas_application_v2_current da
 ...
 LEFT JOIN (SELECT tracking_number,MIN(max_file_inv_date) max_file_inv_date
            FROM(SELECT e.tracking_number,MIN(event_date) max_file_inv_date
                 FROM coverva_dmas.dmas_application_v2_event e
                 ...
                 AND NOT EXISTS(SELECT 1 FROM coverva_dmas.dmas_application_v2_event en WHERE e.tracking_number = en.tracking_number 
                                AND (en.in_app_metric = 'Y' OR en.in_ppit = 'Y' OR en.in_app_metric_pw = 'Y' OR en.in_cviu = 'Y' OR en.in_cpu_incarcerated = 'Y' OR en.in_cm043 = 'Y') AND en.application_event_id > e.application_event_id)
 ...
 LEFT JOIN (SELECT d.tracking_number,MIN(file_inventory_date) min_file_inv_date,MIN(CAST(intake_date AS DATE)) min_intake_date,
            ...
            FROM coverva_dmas.dmas_application_v2_inventory d                          
 ...
 LEFT JOIN (SELECT d.tracking_number,ROW_NUMBER() OVER (PARTITION BY d.tracking_number ORDER BY d.file_inventory_date DESC,d.dmas_application_id DESC) record_sequence,
            ...
            FROM coverva_dmas.dmas_application_v2_inventory d) prevcs ON da.tracking_number = prevcs.tracking_number AND prevcs.record_sequence = 2 
