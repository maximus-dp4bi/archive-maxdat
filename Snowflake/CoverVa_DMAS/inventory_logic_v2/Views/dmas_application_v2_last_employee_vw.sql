CREATE OR REPLACE VIEW COVERVA_DMAS.DMAS_APPLICATION_V2_LAST_EMPLOYEE_VW
AS
WITH lemp AS(
...
FROM coverva_dmas.dmas_raw_application_v2_event_vw e
  ...
  LEFT JOIN (SELECT DISTINCT application_number,mcpu_activity_date,mcpu_last_employee                                   
                          FROM(SELECT application_number                                     
                                     ...
                                     ,ROW_NUMBER() OVER(PARTITION BY application_number ORDER BY CAST(activity_date AS DATE) DESC,DECODE(COALESCE(noa,vcl,approved,denied,sent_to_ldss),'1',1,2),record_id DESC) rnk 
