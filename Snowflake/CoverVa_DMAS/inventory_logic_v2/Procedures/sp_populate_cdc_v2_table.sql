...
create or replace procedure SP_POPULATE_CDC_V2_TABLE()
  ...
  //snowflake.execute( {sqlText: "BEGIN;"} );
   try {  
    ...
    var strSQLText = ` UPDATE coverva_dmas.dmas_file_log fl 
                     SET cdc_v2_processed = 'N',cdc_v2_processed_date = null 
                     FROM(SELECT COALESCE(LEAST(fl.file_date,CAST(cc.config_value AS DATE)),CAST(cc.config_value AS DATE)) min_file_date, 
                          ...
                          FROM coverva_dmas.dmas_config_control cc 
                             JOIN (SELECT MIN(CAST(fl.file_date AS DATE)) file_date 
                                   ...
                                   WHERE ll.use_in_v2_inventory = 'Y' AND fl.load_status = 'COMPLETED' AND fl.cdc_v2_processed = 'N' 
                                   AND NOT EXISTS(SELECT 1 FROM coverva_dmas.dmas_file_log pfl WHERE pfl.filename_prefix = fl.filename_prefix AND CAST(pfl.file_date AS DATE) = CAST(fl.file_date AS DATE) AND pfl.cdc_v2_processed = 'Y') ) fl  
                           WHERE cc.config_name = 'INVENTORY_V2_FILE_DATE' ) mf WHERE fl.file_date >= mf.min_file_date AND cdc_v2_processed = 'Y' AND mf.upd_cdc = 'Y'; `;
                     ...
                     FROM(SELECT cc.config_name,TO_CHAR(COALESCE(LEAST(fl.file_date,CAST(cc.config_value AS DATE)),CAST(cc.config_value AS DATE)),'MM/DD/YYYY') min_file_date 
                          FROM coverva_dmas.dmas_config_control cc 
                             JOIN (SELECT MIN(CAST(fl.file_date AS DATE)) file_date 
                                   ...
                                   WHERE ll.use_in_v2_inventory = 'Y' AND fl.load_status = 'COMPLETED' AND fl.cdc_v2_processed = 'N' 
                                   AND NOT EXISTS(SELECT 1 FROM coverva_dmas.dmas_file_log pfl WHERE pfl.filename_prefix = fl.filename_prefix AND CAST(pfl.file_date AS DATE) = CAST(fl.file_date AS DATE) AND pfl.cdc_v2_processed = 'Y') ) fl   
                           WHERE cc.config_name = 'INVENTORY_V2_FILE_DATE') mf WHERE cc.config_name = mf.config_name AND cc.config_value != mf.min_file_date; `;
      ...
      strSQLText = `SELECT DISTINCT d_date,COALESCE(file_count,0) file_count 
                        FROM public.d_dates dd JOIN coverva_dmas.dmas_config_control ccd ON ccd.config_name = 'INVENTORY_DAYS_TO_PROCESS' 
                          ...
                          LEFT JOIN (SELECT CAST(fl.file_date AS DATE) file_date,COUNT(DISTINCT fl.filename_prefix) file_count 
                                     ...
                                     WHERE ll.use_in_v2_inventory = 'Y' AND fl.load_status = 'COMPLETED' AND fl.cdc_v2_processed = 'N' GROUP BY CAST(fl.file_date AS DATE)) fl ON fl.file_date = dd.d_date 
                        WHERE cc.config_name = 'INVENTORY_V2_FILE_DATE' 
                        ORDER BY d_date; `;  
      strSQLStmt = snowflake.createStatement({sqlText: strSQLText});
      ...
      try{
        ...
        if (fileCount >= invNumFile)          
          {      
               ...
               strSQLText = "CALL SP_POPULATE_APPLICATION_V2_LIST(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               var ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_CREATE_APPLICATION_V2_EVENT(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               var ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_SOURCE(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               /* App Received stored proc also updates the Maximus Source Date*/
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_RECEIVED_DATE(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_UNIT(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_TYPE(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_CASE_TYPE_STAGE(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_IN_CP(:1);";                   
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               /* Current State stored proc also updates the App End Date, CM44 verified, Non Maximus Initial and Returned dates*/
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_CURRENT_STATE(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_REVIEW_DATE(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
                     
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_WORKER(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_MIGRATED(:1);";                   
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_VCL_DUEDATE(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_UPDATE_APPLICATION_V2_RENEWAL_CLOSURE_DATE(:1);";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = "CALL SP_POPULATE_APPLICATION_V2_CURRENT();";                     
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
               
               strSQLText = `UPDATE COVERVA_DMAS.dmas_file_log fl 
                       SET cdc_v2_processed = 'Y', cdc_v2_processed_date = current_timestamp() 
                       WHERE CAST(file_date AS DATE) = CAST(:1 AS DATE) 
                       AND cdc_v2_processed = 'N' 
                       AND EXISTS(SELECT 1 FROM coverva_dmas.dmas_file_load_lkup ll WHERE fl.filename_prefix = ll.filename_prefix AND ll.use_in_v2_inventory= 'Y'); `;       
               strSQLStmt = snowflake.createStatement({sqlText: strSQLText,binds: [invStartDate]});
               ret_value = strSQLStmt.execute();
   ...
   strSQLText = `UPDATE COVERVA_DMAS.dmas_config_control SET config_value = max_file_date 
               ...
               WHERE config_name = 'INVENTORY_V2_FILE_DATE';`;                 
   strSQLStmt = snowflake.createStatement({sqlText: strSQLText});
    ret_value = strSQLStmt.execute();
