...
create or replace procedure SP_UPDATE_APPLICATION_V2_TYPE(INV_FILE_DATE VARCHAR)
   ...
   try {  
       ...
       var strSQLText = `UPDATE coverva_dmas.dmas_application_v2_inventory dmas 
                ...
                FROM (SELECT DISTINCT da.tracking_number, da.event_date 
                        ,COALESCE(o.override_app_type,pw.pw_app_type,cm.cm044_app_type,cviu.cviu_app_type,cpui.cpui_app_type,cpu.cpu_app_type,'non-PW') application_type 
                      ...
                      FROM (SELECT tracking_number, event_date 
                        ...
                        LEFT JOIN (SELECT tracking_number,'PW' pw_app_type,CAST(f.file_date AS DATE) file_date 
                                   FROM coverva_dmas.dmas_app_metric_and_renewal_v2_vw pw 
                                     JOIN coverva_dmas.dmas_file_log f ON UPPER(pw.filename) = UPPER(f.filename)
                                     WHERE UPPER(ma_pg_indicator) IN('YES','Y')
                                     QUALIFY ROW_NUMBER() OVER (PARTITION BY tracking_number,CAST(f.file_date AS DATE) ORDER BY COALESCE(appmetric_data_id,renewal_data_id) DESC) = 1) pw ON da.tracking_number = pw.tracking_number AND da.event_date = pw.file_date 
