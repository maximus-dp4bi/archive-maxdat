...
create or replace procedure SP_UPDATE_APPLICATION_V2_SOURCE(INV_FILE_DATE VARCHAR)
   ...
   try {  
       ...
       var strSQLText = `UPDATE coverva_dmas.dmas_application_v2_inventory dmas 
             ...
             FROM (SELECT DISTINCT da.tracking_number, da.event_date 
                     ,COALESCE(CASE WHEN UPPER(COALESCE(o.override_source,rcpu.running_cpu_source,cpui.cpui_source,cviu.cviu_source,pp.ppit_source,am.am_source,cmd.cm043_source,cp.channel,mcpu.mcpu_source,mcviu.mcviu_source)) LIKE 'FFM_%' 
                        THEN UPPER(REPLACE(COALESCE(o.override_source,rcpu.running_cpu_source,cpui.cpui_source,cviu.cviu_source,pp.ppit_source,am.am_source,cmd.cm043_source,cp.channel,mcpu.mcpu_source,mcviu.mcviu_source),'_',' ')) 
                     WHEN UPPER(COALESCE(o.override_source,rcpu.running_cpu_source,cpui.cpui_source,cviu.cviu_source,pp.ppit_source,am.am_source,cmd.cm043_source,cp.channel,mcpu.mcpu_source,mcviu.mcviu_source)) LIKE 'COMMON%' THEN 'CommonHelp' 
                        ELSE COALESCE(o.override_source,rcpu.running_cpu_source,cpui.cpui_source,cviu.cviu_source,pp.ppit_source,am.am_source,cmd.cm043_source,cp.channel,mcpu.mcpu_source,mcviu.mcviu_source) END,prsrc.prev_source) application_source  
                   ...
                   FROM (SELECT tracking_number, event_date 
                     ...
                     LEFT JOIN (SELECT DISTINCT application_number,mcpu_source 
                                FROM(SELECT application_number, source mcpu_source 
                                       ,RANK() OVER(PARTITION BY application_number ORDER BY CAST(activity_date AS DATE),record_id) rnk 
