...
raw_event AS(SELECT DISTINCT da.tracking_number, da.event_date 
             ...
             ,p.ppit_vcl_due_date,COALESCE(am.case_type,'Application') case_type,am.sd_stage,am.am_processing_status,ram.running_am_processing_status,running_case_type
FROM  tlist da             
--am
...
LEFT JOIN (SELECT case_number tracking_number, ldap_id mio_term_last_employee,task_status mio_term_orig_status                                 
 ...
 SELECT e.tracking_number,e.event_date        
               ,CASE WHEN NOT EXISTS(SELECT 1 FROM coverva_dmas.cp_application_inventory cpi WHERE cpi.tracking_number = e.tracking_number AND e.event_date >= cpi.sr_create_date )
                    ...
                    THEN 'Intake' ELSE NULL END current_state_0
               ...
               ,cpu_last_event,min_cpu_submit_date,state_app_received_date,inv.previous_current_state,inv.previous_end_date
             FROM (SELECT e.*,
                    CASE WHEN manual_cpu_status = 'Transferred to LDSS' THEN mcpu_activity_date ELSE NULL END mcpu_ldss_date
                    ,CASE WHEN manual_cpu_status IN('Approved','Denied') THEN mcpu_activity_date ELSE NULL END mcpu_complete_date
                    ,CASE WHEN manual_cviu_status = 'Transferred to LDSS' THEN mcviu_activity_date ELSE NULL END mcviu_ldss_date
              ...
              LEFT JOIN(SELECT tracking_number,MIN(last_date_in_file_mwrk) last_date_in_file_mwrk
               ...
               LEFT JOIN (SELECT tracking_number,file_inventory_date latest_inventory_date,state_app_received_date,previous_current_state,previous_end_date 
...
(SELECT cs.tracking_number
  ...
  ,current_state_1,current_state_1a,current_state_2,current_state_3,current_state_3b,current_state_4,current_state_5,current_state_5a ,current_state_6,current_state_7
  ...
  ,mio_complete_date,last_date_in_file_mwrk,cpu_last_event,min_cpu_submit_date,previous_end_date
 ...
 SELECT fcs.*,
   ...
   CASE WHEN final_current_state = 'Waiting for Verification Documents' THEN event_date ELSE NULL END waiting_for_vdoc_date
...
FROM(
   SELECT 
        ...
        CASE WHEN CONCAT(UPPER(task_status),', ',UPPER(why)) LIKE 'PENDING%VCL%' 
         ...
         CASE WHEN UPPER(task_status) IN('TRANSFERRED TO LDSS','SENT TO LDSS') THEN 'Transferred to LDSS' 
                                       ...
                                       WHEN CONCAT(UPPER(task_status),', ',UPPER(why)) LIKE 'PENDING%VCL%' 
                                          OR UPPER(task_status) = 'VCL SENT' THEN 'Waiting for Verification Documents'  
                                     ELSE 'Waiting Initial Review' END mio_term_state,
        CASE WHEN UPPER(task_status) IN('TRANSFERRED TO LDSS','SENT TO LDSS') THEN 1 
                                     ...
                                     ELSE 4 END mio_term_state_rank,
         case_put_on_hold,
         case_type
