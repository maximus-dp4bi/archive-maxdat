create table coverva_dmas.cp_initial_application_review
as
...
LEFT JOIN (SELECT distinct u.user_id, concat(s.first_name, ' ',s.last_name) as name, s.maximus_id, t.TEAM_NAME, pr.role_name
           ...
           ) emp on (emp.user_id = fcomp.updated_by)
LEFT JOIN (select * from
...
LEFT JOIN (select * from (                                                          
          ...
          ) where rn = 1 ) extappid2 on (extappid2.proc_app_task_id = f.task_id)  
...
alter table coverva_dmas.cp_initial_application_review add primary key (task_id);
