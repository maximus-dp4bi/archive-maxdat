create or replace view PUBLIC.D_PM_PROVIDER_CREDENTIAL_INSTANCE_SV(
	APPLICATION_ID,
	PROCESS_ID,
	FILE_ID,
	FILE_AGE,
...
SELECT r.reg_id application_id  
  ...
  ,CASE WHEN rc.credentialing_status_name IN('Approved','Administrative Denial','Administrative Termination','Credentialing Committee Denial','Credentialing Committee Termination')
    ...
    THEN DATEDIFF(second,COALESCE(wspc.cs_start_date_time,rc.start_date_time),COALESCE(ccr.cc_cr_end_dt,ocr.odm_cr_end_dt,rc.end_date_time))
   ELSE DATEDIFF(second,COALESCE(wspc.cs_start_date_time,rc.start_date_time),CONVERT_TIMEZONE('America/Los_Angeles','America/New_York',current_timestamp())) END AS file_age
  ...
  ,COALESCE(wspc.cs_start_date_time,rc.start_date_time) AS date_credentialing
  ,CASE WHEN rc.credentialing_status_name IN('Approved','Administrative Denial','Administrative Termination','Credentialing Committee Denial','Credentialing Committee Termination')
      OR (rc.credentialing_status_name = 'Process Discontinued' AND ocr.odm_action = 'Enrollment Approved') 
    THEN DATEDIFF(second,COALESCE(wspc.cs_start_date_time,rc.start_date_time),COALESCE(ccr.cc_cr_end_dt,ocr.odm_cr_end_dt,rc.end_date_time))   
  ...
  ,CASE WHEN rc.credentialing_status_name IN('Approved','Administrative Denial','Administrative Termination','Credentialing Committee Denial','Credentialing Committee Termination')
                 OR (rc.credentialing_status_name = 'Process Discontinued' AND ocr.odm_action = 'Enrollment Approved') 
        THEN (DATEDIFF(second,COALESCE(rc.start_date_time,wspc.cs_start_date_time),COALESCE(ccr.cc_cr_end_dt,ocr.odm_cr_end_dt,rc.end_date_time)) - COALESCE(DATEDIFF(second,ocr.odm_cr_start_dt,ocr.odm_cr_end_dt),0))
...
FROM ohpnm_dp4bi.registration r
 ...
 --CS queue    
 LEFT JOIN (SELECT *
       FROM (SELECT curws.process_id,curws.owner_id,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',ua.DateOfAction) assigned_to_ts,ua.contact_name assigned_to_user_name
             ...
             FROM ohpnm_dp4bi.wf_step curws -- current task 			    
              ...
              LEFT JOIN (SELECT *
                          FROM(SELECT hwf.step_id,hwf.owner_id, ua.contact_name,hwf.DateOfAction,
                    		...
                    		FROM ohpnm_dp4bi.a_wf_step hwf
                    			JOIN ohpnm_dp4bi.wf_task wf ON hwf.task_id = wf.task_id
                    			JOIN ohpnm_dp4bi.user_account_information ua ON hwf.owner_id = ua.UserId) ua
                    		WHERE rnk = 1)	ua ON curws.step_id = ua.step_id			                
              WHERE curtsk.name IN('Initiate Provider Credentialing','Provider Credentialing','Provider Credentialing ODM')
                ) s 
