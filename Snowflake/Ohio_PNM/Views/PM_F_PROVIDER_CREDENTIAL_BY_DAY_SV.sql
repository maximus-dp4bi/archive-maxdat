CREATE OR REPLACE VIEW OHIO_PROVIDER_DP4BI_DEV_DB.PUBLIC.PM_F_PROVIDER_CREDENTIAL_BY_DAY_SV
AS
SELECT d_date,
        ...
        LEFT JOIN (SELECT *
                   FROM (SELECT curws.process_id,curws.owner_id,curws.start_date_time assigned_to_ts,
	                     ...
	                     FROM ohpnm_dp4bi_dev.wf_step curws -- current task 			    
					        ...
					        JOIN ohpnm_dp4bi_dev.wf_step nextws ON nextws.calling_step_id = curws.step_id --next task
					        JOIN ohpnm_dp4bi_dev.wf_task nextsk ON nextws.task_id = nextsk.task_id
					        JOIN ohpnm_dp4bi_dev.wf_action wa ON wa.task_id = curtsk.task_id AND wa.next_task_id = nextsk.task_id --current task action			   
        ...
        LEFT JOIN (SELECT *
                   FROM (SELECT curws.process_id,curws.start_date_time csp_cr_start_dt
				         ...
				         FROM ohpnm_dp4bi_dev.wf_step  curws -- current task 			    
					        ...
					        JOIN ohpnm_dp4bi_dev.wf_task nextsk ON nextws.task_id = nextsk.task_id
					        JOIN ohpnm_dp4bi_dev.wf_action wa ON wa.task_id = curtsk.task_id AND wa.next_task_id = nextsk.task_id --current task action	
					        LEFT JOIN ohpnm_dp4bi_dev.user_account_information ua ON curws.owner_id = ua.UserId						
