       ...
       FROM  ohpnm_dp4bi.registration r 
         ...
         LEFT JOIN (SELECT *
                    FROM(SELECT ra.reg_id, ras.application_status_desc application_status, RANK() OVER (PARTITION BY ra.reg_id ORDER BY ra.reg_application_id) srnk
                         FROM ohpnm_dp4bi.reg_application ra
                           JOIN ohpnm_dp4bi.application_status ras ON ra.pnm_application_status_id = ras.application_status_id)
...
FROM curinv       
  LEFT JOIN (SELECT dd.d_date,process_id,t.step_id,t.name,t.start_date_time,t.date_of_action,assigned_to_dt
             FROM(SELECT *
                  ...
                  FROM ohpnm_dp4bi.wf_step t
                    JOIN ohpnm_dp4bi.wf_task tt ON t.task_id = tt.task_id
                    JOIN ohpnm_dp4bi.a_wf_step ht ON t.step_id = ht.step_id
