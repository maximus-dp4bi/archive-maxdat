create or replace view OHIO_PROVIDER_DP4BI_PROD_DB.PUBLIC.D_PM_PROVIDER_CREDENTIAL_INSTANCE_SV(
	APPLICATION_ID,
	PROCESS_ID,
	FILE_ID,
	FILE_AGE,
...
--      OR (rc.credentialing_status_name = 'Process Discontinued' AND ocr.odm_action = 'Enrollment Approved') 
 ...
 ,CASE WHEN rc.credentialing_status_name IN('Administrative Denial','Approved','Administrative Termination') THEN 
   COALESCE(setp.csr_end_date_time,ccr.cqr_end_date_time)
   ELSE NULL END AS final_decision_dt
  ...
  ,CASE WHEN rc.credentialing_status_name IN ('Administrative Denial','Approved','Administrative Termination')
  THEN DATEDIFF(second,minstepid.ipcs_start_date_time,COALESCE(setp.csr_end_date_time,ccr.cqr_end_date_time))
  ...
  ,DATEDIFF(second,pcs.cur_status_start_dt, CASE WHEN rc.credentialing_status_name IN('Approved','Administrative Denial','Administrative Termination','Credentialing Committee Denial','Credentialing Committee Termination')
                 --OR (rc.credentialing_status_name = 'Process Discontinued' AND ocr.odm_action = 'Enrollment Approved') 
            THEN cur_status_end_dt ELSE CONVERT_TIMEZONE('America/Los_Angeles','America/New_York',current_timestamp()) END) AS current_credentialing_status_tat
  ...
  ,CASE when r.workflow_event_type_id = 1 then 'New' 
         when r.workflow_event_type_id = 2 then 'Update' 
         when r.workflow_event_type_id = 3 then 'Revalidation' end APPLICATION_STATUS_DESCRIPTION
FROM ohpnm_dp4bi.registration r
 ...
 LEFT JOIN (SELECT s.*,pt.name prior_credentialing_queue
       FROM (SELECT s.process_id,s.owner_id,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',s.create_date_time) cur_queue_start_dt
                   ...
                   ,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',ua.DateOfAction) reassigned_to_ts
                   ,ua.contact_name reassigned_to_user_name
                   ,t.name credentialing_type
	         ...
	         FROM ohpnm_dp4bi.wf_step s
			   ...
			   LEFT JOIN (SELECT hwf.a_wf_step_id,hwf.step_id,hwf.owner_id, ua.contact_name,hwf.DateOfAction--,RANK() OVER (PARTITION BY hwf.step_id ORDER BY hwf.a_wf_step_id) rnk
                           , CASE WHEN EXISTS(SELECT 1 FROM ohpnm_dp4bi.aspnet_usersinroles ur LEFT JOIN ohpnm_dp4bi.aspnet_roles r on ur.roleid = r.roleid 
                                              WHERE ua.userid = ur.userid 
                                              AND (r.rolename = 'CredentialingSpecialist' or r.rolename = 'CredentialingQualityAssurance' or r.rolename = 'CredentialingSupervisor'))
...
--First Risk Level Date
 ...
 LEFT JOIN (SELECT *
           ...
           LEFT JOIN (SELECT *
           				FROM (SELECT curws.process_id,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',curws.create_date_time) cc_cr_start_dt
           					  	,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',curws.end_date_time) cc_cr_end_dt
           						, wa.name ccr_action
                               -- , prevtsk.name prior_queue_before_cc_cr         
                                ,ua.contact_name cc_cr_credential_assigned_to
           						,RANK() OVER (PARTITION BY curws.process_id ORDER BY curws.step_id) rnk,
           						--prevws.end_date_time cqr_end_date_time,
           					--	CASE WHEN prior_queue_before_cc_cr <>'Credentialing Quality Review' THEN cqr.cqr_cr_end_dt ELSE CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',prevws.end_date_time) END
                                cqr.cqr_cr_end_dt cqr_end_date_time, --If prior task isn't CQR, we need to get end dt from the CQR task
           						curtsk.name queue_name
           					FROM ohpnm_dp4bi.wf_step curws -- current task
           					LEFT JOIN ohpnm_dp4bi.wf_task curtsk ON curws.task_id = curtsk.task_id
           					LEFT JOIN ohpnm_dp4bi.wf_step nextws ON nextws.calling_step_id = curws.step_id --next task
           					LEFT JOIN ohpnm_dp4bi.wf_task nextsk ON nextws.task_id = nextsk.task_id
           					...
           					--LEFT JOIN ohpnm_dp4bi.wf_task prevtsk ON prevws.task_id = prevtsk.task_id
           					LEFT JOIN ohpnm_dp4bi.user_account_information ua ON curws.owner_id = ua.UserId
           					LEFT JOIN (SELECT curws.process_id,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',curws.create_date_time) cqr_cr_start_dt
           			 					,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',curws.end_date_time) cqr_cr_end_dt
                     					FROM ohpnm_dp4bi.wf_step curws -- current task
                     					LEFT JOIN ohpnm_dp4bi.wf_task curtsk ON curws.task_id = curtsk.task_id
                     					LEFT JOIN ohpnm_dp4bi.wf_step nextws ON nextws.calling_step_id = curws.step_id --next task
                     					...
                     					LEFT JOIN (SELECT w.*, RANK() OVER (PARTITION BY task_id, next_task_id ORDER BY action_id) action_rnk
                     								FROM ohpnm_dp4bi.wf_action w) wa ON wa.task_id = curtsk.task_id AND wa.next_task_id = nextsk.task_id AND wa.action_rnk = 1--current task action
                               					 WHERE curtsk.name IN ('Credentialing Quality Review')
                                					QUALIFY RANK() OVER (PARTITION BY curws.process_id ORDER BY curws.step_id desc) = 1 ) cqr ON curws.process_id=cqr.process_id
                                				WHERE curtsk.name IN ('Credentialing Committee Review') ) tmp
        				   WHERE rnk = 1 --and prior_queue_before_cc_cr <>'Check for Credential Risk Level'
        		) ccr ON ccr.process_id = rc.process_id
		LEFT JOIN ( SELECT *
					FROM (SELECT curws.process_id,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',curws.create_date_time) setp_start_dt
            				,CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',curws.end_date_time) setp_end_dt
            				,wa.name setp_action,prevtsk.name prior_queue_before_setp, ua.contact_name setp_credential_assigned_to
            				,RANK() OVER (PARTITION BY curws.process_id ORDER BY curws.step_id) rnk, CONVERT_TIMEZONE('UTC', 'America/Los_Angeles',prevws.end_date_time) csr_end_date_time,curtsk.name queue_name
            				FROM ohpnm_dp4bi.wf_step curws -- current task
                			LEFT JOIN ohpnm_dp4bi.wf_task curtsk ON curws.task_id = curtsk.task_id
                    		LEFT JOIN ohpnm_dp4bi.wf_step nextws ON nextws.calling_step_id = curws.step_id --next task
                    		LEFT JOIN ohpnm_dp4bi.wf_task nextsk ON nextws.task_id = nextsk.task_id
                    		...
                    		LEFT JOIN ohpnm_dp4bi.wf_task prevtsk ON prevws.task_id = prevtsk.task_id
                    		LEFT JOIN ohpnm_dp4bi.user_account_information ua ON curws.owner_id = ua.UserId
                	WHERE curtsk.name IN ('Send Email to Provider(Credentialing Admin Deny or Term)') ) tmp
            WHERE rnk = 1
        ) setp ON setp.process_id = rc.process_id
      LEFT JOIN (select distinct userid,contact_name,'Y' as credentialing_role from ohpnm_dp4bi.user_account_information ua where ua.userid in 
        (select distinct ur.userid from ohpnm_dp4bi.aspnet_usersinroles ur where ur.roleid in
        (select r.roleid from ohpnm_dp4bi.aspnet_roles r where (r.rolename = 'CredentialingSpecialist' or r.rolename = 'CredentialingQualityAssurance' or r.rolename = 'CredentialingSupervisor')))) crole
        ON ws.owner_id=crole.userid
    )
SELECT pr.*
