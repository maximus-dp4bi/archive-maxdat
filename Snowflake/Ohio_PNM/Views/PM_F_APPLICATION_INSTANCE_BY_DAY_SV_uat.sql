CREATE OR REPLACE VIEW PUBLIC.PM_F_APPLICATION_INSTANCE_BY_DAY_SV 
AS
            ...
            SELECT  dd.d_date,pcbd.*,
              ...
              FROM  (
                        SELECT  r.reg_id                                                                AS  application_id,     
                          ...
                          JOIN  ohpnm_dp4bi.wf_process          wc  ON wc.process_id = wp.process_id 
                          LEFT JOIN
                          (
                              SELECT        rp.reg_id,
                                            pt.mmis_provider_type_id
                                FROM        ohpnm_dp4bi.reg_provider                rp
                                JOIN        ohpnm_dp4bi.provider_type               pt      ON rp.provider_type_id = pt.provider_type_id
                          ) pt
                          ON pt.reg_id = r.reg_id 
                          LEFT JOIN         ohpnm_dp4bi.registration_status_type    rst     ON rst.registration_status_type_id = r.registration_status_type_id
                          LEFT JOIN
                          (
                              SELECT        rs.reg_id,
                                ...
                                LEFT JOIN   ohpnm_dp4bi.screening                                   s       ON rs.screening_id = s.screening_id
                                LEFT JOIN   ohpnm_dp4bi.screening_result                            sr      ON  s.screening_result_id = sr.screening_result_id
                          ) rsr
                                ...
                                SELECT  t.*
                                  FROM  (
                                          SELECT        curws.process_id,
                                           ...
                                           LEFT JOIN    ohpnm_dp4bi.wf_step nextws ON nextws.calling_step_id = curws.step_id 
                                           LEFT JOIN    ohpnm_dp4bi.wf_task nextsk ON nextws.task_id = nextsk.task_id
                                           LEFT JOIN    ohpnm_dp4bi.wf_action wa ON wa.task_id = curtsk.task_id AND wa.next_task_id = nextsk.task_id
                          ...
                          (
                              SELECT        rc.reg_id,
                                ...
                                LEFT JOIN   ohpnm_dp4bi.credentialing                               c       ON rc.credentialing_id = c.credentialing_id
                          ) rc
