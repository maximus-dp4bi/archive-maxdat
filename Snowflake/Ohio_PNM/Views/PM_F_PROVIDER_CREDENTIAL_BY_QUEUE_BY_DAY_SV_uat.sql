CREATE OR REPLACE VIEW PUBLIC.PM_F_PROVIDER_CREDENTIAL_BY_QUEUE_BY_DAY_SV
AS
SELECT d_date,
  credentialing_queue,
...
FROM ohpnm_dp4bi.registration r
 ...
 JOIN ohpnm_dp4bi.wf_step ws ON wp.process_id = ws.process_id --current task
 JOIN ohpnm_dp4bi.wf_task ts ON ws.task_id = ts.task_id 
 LEFT JOIN ohpnm_dp4bi.wf_step nextws ON nextws.calling_step_id = ws.step_id --next task
 LEFT JOIN ohpnm_dp4bi.wf_task nextsk ON nextws.task_id = nextsk.task_id
 LEFT JOIN ohpnm_dp4bi.wf_action wa ON wa.task_id = ts.task_id AND wa.next_task_id = nextsk.task_id 
