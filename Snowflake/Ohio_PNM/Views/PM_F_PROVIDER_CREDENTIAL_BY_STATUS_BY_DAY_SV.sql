CREATE OR REPLACE VIEW PM_F_PROVIDER_CREDENTIAL_BY_STATUS_BY_DAY_SV
AS
SELECT d_date,
...
FROM (SELECT rc.reg_id,c.credentialing_id,acs.credentialing_status_name credentialing_status,convert_timezone('UTC', 'America/Los_Angeles',aac.start_date_time) start_date_time,
	  ...
	  FROM ohpnm_dp4bi_dev.reg_credentialing rc 
		...
		LEFT JOIN ohpnm_dp4bi_dev.a_credentialing aac ON c.credentialing_id = aac.credentialing_id
		LEFT JOIN ohpnm_dp4bi_dev.credentialing_status acs ON acs.credentialing_status_id = aac.credentialing_status_id ) pcbd
JOIN public.d_dates dd ON dd.d_date >= CAST(pcbd.start_date_time AS DATE) AND dd.d_date <= CAST(COALESCE(pcbd.end_date_time,CONVERT_TIMEZONE('America/Los_Angeles','America/New_York',current_timestamp())) AS DATE)
) t
